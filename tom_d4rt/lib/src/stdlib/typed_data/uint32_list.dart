import 'dart:typed_data';
import 'package:tom_d4rt/d4rt.dart';

import 'inherited_list_methods.dart';

class Uint32ListTypedData {
  static BridgedClass get definition => BridgedClass(
    name: 'Uint32List',
    nativeType: Uint32List,
    isAssignable: (v) => v is Uint32List,
    typeParameterCount: 0,
    constructors: {
      '': (visitor, positionalArgs, namedArgs) {
        if (positionalArgs.length == 1 && positionalArgs[0] is int) {
          return Uint32List(positionalArgs[0] as int);
        }
        throw RuntimeD4rtException(
          "Uint32List constructor expects one int argument (length).",
        );
      },
      'fromList': (visitor, positionalArgs, namedArgs) {
        if (positionalArgs.length == 1 && positionalArgs[0] is List) {
          final sourceList = positionalArgs[0] as List;
          final intList = sourceList.toNativeList().map((e) {
            if (e is int) return e;
            throw RuntimeD4rtException(
              "Uint32List.fromList expects a List<int>.",
            );
          }).toList();
          return Uint32List.fromList(intList);
        }
        throw RuntimeD4rtException(
          "Uint32List.fromList expects one List<int> argument.",
        );
      },
      'view': (visitor, positionalArgs, namedArgs) {
        if (positionalArgs.isNotEmpty && positionalArgs[0] is ByteBuffer) {
          final buffer = positionalArgs[0] as ByteBuffer;
          final offsetInBytes = positionalArgs.length > 1
              ? positionalArgs[1] as int? ?? 0
              : 0;
          final length = positionalArgs.length > 2
              ? positionalArgs[2] as int?
              : null;
          return Uint32List.view(buffer, offsetInBytes, length);
        }
        throw RuntimeD4rtException(
          "Uint32List.view expects ByteBuffer and optional offset/length arguments.",
        );
      },
      'sublistView': (visitor, positionalArgs, namedArgs) {
        if (positionalArgs.isNotEmpty && positionalArgs[0] is TypedData) {
          final data = positionalArgs[0] as TypedData;
          final start = positionalArgs.length > 1
              ? positionalArgs[1] as int? ?? 0
              : 0;
          final end = positionalArgs.length > 2
              ? positionalArgs[2] as int?
              : null;
          return Uint32List.sublistView(data, start, end);
        }
        throw RuntimeD4rtException(
          "Uint32List.sublistView expects TypedData and optional start/end arguments.",
        );
      },
    },
    methods: {
      // Index operators
      '[]': (visitor, target, positionalArgs, namedArgs, _) {
        if (target is Uint32List &&
            positionalArgs.length == 1 &&
            positionalArgs[0] is int) {
          return target[positionalArgs[0] as int];
        }
        throw RuntimeD4rtException("Uint32List[index] expects an int index.");
      },
      '[]=': (visitor, target, positionalArgs, namedArgs, _) {
        if (target is Uint32List &&
            positionalArgs.length == 2 &&
            positionalArgs[0] is int &&
            positionalArgs[1] is int) {
          final index = positionalArgs[0] as int;
          final value = positionalArgs[1] as int;
          target[index] = value;
          return value;
        }
        throw RuntimeD4rtException(
          "Uint32List[index] = value expects int index and int value.",
        );
      },

      // List methods
      'sublist': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.sublist', atMost: 2);
        final start = positionalArgs.isNotEmpty ? positionalArgs[0] as int : 0;
        final end = positionalArgs.length > 1
            ? positionalArgs[1] as int?
            : null;
        return (target as Uint32List).sublist(start, end);
      },
      'getRange': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.getRange', atMost: 2);
        final start = positionalArgs[0] as int;
        final end = positionalArgs[1] as int;
        return (target as Uint32List).getRange(start, end);
      },
      'setRange': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.setRange', atMost: 4);
        final start = positionalArgs[0] as int;
        final end = positionalArgs[1] as int;
        final iterable = coerceElements<int>(
          positionalArgs[2],
          'Uint32List.setRange',
        );
        final skipCount = positionalArgs.length > 3
            ? positionalArgs[3] as int
            : 0;
        (target as Uint32List).setRange(start, end, iterable, skipCount);
        return null;
      },
      'setAll': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.setAll', atMost: 2);
        final at = positionalArgs[0] as int;
        final iterable = coerceElements<int>(
          positionalArgs[1],
          'Uint32List.setAll',
        );
        (target as Uint32List).setAll(at, iterable);
        return null;
      },
      'fillRange': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.fillRange', atMost: 3);
        final start = positionalArgs[0] as int;
        final end = positionalArgs[1] as int;
        final fill = positionalArgs.length > 2
            ? positionalArgs[2] as int?
            : null;
        (target as Uint32List).fillRange(start, end, fill);
        return null;
      },

      // SCD27: `buffer` was registered HERE as well as in `getters`,
      // so `l.buffer()` resolved. In the SDK it is a getter inherited
      // from `TypedData` and that call does not compile as Dart — the
      // widening shape, where a script is green here and invalid there.
      // The getter registration below is the correct one and stays.
      // Absence pinned by F-SCD27-*.
      // Typed methods
      'asUint8ListView': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.asUint8ListView', atMost: 2);
        final offsetInBytes = positionalArgs.isNotEmpty
            ? positionalArgs[0] as int?
            : null;
        final length = positionalArgs.length > 1
            ? positionalArgs[1] as int?
            : null;
        return (target as Uint32List).buffer.asUint8List(
          offsetInBytes ?? 0,
          length,
        );
      },

      // Standard methods
      'toString': (visitor, target, positionalArgs, namedArgs, _) {
        return (target as Uint32List).toString();
      },
      '==': (visitor, target, positionalArgs, namedArgs, _) {
        D4.checkArity(positionalArgs, 'Uint32List.==', atMost: 1);
        return (target as Uint32List) == positionalArgs[0];
      },

      // Inherited Iterable<int> / List<int> read-only methods.
      // See inherited_list_methods.dart — these are declared per variant
      // because the element type has to be reified at the native boundary,
      // which the generic `List` bridge cannot do. The supertype walk does
      // reach `List`, so pruning this spread looks safe and is not.
      ...inheritedListMethods<int>(
        (t) => t as Uint32List,
        unmodifiableView: (t) => (t as Uint32List).asUnmodifiableView(),
      ),
    },
    staticGetters: typedListStaticGetters(Uint32List.bytesPerElement),
    setters: inheritedListSetters<int>((t) => t as Uint32List),
    getters: {
      'length': (visitor, target) {
        if (target is Uint32List) return target.length;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'length'",
        );
      },
      'lengthInBytes': (visitor, target) {
        if (target is Uint32List) return target.lengthInBytes;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'lengthInBytes'",
        );
      },
      'elementSizeInBytes': (visitor, target) {
        if (target is Uint32List) return target.elementSizeInBytes;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'elementSizeInBytes'",
        );
      },
      'offsetInBytes': (visitor, target) {
        if (target is Uint32List) return target.offsetInBytes;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'offsetInBytes'",
        );
      },
      'buffer': (visitor, target) {
        if (target is Uint32List) return target.buffer;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'buffer'",
        );
      },
      'first': (visitor, target) {
        if (target is Uint32List) return target.first;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'first'",
        );
      },
      'last': (visitor, target) {
        if (target is Uint32List) return target.last;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'last'",
        );
      },
      'isEmpty': (visitor, target) {
        if (target is Uint32List) return target.isEmpty;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'isEmpty'",
        );
      },
      'isNotEmpty': (visitor, target) {
        if (target is Uint32List) return target.isNotEmpty;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'isNotEmpty'",
        );
      },
      'hashCode': (visitor, target) {
        if (target is Uint32List) return target.hashCode;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'hashCode'",
        );
      },
      'runtimeType': (visitor, target) {
        if (target is Uint32List) return target.runtimeType;
        throw RuntimeD4rtException(
          "Target is not an Uint32List for getter 'runtimeType'",
        );
      },

      // Inherited getters (single, iterator, reversed).
      ...inheritedListGetters<int>((t) => t as Uint32List),
    },
  );
}
