...
class Environment {
  ...
  dynamic get(String name) {
    ...
    while (env != null) {
      if (Logger.isDebug) {
        Logger.debug(
            '[Env.get] Attempting to get "$name" in env: ${env.hashCode}'); // Log attempt + env hash
      }
  ...
  // ── T9 (perf plan_2 §6, bounded realization of 4b indexed access) ──────────
  //
  // The residual `Environment.get` cost after T1 is the per-level name-hashing
  // paid at EVERY scope on the chain walk. Indexed lexical access removes that
  // by resolving an identifier-use to a fixed scope depth once, then reading at
  // that depth via pointer-hops with a single terminal hash. We ship the
  // depth-only slice (not slot arrays) — it captures the chain-walk-depth cost
  // 5d/F1 flagged with no AST-model change and a self-validating fast path that
  // cannot return a stale or wrong binding (the terminal frame is re-checked,
  // and any miss falls back to the full [get] and re-resolves the depth).

  /// Sentinel returned by [getAtDepthOrMiss] when the cached depth no longer
  /// resolves [name] to a local [_values] binding (env structure or shadowing
  /// changed). Distinct from a real `null` binding, so the caller can tell a
  /// stale cache from a legitimately-null variable and fall back to [get].
  static final Object slotMiss = Object();

  /// Fast lexical read of [name] at a previously-resolved fixed [depth]
  /// (number of enclosing hops). Walks pointer-hops only — no per-level name
  /// hashing at intermediate scopes — and probes [_values] exactly ONCE at the
  /// terminal frame. Returns [slotMiss] if that frame does not hold [name],
  /// signalling the inline cache is stale.
  ///
  /// Correctness rests on the lexical-addressing invariant that the first-hit
  /// scope depth for a given identifier-use node is stable across executions
  /// (the runtime env chain mirrors static block nesting). The caller stores
  /// the depth only after a real [get] hit and re-establishes it via
  /// [resolveDepthOf] on any miss, so a changed chain self-heals rather than
  /// returning a wrong value.
  Object? getAtDepthOrMiss(String name, int depth) {
    Environment? env = this;
    for (var i = 0; i < depth; i++) {
      env = env?._enclosing;
      if (env == null) return slotMiss;
    }
    final v = env!._values[name];
    if (v != null || env._values.containsKey(name)) {
      return v is GlobalGetter ? v() : v;
    }
    return slotMiss;
  }

  /// Resolves the number of enclosing hops from this environment to the first
  /// scope whose [_values] holds [name]. Returns `-1` when the name resolves
  /// via a NON-local path (prefixed import, bridged class, or bridged enum) or
  /// is not found as a plain local at any level — those reads are ineligible
  /// for the depth inline-cache and must always take the full [get] path.
  /// Called only on a cache miss (≈ once per identifier-use node after warmup).
  int resolveDepthOf(String name) {
    Environment? env = this;
    var depth = 0;
    while (env != null) {
      // A prefixed-import scope can intercept the name non-locally; mark the
      // node ineligible so the inline cache never short-circuits past it.
      if (env._prefixedImports.isNotEmpty &&
          (env._prefixedImports.containsKey(name) || name.contains('.'))) {
        return -1;
      }
      final v = env._values[name];
      if (v != null || env._values.containsKey(name)) {
