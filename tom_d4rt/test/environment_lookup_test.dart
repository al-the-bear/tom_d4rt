...
/// tests pin the contract that makes the throw avoidable:
///   - a present binding resolves to its value (including `null`);
///   - an absent binding resolves to the [Environment.kNotFound] sentinel
///     WITHOUT throwing;
///   - present-but-`null` is distinguishable from absent;
///   - `lookup` walks the enclosing chain like `get`;
///   - `get` still throws on a miss (its public contract is unchanged).
...
void main() {
  group('Environment.lookup', () {
    ...
    test('returns kNotFound (without throwing) for an undefined name', () {
      // The whole point: a miss must NOT construct/throw an exception.
