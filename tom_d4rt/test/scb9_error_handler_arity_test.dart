...
void main() {
  group('SCB9: Stream.listen onError arity', () {
    ...
    test('F-SCB9-2: a binary onError still receives both arguments [2026-07-28]',
        () async {
      ...
      final result = await listenOnError(
          "(e, st) => seen.add('e:\$e st:\${st != null}')") as List;
      expect(result, orderedEquals(['e:Bad state: boom st:true']));
    });

    test('F-SCB9-3: an onError with an optional second parameter still receives the stack trace [2026-07-28]',
        () async {
  ...
  group('SCB9: Future error-handler arity', () {
    test('F-SCB9-4: Future.catchError accepts a unary handler [2026-07-28]',
        () async {
    ...
    test('F-SCB9-5: Future.catchError still accepts a binary handler [2026-07-28]',
        () async {
      final result = await executeAsync('''
        import 'dart:async';
        main() async {
    ...
    test('F-SCB9-6: Future.then onError accepts a unary handler [2026-07-28]',
        () async {
    ...
    test('F-SCB9-7: the Future.onError extension accepts a unary handler [2026-07-28]',
        () async {
  ...
  group('SCB9: Stream.handleError and subscription handlers', () {
    test('F-SCB9-8: the StreamSubscription.onError setter accepts a unary handler [2026-07-28]',
        () async {
      final result = await executeAsync('''
        import 'dart:async';
        main() async {
    ...
    test('F-SCB9-9: Stream.handleError returns a usable stream [2026-07-28]',
        () async {
    ...
    test('F-SCB9-10: Stream.handleError accepts a binary handler [2026-07-28]',
        () async {
      final result = await executeAsync('''
        import 'dart:async';
        main() async {
    ...
    test('F-SCB9-11: Stream.handleError passes the stack trace to an optional second parameter [2026-07-28]',
        () async {
  ...
  group('SCB9: the fix does not swallow genuine arity errors', () {
    test('F-SCB9-12: a three-parameter error handler still fails [2026-07-28]',
        () async {
    ...
    test('F-SCB9-14: a bad handler in a stream callback still reports, via the zone [2026-07-28]',
        () async {
      ...
      runZonedGuarded(() async {
        finished.complete(await executeAsync('''
          import 'dart:async';
          main() async {
            final c = StreamController();
    ...
    test('F-SCB9-13: StreamTransformer.fromHandlers still receives all three arguments [2026-07-28]',
        () async {
