...
void main() {
  ...
  group('IMP-OPT-12: pool / security regression (analyzer D4rt)', () {
    ...
    test('IMP-OPT-12b: a package provided by interpreter 1 is NOT exposed in '
      ...
      expect(
        firstParent.toBridgedInstance(_AClassNative()),
        isNotNull,
      ...
      expect(
        D4rt.debugPooledClassCount('pkg_a'),
      ...
      expect(
        () => secondParent.toBridgedInstance(_AClassNative()),
        throwsA(anything),
        reason:
            'interpreter 2 was not granted pkg_a — AClass is out of '
            'scope',
      );
      ...
      expect(
        () => thirdParent.toBridgedInstance(_AClassNative()),
        throwsA(anything),
        reason:
            'a legacy interpreter never pulls pooled bridges it did not '
            'register itself',
      );
    });
    ...
    test(
      'IMP-OPT-12d: no script-decl leak between executes or interpreters',
      () {
        final first = D4rt();
