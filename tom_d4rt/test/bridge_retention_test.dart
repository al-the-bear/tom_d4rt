  ...
  group('PERF-3: per-run BridgedClass / AST retention (analyzer D4rt)', () {
    ...
    test(
      ...
      for (var i = 0; i < n; i++) {
        ...
        if (i == 0) {
          ...
          expect(D4rt.debugBridgedModuleEnvBuildCount, buildsAfterFirst,
              reason:
                  'run $i must reuse the cached bridge surface — no rebuild');
          expect(D4rt.debugPooledClassCount('pkg_w'), pooledAfterFirst,
    ...
    test(
      ...
      final first = interpreter.execute(
          source: "import 'package:w/w.dart';\nint main() => 7;");
      expect(first, 7);
      expect(interpreter.debugLoadedModuleCount, greaterThan(0),
