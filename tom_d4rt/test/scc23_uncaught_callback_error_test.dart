...
Future<(Object? result, List<_Escape> escapes)> runWithHook(
  ...
  d4rt.onUncaughtError =
      hook ?? (error, stackTrace) => escapes.add(_Escape(error, stackTrace));
...
void main() {
  group('SCC23: escapes reach the embedder hook', () {
    test(
      'F-SCC23-1: an error thrown by onData reaches onUncaughtError [2026-09-04]',
      () async {
        final (result, escapes) = await runWithHook(
          streamScript("(v) { throw StateError('od'); }"),
        );
        expect(
          result,
          'script-completed',
        ...
        expect(
          escapes.single.error,
          isA<StateError>(),
          reason:
              'the host must get the script error, not the interpreter '
    ...
    test(
      'F-SCC23-2: an error thrown by onError reaches onUncaughtError [2026-09-04]',
      () async {
    ...
    test(
      'F-SCC23-3: an error thrown by onDone reaches onUncaughtError [2026-09-04]',
      () async {
        final (_, escapes) = await runWithHook(
    ...
    test(
      'F-SCC23-4: an error thrown by a handleError handler reaches onUncaughtError [2026-09-04]',
      () async {
        final (_, escapes) = await runWithHook('''
        import 'dart:async';
        main() async {
    ...
    test(
      'F-SCC23-5: a Timer callback error reaches onUncaughtError too [2026-09-04]',
      () async {
    ...
    test(
      'F-SCC23-6: a script-class object thrown from a callback arrives unwrapped [2026-09-04]',
      () async {
      ...
      ''');
        ...
        expect(
          ...
          reason:
              'the interpreter-internal wrapper must not cross the boundary',
        );
        expect(
          escapes.single.error,
        ...
        expect(
          (escapes.single.error as InterpretedInstance).klass.name,
          'MyErr',
    ...
    test(
      'F-SCC23-7: a native stream error with no onError routes through the same hook [2026-09-04]',
      () async {
    ...
    test(
      'F-SCC23-8: the hook receives a usable stack trace [2026-09-04]',
      () async {
        final (_, escapes) = await runWithHook(
          streamScript("(v) { throw StateError('st'); }"),
        );
        expect(escapes, hasLength(1));
  ...
  group('SCC23: containment and the no-hook default', () {
    test(
      'F-SCC23-9: with no hook the error still reaches the enclosing zone [2026-09-04]',
      () async {
        ...
        final (result, zoneErrors) = await runWithoutHook(
          streamScript("(v) { throw StateError('nz'); }"),
        );
        expect(result, 'script-completed');
    ...
    test(
      'F-SCC23-11: a hook contains the error — it does not also reach the zone [2026-09-04]',
      () async {
        ...
        runZonedGuarded(() async {
          ...
          final raw = d4rt.execute(
            library: 'package:test/main.dart',
            sources: {
              'package:test/main.dart': streamScript(
                "(v) { throw StateError('cz'); }",
              ),
            },
          );
          finished.complete(raw is Future ? await raw : raw);
    ...
    test(
      'F-SCC23-12: an error thrown by the hook itself is not lost [2026-09-04]',
      () async {
        ...
        expect(
          zoneErrors.map((e) => e.toString()).join('\n'),
    ...
    test(
      'F-SCC23-13: a synchronous script error still throws out of execute [2026-09-04]',
      () async {
        ...
        try {
          final raw = d4rt.execute(
            library: 'package:test/main.dart',
            sources: {
              'package:test/main.dart': "main() { throw StateError('sync'); }",
            },
          );
    ...
    test(
      'F-SCC23-16: an async script error still reaches the caller with a hook installed [2026-09-04]',
      () async {
  ...
  group('SCC23: the delivery paths agree', () {
    ...
    test(
      'F-SCC23-14: listen onError receives the script object the script threw [2026-09-04]',
      () async {
        final result =
            await delivered(
                  "c.stream.listen((v) {}, onError: (e) { seen.add('\$e'); });",
                )
                as List;
    ...
    test(
      'F-SCC23-15: handleError receives the identical object [2026-09-04]',
      () async {
        final result =
            await delivered(
        ...
        expect(
          result,
          orderedEquals(['MyErr:sent']),
          reason:
              'the two paths must agree — this is the RELATED claim in the '
