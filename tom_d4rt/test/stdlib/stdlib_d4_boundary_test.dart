...
void main() {
  ...
  List<MapEntry<String, String>> stdlibSources() {
    expect(
      ...
      isTrue,
      reason:
          'lib/src/stdlib is missing, so this guard would pass by checking '
  ...
  group('SCC87: the D4 argument-reading boundary', () {
    test('F-SCC87-1: the guard is looking at a real corpus [2026-09-07]', () {
    ...
    test('F-SCC87-2: no stdlib file uses the generated-code argument readers '
        '[2026-09-07]', () {
    ...
    test('F-SCC87-3: the arity guard the stdlib DOES use is the one that '
        'matches its dispatch path [2026-09-07]', () {
      // The positive half. If `checkArity` ever stopped being the stdlib's
      // guard — replaced wholesale, or renamed — F-SCC87-2 would still pass
