...
void main() {
  ...
  group('LinkedList and LinkedListEntry Tests', () {
    ...
    test('I-COLL-42: the head entry is removed by unlinking it. [2026-09-04]', () {
    ...
    test('F-SCC8-1: addAll links every entry, in order, at the tail [2026-09-04]',
        () {
    ...
    test('F-SCC8-2: addAll accepts any Iterable, not only a list literal [2026-09-04]',
        () {
    ...
    test('F-SCC8-4: addFirst prepends and relinks the previous head [2026-09-04]',
        () {
      final result = executeTestScript('''
        var list = LinkedList();
    ...
    test('F-SCC8-5: removeFirst is absent, because Dart LinkedList has no such member [2026-09-04]',
        () {
      ...
      expect(
        ...
        throwsA(isA<NoSuchMethodError>().having(
            (e) => e.toString(), 'message', contains('removeFirst'))),
      );
    });

    test('F-SCC8-6: addAll rejects a non-entry element without linking any [2026-09-04]',
        () {
      ...
      expect(result['length'], 0, reason: 'nothing may be linked on rejection');
