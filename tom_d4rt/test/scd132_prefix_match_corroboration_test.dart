// REPO-WIDE GUARD (tom_d4rt) — no, it is not one; this banner is here only to
// say so, because the file sits beside fifteen that are.
//
// (Deliberately not matching the reach detector: this test reads nothing
// outside its own package. `scd129_repo_wide_guard_index_test.dart` only
// demands a banner from files that DO reach out, so this note is prose, not a
// claim.)
//
// ─────────────────────────────────────────────────────────────────────────────
//
// SCD132 — a bare name prefix is not evidence of a bridge relationship.
//
...
//
// F-SCD132-4 IS WHY THIS IS A NARROWING AND NOT A DELETION. Removing PASS B
...
void main() {
  group('SCD132: a prefix match needs corroboration', () {
    ...
    test(
      'F-SCD132-1: an UNCORROBORATED prefix no longer resolves [2026-09-15]',
      () {
    ...
    test(
      'F-SCD132-2: a prefix corroborated by nativeNames resolves [2026-09-15]',
      () {
    ...
    test('F-SCD132-3: a prefix corroborated by the supertype registry resolves '
        '[2026-09-15]', () {
    ...
    test('F-SCD132-4 (control): PASS A is untouched — a suffix match still '
