...
void main() {
  ...
  Object? runWith(String imports, String body) => D4rt().execute(
    source:
        '''
          $imports
          main() {
  ...
  Object? call(
    ...
    for (final name in [
      ...
      if (adapter != null) return adapter(visitor, target, args, {}, []);
    }
  ...
  group('SCE185: what the widened differential found', () {
    test('F-SCE195-1: ServerSocket declares only what it adds over Stream, and '
      ...
      final subscription =
          call('ServerSocket', server, 'listen', [accept])
    ...
    test('F-SCE185-2: Runes members that take a callback run it '
      ...
      expect(run("return 'abc'.runes.where((r) => r > 97).toList();"), [
        98,
        99,
      ]);
      expect(run("return 'abc'.runes.fold(0, (a, b) => a + b);"), 294);
      expect(run("return 'abc'.runes.any((r) => r > 97);"), isTrue);
      expect(run("return 'abc'.runes.reduce((a, b) => a + b);"), 294);
    });

    test('F-SCE185-3: LinkedList.contains answers false for a non-entry '
        '[2026-09-25]', () {
      expect(run('return LinkedList().contains(1);'), isFalse);
    });
    ...
    test('F-SCE185-4: List.shuffle honours the Random it is given '
      ...
      expect(
        runWith(
          "import 'dart:math';",
          'final l = [1, 2, 3, 4, 5, 6, 7, 8]; l.shuffle(Random(3)); return l;',
        ),
        expected,
      );
    });
    ...
    test('F-SCE185-5: a natively typed List<int> takes a script\'s list '
      ...
      // `'ab'.codeUnits.toList()` is a real `List<int>` that dispatches to
      // the `List` bridge; the script's literal and closure are untyped.
      expect(
        run(
          "final l = 'ab'.codeUnits.toList(); "
          'return [l.followedBy([1]).toList(), l.reduce((a, b) => a + b)];',
        ),
        [
          [97, 98, 1],
          195,
      ...
      expect(
        run(
          "final l = 'ab'.codeUnits.toList(); l.setRange(0, 1, [7]); return l;",
        ),
        [7, 98],
      );
      expect(
        run(
          "final l = 'ab'.codeUnits.toList(); "
          'try { l.setRange(0, 2, [7]); return "no throw"; } '
          'on StateError catch (e) { return "StateError"; }',
        ),
        'StateError',
      );
    ...
    test('F-SCE185-7: Sink.close hands back the Future the sink returns '
      ...
      final closing = env.findBridgedClassByName('Sink')!.methods['close']!(
        visitor,
        controller,
        const [],
        {},
