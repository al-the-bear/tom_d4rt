...
- **Source-based (analyzer)** — *this package*, plus [`tom_d4rt_dcli`](../tom_d4rt_dcli/) (REPL) and [`tom_d4rt_flutter`](../tom_d4rt_flutter/) (Flutter Material bridges). The host parses Dart source with the `analyzer` package and interprets it directly.
- **Analyzer-free (mirror AST)** — [`tom_ast_model`](../tom_ast_model/), [`tom_d4rt_ast`](../tom_d4rt_ast/), [`tom_ast_generator`](../tom_ast_generator/), [`tom_d4rt_exec`](../tom_d4rt_exec/), [`tom_dcli_exec`](../tom_dcli_exec/). These run from pre-compiled `SAstNode` trees with **no analyzer dependency**:
...
Runnable, self-contained samples live in [`tom_d4rt_samples/`](../tom_d4rt_samples/). They are ordered as a learning path — each one introduces exactly one new capability on top of the last:

| Sample | What it teaches | Interpreter |
|--------|-----------------|-------------|
| [d4rt_introduction_sample](../tom_d4rt_samples/d4rt_introduction_sample/) | Run multi-file D4rt programs with **nothing but the interpreter** — no bridges, no host wiring. Includes shebang launchers (`#!/usr/bin/env run_example`). Start here. | `tom_d4rt` |
| [d4rt_advanced_sample](../tom_d4rt_samples/d4rt_advanced_sample/) | Cross the first boundary: bridge a **native Dart library** into scripts using [`tom_d4rt_generator`](../tom_d4rt_generator/). | `tom_d4rt` |
| [d4rt_userbridges_sample](../tom_d4rt_samples/d4rt_userbridges_sample/) | Hand-write `D4UserBridge` overrides for what the generator can't bridge automatically. | `tom_d4rt` |
| [d4rt_dcli_sample](../tom_d4rt_samples/d4rt_dcli_sample/) | Shell scripting plus your own bridges via the [`tom_d4rt_dcli`](../tom_d4rt_dcli/) REPL. | `tom_d4rt` |
| [d4rt_flutter_sample](../tom_d4rt_samples/d4rt_flutter_sample/) | Interpret a live Flutter UI from source at runtime. | `tom_d4rt_flutter` |

If you are new to D4rt, clone the [introduction sample](../tom_d4rt_samples/d4rt_introduction_sample/) and run one of its examples directly — it is the shortest path from "installed" to "running a script".
...
the [User Guide → Extension Registration and Facades](doc/d4rt_user_guide.md#extension-registration-and-facades)
...
For large bridge surfaces, use [`tom_d4rt_generator`](../tom_d4rt_generator/) to generate all adapter boilerplate from annotated native source. Duplicated registrations are safely deduplicated via `sourceUri` tracking.

For the complete registration reference see the [Bridging Guide](doc/BRIDGING_GUIDE.md); for a worked end-to-end example, the [d4rt_advanced_sample](../tom_d4rt_samples/d4rt_advanced_sample/) bridges a native library with the generator, and the [d4rt_userbridges_sample](../tom_d4rt_samples/d4rt_userbridges_sample/) shows hand-written overrides.
...
- [Issues](doc/issues.md) — tracked interpreter issues and their status
