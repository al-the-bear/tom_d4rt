...
d4rt_flutter_sample/
├── lib/
│   └── main.dart                      # the app shell (gallery + host page)
├── example/                           # the samples — plain Dart source
│   ├── counter_app/
...
The crucial split: `example/` holds the samples as **editable source**;
`assets/samples/` holds a **bundled snapshot** of that source, generated by the
tool in `tool/`. The app only ever reads the snapshot — see §8 for why.
...
In the app, pick a sample by tapping its name in the list, or type the name into
the field and press **Run**. The chosen sample is interpreted and shown
full-screen.
