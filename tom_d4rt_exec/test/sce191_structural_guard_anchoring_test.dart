// REPO-WIDE GUARD (tom_d4rt_exec) — every test here that reads a sibling tree declares which package it must run in.
//
...
// `tom_d4rt_exec`; the reasoning is there. A test that resolves paths into a SIBLING
...
final RegExp _anchorCall = RegExp(r"requirePackage\(\s*'tom_d4rt_exec'");
...
void main() {
  ...
  for (final entity in Directory('test').listSync(recursive: true)) {
    ...
    final call = RegExp(
      r"requirePackage\(\s*'tom_d4rt_exec'\s*(,\s*subject:\s*(.*?))?\)",
      dotAll: true,
    ).firstMatch(code);
    ...
    test('F-SCE191-1: every test reading a sibling tree calls requirePackage '
      ...
      expect(
        ...
        reason:
            ...
            "    requirePackage('tom_d4rt_exec', subject: '<what it reads>');\n"
            '\n${unanchored.join('\n')}',
      );
    });
