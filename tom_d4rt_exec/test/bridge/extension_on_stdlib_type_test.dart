      ...
      try {
        ...
        d4rt.registerBridgedExtension(
          BridgedExtensionDefinition(
            ...
            onTypeName: 'CompletelyUnknownType',
            getters: {
              'summary': (visitor, target) => 'unknown',
            },
          ),
          'package:test_pkg/test_pkg.dart',
        );
        ...
        expect(
          ErrorReporter.errors.map((e) => e.message).toList(),
          isNot(contains(contains('Undefined variable: CompletelyUnknownType'))),
          reason: 'A stdlib probe that misses must not be reported as an error',
        );
      } finally {
        ErrorReporter.clear();
        ErrorReporter.disableTracking();
