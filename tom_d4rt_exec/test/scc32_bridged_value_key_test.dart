...
void main() {
  ...
  // NOT PORTED: the reference file's third group (F-SCC32-20/21) is a source
  // scan over both mirrored trees rather than a script run. It reads the same
  // two files off disk and asks the same question `tom_d4rt`'s copy already
  // answers, so a second copy here could only ever produce a duplicate
  // failure — the same reason `release_hygiene_test.dart` is marked NOT
  // PORTABLE in `conformance_drift_test.dart`, and the same call SCC31 makes
  // for its own F-SCC31-17/18. The omission is recorded as this file's entry
  // in `_divergentBaseline`; it is not a coverage gap.
}
