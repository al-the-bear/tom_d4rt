...
void main() {
  ...
  group('SC3: UnmodifiableSetView collection bridge', () {
    test('F-SC3-11: the constructor wraps a set and reads through it [2026-07-27]',
        () {
    ...
    test('F-SC3-14: set algebra returns plain modifiable sets [2026-07-27]', () {
      final result = d4rt.execute(
        source: viewSource('{1, 2, 3}', '''
          return [
    ...
    test('F-SC3-15: the view reflects later changes to the backing set [2026-07-27]',
        () {
      final result = d4rt.execute(
        source: viewSource('{1}', '''
    ...
    test('F-SC3-16: add throws an UnsupportedError catchable by type [2026-07-27]',
        () {
      final result = d4rt.execute(
        source: viewSource('{1}', '''
          try {
    ...
    test('F-SC3-17: the other mutators also throw UnsupportedError [2026-07-27]',
        () {
    ...
    test('F-SC3-19: Set.unmodifiable() results are the same bridged type [2026-07-27]',
        () {
      final result = d4rt.execute(
        source: '''
          import 'dart:collection';
