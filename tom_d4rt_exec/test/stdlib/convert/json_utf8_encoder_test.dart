...
void main() {
  ...
  group('JsonUtf8Encoder (SC9)', () {
    test(
      'F-SC9-1: converts an object straight to UTF-8 JSON bytes. [2026-07-27]',
      () {
        final result = run('''
    ...
    test(
      'F-SC9-2: the indent argument selects pretty output. [2026-07-27]',
      () {
        final result = run(r'''
    ...
    test(
      'F-SC9-4: toEncodable is invoked for unsupported values. [2026-07-27]',
      () {
        final result = run('''
    ...
    test(
      'F-SC9-5: JsonEncoder.fuse(Utf8Encoder()) is now usable. [2026-07-27]',
      () {
        final result = run('''
    ...
    test(
      'F-SC9-6: startChunkedConversion drives a byte sink. [2026-07-27]',
      () {
        final result = run('''
        final out = [];
      ...
      expect(
        ...
        throwsA(
          isA<RuntimeD4rtException>().having(
            (e) => e.toString(),
            'message',
            contains('indent must be a String or null'),
          ),
        ),
      );
    });
      ...
      expect(
        ...
        throwsA(
          isA<RuntimeD4rtException>().having(
            (e) => e.toString(),
            'message',
            contains('toEncodable must be a Function or null'),
          ),
        ),
      );
    });
      ...
      expect(
        ...
        throwsA(
          isA<RuntimeD4rtException>().having(
            (e) => e.toString(),
            'message',
            contains('bufferSize must be an int or null'),
          ),
        ),
      );
    });
