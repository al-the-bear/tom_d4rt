...
void main() {
  ...
  if (rect.width != 2.0)
    errors.add('Rectangle.width expected 2.0, got ${rect.width}');
  if (rect.height != 3.0)
    errors.add('Rectangle.height expected 3.0, got ${rect.height}');

  // Test BankAccount(accountNumber, balance)
  var ba = BankAccount('B', 50.0);
  if (ba.accountNumber != 'B')
    errors.add(
      'BankAccount.accountNumber expected "B", got "${ba.accountNumber}"',
    );
  if (ba.balance != 50.0)
    errors.add('BankAccount.balance expected 50.0, got ${ba.balance}');

  // Test Circle(radius)
  var circle = Circle(4.0);
  if (circle.radius != 4.0)
    errors.add('Circle.radius expected 4.0, got ${circle.radius}');

  // Test User(name, email)
  var user = User('C', 'c@d.com');
  if (user.name != 'C')
    errors.add('User.name expected "C", got "${user.name}"');
  if (user.email != 'c@d.com')
    errors.add('User.email expected "c@d.com", got "${user.email}"');
