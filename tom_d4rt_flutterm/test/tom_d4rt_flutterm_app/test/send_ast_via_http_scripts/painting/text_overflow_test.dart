...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting TextOverflow enum verification');

  final values = TextOverflow.values;
  _check(
  ...
  final roundTripValid = values
      .map((value) => TextOverflow.values.byName(value.name) == value)
  ...
  try {
    TextOverflow.values.byName('__invalid_text_overflow__');
  } catch (_) {
  ...
  final toStringIncludesType =
      values.every((value) => value.toString().startsWith('TextOverflow.'));
  _check(
  ...
  final summaryLine =
      'TextOverflow summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('TextOverflow Test Summary'),
      Text(summaryLine),
