...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting TextWidthBasis enum verification');

  final values = TextWidthBasis.values;
  _check(
  ...
  final probes = values
      .map(
        (value) => _EnumProbe<TextWidthBasis>(
          value: value,
          name: value.name,
          index: value.index,
        ),
  ...
  final roundTripValid = values
      .map((value) => TextWidthBasis.values.byName(value.name) == value)
  ...
  try {
    TextWidthBasis.values.byName('__invalid_text_width_basis__');
  } catch (_) {
  ...
  final summaryLine =
      'TextWidthBasis summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('TextWidthBasis Test Summary'),
      Text(summaryLine),
