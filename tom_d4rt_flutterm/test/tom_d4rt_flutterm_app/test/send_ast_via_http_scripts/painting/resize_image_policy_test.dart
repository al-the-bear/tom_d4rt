...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting ResizeImagePolicy checks');

  final values = ResizeImagePolicy.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<ResizeImagePolicy>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => ResizeImagePolicy.values.byName(value.name) == value)
  ...
  try {
    ResizeImagePolicy.values.byName('__ResizeImagePolicy_invalid__');
  } catch (_) {
  ...
  final summary =
      'ResizeImagePolicy summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
  ...
  return ListView(
    ...
    children: [
      const Text(
        'ResizeImagePolicy Test Summary',
