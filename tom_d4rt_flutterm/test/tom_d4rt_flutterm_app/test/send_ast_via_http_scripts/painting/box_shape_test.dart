...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting BoxShape checks');

  final values = BoxShape.values;
  _check(
  ...
  final holders = values
      .map((value) => _EnumHolder<BoxShape>(value: value, label: value.name))
  ...
  final allRoundTrips = values
      .map((value) => BoxShape.values.byName(value.name) == value)
  ...
  try {
    BoxShape.values.byName('__BoxShape_invalid__');
  } catch (_) {
  ...
  final summary =
      'BoxShape summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
