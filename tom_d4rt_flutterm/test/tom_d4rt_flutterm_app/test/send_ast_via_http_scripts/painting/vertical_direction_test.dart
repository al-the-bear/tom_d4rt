...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting VerticalDirection enum verification');

  final values = VerticalDirection.values;
  _check(
  ...
  final probes = values
      .map(
        (value) => _EnumProbe<VerticalDirection>(
          value: value,
          name: value.name,
          index: value.index,
        ),
  ...
  final roundTripValid = values
      .map((value) => VerticalDirection.values.byName(value.name) == value)
  ...
  try {
    VerticalDirection.values.byName('__invalid_vertical_direction__');
  } catch (_) {
  ...
  final toStringIncludesType = values
      .every((value) => value.toString().startsWith('VerticalDirection.'));
  _check(
  ...
  final summaryLine =
      'VerticalDirection summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('VerticalDirection Test Summary'),
      Text(summaryLine),
