...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting FlutterLogoStyle checks');

  final values = FlutterLogoStyle.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<FlutterLogoStyle>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => FlutterLogoStyle.values.byName(value.name) == value)
  ...
  try {
    FlutterLogoStyle.values.byName('__FlutterLogoStyle_invalid__');
  } catch (_) {
  ...
  final summary =
      'FlutterLogoStyle summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
