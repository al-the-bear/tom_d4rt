...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting AxisDirection checks');
  ...
  final holders = values
      .map(
        (value) => _EnumHolder<AxisDirection>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => AxisDirection.values.byName(value.name) == value)
  ...
  try {
    AxisDirection.values.byName('__AxisDirection_invalid__');
  } catch (_) {
  ...
  final summary =
      'AxisDirection summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
