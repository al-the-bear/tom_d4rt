...
class CanvasOperationsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(40, size.height - 32), 30, clipPaint);
    canvas.restore();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Computed size: ${textPainter.size.width.toStringAsFixed(1)} x ${textPainter.size.height.toStringAsFixed(1)}',
          ),
...
dynamic build(BuildContext context) {
  ...
  final filterQualityPaint = Paint()
    ...
    ..color = Colors.purple;
  print(
    'FilterQuality Paint: filterQuality=${filterQualityPaint.filterQuality}',
  );
  ...
  print(
    'LinearGradient with rotation transform: ${transformedGradient.colors}',
  );

  final scaledLinear = linearGradient.scale(0.5);
  print('Scaled gradient to 50%: $scaledLinear');

  final lerpedGradient = LinearGradient.lerp(
    linearGradient,
    transformedGradient,
    0.5,
  );
  ...
  print('\nColorFilter usage:');
  final colorFilterMode = ColorFilter.mode(
  ...
  final edgeInsetsSymmetric = EdgeInsets.symmetric(
    horizontal: 24,
    vertical: 12,
  );
  ...
  final mergedStyle = comprehensiveTextStyle.merge(
  ...
  print(
    'StrutStyle: height=${strutStyle.height}, forceStrutHeight=${strutStyle.forceStrutHeight}',
  );
  ...
  return SingleChildScrollView(
    ...
    child: Column(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: Container(
                ...
                child: Center(
                  child: Text('Base', style: TextStyle(color: Colors.white)),
                ),
              ),
            ),
            Expanded(
            ...
            Expanded(
              child: Container(
                ...
                child: Center(
                  child: Text('Lerped', style: TextStyle(color: Colors.white)),
                ),
              ),
            ),
          ],
        ...
        buildInfoCard(
          'HSL',
          'H:${hslColor.hue.toStringAsFixed(0)}° S:${(hslColor.saturation * 100).toStringAsFixed(0)}% L:${(hslColor.lightness * 100).toStringAsFixed(0)}%',
        ),
        buildInfoCard(
          'HSV',
          'H:${hsvColor.hue.toStringAsFixed(0)}° S:${(hsvColor.saturation * 100).toStringAsFixed(0)}% V:${(hsvColor.value * 100).toStringAsFixed(0)}%',
        ),
        ...
        TextPainterDemo(
          text:
              'The TextPainter class is used to paint text onto a canvas. It can compute the width and height of text before painting. This is a longer text to demonstrate how TextPainter handles overflow.',
        ...
        Container(
          ...
          child: Center(child: Text('RoundedRectangleBorder shape')),
        ),
        SizedBox(height: 12),
        buildSubsectionTitle('FlutterLogoDecoration'),
        Container(height: 80, decoration: flutterLogoDecoration),
        buildInfoCard(
          'BoxPainter',
          'createBoxPainter() -> paint() -> dispose()',
        ),
