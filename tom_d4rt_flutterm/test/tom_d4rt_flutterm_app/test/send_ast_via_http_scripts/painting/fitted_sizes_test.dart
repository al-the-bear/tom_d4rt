...
dynamic build(BuildContext context) {
  ...
  results.add('FittedSizes: source=${fitted1.source}, dest=${fitted1.destination}');
  ...
  assert(fitted2.source.width == fitted2.source.height, 'Source should be square');
  assert(fitted2.destination.width == fitted2.destination.height, 'Dest should be square');
  results.add('Square FittedSizes: ${fitted2.source} -> ${fitted2.destination}');
  ...
  assert(containResult.source == inputSize, 'Contain source should be full input');
  results.add('BoxFit.contain: source=${containResult.source}, dest=${containResult.destination}');
  ...
  results.add('BoxFit.cover: source=${coverResult.source}, dest=${coverResult.destination}');
  ...
  results.add('BoxFit.fill: source=${fillResult.source}, dest=${fillResult.destination}');
  ...
  for (final fit in BoxFit.values) {
    ...
    results.add('BoxFit.${fit.name}: ${result.destination.width.toStringAsFixed(1)}x${result.destination.height.toStringAsFixed(1)}');
  ...
  final zeroDestResult = applyBoxFit(BoxFit.contain, Size(100.0, 100.0), Size.zero);
  results.add('Zero dest: ${zeroDestResult.destination}');
  print('Zero dest: ${zeroDestResult.destination}');

  // Test 12: Very small source
  final smallResult = applyBoxFit(BoxFit.contain, Size(1.0, 1.0), Size(100.0, 100.0));
  results.add('Small source: ${smallResult.destination}');
  print('Small source: ${smallResult.destination}');

  // Test 13: Very large source
  final largeResult = applyBoxFit(BoxFit.contain, Size(10000.0, 10000.0), Size(100.0, 100.0));
  ...
  for (final ar in aspectRatios) {
    ...
    results.add('Aspect ${ar['name']}: ${result.destination.width.toStringAsFixed(0)}x${result.destination.height.toStringAsFixed(0)}');
  ...
  assert(fittedA.destination == fittedB.destination, 'Destinations should match');
  results.add('Equality: A==B (same) ${fittedA.source == fittedB.source && fittedA.destination == fittedB.destination}');
  results.add('Inequality: A!=C (diff dest) ${fittedA.destination != fittedC.destination}');
  ...
  return Column(
    ...
    children: [
      Text('FittedSizes Tests', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
