...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting WebHtmlElementStrategy enum verification');

  final values = WebHtmlElementStrategy.values;
  _check(
  ...
  final probes = values
      .map(
        (value) => _EnumProbe<WebHtmlElementStrategy>(
          value: value,
          name: value.name,
          index: value.index,
        ),
  ...
  final roundTripValid = values
      .map((value) => WebHtmlElementStrategy.values.byName(value.name) == value)
  ...
  final summaryLine =
      'WebHtmlElementStrategy summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('WebHtmlElementStrategy Test Summary'),
      Text(summaryLine),
