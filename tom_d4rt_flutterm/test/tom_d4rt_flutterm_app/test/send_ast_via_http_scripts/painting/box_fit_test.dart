...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting BoxFit checks');

  final values = BoxFit.values;
  _check(
  ...
  final holders = values
      .map((value) => _EnumHolder<BoxFit>(value: value, label: value.name))
  ...
  final allRoundTrips = values
      .map((value) => BoxFit.values.byName(value.name) == value)
  ...
  try {
    BoxFit.values.byName('__BoxFit_invalid__');
  } catch (_) {
  ...
  final summary =
      'BoxFit summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
