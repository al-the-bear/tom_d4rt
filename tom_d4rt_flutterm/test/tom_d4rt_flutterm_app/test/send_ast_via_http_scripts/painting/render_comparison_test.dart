...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting RenderComparison checks');

  final values = RenderComparison.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<RenderComparison>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => RenderComparison.values.byName(value.name) == value)
  ...
  try {
    RenderComparison.values.byName('__RenderComparison_invalid__');
  } catch (_) {
  ...
  final summary =
      'RenderComparison summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
