...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting ImageRepeat checks');

  final values = ImageRepeat.values;
  _check(
  ...
  final holders = values
      .map((value) => _EnumHolder<ImageRepeat>(value: value, label: value.name))
  ...
  final allRoundTrips = values
      .map((value) => ImageRepeat.values.byName(value.name) == value)
  ...
  try {
    ImageRepeat.values.byName('__ImageRepeat_invalid__');
  } catch (_) {
  ...
  final summary =
      'ImageRepeat summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
