...
dynamic build(BuildContext context) {
  ...
  final holders = values
      .map((value) => _EnumHolder<Axis>(value: value, label: value.name))
  ...
  final allRoundTrips = values
      .map((value) => Axis.values.byName(value.name) == value)
  ...
  try {
    Axis.values.byName('__Axis_invalid__');
  } catch (_) {
  ...
  final summary =
      'Axis summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
