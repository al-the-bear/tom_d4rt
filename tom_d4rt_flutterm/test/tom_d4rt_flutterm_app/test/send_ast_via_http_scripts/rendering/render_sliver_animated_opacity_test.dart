// D4rt test script: Comprehensive checks for RenderSliverAnimatedOpacity in rendering
...
Widget _buildRelatedWidgetForRenderSliverAnimatedOpacity() {
  if ('RenderSliverAnimatedOpacity'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSliverAnimatedOpacity sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSliverAnimatedOpacity host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSliverAnimatedOpacity: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSliverAnimatedOpacity;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSliverAnimatedOpacity'));
  ...
  mark('Direct class reference resolved for RenderSliverAnimatedOpacity');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Sliver Animated Opacity',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSliverAnimatedOpacity();
  ...
  print(
    'RenderSliverAnimatedOpacity test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Sliver Animated Opacity'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSliverAnimatedOpacity checks passed'),
    ],
  );
}
