// D4rt test script: Comprehensive checks for RenderShaderMask in rendering
...
Widget _buildRelatedWidgetForRenderShaderMask() {
  if ('RenderShaderMask'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderShaderMask sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderShaderMask host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) =>
              print('RenderShaderMask: pointer down observed via Listener'),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Widget relatedWidgetA = _buildRelatedWidgetForRenderShaderMask();
  final Widget relatedWidgetB = _buildRelatedWidgetForRenderShaderMask();
  ...
  print(
    'RenderShaderMask test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Shader Mask'),
      const SizedBox(height: 6),
      _buildRelatedWidgetForRenderShaderMask(),
      const SizedBox(height: 8),
      const Text('Summary:'),
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderShaderMask checks passed'),
    ],
  );
}
