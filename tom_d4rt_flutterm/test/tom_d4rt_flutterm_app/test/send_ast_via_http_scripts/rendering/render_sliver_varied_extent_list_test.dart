// D4rt test script: comprehensive checks for RenderSliverVariedExtentList
...
class _RenderSliverVariedExtentListScript extends StatelessWidget {
  const _RenderSliverVariedExtentListScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverVariedExtentList');

    final Type targetType = RenderSliverVariedExtentList;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_varied_extent_list_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverVariedExtentList',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverVariedExtentList');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = SliverVariedExtentList.builder(
      itemExtentBuilder: (int index, SliverLayoutDimensions dimensions) => 18 + (index * 6),
      itemBuilder: (BuildContext context, int index) => Text('varied-$index'),
      itemCount: 4,
    );
    ...
    final String normalized = 'RenderSliverVariedExtentList'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverVariedExtentList'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverVariedExtentList] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverVariedExtentList'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverVariedExtentList');
  return const _RenderSliverVariedExtentListScript();
}
