// D4rt test script: Comprehensive PlatformViewHitTestBehavior coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(PlatformViewHitTestBehavior value) => '${value.name}#${value.index}';
  ...
  for (final value in values) {
    ...
    final byName = PlatformViewHitTestBehavior.values.byName(value.name);
    assert(identical(byName, value));
    assert(PlatformViewHitTestBehavior.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <PlatformViewHitTestBehavior, String>{
    for (final value in values) value: 'platform-view-hit-test:${value.name}',
  };
  ...
  try {
    PlatformViewHitTestBehavior.values.byName('__not_a_platform_view_hit_test_behavior__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => PlatformViewHitTestBehavior.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('PlatformViewHitTestBehavior Tests'),
