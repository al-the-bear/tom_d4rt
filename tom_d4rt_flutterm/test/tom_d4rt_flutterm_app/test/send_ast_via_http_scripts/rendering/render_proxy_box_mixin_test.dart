...
class _ProxyProbe extends RenderBox
    with
...
dynamic build(BuildContext context) {
  ...
  final child = RenderConstrainedBox(
    additionalConstraints: const BoxConstraints.tightFor(width: 22, height: 11),
  );

  _check(
    logs: logs,
    label: 'probe instantiated',
    condition: probe is RenderProxyBoxMixin<RenderBox>,
  );
  ...
  _check(
    logs: logs,
    label: 'layout proxies child size',
    condition: probe.size == const Size(22, 11),
  );
  _check(
    logs: logs,
    label: 'child retained',
    condition: identical(probe.child, child),
  );
  ...
  _check(
    logs: logs,
    label: 'intrinsics are valid numbers',
    condition: minW >= 0 && maxW >= minW,
  );
  ...
  _check(
    logs: logs,
    label: 'height intrinsics are valid numbers',
    condition: minH >= 0 && maxH >= minH,
  );
  ...
  _check(
    logs: logs,
    label: 'no-child layout falls back to constraints.smallest',
    condition: noChild.size == const Size(4, 3),
  );
  ...
  _check(
    logs: logs,
    label: 'hitTestChildren returns bool',
    condition: hit is bool,
  );
  ...
  print(
    'RenderProxyBoxMixin comprehensive test finished: pass=$pass fail=$fail',
  );
