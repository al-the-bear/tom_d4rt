// D4rt comprehensive test script for DebugOverflowIndicatorMixin
...
dynamic build(BuildContext context) {
  const targetClassName = 'DebugOverflowIndicatorMixin';
  const testLabel = 'DebugOverflowIndicatorMixin';
  final logs = <String>[];
  ...
  final flexA = RenderFlex(direction: Axis.horizontal);
  final flexB = RenderFlex(direction: Axis.vertical);
  expectCondition(flexA is RenderBox, 'flexA is RenderBox', logs, counters);
  expectCondition(flexA is DebugOverflowIndicatorMixin, 'RenderFlex uses DebugOverflowIndicatorMixin indirectly', logs, counters);
  expectCondition(flexB.direction == Axis.vertical, 'axis property supports vertical value', logs, counters);
  expectCondition(flexA.toStringShort().contains('RenderFlex'), 'toStringShort includes class name', logs, counters);
  details['flexA'] = flexA.runtimeType.toString();
  details['flexB'] = flexB.runtimeType.toString();
