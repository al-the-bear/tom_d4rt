// D4rt test script: comprehensive checks for RenderSliverFloatingPinnedPersistentHeader
...
class _RenderSliverFloatingPinnedPersistentHeaderScript extends StatelessWidget {
  const _RenderSliverFloatingPinnedPersistentHeaderScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverFloatingPinnedPersistentHeader');

    final Type targetType = RenderSliverFloatingPinnedPersistentHeader;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_floating_pinned_persistent_header_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverFloatingPinnedPersistentHeader',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverFloatingPinnedPersistentHeader');
    ...
    final Widget indirectWidget = SliverPersistentHeader(
      floating: true,
      pinned: true,
      delegate: _SimpleHeaderDelegate(label: 'floating-pinned', maxExtentValue: 80, minExtentValue: 36),
    );
    ...
    final String normalized = 'RenderSliverFloatingPinnedPersistentHeader'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverFloatingPinnedPersistentHeader'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverFloatingPinnedPersistentHeader] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverFloatingPinnedPersistentHeader'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverFloatingPinnedPersistentHeader');
  return const _RenderSliverFloatingPinnedPersistentHeaderScript();
}
