// D4rt test script: comprehensive checks for RenderSliverScrollingPersistentHeader
...
class _RenderSliverScrollingPersistentHeaderScript extends StatelessWidget {
  const _RenderSliverScrollingPersistentHeaderScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverScrollingPersistentHeader');

    final Type targetType = RenderSliverScrollingPersistentHeader;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_scrolling_persistent_header_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverScrollingPersistentHeader',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverScrollingPersistentHeader');
    ...
    final Widget indirectWidget = SliverPersistentHeader(
      floating: false,
      pinned: false,
      delegate: _SimpleHeaderDelegate(label: 'scrolling-header', maxExtentValue: 68, minExtentValue: 26),
    );
    ...
    final String normalized = 'RenderSliverScrollingPersistentHeader'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverScrollingPersistentHeader'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverScrollingPersistentHeader] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverScrollingPersistentHeader'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverScrollingPersistentHeader');
  return const _RenderSliverScrollingPersistentHeaderScript();
}
