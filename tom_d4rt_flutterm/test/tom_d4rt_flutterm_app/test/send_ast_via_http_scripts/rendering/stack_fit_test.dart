// D4rt test script: Comprehensive StackFit coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(StackFit value) => '${value.name}#${value.index}';
  ...
  for (final value in values) {
    ...
    final byName = StackFit.values.byName(value.name);
    assert(identical(byName, value));
    assert(StackFit.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <StackFit, String>{
    for (final value in values) value: 'stack-fit:${value.name}',
  };
  ...
  try {
    StackFit.values.byName('__not_a_stack_fit__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => StackFit.values.byName(value.name))
