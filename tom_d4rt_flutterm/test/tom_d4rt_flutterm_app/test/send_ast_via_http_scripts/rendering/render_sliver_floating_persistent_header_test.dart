// D4rt test script: comprehensive checks for RenderSliverFloatingPersistentHeader
...
class _RenderSliverFloatingPersistentHeaderScript extends StatelessWidget {
  const _RenderSliverFloatingPersistentHeaderScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverFloatingPersistentHeader');

    final Type targetType = RenderSliverFloatingPersistentHeader;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_floating_persistent_header_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverFloatingPersistentHeader',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverFloatingPersistentHeader');
    ...
    final Widget indirectWidget = SliverPersistentHeader(
      floating: true,
      pinned: false,
      delegate: _SimpleHeaderDelegate(label: 'floating-only', maxExtentValue: 72, minExtentValue: 32),
    );
    ...
    final String normalized = 'RenderSliverFloatingPersistentHeader'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverFloatingPersistentHeader'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverFloatingPersistentHeader] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverFloatingPersistentHeader'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverFloatingPersistentHeader');
  return const _RenderSliverFloatingPersistentHeaderScript();
}
