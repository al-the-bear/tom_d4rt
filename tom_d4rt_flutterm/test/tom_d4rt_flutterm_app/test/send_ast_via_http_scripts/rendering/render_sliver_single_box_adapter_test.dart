// D4rt test script: comprehensive checks for RenderSliverSingleBoxAdapter
...
class _RenderSliverSingleBoxAdapterScript extends StatelessWidget {
  const _RenderSliverSingleBoxAdapterScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverSingleBoxAdapter');

    final Type targetType = RenderSliverSingleBoxAdapter;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_single_box_adapter_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverSingleBoxAdapter',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverSingleBoxAdapter');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = const SliverToBoxAdapter(
      child: SizedBox(height: 24, child: Text('single-box')),
    );
    ...
    final String normalized = 'RenderSliverSingleBoxAdapter'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverSingleBoxAdapter'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverSingleBoxAdapter] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverSingleBoxAdapter'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverSingleBoxAdapter');
  return const _RenderSliverSingleBoxAdapterScript();
}
