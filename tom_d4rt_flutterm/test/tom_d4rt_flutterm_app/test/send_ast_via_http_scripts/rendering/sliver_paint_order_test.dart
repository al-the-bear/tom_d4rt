// D4rt test script: Comprehensive SliverPaintOrder coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(SliverPaintOrder value) => '${value.name}#${value.index}';
  ...
  final values = SliverPaintOrder.values;
  ...
  for (final value in values) {
    ...
    final byName = SliverPaintOrder.values.byName(value.name);
    assert(identical(byName, value));
    assert(SliverPaintOrder.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SliverPaintOrder, String>{
    for (final value in values) value: 'sliver-paint-order:${value.name}',
  };
  ...
  try {
    SliverPaintOrder.values.byName('__not_a_sliver_paint_order__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => SliverPaintOrder.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SliverPaintOrder Tests'),
