...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting DecorationPosition enum verification');

  final values = DecorationPosition.values;
  _check(
  ...
  final probes = values
      .map(
        (value) => _EnumProbe<DecorationPosition>(
          value: value,
          name: value.name,
          index: value.index,
        ),
  ...
  final roundTripValid = values
      .map((value) => DecorationPosition.values.byName(value.name) == value)
  ...
  try {
    DecorationPosition.values.byName('__invalid_decoration_position__');
  } catch (_) {
  ...
  final toStringIncludesType = values
      .every((value) => value.toString().startsWith('DecorationPosition.'));
  _check(
  ...
  final summaryLine =
      'DecorationPosition summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('DecorationPosition Test Summary'),
      Text(summaryLine),
