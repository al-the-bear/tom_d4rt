// D4rt test script: Comprehensive RenderingServiceExtensions coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(RenderingServiceExtensions value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = RenderingServiceExtensions.values;
  ...
  for (final value in values) {
    ...
    final byName = RenderingServiceExtensions.values.byName(value.name);
    assert(identical(byName, value));
    assert(RenderingServiceExtensions.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <RenderingServiceExtensions, String>{
    for (final value in values) value: 'service-extension:${value.name}',
  };
  ...
  try {
    RenderingServiceExtensions.values.byName('__not_a_rendering_service_extension__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => RenderingServiceExtensions.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('RenderingServiceExtensions Tests'),
