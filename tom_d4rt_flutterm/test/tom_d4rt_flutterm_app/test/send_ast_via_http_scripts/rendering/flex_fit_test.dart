...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting FlexFit enum verification');

  final values = FlexFit.values;
  _check(
  ...
  final roundTripValid = values
      .map((value) => FlexFit.values.byName(value.name) == value)
  ...
  try {
    FlexFit.values.byName('__invalid_flex_fit__');
  } catch (_) {
  ...
  final toStringIncludesType =
      values.every((value) => value.toString().startsWith('FlexFit.'));
  _check(
  ...
  final summaryLine =
      'FlexFit summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('FlexFit Test Summary'),
      Text(summaryLine),
