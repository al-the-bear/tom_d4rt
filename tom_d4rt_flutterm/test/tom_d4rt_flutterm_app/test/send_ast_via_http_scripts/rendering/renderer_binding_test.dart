...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'WidgetsFlutterBinding initialized',
    condition: binding is WidgetsBinding,
  );

  final renderer = RendererBinding.instance;
  _check(
    logs: logs,
    label: 'RendererBinding.instance available',
    condition: renderer is RendererBinding,
  );

  final rootOwner = renderer.rootPipelineOwner;
  _check(
    logs: logs,
    label: 'rootPipelineOwner exists',
    condition: rootOwner is PipelineOwner,
  );

  final renderViews = renderer.renderViews;
  _check(
    logs: logs,
    label: 'renderViews iterable exists',
    condition: renderViews is Iterable<RenderView>,
  );

  final list = renderViews.toList(growable: false);
  _check(
    logs: logs,
    label: 'renderViews toList works',
  ...
  _check(
    logs: logs,
    label: 'defer/allow first frame sequence executed',
    condition: true,
  );

  final hit = HitTestResult();
  final hasHit = renderer.hitTestInView(
    hit,
    const Offset(0, 0),
    renderer.platformDispatcher.implicitView!.viewId,
  );
  _check(
    logs: logs,
    label: 'hitTestInView returns bool',
    condition: hasHit is bool,
  );

  final sameRenderer = identical(renderer, RendererBinding.instance);
  _check(
    logs: logs,
    label: 'RendererBinding singleton stable',
    condition: sameRenderer,
  );
