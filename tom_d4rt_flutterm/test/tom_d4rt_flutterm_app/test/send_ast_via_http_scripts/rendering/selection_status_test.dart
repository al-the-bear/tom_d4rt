// D4rt test script: Comprehensive SelectionStatus coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(SelectionStatus value) => '${value.name}#${value.index}';
  ...
  final values = SelectionStatus.values;
  ...
  for (final value in values) {
    ...
    final byName = SelectionStatus.values.byName(value.name);
    assert(identical(byName, value));
    assert(SelectionStatus.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SelectionStatus, String>{
    for (final value in values) value: 'selection-status:${value.name}',
  };
  ...
  try {
    SelectionStatus.values.byName('__not_a_selection_status__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => SelectionStatus.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SelectionStatus Tests'),
