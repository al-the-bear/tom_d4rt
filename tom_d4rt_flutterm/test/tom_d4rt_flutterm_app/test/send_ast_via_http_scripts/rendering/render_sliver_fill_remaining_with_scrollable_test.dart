// D4rt test script: Comprehensive checks for RenderSliverFillRemainingWithScrollable in rendering
...
Widget _buildRelatedWidgetForRenderSliverFillRemainingWithScrollable() {
  if ('RenderSliverFillRemainingWithScrollable'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSliverFillRemainingWithScrollable sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSliverFillRemainingWithScrollable host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSliverFillRemainingWithScrollable: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSliverFillRemainingWithScrollable;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSliverFillRemainingWithScrollable'));
  ...
  mark(
    'Direct class reference resolved for RenderSliverFillRemainingWithScrollable',
  );

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Sliver Fill Remaining With Scrollable',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSliverFillRemainingWithScrollable();
  final Widget relatedWidgetB =
      _buildRelatedWidgetForRenderSliverFillRemainingWithScrollable();
  ...
  if (sliverCase) {
    print(
      'Edge case: Sliver lifecycle and constraint propagation are relevant.',
    );
    print(
      'Edge case: cross-axis / main-axis extents can produce unusual layouts.',
    );
    mark('Sliver-specific edge notes captured');
  } else {
    print(
      'Edge case: Box render object interactions and hit testing are relevant.',
    );
    print(
      'Edge case: painting / compositing behavior can vary with child presence.',
    );
  ...
  print(
    'RenderSliverFillRemainingWithScrollable test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Sliver Fill Remaining With Scrollable'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSliverFillRemainingWithScrollable checks passed'),
    ],
  );
}
