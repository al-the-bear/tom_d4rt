...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting GrowthDirection enum verification');

  final values = GrowthDirection.values;
  _check(
  ...
  final probes = values
      .map(
        (value) => _EnumProbe<GrowthDirection>(
          value: value,
          name: value.name,
          index: value.index,
        ),
  ...
  final roundTripValid = values
      .map((value) => GrowthDirection.values.byName(value.name) == value)
  ...
  try {
    GrowthDirection.values.byName('__invalid_growth_direction__');
  } catch (_) {
  ...
  final toStringIncludesType = values
      .every((value) => value.toString().startsWith('GrowthDirection.'));
  _check(
  ...
  final summaryLine =
      'GrowthDirection summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('GrowthDirection Test Summary'),
      Text(summaryLine),
