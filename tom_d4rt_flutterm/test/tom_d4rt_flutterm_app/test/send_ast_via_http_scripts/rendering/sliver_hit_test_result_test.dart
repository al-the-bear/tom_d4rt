...
class TestRenderSliver extends RenderSliver {
  ...
  @override
  String toString({DiagnosticLevel minLevel = DiagnosticLevel.info}) => 'TestRenderSliver($name)';
}
...
dynamic build(BuildContext context) {
  ...
  for (var i = 0; i < slivers.length; i++) {
    multiResult.add(SliverHitTestEntry(
      slivers[i],
  ...
  for (final e in multiResult.path) {
    if (e is SliverHitTestEntry) {
      print('  - ${e.target}: main=${e.mainAxisPosition}, cross=${e.crossAxisPosition}');
    }
  }
  ...
  for (var i = 0; i < 5; i++) {
    accumulatedResult.add(SliverHitTestEntry(
  ...
  complexResult.add(SliverHitTestEntry(parentSliver, mainAxisPosition: 0.0, crossAxisPosition: 0.0));
  complexResult.add(SliverHitTestEntry(child1, mainAxisPosition: 50.0, crossAxisPosition: 0.0));
  complexResult.add(SliverHitTestEntry(child2, mainAxisPosition: 100.0, crossAxisPosition: 0.0));
  ...
  final iterableCount = complexResult.path.where((e) => e is SliverHitTestEntry).length;
