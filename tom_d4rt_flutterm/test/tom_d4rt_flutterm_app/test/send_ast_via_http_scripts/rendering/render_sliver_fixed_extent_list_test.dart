// D4rt test script: comprehensive checks for RenderSliverFixedExtentList
...
class _RenderSliverFixedExtentListScript extends StatelessWidget {
  const _RenderSliverFixedExtentListScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverFixedExtentList');

    final Type targetType = RenderSliverFixedExtentList;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_fixed_extent_list_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverFixedExtentList',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverFixedExtentList');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = SliverFixedExtentList(
      itemExtent: 24,
      delegate: SliverChildListDelegate.fixed([
        const SizedBox(height: 24, child: Text('row-a')),
        const SizedBox(height: 24, child: Text('row-b')),
      ]),
    );
    ...
    final String normalized = 'RenderSliverFixedExtentList'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverFixedExtentList'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverFixedExtentList] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverFixedExtentList'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverFixedExtentList');
  return const _RenderSliverFixedExtentListScript();
}
