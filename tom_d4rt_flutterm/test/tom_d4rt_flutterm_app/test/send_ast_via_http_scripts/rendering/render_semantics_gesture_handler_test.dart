// D4rt test script: Comprehensive checks for RenderSemanticsGestureHandler in rendering
...
Widget _buildRelatedWidgetForRenderSemanticsGestureHandler() {
  if ('RenderSemanticsGestureHandler'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSemanticsGestureHandler sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSemanticsGestureHandler host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSemanticsGestureHandler: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSemanticsGestureHandler;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSemanticsGestureHandler'));
  ...
  mark('Direct class reference resolved for RenderSemanticsGestureHandler');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Semantics Gesture Handler',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSemanticsGestureHandler();
  ...
  print(
    'RenderSemanticsGestureHandler test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Semantics Gesture Handler'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSemanticsGestureHandler checks passed'),
    ],
  );
}
