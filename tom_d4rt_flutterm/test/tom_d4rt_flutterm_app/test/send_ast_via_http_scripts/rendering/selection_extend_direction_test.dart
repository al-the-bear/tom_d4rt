// D4rt test script: Comprehensive SelectionExtendDirection coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = SelectionExtendDirection.values.byName(value.name);
    assert(identical(byName, value));
    assert(SelectionExtendDirection.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SelectionExtendDirection, String>{
    for (final value in values) value: 'selection-extend:${value.name}',
  };
  ...
  final roundTripStable = values
      .map((value) => SelectionExtendDirection.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SelectionExtendDirection Tests'),
