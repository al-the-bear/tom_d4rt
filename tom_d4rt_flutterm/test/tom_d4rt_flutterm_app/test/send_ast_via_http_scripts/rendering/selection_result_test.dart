// D4rt test script: Comprehensive SelectionResult coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(SelectionResult value) => '${value.name}#${value.index}';
  ...
  final values = SelectionResult.values;
  ...
  for (final value in values) {
    ...
    final byName = SelectionResult.values.byName(value.name);
    assert(identical(byName, value));
    assert(SelectionResult.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SelectionResult, String>{
    for (final value in values) value: 'selection-result:${value.name}',
  };
  ...
  try {
    SelectionResult.values.byName('__not_a_selection_result__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => SelectionResult.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SelectionResult Tests'),
