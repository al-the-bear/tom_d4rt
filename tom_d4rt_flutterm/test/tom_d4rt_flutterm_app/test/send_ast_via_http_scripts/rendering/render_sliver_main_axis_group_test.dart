// D4rt test script: comprehensive checks for RenderSliverMainAxisGroup
...
class _RenderSliverMainAxisGroupScript extends StatelessWidget {
  const _RenderSliverMainAxisGroupScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverMainAxisGroup');

    final Type targetType = RenderSliverMainAxisGroup;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_main_axis_group_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverMainAxisGroup',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverMainAxisGroup');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = SliverMainAxisGroup(
      slivers: const [
        SliverToBoxAdapter(child: Text('group-a')),
        SliverToBoxAdapter(child: Text('group-b')),
      ],
    );
    ...
    final String normalized = 'RenderSliverMainAxisGroup'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverMainAxisGroup'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverMainAxisGroup] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverMainAxisGroup'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverMainAxisGroup');
  return const _RenderSliverMainAxisGroupScript();
}
