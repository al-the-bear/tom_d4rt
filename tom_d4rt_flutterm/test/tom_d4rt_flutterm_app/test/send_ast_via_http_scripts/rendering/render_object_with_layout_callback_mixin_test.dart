...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    box is RenderObjectWithLayoutCallbackMixin,
    'Custom render box mixes in RenderObjectWithLayoutCallbackMixin',
    logs,
    counters,
  );
  ...
  expectCondition(
    box.size.width == 40,
    'Layout width matches tight constraint',
    logs,
    counters,
  );
  expectCondition(
    box.size.height == 24,
    'Layout height matches tight constraint',
    logs,
    counters,
  );
  expectCondition(
    box.layoutCallbackCount == 1,
    'Layout callback ran once on first layout',
    logs,
    counters,
  );
  ...
  expectCondition(
    box.layoutCallbackCount >= 2,
    'Layout callback runs again after scheduling',
    logs,
    counters,
  );
  expectCondition(
    box.size.width == 60,
    'Second layout updates width',
    logs,
    counters,
  );
  expectCondition(
    box.size.height == 30,
    'Second layout updates height',
    logs,
    counters,
  );
  for (var i = 0; i < 8; i++) {
    ...
    box.layout(
      BoxConstraints.tightFor(
        width: 20 + i.toDouble(),
        height: 10 + i.toDouble(),
      ),
    );
    expectCondition(
      box.layoutCallbackCount >= 3 + i,
      'Callback count increases after scheduled layout',
      logs,
      counters,
    );
  }
  expectCondition(
    box.layoutCallbackCount >= 10,
    'Multiple callbacks executed',
    logs,
    counters,
  );
  expectCondition(
    box.toStringShort().isNotEmpty,
    'Render object short string is available',
    logs,
    counters,
  );
  expectCondition(
...
class _LayoutCallbackRenderBox extends RenderBox
