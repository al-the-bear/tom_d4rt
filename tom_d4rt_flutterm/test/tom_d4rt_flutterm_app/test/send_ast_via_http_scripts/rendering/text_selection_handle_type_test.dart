// D4rt test script: Comprehensive TextSelectionHandleType coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(TextSelectionHandleType value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = TextSelectionHandleType.values;
  ...
  for (final value in values) {
    ...
    final byName = TextSelectionHandleType.values.byName(value.name);
    assert(identical(byName, value));
    assert(TextSelectionHandleType.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <TextSelectionHandleType, String>{
  ...
  try {
    TextSelectionHandleType.values.byName('__not_a_text_selection_handle_type__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => TextSelectionHandleType.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('TextSelectionHandleType Tests'),
