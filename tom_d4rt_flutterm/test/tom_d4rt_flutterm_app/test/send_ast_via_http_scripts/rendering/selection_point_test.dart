...
dynamic build(BuildContext context) {
  ...
  results.add('TextSelectionHandleType values: ${TextSelectionHandleType.values.length}');
  ...
  final selectionWidth = endSelection.localPosition.dx - startSelection.localPosition.dx;
  ...
  for (var line = 0; line < 5; line++) {
    linePoints.add(SelectionPoint(
      localPosition: Offset(0.0, line * 20.0),
      lineHeight: 18.0,
      handleType: line == 0 ? TextSelectionHandleType.left : TextSelectionHandleType.right,
    ));
  }
  print('Created ${linePoints.length} line selection points');
  for (var i = 0; i < linePoints.length; i++) {
    print('  Line $i: y=${linePoints[i].localPosition.dy}, handle=${linePoints[i].handleType}');
  }
