// D4rt test script: Comprehensive TextGranularity coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(TextGranularity value) => '${value.name}#${value.index}';
  ...
  final values = TextGranularity.values;
  ...
  for (final value in values) {
    ...
    final byName = TextGranularity.values.byName(value.name);
    assert(identical(byName, value));
    assert(TextGranularity.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <TextGranularity, String>{
  ...
  try {
    TextGranularity.values.byName('__not_a_text_granularity__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => TextGranularity.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('TextGranularity Tests'),
