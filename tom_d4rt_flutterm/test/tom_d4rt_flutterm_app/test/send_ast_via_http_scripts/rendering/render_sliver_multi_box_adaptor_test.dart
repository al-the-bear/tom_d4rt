// D4rt test script: comprehensive checks for RenderSliverMultiBoxAdaptor
...
class _RenderSliverMultiBoxAdaptorScript extends StatelessWidget {
  const _RenderSliverMultiBoxAdaptorScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverMultiBoxAdaptor');

    final Type targetType = RenderSliverMultiBoxAdaptor;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_multi_box_adaptor_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverMultiBoxAdaptor',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverMultiBoxAdaptor');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = SliverList(
      delegate: SliverChildBuilderDelegate(
        (BuildContext context, int index) => Text('cell-$index'),
        childCount: 3,
      ),
    );
    ...
    final String normalized = 'RenderSliverMultiBoxAdaptor'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverMultiBoxAdaptor'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverMultiBoxAdaptor] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverMultiBoxAdaptor'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverMultiBoxAdaptor');
  return const _RenderSliverMultiBoxAdaptorScript();
}
