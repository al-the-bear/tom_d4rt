// D4rt test script: Comprehensive WrapAlignment coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = WrapAlignment.values.byName(value.name);
    assert(identical(byName, value));
    assert(WrapAlignment.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <WrapAlignment, String>{
  ...
  try {
    WrapAlignment.values.byName('__not_a_wrap_alignment__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => WrapAlignment.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('WrapAlignment Tests'),
