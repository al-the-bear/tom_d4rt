...
dynamic build(BuildContext context) {
  ...
  _check(
    ...
    condition: probe is RelayoutWhenSystemFontsChangeMixin,
  );

  probe.layout(const BoxConstraints.tightFor(width: 20, height: 15));
  _check(
    logs: logs,
    label: 'layout assigns size',
    condition: probe.size == const Size(20, 15),
  );

  probe.systemFontsDidChange();
  _check(
    logs: logs,
    label: 'systemFontsDidChange increments counter',
    condition: probe.didChangeCount == 1,
  );
  _check(
    logs: logs,
    label: 'markNeedsLayout flag can be set',
    condition: probe.debugNeedsLayout || !kDebugMode,
  );

  probe.systemFontsDidChange();
  _check(
    logs: logs,
    label: 'second callback increments counter',
    condition: probe.didChangeCount == 2,
  );
  ...
  _check(
    logs: logs,
    label: 'detached object handles callback',
    condition: detached.didChangeCount == 1,
  );

  final list = <RenderObject>[probe, detached];
  _check(
    logs: logs,
    label: 'objects are render objects',
    condition: list.every((e) => e is RenderObject),
  );
  ...
  print(
    'RelayoutWhenSystemFontsChangeMixin comprehensive test finished: pass=$pass fail=$fail',
  );
