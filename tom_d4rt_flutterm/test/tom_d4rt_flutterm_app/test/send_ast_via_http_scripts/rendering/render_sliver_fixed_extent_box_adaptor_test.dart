// D4rt test script: Comprehensive checks for RenderSliverFixedExtentBoxAdaptor in rendering
...
Widget _buildRelatedWidgetForRenderSliverFixedExtentBoxAdaptor() {
  if ('RenderSliverFixedExtentBoxAdaptor'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSliverFixedExtentBoxAdaptor sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSliverFixedExtentBoxAdaptor host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSliverFixedExtentBoxAdaptor: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSliverFixedExtentBoxAdaptor;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSliverFixedExtentBoxAdaptor'));
  ...
  mark('Direct class reference resolved for RenderSliverFixedExtentBoxAdaptor');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Sliver Fixed Extent Box Adaptor',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSliverFixedExtentBoxAdaptor();
  ...
  print(
    'RenderSliverFixedExtentBoxAdaptor test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Sliver Fixed Extent Box Adaptor'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSliverFixedExtentBoxAdaptor checks passed'),
    ],
  );
}
