// D4rt test script: Comprehensive SelectionEventType coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(SelectionEventType value) => '${value.name}#${value.index}';
  ...
  final values = SelectionEventType.values;
  ...
  for (final value in values) {
    ...
    final byName = SelectionEventType.values.byName(value.name);
    assert(identical(byName, value));
    assert(SelectionEventType.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SelectionEventType, String>{
    for (final value in values) value: 'selection-event:${value.name}',
  };
  ...
  try {
    SelectionEventType.values.byName('__not_a_selection_event_type__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => SelectionEventType.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SelectionEventType Tests'),
