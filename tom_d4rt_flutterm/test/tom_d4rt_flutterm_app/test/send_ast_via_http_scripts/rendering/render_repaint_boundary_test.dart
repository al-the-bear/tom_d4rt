// D4rt test script: Comprehensive checks for RenderRepaintBoundary in rendering
...
Widget _buildRelatedWidgetForRenderRepaintBoundary() {
  if ('RenderRepaintBoundary'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderRepaintBoundary sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderRepaintBoundary host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderRepaintBoundary: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  mark('Direct class reference resolved for RenderRepaintBoundary');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Repaint Boundary',
  ...
  final Widget relatedWidgetA = _buildRelatedWidgetForRenderRepaintBoundary();
  final Widget relatedWidgetB = _buildRelatedWidgetForRenderRepaintBoundary();
  ...
  print(
    'RenderRepaintBoundary test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Repaint Boundary'),
      const SizedBox(height: 6),
      _buildRelatedWidgetForRenderRepaintBoundary(),
      const SizedBox(height: 8),
      const Text('Summary:'),
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderRepaintBoundary checks passed'),
    ],
  );
}
