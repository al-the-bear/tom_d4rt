// D4rt test script: Comprehensive TableCellVerticalAlignment coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = TableCellVerticalAlignment.values.byName(value.name);
    assert(identical(byName, value));
    assert(TableCellVerticalAlignment.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <TableCellVerticalAlignment, String>{
  ...
  final roundTripStable = values
      .map((value) => TableCellVerticalAlignment.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('TableCellVerticalAlignment Tests'),
