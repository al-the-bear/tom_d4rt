// D4rt test script: Comprehensive checks for RenderShrinkWrappingViewport in rendering
...
Widget _buildRelatedWidgetForRenderShrinkWrappingViewport() {
  if ('RenderShrinkWrappingViewport'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderShrinkWrappingViewport sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderShrinkWrappingViewport host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderShrinkWrappingViewport: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderShrinkWrappingViewport;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderShrinkWrappingViewport'));
  ...
  mark('Direct class reference resolved for RenderShrinkWrappingViewport');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Shrink Wrapping Viewport',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderShrinkWrappingViewport();
  ...
  print(
    'RenderShrinkWrappingViewport test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Shrink Wrapping Viewport'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderShrinkWrappingViewport checks passed'),
    ],
  );
}
