// D4rt test script: Comprehensive checks for RenderSliverFillViewport in rendering
...
Widget _buildRelatedWidgetForRenderSliverFillViewport() {
  if ('RenderSliverFillViewport'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSliverFillViewport sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSliverFillViewport host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSliverFillViewport: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSliverFillViewport;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSliverFillViewport'));
  ...
  mark('Direct class reference resolved for RenderSliverFillViewport');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Sliver Fill Viewport',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSliverFillViewport();
  ...
  print(
    'RenderSliverFillViewport test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Sliver Fill Viewport'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSliverFillViewport checks passed'),
    ],
  );
}
