// D4rt test script: Comprehensive WrapCrossAlignment coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(WrapCrossAlignment value) => '${value.name}#${value.index}';
  ...
  final values = WrapCrossAlignment.values;
  ...
  for (final value in values) {
    ...
    final byName = WrapCrossAlignment.values.byName(value.name);
    assert(identical(byName, value));
    assert(WrapCrossAlignment.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <WrapCrossAlignment, String>{
  ...
  try {
    WrapCrossAlignment.values.byName('__not_a_wrap_cross_alignment__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => WrapCrossAlignment.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('WrapCrossAlignment Tests'),
