// D4rt test script: comprehensive checks for RenderSliverIgnorePointer
...
class _RenderSliverIgnorePointerScript extends StatelessWidget {
  const _RenderSliverIgnorePointerScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverIgnorePointer');

    final Type targetType = RenderSliverIgnorePointer;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_ignore_pointer_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverIgnorePointer',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverIgnorePointer');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = SliverIgnorePointer(
      ignoring: true,
      sliver: const SliverToBoxAdapter(child: SizedBox(height: 20, child: Text('ignore-me'))),
    );
    ...
    final String normalized = 'RenderSliverIgnorePointer'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverIgnorePointer'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverIgnorePointer] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverIgnorePointer'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverIgnorePointer');
  return const _RenderSliverIgnorePointerScript();
}
