...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting CacheExtentStyle enum verification');

  final values = CacheExtentStyle.values;
  _check(
  ...
  final probes = values
      .map(
        (value) => _EnumProbe<CacheExtentStyle>(
          value: value,
          name: value.name,
          index: value.index,
        ),
  ...
  final roundTripValid = values
      .map((value) => CacheExtentStyle.values.byName(value.name) == value)
  ...
  try {
    CacheExtentStyle.values.byName('__invalid_cache_extent_style__');
  } catch (_) {
  ...
  final toStringIncludesType =
      values.every((value) => value.toString().startsWith('CacheExtentStyle.'));
  _check(
  ...
  final summaryLine =
      'CacheExtentStyle summary: checks=${summary.checks}, failures=${summary.failures}, values=${summary.valueCount}';
  ...
  return ListView(
    ...
    children: [
      const Text('CacheExtentStyle Test Summary'),
      Text(summaryLine),
