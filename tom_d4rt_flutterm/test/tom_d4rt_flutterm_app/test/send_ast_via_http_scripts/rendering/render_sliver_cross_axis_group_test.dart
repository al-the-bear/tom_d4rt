// D4rt test script: Comprehensive checks for RenderSliverCrossAxisGroup in rendering
...
Widget _buildRelatedWidgetForRenderSliverCrossAxisGroup() {
  if ('RenderSliverCrossAxisGroup'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSliverCrossAxisGroup sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSliverCrossAxisGroup host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSliverCrossAxisGroup: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSliverCrossAxisGroup;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSliverCrossAxisGroup'));
  ...
  mark('Direct class reference resolved for RenderSliverCrossAxisGroup');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Sliver Cross Axis Group',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSliverCrossAxisGroup();
  ...
  print(
    'RenderSliverCrossAxisGroup test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Sliver Cross Axis Group'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSliverCrossAxisGroup checks passed'),
    ],
  );
}
