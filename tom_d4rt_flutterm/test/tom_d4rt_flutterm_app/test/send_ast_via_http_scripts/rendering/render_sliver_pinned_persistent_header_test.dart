// D4rt test script: comprehensive checks for RenderSliverPinnedPersistentHeader
...
class _RenderSliverPinnedPersistentHeaderScript extends StatelessWidget {
  const _RenderSliverPinnedPersistentHeaderScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverPinnedPersistentHeader');

    final Type targetType = RenderSliverPinnedPersistentHeader;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_pinned_persistent_header_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverPinnedPersistentHeader',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverPinnedPersistentHeader');
    ...
    final Widget indirectWidget = SliverPersistentHeader(
      floating: false,
      pinned: true,
      delegate: _SimpleHeaderDelegate(label: 'pinned-header', maxExtentValue: 70, minExtentValue: 30),
    );
    ...
    final String normalized = 'RenderSliverPinnedPersistentHeader'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverPinnedPersistentHeader'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverPinnedPersistentHeader] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverPinnedPersistentHeader'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverPinnedPersistentHeader');
  return const _RenderSliverPinnedPersistentHeaderScript();
}
