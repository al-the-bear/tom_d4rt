// D4rt test script: comprehensive checks for RenderSliverToBoxAdapter
...
class _RenderSliverToBoxAdapterScript extends StatelessWidget {
  const _RenderSliverToBoxAdapterScript();

  List<String> _runAssertions() {
    final List<String> logs = <String>[];
    logs.add('start:RenderSliverToBoxAdapter');

    final Type targetType = RenderSliverToBoxAdapter;
    logs.add('type=' + targetType.toString());
    assert(targetType.toString().isNotEmpty);

    const String filenameTag = 'render_sliver_to_box_adapter_test.dart';
    assert(filenameTag.contains('_test.dart'));
    logs.add('file=' + filenameTag);

    final Map<String, Object?> contextMap = <String, Object?>{
      'target': 'RenderSliverToBoxAdapter',
      'group': 'rendering',
      'hasBuild': true,
      'lineGoal': 110,
    };

    assert(contextMap['target'] == 'RenderSliverToBoxAdapter');
    assert(contextMap['hasBuild'] == true);
    assert((contextMap['lineGoal'] as int) >= 110);
    logs.add('context-ok');

    final Widget indirectWidget = const SliverToBoxAdapter(
      child: SizedBox(height: 26, child: Text('to-box')),
    );
    ...
    final String normalized = 'RenderSliverToBoxAdapter'.toLowerCase();
    ...
    final bool containsSliver = 'RenderSliverToBoxAdapter'.contains('Sliver');
    logs.add('contains-sliver=' + containsSliver.toString());

    for (final String log in logs) {
      print('[RenderSliverToBoxAdapter] ' + log);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      ...
      Text('Target: RenderSliverToBoxAdapter'),
...
dynamic build(BuildContext context) {
  print('Executing comprehensive D4rt script for RenderSliverToBoxAdapter');
  return const _RenderSliverToBoxAdapterScript();
}
