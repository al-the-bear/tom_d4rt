// D4rt comprehensive test script for ContainerRenderObjectMixin
...
dynamic build(BuildContext context) {
  const targetClassName = 'ContainerRenderObjectMixin';
  const testLabel = 'ContainerRenderObjectMixin';
  final logs = <String>[];
  ...
  final renderStackA = RenderStack(textDirection: TextDirection.ltr);
  final renderStackB = RenderStack(textDirection: TextDirection.rtl);
  expectCondition(renderStackA is RenderObject, 'renderStackA is RenderObject', logs, counters);
  expectCondition(renderStackA is ContainerRenderObjectMixin<RenderBox, ContainerBoxParentData<RenderBox>>, 'RenderStack uses ContainerRenderObjectMixin indirectly', logs, counters);
  expectCondition(renderStackB.textDirection == TextDirection.rtl, 'textDirection property is configurable', logs, counters);
  expectCondition(renderStackA.childCount == 0, 'new stack starts with zero children edge case', logs, counters);
  details['renderStackA'] = renderStackA.runtimeType.toString();
  details['renderStackB'] = renderStackB.runtimeType.toString();
