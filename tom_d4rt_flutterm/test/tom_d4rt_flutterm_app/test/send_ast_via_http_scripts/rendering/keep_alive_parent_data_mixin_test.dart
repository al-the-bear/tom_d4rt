// D4rt comprehensive test script for KeepAliveParentDataMixin
import 'package:flutter/rendering.dart';
import 'package:flutter/material.dart';

class _KeepAliveParentDataProbe extends ParentData with KeepAliveParentDataMixin {
  @override
  String toString() => 'KeepAliveParentData(keepAlive=$keepAlive)';
}
...
dynamic build(BuildContext context) {
  const targetClassName = 'KeepAliveParentDataMixin';
  const testLabel = 'KeepAliveParentDataMixin';
  final logs = <String>[];
  ...
  final dataA = _KeepAliveParentDataProbe()..keepAlive = true;
  final dataB = _KeepAliveParentDataProbe()..keepAlive = false;
  expectCondition(dataA is KeepAliveParentDataMixin, 'dataA uses KeepAliveParentDataMixin', logs, counters);
  expectCondition(dataA.keepAlive == true, 'keepAlive true assignment preserved', logs, counters);
  expectCondition(dataB.keepAlive == false, 'keepAlive false edge case preserved', logs, counters);
  dataA.keepAlive = false;
  expectCondition(dataA.keepAlive == false, 'keepAlive mutation works', logs, counters);
  details['dataA'] = dataA.toString();
  details['dataB'] = dataB.toString();
