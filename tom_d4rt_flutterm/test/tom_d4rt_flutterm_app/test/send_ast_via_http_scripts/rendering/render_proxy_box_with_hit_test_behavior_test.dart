// D4rt test script: Comprehensive checks for RenderProxyBoxWithHitTestBehavior in rendering
...
Widget _buildRelatedWidgetForRenderProxyBoxWithHitTestBehavior() {
  if ('RenderProxyBoxWithHitTestBehavior'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderProxyBoxWithHitTestBehavior sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderProxyBoxWithHitTestBehavior host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderProxyBoxWithHitTestBehavior: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderProxyBoxWithHitTestBehavior;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderProxyBoxWithHitTestBehavior'));
  ...
  mark('Direct class reference resolved for RenderProxyBoxWithHitTestBehavior');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Proxy Box With Hit Test Behavior',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderProxyBoxWithHitTestBehavior();
  ...
  print(
    'RenderProxyBoxWithHitTestBehavior test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Proxy Box With Hit Test Behavior'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderProxyBoxWithHitTestBehavior checks passed'),
    ],
  );
}
