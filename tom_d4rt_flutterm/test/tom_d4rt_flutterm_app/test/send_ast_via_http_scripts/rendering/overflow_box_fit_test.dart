// D4rt test script: Comprehensive OverflowBoxFit coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = OverflowBoxFit.values.byName(value.name);
    assert(identical(byName, value));
    assert(OverflowBoxFit.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <OverflowBoxFit, String>{
    for (final value in values) value: 'fit:${value.name}',
  };
  ...
  try {
    OverflowBoxFit.values.byName('__not_an_overflow_box_fit__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => OverflowBoxFit.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('OverflowBoxFit Tests'),
