// D4rt test script: Comprehensive checks for RenderSliverConstrainedCrossAxis in rendering
...
Widget _buildRelatedWidgetForRenderSliverConstrainedCrossAxis() {
  if ('RenderSliverConstrainedCrossAxis'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSliverConstrainedCrossAxis sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSliverConstrainedCrossAxis host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSliverConstrainedCrossAxis: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSliverConstrainedCrossAxis;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSliverConstrainedCrossAxis'));
  ...
  mark('Direct class reference resolved for RenderSliverConstrainedCrossAxis');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Sliver Constrained Cross Axis',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSliverConstrainedCrossAxis();
  ...
  print(
    'RenderSliverConstrainedCrossAxis test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Sliver Constrained Cross Axis'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSliverConstrainedCrossAxis checks passed'),
    ],
  );
}
