// D4rt test script: Comprehensive MainAxisAlignment coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(MainAxisAlignment value) => '${value.name}#${value.index}';
  ...
  for (final value in values) {
    ...
    final byName = MainAxisAlignment.values.byName(value.name);
    assert(identical(byName, value));
    assert(MainAxisAlignment.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <MainAxisAlignment, String>{
  ...
  try {
    MainAxisAlignment.values.byName('__not_a_main_axis_alignment__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => MainAxisAlignment.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('MainAxisAlignment Tests'),
