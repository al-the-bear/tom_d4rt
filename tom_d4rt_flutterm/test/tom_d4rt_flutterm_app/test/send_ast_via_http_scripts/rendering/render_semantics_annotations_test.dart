// D4rt test script: Comprehensive checks for RenderSemanticsAnnotations in rendering
...
Widget _buildRelatedWidgetForRenderSemanticsAnnotations() {
  if ('RenderSemanticsAnnotations'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderSemanticsAnnotations sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderSemanticsAnnotations host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderSemanticsAnnotations: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  final Type renderType = RenderSemanticsAnnotations;
  final String className = renderType.toString();
  _logKV('renderType', renderType);
  _logKV('className', className);
  assert(className.contains('RenderSemanticsAnnotations'));
  ...
  mark('Direct class reference resolved for RenderSemanticsAnnotations');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Semantics Annotations',
  ...
  final Widget relatedWidgetA =
      _buildRelatedWidgetForRenderSemanticsAnnotations();
  ...
  print(
    'RenderSemanticsAnnotations test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Semantics Annotations'),
      ...
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderSemanticsAnnotations checks passed'),
    ],
  );
}
