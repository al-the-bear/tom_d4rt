// D4rt test script: Comprehensive ScrollDirection coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(ScrollDirection value) => '${value.name}#${value.index}';
  ...
  final values = ScrollDirection.values;
  ...
  for (final value in values) {
    ...
    final byName = ScrollDirection.values.byName(value.name);
    assert(identical(byName, value));
    assert(ScrollDirection.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <ScrollDirection, String>{
    for (final value in values) value: 'scroll:${value.name}',
  };
  ...
  try {
    ScrollDirection.values.byName('__not_a_scroll_direction__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => ScrollDirection.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('ScrollDirection Tests'),
