...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    paragraph is RenderInlineChildrenContainerDefaults,
    'RenderParagraph uses RenderInlineChildrenContainerDefaults',
    logs,
    counters,
  );
  expectCondition(
    paragraph.text.toPlainText().contains('hello'),
    'Paragraph plain text contains hello',
    logs,
    counters,
  );
  expectCondition(
    paragraph.text.toPlainText().contains('world'),
    'Paragraph plain text contains world',
    logs,
    counters,
  );
  paragraph.text = const TextSpan(text: "updated");
  expectCondition(
    paragraph.text.toPlainText() == 'updated',
    'Paragraph text can be replaced',
    logs,
    counters,
  );
  ...
  expectCondition(
    paragraph.textDirection == TextDirection.ltr,
    'TextDirection can be changed back',
    logs,
    counters,
  );
  paragraph.layout(const BoxConstraints(maxWidth: 200));
  expectCondition(
    paragraph.size.width <= 200,
    'Paragraph width respects constraints',
    logs,
    counters,
  );
  expectCondition(
    paragraph.size.height >= 0,
    'Paragraph height is non-negative',
    logs,
    counters,
  );
  final placeholders = paragraph.text.getPlaceholderSpans();
  expectCondition(
    placeholders.isEmpty || placeholders.isNotEmpty,
    'Placeholder query executes',
    logs,
    counters,
  );
  for (var i = 0; i < 8; i++) {
    ...
    expectCondition(
      paragraph.size.width <= 127,
      'Repeated layout obeys maxWidth constraint',
      logs,
      counters,
    );
  }
