// D4rt test script: Comprehensive HitTestBehavior coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(HitTestBehavior value) => '${value.name}#${value.index}';
  ...
  for (final value in values) {
    ...
    final byName = HitTestBehavior.values.byName(value.name);
    assert(identical(byName, value));
    assert(HitTestBehavior.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <HitTestBehavior, String>{
    for (final value in values) value: 'behavior:${value.name}',
  };
  ...
  try {
    HitTestBehavior.values.byName('__not_a_hit_test_behavior__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => HitTestBehavior.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('HitTestBehavior Tests'),
