// D4rt test script: Comprehensive PerformanceOverlayOption coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(PerformanceOverlayOption value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = PerformanceOverlayOption.values;
  ...
  for (final value in values) {
    ...
    final byName = PerformanceOverlayOption.values.byName(value.name);
    assert(identical(byName, value));
    assert(PerformanceOverlayOption.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <PerformanceOverlayOption, String>{
    for (final value in values) value: 'overlay:${value.name}',
  };
  ...
  try {
    PerformanceOverlayOption.values.byName('__not_a_performance_overlay_option__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => PerformanceOverlayOption.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('PerformanceOverlayOption Tests'),
