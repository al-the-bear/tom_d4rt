// D4rt test script: Comprehensive checks for RenderPointerListener in rendering
...
Widget _buildRelatedWidgetForRenderPointerListener() {
  if ('RenderPointerListener'.contains('Sliver')) {
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(
            ...
            child: Text('RenderPointerListener sliver host'),
          ),
        ),
        SliverPadding(
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        Text('RenderPointerListener host widget'),
        const SizedBox(height: 4),
        Listener(
          behavior: HitTestBehavior.opaque,
          onPointerDown: (_) => print(
            'RenderPointerListener: pointer down observed via Listener',
          ),
          child: Container(
            width: 120,
            height: 28,
...
dynamic build(BuildContext context) {
  ...
  mark('Direct class reference resolved for RenderPointerListener');

  _section('Constructor / property-oriented checks');
  final Map<String, Object?> observed = <String, Object?>{
    'title': 'Render Pointer Listener',
  ...
  final Widget relatedWidgetA = _buildRelatedWidgetForRenderPointerListener();
  final Widget relatedWidgetB = _buildRelatedWidgetForRenderPointerListener();
  ...
  print(
    'RenderPointerListener test completed with ${checkpoints.length} checkpoints.',
  );
  ...
  return Column(
    ...
    children: [
      ...
      Text('Topic: Render Pointer Listener'),
      const SizedBox(height: 6),
      _buildRelatedWidgetForRenderPointerListener(),
      const SizedBox(height: 8),
      const Text('Summary:'),
      ...summaryLines,
      const SizedBox(height: 8),
      Text('All RenderPointerListener checks passed'),
    ],
  );
}
