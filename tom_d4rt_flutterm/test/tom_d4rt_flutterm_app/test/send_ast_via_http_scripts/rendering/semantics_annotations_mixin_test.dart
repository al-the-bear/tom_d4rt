...
dynamic build(BuildContext context) {
  ...
  _check(
    ...
    condition: probe is SemanticsAnnotationsMixin,
  );

  _check(
    logs: logs,
    label: 'initial container true',
    condition: probe.container,
  );
  _check(
    logs: logs,
    label: 'initial textDirection ltr',
    condition: probe.textDirection == TextDirection.ltr,
  );
  ...
  _check(
    logs: logs,
    label: 'explicitChildNodes toggled',
    condition: probe.explicitChildNodes,
  );
  _check(
    logs: logs,
    label: 'excludeSemantics toggled',
    condition: probe.excludeSemantics,
  );
  _check(
    logs: logs,
    label: 'blockUserActions toggled',
    condition: probe.blockUserActions,
  );
  _check(
    logs: logs,
    label: 'textDirection updated',
    condition: probe.textDirection == TextDirection.rtl,
  );
  _check(
    logs: logs,
    label: 'properties label updated',
    condition: probe.properties.label == 'updated',
  );

  probe.layout(const BoxConstraints.tightFor(width: 7, height: 6));
  _check(
    logs: logs,
    label: 'layout still works',
    condition: probe.size == const Size(7, 6),
  );
  ...
  print(
    'SemanticsAnnotationsMixin comprehensive test finished: pass=$pass fail=$fail',
  );
