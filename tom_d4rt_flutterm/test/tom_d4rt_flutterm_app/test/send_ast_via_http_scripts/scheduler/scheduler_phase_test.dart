// D4rt test script: Comprehensive SchedulerPhase coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(SchedulerPhase value) => '${value.name}#${value.index}';
  ...
  final values = SchedulerPhase.values;
  ...
  for (final value in values) {
    ...
    final byName = SchedulerPhase.values.byName(value.name);
    assert(identical(byName, value));
    assert(SchedulerPhase.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SchedulerPhase, String>{
  ...
  try {
    SchedulerPhase.values.byName('__not_a_scheduler_phase__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => SchedulerPhase.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SchedulerPhase Tests'),
