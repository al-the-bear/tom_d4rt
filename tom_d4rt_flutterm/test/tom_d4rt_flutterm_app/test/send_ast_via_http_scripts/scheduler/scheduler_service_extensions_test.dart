// D4rt test script: Comprehensive SchedulerServiceExtensions coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(SchedulerServiceExtensions value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = SchedulerServiceExtensions.values;
  ...
  for (final value in values) {
    ...
    final byName = SchedulerServiceExtensions.values.byName(value.name);
    assert(identical(byName, value));
    assert(SchedulerServiceExtensions.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <SchedulerServiceExtensions, String>{
  ...
  try {
    SchedulerServiceExtensions.values.byName('__not_a_scheduler_service_extensions__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => SchedulerServiceExtensions.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('SchedulerServiceExtensions Tests'),
