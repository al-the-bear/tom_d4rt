...
class PointerDataPacketCaseStudy {
  ...
  List<String> run() {
    ...
    final populatedPacket = ui.PointerDataPacket(
      data: const <ui.PointerData>[ui.PointerData(pointerIdentifier: 7)],
    );

    assert(
      populatedPacket.data.length == 1,
      'Populated packet should have one entry',
    );
    assert(
      populatedPacket.data.first.pointerIdentifier == 7,
      'Pointer identifier mismatch',
    );
    logs.add('populated-size:${populatedPacket.data.length}');
    logs.add('pointer-id:${populatedPacket.data.first.pointerIdentifier}');
    print(
      'PointerDataPacket populated id: ${populatedPacket.data.first.pointerIdentifier}',
    );

    final copied = ui.PointerDataPacket(
      data: List<ui.PointerData>.from(populatedPacket.data),
    );
    assert(
      copied.data.length == populatedPacket.data.length,
      'Copied packet length mismatch',
    );
    logs.add('copied-size:${copied.data.length}');

    if (includeEdgeCases) {
      final identifiers = populatedPacket.data
          .map((value) => value.pointerIdentifier)
          .toList();
...
dynamic build(BuildContext context) {
  ...
  assert(
    logs.any((line) => line.startsWith('pointer-id:')),
    'Expected pointer id log',
  );
  assert(
