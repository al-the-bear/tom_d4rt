// D4rt test script: Comprehensive tests for Brightness
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for Brightness: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== Brightness comprehensive test start ===');

  final values = Brightness.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('Brightness.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = Brightness.values.byName(value.name);
    _expect(
  ...
  try {
    Brightness.values.byName('__definitely_not_a_brightness__');
  } catch (error) {
  ...
  print('=== Brightness comprehensive test completed ===');
  return Column(
