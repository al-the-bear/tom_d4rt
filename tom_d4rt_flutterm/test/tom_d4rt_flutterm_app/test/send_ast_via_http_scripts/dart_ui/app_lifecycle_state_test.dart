...
dynamic build(BuildContext context) {
  ...
  log('=== AppLifecycleState comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.AppLifecycleState> values = ui.AppLifecycleState.values;
  expectCondition(values.isNotEmpty, 'AppLifecycleState.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'AppLifecycleState.values contains unique entries',
  );

  final ui.AppLifecycleState first = values.first;
  final ui.AppLifecycleState last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.AppLifecycleState,
    'First value runtimeType is AppLifecycleState',
  );
  ...
  for (final ui.AppLifecycleState value in values) {
  ...
  final Map<String, ui.AppLifecycleState> byName =
      <String, ui.AppLifecycleState>{
        for (final ui.AppLifecycleState value in values) value.name: value,
      };
  expectCondition(
  ...
  log('=== AppLifecycleState comprehensive test completed ===');
  return Column(
