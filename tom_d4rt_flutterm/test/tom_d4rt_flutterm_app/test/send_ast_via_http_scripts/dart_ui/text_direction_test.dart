...
dynamic build(BuildContext context) {
  ...
  _expectCondition(
    ...
    'Index map covers all TextDirection values',
  );
  _expectCondition(
    names.toSet().length == names.length,
    'TextDirection names are unique',
  );
  _expectCondition(
    indexes.toSet().length == indexes.length,
    'TextDirection indexes are unique',
  );
  _expectCondition(indexes.first == 0, 'First TextDirection index is 0');
  _expectCondition(
    indexes.last == values.length - 1,
    'Last TextDirection index is values.length - 1',
  );
  ...
  _expectCondition(
    TextDirection.values.byName(first.name) == first,
    'byName resolves first TextDirection',
  );
  _expectCondition(
    TextDirection.values.byName(last.name) == last,
    'byName resolves last TextDirection',
  );
  _expectCondition(
    indexMap[first.index] == first,
    'Index map resolves first TextDirection',
  );
  _expectCondition(
    indexMap[last.index] == last,
    'Index map resolves last TextDirection',
  );
  _expectCondition(
    first == TextDirection.values[first.index],
    'First TextDirection round-trips from index',
  );
  _expectCondition(
    last == TextDirection.values[last.index],
    'Last TextDirection round-trips from index',
  );
  _expectCondition(
  ...
  _expectCondition(
    first == first,
    'Reflexive equality for first TextDirection',
  );
