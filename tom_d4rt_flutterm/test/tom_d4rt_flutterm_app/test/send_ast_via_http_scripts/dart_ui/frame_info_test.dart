...
dynamic build(BuildContext context) {
  ...
  final totalDuration = gifFrames.fold<int>(
    0, (sum, f) => sum + f.duration.inMilliseconds);
            ...
            children: [
                  ...
                  children: [
                    ...
                    Text('FrameInfo',
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold,
                        color: Colors.white)),
                    SizedBox(height: 6),
                    Text('Decoded image frame with pixel data and display duration',
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      _propRow('image', 'ui.Image', 'Decoded pixel data for this frame',
                        Color(0xFF00695C)),
                      _propRow('duration', 'Duration', 'How long to display before advancing',
                        Color(0xFF1565C0)),
                      SizedBox(height: 10),
                      Container(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _pipeStep(1, 'Load bytes', 'rootBundle.load / http.get / File.readAsBytes',
                      Icons.file_download, Color(0xFF455A64)),
                    _pipeArrow(),
                    _pipeStep(2, 'Create Codec', 'ui.instantiateImageCodec(bytes)',
                      Icons.settings, Color(0xFF00695C)),
                    _pipeArrow(),
                    _pipeStep(3, 'Check frame count', 'codec.frameCount → N frames',
                      Icons.format_list_numbered, Color(0xFF1565C0)),
                    _pipeArrow(),
                    _pipeStep(4, 'Get FrameInfo', 'codec.getNextFrame() → FrameInfo',
                      Icons.burst_mode, Color(0xFFE65100)),
                    _pipeArrow(),
                    _pipeStep(5, 'Use Image', 'frameInfo.image → paint on Canvas',
                      Icons.image, Color(0xFF2E7D32)),
                    _pipeArrow(),
                    _pipeStep(6, 'Dispose', 'frameInfo.image.dispose()',
                      Icons.delete_outline, Color(0xFFC62828)),
                  ],
                ),
              ),
                ...
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        _statBadge('${gifFrames.length}', 'frames', Color(0xFF00695C)),
                        SizedBox(width: 8),
                        _statBadge('${totalDuration}ms', 'total', Color(0xFF1565C0)),
                        SizedBox(width: 8),
                        _statBadge('${avgDuration}ms', 'avg', Color(0xFFE65100)),
                      ],
                    ),
                    SizedBox(height: 12),
                    ...
                    SizedBox(
                      height: 60,
                      child: Row(
                        children: gifFrames.map((f) => Expanded(
                          child: Container(
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(child: _typeCard('Still Image', stillFrame,
                      ['1 frame only', 'Duration.zero', 'PNG, JPEG, BMP', 'No animation'],
                      Color(0xFF455A64))),
                    SizedBox(width: 10),
                    Expanded(child: _typeCard('Animated Image', gifFrames.first,
                      ['N frames', 'Each has duration', 'GIF, WebP, APNG', 'Loop control'],
                      Color(0xFF00695C))),
                  ],
                ),
              ),
    ...
    children: [
      ...
      Expanded(child: Column(
        ...
        children: [
          ...
          Text(desc, style: TextStyle(fontSize: 9, fontFamily: 'monospace',
...
Color _frameColor(int index) {
  final colors = [
    ...
    Color(0xFFBDBDBD), Color(0xFFE53935),
  ];
...
Widget _frameDurationRow(_SimFrame f, int maxMs) {
  ...
  return Padding(
    ...
    child: Row(
      children: [
        SizedBox(width: 28, child: Text('#${f.index}',
        ...
        SizedBox(width: 6),
        Text('${f.duration.inMilliseconds}ms',
...
Widget _typeCard(String title, _SimFrame sample, List<String> points, Color color) {
  return Container(
...
Widget _formatRow(String format, bool still, bool animated, Color color) {
  return Padding(
    ...
    child: Row(
      children: [
        SizedBox(width: 50, child: Text(format,
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: color))),
        SizedBox(width: 8),
        Container(
          width: 50, height: 18,
          decoration: BoxDecoration(
            color: still ? Color(0xFF1565C0).withValues(alpha: 0.1) : Colors.grey[100],
            borderRadius: BorderRadius.circular(4)),
          child: Center(child: Text(still ? 'Still' : '—',
        ...
        Container(
          width: 60, height: 18,
          decoration: BoxDecoration(
            color: animated ? Color(0xFFE65100).withValues(alpha: 0.1) : Colors.grey[100],
            borderRadius: BorderRadius.circular(4)),
          child: Center(child: Text(animated ? 'Animated' : '—',
            style: TextStyle(fontSize: 9,
