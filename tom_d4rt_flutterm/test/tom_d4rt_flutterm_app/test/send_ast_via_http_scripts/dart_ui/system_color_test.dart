...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    systemA.name == 'primaryLike',
    'systemA name set through constructor',
    logs,
    counters,
  );
  expectCondition(
    systemB.value == colorB,
    'systemB stores provided color value',
    logs,
    counters,
  );
  expectCondition(
    paletteLight.length >= 0,
    'light palette query works',
    logs,
    counters,
  );
  expectCondition(
    paletteDark.length >= 0,
    'dark palette query works',
    logs,
    counters,
  );
  details['systemA'] = '${systemA.name}:${systemA.value.value}';
  details['systemB'] = '${systemB.name}:${systemB.value.value}';
  details['platformProvidesSystemColors'] =
