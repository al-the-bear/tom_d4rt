...
dynamic build(BuildContext context) {
  ...
  _expect(
    ...
    'display.size dimensions are non-negative',
    logs,
  );
  assertionCount++;
  _expect(
    display.devicePixelRatio > 0,
    'display.devicePixelRatio is positive',
    logs,
  );
  assertionCount++;
  _expect(
    display.refreshRate > 0 || display.refreshRate == 0,
    'display.refreshRate is accessible',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    sameDisplay.id == display.id,
    're-reading display from same view is stable',
    logs,
  );
  assertionCount++;
  ...
  print(
    'Display id=${display.id} size=${display.size} dpr=${display.devicePixelRatio} refresh=${display.refreshRate}',
  );
  print(
    'View count=$viewCount implicitView=${dispatcher.implicitView != null}',
  );
