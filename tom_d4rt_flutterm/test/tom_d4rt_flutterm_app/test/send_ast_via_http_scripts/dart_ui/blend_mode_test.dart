...
dynamic build(BuildContext context) {
  ...
  log('=== BlendMode comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.BlendMode> values = ui.BlendMode.values;
  expectCondition(values.isNotEmpty, 'BlendMode.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'BlendMode.values contains unique entries',
  );

  final ui.BlendMode first = values.first;
  final ui.BlendMode last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.BlendMode,
    'First value runtimeType is BlendMode',
  );
  ...
  for (final ui.BlendMode value in values) {
  ...
  final Map<String, ui.BlendMode> byName = <String, ui.BlendMode>{
    for (final ui.BlendMode value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.BlendMode> sortedByIndex = <ui.BlendMode>[...values]
    ..sort((ui.BlendMode a, ui.BlendMode b) => a.index - b.index);
  ...
  log('=== BlendMode comprehensive test completed ===');
  return Column(
