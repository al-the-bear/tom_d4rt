...
Map<int, SemanticsValidationResult> _buildIndexMap(List<SemanticsValidationResult> values) {
...
dynamic build(BuildContext context) {
  ...
  print('SemanticsValidationResult detailed values:\n${_formatEnumValues(values)}');

  _expectCondition(values.isNotEmpty, 'SemanticsValidationResult has at least one value');
  ...
  print('First SemanticsValidationResult value: $first (${first.name}, ${first.index})');
  print('Last SemanticsValidationResult value: $last (${last.name}, ${last.index})');

  _expectCondition(SemanticsValidationResult.values.byName(first.name) == first, 'byName resolves first value');
  _expectCondition(SemanticsValidationResult.values.byName(last.name) == last, 'byName resolves last value');
  ...
  _expectCondition(first == SemanticsValidationResult.values[first.index], 'First value round-trips from index');
  _expectCondition(last == SemanticsValidationResult.values[last.index], 'Last value round-trips from index');
  ...
  try {
    SemanticsValidationResult.values.byName('__not_a_valid_semanticsvalidationresult_name__');
  } catch (error) {
    invalidNameThrows = true;
    print('Expected byName failure captured for SemanticsValidationResult: $error');
  }
  _expectCondition(invalidNameThrows, 'Invalid byName lookup throws for SemanticsValidationResult');
