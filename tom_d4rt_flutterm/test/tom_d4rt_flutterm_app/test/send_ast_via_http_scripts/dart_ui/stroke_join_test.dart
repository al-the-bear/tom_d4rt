...
dynamic build(BuildContext context) {
  ...
  _expectCondition(
    ...
    'Index map covers all StrokeJoin values',
  );
  _expectCondition(
    names.toSet().length == names.length,
    'StrokeJoin names are unique',
  );
  _expectCondition(
    indexes.toSet().length == indexes.length,
    'StrokeJoin indexes are unique',
  );
  _expectCondition(indexes.first == 0, 'First StrokeJoin index is 0');
  _expectCondition(
    indexes.last == values.length - 1,
    'Last StrokeJoin index is values.length - 1',
  );
  ...
  _expectCondition(
    StrokeJoin.values.byName(first.name) == first,
    'byName resolves first StrokeJoin',
  );
  _expectCondition(
    StrokeJoin.values.byName(last.name) == last,
    'byName resolves last StrokeJoin',
  );
  _expectCondition(
    indexMap[first.index] == first,
    'Index map resolves first StrokeJoin',
  );
  _expectCondition(
    indexMap[last.index] == last,
    'Index map resolves last StrokeJoin',
  );
  _expectCondition(
    first == StrokeJoin.values[first.index],
    'First StrokeJoin round-trips from index',
  );
  _expectCondition(
    last == StrokeJoin.values[last.index],
    'Last StrokeJoin round-trips from index',
  );
  _expectCondition(
  ...
  _expectCondition(
    ...
    'Sorting by index preserves StrokeJoin order',
  );
