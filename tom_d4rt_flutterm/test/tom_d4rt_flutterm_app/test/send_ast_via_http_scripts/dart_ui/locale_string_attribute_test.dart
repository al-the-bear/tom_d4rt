...
dynamic build(BuildContext context) {
  ...
  _expect(
    attrA.range.start == 0 && attrA.range.end == 5,
    'constructor sets range for attrA',
    logs,
  );
  assertionCount++;
  _expect(
    attrA.locale.languageCode == 'en',
    'constructor sets locale language for attrA',
    logs,
  );
  assertionCount++;
  _expect(
    attrB.range.start == 5 && attrB.range.end == 10,
    'constructor sets range for attrB',
    logs,
  );
  assertionCount++;
  _expect(
    attrB.locale.countryCode == 'DE',
    'constructor sets locale country for attrB',
    logs,
  );
  assertionCount++;

  _expect(
    attrA.toString().isNotEmpty,
    'toString produces non-empty representation',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    edge.range.isCollapsed,
    'edge case supports collapsed TextRange',
    logs,
  );
  assertionCount++;
  ...
  return Column(
    ...
    children: [
      ...
      Text(
        'A locale: ${attrA.locale.languageCode}-${attrA.locale.countryCode ?? ''}',
      ),
      Text(
        'B locale: ${attrB.locale.languageCode}-${attrB.locale.countryCode ?? ''}',
      ),
