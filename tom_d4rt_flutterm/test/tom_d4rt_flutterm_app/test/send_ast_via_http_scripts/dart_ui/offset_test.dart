...
dynamic build(BuildContext context) {
  ...
  final a = Offset(100.0, 200.0);
  final b = Offset(-25.0, 50.0);
  final zero = Offset.zero;
  log('a=$a b=$b zero=$zero');

  expectCheck(
    a.dx == 100.0 && a.dy == 200.0,
    'constructor should assign dx/dy',
  );
  expectCheck(zero == Offset(0.0, 0.0), 'Offset.zero should be origin');
  ...
  log(
    'sum=$sum diff=$diff mul=$mul div=$div translate=$translate scale=$scale',
  );
  expectCheck(sum == Offset(75.0, 250.0), 'sum must match expected');
  expectCheck(diff == Offset(125.0, 150.0), 'diff must match expected');
  expectCheck(mul == Offset(200.0, 400.0), 'multiply must match expected');
  expectCheck(div == Offset(50.0, 100.0), 'divide must match expected');
  expectCheck(
    translate == Offset(105.0, 195.0),
    'translate must match expected',
  );
  expectCheck(scale == Offset(50.0, 300.0), 'scale must match expected');
  ...
  log('lerp(0.5)=$mid');
  expectCheck(
    mid == Offset(37.5, 125.0),
    'midpoint lerp should match expected',
  );
