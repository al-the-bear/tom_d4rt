...
dynamic build(BuildContext context) {
  ...
  print('FontFeature liga on: $ligaOn (feature=${ligaOn.feature}, value=${ligaOn.value})');
  ...
  final features = <_FeatureInfo>[
    ...
    _FeatureInfo('kern', 'Kerning', 'Adjusts spacing between letter pairs', true),
    _FeatureInfo('smcp', 'Small Caps', 'Lowercase → small capital forms', false),
    _FeatureInfo('onum', 'Oldstyle Nums', 'Numerals with varying heights', false),
    _FeatureInfo('tnum', 'Tabular Figs', 'Fixed-width numerals for alignment', false),
    _FeatureInfo('pnum', 'Proportional', 'Natural-width numerals', false),
    _FeatureInfo('frac', 'Fractions', 'Stacked fraction rendering', false),
    _FeatureInfo('ss01', 'Stylistic Set', 'Alternate glyph designs (#1)', false),
  ];
  ...
  final variations = <_VariationInfo>[
    ...
    _VariationInfo('opsz', 'Optical Size', 8, 144, 14, 'Optimize for display size'),
  ];
          ...
          child: Column(
            ...
            children: [
                  ...
                  children: [
                    ...
                    Text('Font APIs',
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold,
                        color: Colors.white)),
                    SizedBox(height: 6),
                    Text('FontFeature, FontVariation, loadFontFromList',
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _apiRow("FontFeature('tag')", 'Enable with value 1', Color(0xFF880E4F)),
                    _apiRow("FontFeature('tag', 0)", 'Disable feature', Color(0xFF880E4F)),
                    _apiRow("FontFeature.enable('tag')", 'Named enable', Color(0xFF0D47A1)),
                    _apiRow("FontFeature.disable('tag')", 'Named disable', Color(0xFF0D47A1)),
                    _apiRow('.tabularFigures()', 'tnum feature', Color(0xFF2E7D32)),
                    _apiRow('.proportionalFigures()', 'pnum feature', Color(0xFF2E7D32)),
                    _apiRow('.oldstyleFigures()', 'onum feature', Color(0xFF2E7D32)),
                  ],
                ),
              ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _pipeStep(1, 'Get font bytes', 'http.get() or rootBundle.load()',
                      Icons.download, Color(0xFF880E4F)),
                    _arrow(),
                    _pipeStep(2, 'Convert to Uint8List', 'response.bodyBytes',
                      Icons.memory, Color(0xFF0D47A1)),
                    _arrow(),
                    _pipeStep(3, 'Register font', 'loadFontFromList(bytes, fontFamily: name)',
                      Icons.app_registration, Color(0xFF2E7D32)),
                    _arrow(),
                    _pipeStep(4, 'Await completion', 'await future; // font now available',
                      Icons.check_circle, Color(0xFFE65100)),
                    _arrow(),
                    _pipeStep(5, 'Use in TextStyle', 'TextStyle(fontFamily: name)',
                      Icons.text_fields, Color(0xFF6A1B9A)),
                  ],
                ),
              ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Text('FontFeature', style: TextStyle(
                      fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF880E4F))),
                    SizedBox(height: 6),
                    _eqRow('enable("liga") == enable("liga")', ligaOn == ligaOn2),
                    _eqRow('enable("liga") == disable("liga")', ligaOn == ligaOff),
                    _eqRow('.feature property', true, suffix: '"${ligaOn.feature}"'),
                    _eqRow('.value property', true, suffix: '${ligaOn.value}'),
                    SizedBox(height: 12),
                    Text('FontVariation', style: TextStyle(
                      fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF0D47A1))),
                    SizedBox(height: 6),
...
class _VariationInfo {
  ...
  final String desc;
  _VariationInfo(this.axis, this.name, this.min, this.max, this.defaultVal, this.desc);
}
    ...
    child: Row(
      children: [
        ...
        Expanded(child: Column(
          ...
          children: [
            Text(f.name, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
            Text(f.desc, style: TextStyle(fontSize: 10, color: Colors.grey[600])),
          ],
        )),
        Container(
  ...
  return Padding(
    ...
    child: Row(
      children: [
        ...
        SizedBox(width: 8),
        Expanded(child: Text(api, style: TextStyle(fontSize: 10, fontFamily: 'monospace'))),
    ...
    child: Row(
      children: [
        Container(
          ...
          child: Center(child: Text(v.axis,
            style: TextStyle(fontFamily: 'monospace', fontWeight: FontWeight.bold,
              fontSize: 10, color: color))),
        ),
        SizedBox(width: 8),
        SizedBox(width: 80, child: Text(v.name,
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
        Expanded(child: Text(v.desc,
...
Widget _variationBar(_VariationInfo v) {
  ...
  return Padding(
    ...
    child: Row(
      children: [
        SizedBox(width: 38, child: Text(v.axis,
          style: TextStyle(fontFamily: 'monospace', fontWeight: FontWeight.bold, fontSize: 10))),
        Text('${v.min.toInt()}', style: TextStyle(fontSize: 9, color: Colors.grey[500])),
        SizedBox(width: 4),
        Expanded(
          child: Stack(
            children: [
              Container(height: 12, decoration: BoxDecoration(
                color: Color(0xFFE8EAF6), borderRadius: BorderRadius.circular(6))),
              FractionallySizedBox(
                widthFactor: fraction.clamp(0.0, 1.0),
                child: Container(height: 12, decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [Color(0xFF880E4F), Color(0xFFF06292)]),
                  borderRadius: BorderRadius.circular(6))),
              ),
            ],
          ),
        ...
        SizedBox(width: 4),
        Text('${v.max.toInt()}', style: TextStyle(fontSize: 9, color: Colors.grey[500])),
        SizedBox(width: 8),
        Text('def: ${v.defaultVal.toInt()}', style: TextStyle(
          fontSize: 9, fontWeight: FontWeight.bold, color: Color(0xFF880E4F))),
      ],
    ),
  );
}
    ...
    children: [
      ...
      Expanded(child: Column(
        ...
        children: [
          ...
          Text(desc, style: TextStyle(fontSize: 10, fontFamily: 'monospace', color: Colors.grey[600])),
        ],
      )),
    ],
  );
...
Widget _eqRow(String expr, bool result, {String? suffix}) {
  return Padding(
    ...
    child: Row(
      children: [
        Expanded(child: Text(expr, style: TextStyle(fontSize: 10, fontFamily: 'monospace'))),
        if (suffix != null) ...[
          Text(suffix, style: TextStyle(fontSize: 10, color: Color(0xFF880E4F))),
          SizedBox(width: 6),
        ],
        Container(
  ...
  return Container(
    ...
    child: Text(
      ...
      style: TextStyle(fontFamily: 'monospace', fontSize: 10,
        color: Color(0xFFF48FB1), height: 1.5)),
  );
}
