...
dynamic build(BuildContext context) {
  ...
  log('=== CheckedState comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.CheckedState> values = ui.CheckedState.values;
  expectCondition(values.isNotEmpty, 'CheckedState.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'CheckedState.values contains unique entries',
  );

  final ui.CheckedState first = values.first;
  final ui.CheckedState last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.CheckedState,
    'First value runtimeType is CheckedState',
  );
  ...
  for (final ui.CheckedState value in values) {
  ...
  final Map<String, ui.CheckedState> byName = <String, ui.CheckedState>{
    for (final ui.CheckedState value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.CheckedState> sortedByIndex = <ui.CheckedState>[...values]
    ..sort((ui.CheckedState a, ui.CheckedState b) => a.index - b.index);
  ...
  log('=== CheckedState comprehensive test completed ===');
  return Column(
