...
dynamic build(BuildContext context) {
  ...
  _expectCondition(values.isNotEmpty, 'TextLeadingDistribution has at least one value');
  ...
  _expectCondition(indexMap.length == values.length, 'Index map covers all TextLeadingDistribution values');
  _expectCondition(names.toSet().length == names.length, 'TextLeadingDistribution names are unique');
  _expectCondition(indexes.toSet().length == indexes.length, 'TextLeadingDistribution indexes are unique');
  _expectCondition(indexes.first == 0, 'First TextLeadingDistribution index is 0');
  _expectCondition(indexes.last == values.length - 1, 'Last TextLeadingDistribution index is values.length - 1');
  ...
  print('First TextLeadingDistribution: $first (${first.name}, ${first.index})');
  print('Last TextLeadingDistribution: $last (${last.name}, ${last.index})');

  _expectCondition(TextLeadingDistribution.values.byName(first.name) == first, 'byName resolves first TextLeadingDistribution');
  _expectCondition(TextLeadingDistribution.values.byName(last.name) == last, 'byName resolves last TextLeadingDistribution');
  _expectCondition(indexMap[first.index] == first, 'Index map resolves first TextLeadingDistribution');
  _expectCondition(indexMap[last.index] == last, 'Index map resolves last TextLeadingDistribution');
  _expectCondition(first == TextLeadingDistribution.values[first.index], 'First TextLeadingDistribution round-trips from index');
  _expectCondition(last == TextLeadingDistribution.values[last.index], 'Last TextLeadingDistribution round-trips from index');
  ...
  _expectCondition(first == first, 'Reflexive equality for first TextLeadingDistribution');
  _expectCondition(last == last, 'Reflexive equality for last TextLeadingDistribution');

  bool invalidNameThrows = false;
  try {
    TextLeadingDistribution.values.byName('__invalid_text_leading_distribution__');
  } catch (error) {
    invalidNameThrows = true;
    print('Expected TextLeadingDistribution byName failure: $error');
  }
  _expectCondition(invalidNameThrows, 'Invalid byName throws for TextLeadingDistribution');

  final sortedByIndex = [...values]..sort((a, b) => a.index.compareTo(b.index));
  _expectCondition(sortedByIndex.join('|') == values.join('|'), 'Sorting by index preserves TextLeadingDistribution order');
