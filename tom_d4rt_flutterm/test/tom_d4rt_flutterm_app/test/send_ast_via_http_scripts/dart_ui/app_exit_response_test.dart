...
dynamic build(BuildContext context) {
  ...
  log('=== AppExitResponse comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.AppExitResponse> values = ui.AppExitResponse.values;
  expectCondition(values.isNotEmpty, 'AppExitResponse.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'AppExitResponse.values contains unique entries',
  );

  final ui.AppExitResponse first = values.first;
  final ui.AppExitResponse last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.AppExitResponse,
    'First value runtimeType is AppExitResponse',
  );
  ...
  for (final ui.AppExitResponse value in values) {
  ...
  final Map<String, ui.AppExitResponse> byName = <String, ui.AppExitResponse>{
    for (final ui.AppExitResponse value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.AppExitResponse> sortedByIndex = <ui.AppExitResponse>[...values]
    ..sort((ui.AppExitResponse a, ui.AppExitResponse b) => a.index - b.index);
  ...
  log('=== AppExitResponse comprehensive test completed ===');
  return Column(
