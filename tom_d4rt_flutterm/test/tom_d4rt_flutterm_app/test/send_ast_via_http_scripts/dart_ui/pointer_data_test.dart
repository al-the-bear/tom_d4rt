...
dynamic build(BuildContext context) {
  ...
  _expect(event.kind == PointerDeviceKind.touch, 'stores pointer kind', logs);
  assertionCount++;
  _expect(event.change == PointerChange.move, 'stores pointer change', logs);
  assertionCount++;
  _expect(
    event.physicalX == 100.0 && event.physicalY == 200.0,
    'stores physical coordinates',
    logs,
  );
  assertionCount++;
  _expect(event.pressure > 0.0, 'stores pressure', logs);
  assertionCount++;
  _expect(event.pointerIdentifier == 8, 'stores pointer identifier', logs);
  assertionCount++;

  const edge = PointerData();
  _expect(
    edge.pressureMin <= edge.pressureMax,
    'edge defaults keep pressure bounds valid',
    logs,
  );
  assertionCount++;
  _expect(
    edge.signalKind == PointerSignalKind.none,
    'default signal kind is none',
    logs,
  );
  assertionCount++;

  final eventText = event.toString();
  _expect(eventText.isNotEmpty, 'toString is available for diagnostics', logs);
  assertionCount++;
