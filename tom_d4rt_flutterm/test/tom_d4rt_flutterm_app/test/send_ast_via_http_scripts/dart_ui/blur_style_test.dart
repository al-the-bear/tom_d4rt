...
dynamic build(BuildContext context) {
  ...
  log('=== BlurStyle comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.BlurStyle> values = ui.BlurStyle.values;
  expectCondition(values.isNotEmpty, 'BlurStyle.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'BlurStyle.values contains unique entries',
  );

  final ui.BlurStyle first = values.first;
  final ui.BlurStyle last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.BlurStyle,
    'First value runtimeType is BlurStyle',
  );
  ...
  for (final ui.BlurStyle value in values) {
  ...
  final Map<String, ui.BlurStyle> byName = <String, ui.BlurStyle>{
    for (final ui.BlurStyle value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.BlurStyle> sortedByIndex = <ui.BlurStyle>[...values]
    ..sort((ui.BlurStyle a, ui.BlurStyle b) => a.index - b.index);
  ...
  log('=== BlurStyle comprehensive test completed ===');
  return Column(
