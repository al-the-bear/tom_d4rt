...
dynamic build(BuildContext context) {
  ...
  _expectCondition(
    values.isNotEmpty,
    'PointerDeviceKind has at least one value',
  );
  _expectCondition(
  ...
  print(
    'First PointerDeviceKind value: $first (${first.name}, ${first.index})',
  );
  print('Last PointerDeviceKind value: $last (${last.name}, ${last.index})');

  _expectCondition(
    PointerDeviceKind.values.byName(first.name) == first,
    'byName resolves first value',
  );
  _expectCondition(
    PointerDeviceKind.values.byName(last.name) == last,
  ...
  _expectCondition(
    first == PointerDeviceKind.values[first.index],
    'First value round-trips from index',
  );
  _expectCondition(
    last == PointerDeviceKind.values[last.index],
  ...
  _expectCondition(
    invalidNameThrows,
    'Invalid byName lookup throws for PointerDeviceKind',
  );
