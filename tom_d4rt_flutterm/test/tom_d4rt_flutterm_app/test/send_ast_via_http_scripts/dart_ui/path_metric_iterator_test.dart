...
dynamic build(BuildContext context) {
  ...
  _expect(
    metrics is Iterable<ui.PathMetric>,
    'computeMetrics returns iterable of PathMetric',
    logs,
  );
  assertionCount++;
  _expect(
    iterator is Iterator<ui.PathMetric>,
    'iterator is typed as Iterator<PathMetric>',
    logs,
  );
  assertionCount++;
  ...
  assertionCount++;
  _expect(
    runtimeName.toLowerCase().contains('metric'),
    'iterator runtime hints metric iteration',
    logs,
  );
  assertionCount++;
  ...
  while (iterator.moveNext()) {
    ...
    assertionCount++;
    _expect(
      metric.contourIndex >= 0,
      'metric contourIndex is non-negative',
      logs,
    );
    assertionCount++;
  ...
  _expect(
    !emptyMetrics.iterator.moveNext(),
    'edge case: empty path yields no metrics',
    logs,
  );
  assertionCount++;

  print(
    'iterator runtimeType=$runtimeName segmentCount=$segmentCount length=$accumulatedLength',
  );
