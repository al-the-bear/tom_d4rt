...
dynamic build(BuildContext context) {
  ...
  log('=== ClipOp comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.ClipOp> values = ui.ClipOp.values;
  expectCondition(values.isNotEmpty, 'ClipOp.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'ClipOp.values contains unique entries',
  );

  final ui.ClipOp first = values.first;
  final ui.ClipOp last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.ClipOp,
    'First value runtimeType is ClipOp',
  );
  ...
  for (final ui.ClipOp value in values) {
  ...
  final Map<String, ui.ClipOp> byName = <String, ui.ClipOp>{
    for (final ui.ClipOp value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.ClipOp> sortedByIndex = <ui.ClipOp>[...values]
    ..sort((ui.ClipOp a, ui.ClipOp b) => a.index - b.index);
  ...
  log('=== ClipOp comprehensive test completed ===');
  return Column(
