...
dynamic build(BuildContext context) {
  ...
  expectCheck(
    handleA != handleC,
    'different raw handles should compare different',
  );
  expectCheck(
    handleA.hashCode == handleB.hashCode,
    'equal handles should share hashCode',
  );

  final callback = ui.PluginUtilities.getCallbackFromHandle(handleA);
  log('getCallbackFromHandle(42) => $callback');
  expectCheck(
    callback == null || callback is Function,
    'callback must be null or Function',
  );

  final callback2 = ui.PluginUtilities.getCallbackFromHandle(handleC);
  log('getCallbackFromHandle(77) => $callback2');
  expectCheck(
    callback2 == null || callback2 is Function,
    'callback2 must be null or Function',
  );

  log('PluginUtilities type=${ui.PluginUtilities}');
  expectCheck(
    ui.PluginUtilities.toString().contains('PluginUtilities'),
    'type string should include PluginUtilities',
  );
