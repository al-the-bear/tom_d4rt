...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError(
      'ClipRSuperellipseEngineLayer assertion failed: ' + message,
    );
  }
...
dynamic build(BuildContext context) {
  ...
  final layerA = builder.pushClipRSuperellipse(
    shapeA,
    clipBehavior: ui.Clip.antiAlias,
  );

  _expect(
    layerA is ui.ClipRSuperellipseEngineLayer,
    'pushClipRSuperellipse returns ClipRSuperellipseEngineLayer',
    logs,
  );
  assertionCount++;
  _expect(
    layerA.runtimeType.toString().contains('ClipRSuperellipseEngineLayer'),
    'runtime type includes ClipRSuperellipseEngineLayer',
    logs,
  );
  assertionCount++;
  ...
  final layerB = builder.pushClipRSuperellipse(
    shapeB,
    clipBehavior: ui.Clip.hardEdge,
    oldLayer: layerA,
  );
  _expect(
    layerB is ui.ClipRSuperellipseEngineLayer,
    'push with oldLayer returns ClipRSuperellipseEngineLayer',
    logs,
  );
  assertionCount++;
  ...
  final edgeLayer = builder.pushClipRSuperellipse(
    edgeShape,
    clipBehavior: ui.Clip.none,
  );
  _expect(
    edgeLayer is ui.ClipRSuperellipseEngineLayer,
    'edge shape zero still returns layer',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    shapeA.width > 0 && shapeA.height > 0,
    'shapeA geometry properties are valid',
    logs,
  );
  assertionCount++;
