...
${_formatEnumValues(values)}');
  ...
  _expectCondition(values.isNotEmpty, 'PointerSignalKind has at least one value');
  ...
  print('First PointerSignalKind value: $first (${first.name}, ${first.index})');
  print('Last PointerSignalKind value: $last (${last.name}, ${last.index})');

  _expectCondition(PointerSignalKind.values.byName(first.name) == first, 'byName resolves first value');
  _expectCondition(PointerSignalKind.values.byName(last.name) == last, 'byName resolves last value');
  ...
  _expectCondition(first == PointerSignalKind.values[first.index], 'First value round-trips from index');
  _expectCondition(last == PointerSignalKind.values[last.index], 'Last value round-trips from index');
  ...
  _expectCondition(invalidNameThrows, 'Invalid byName lookup throws for PointerSignalKind');
