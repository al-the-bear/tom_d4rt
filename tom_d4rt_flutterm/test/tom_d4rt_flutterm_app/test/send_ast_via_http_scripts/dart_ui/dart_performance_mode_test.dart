...
dynamic build(BuildContext context) {
  ...
  log('=== DartPerformanceMode comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.DartPerformanceMode> values = ui.DartPerformanceMode.values;
  expectCondition(values.isNotEmpty, 'DartPerformanceMode.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'DartPerformanceMode.values contains unique entries',
  );

  final ui.DartPerformanceMode first = values.first;
  final ui.DartPerformanceMode last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.DartPerformanceMode,
    'First value runtimeType is DartPerformanceMode',
  );
  ...
  for (final ui.DartPerformanceMode value in values) {
  ...
  final Map<String, ui.DartPerformanceMode> byName =
      <String, ui.DartPerformanceMode>{
        for (final ui.DartPerformanceMode value in values) value.name: value,
      };
  expectCondition(
  ...
  log('=== DartPerformanceMode comprehensive test completed ===');
  return Column(
