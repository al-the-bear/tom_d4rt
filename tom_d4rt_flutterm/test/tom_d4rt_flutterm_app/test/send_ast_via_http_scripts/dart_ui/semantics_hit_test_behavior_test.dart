...
Map<int, SemanticsHitTestBehavior> _buildIndexMap(
  List<SemanticsHitTestBehavior> values,
) {
...
dynamic build(BuildContext context) {
  ...
  print(
    'SemanticsHitTestBehavior detailed values:\n${_formatEnumValues(values)}',
  );

  _expectCondition(
    values.isNotEmpty,
    'SemanticsHitTestBehavior has at least one value',
  );
  _expectCondition(
  ...
  print(
    'First SemanticsHitTestBehavior value: $first (${first.name}, ${first.index})',
  );
  print(
    'Last SemanticsHitTestBehavior value: $last (${last.name}, ${last.index})',
  );

  _expectCondition(
    SemanticsHitTestBehavior.values.byName(first.name) == first,
    'byName resolves first value',
  );
  _expectCondition(
    SemanticsHitTestBehavior.values.byName(last.name) == last,
  ...
  _expectCondition(
    first == SemanticsHitTestBehavior.values[first.index],
    'First value round-trips from index',
  );
  _expectCondition(
    last == SemanticsHitTestBehavior.values[last.index],
  ...
  try {
    SemanticsHitTestBehavior.values.byName(
      '__not_a_valid_semanticshittestbehavior_name__',
    );
  } catch (error) {
    invalidNameThrows = true;
    print(
      'Expected byName failure captured for SemanticsHitTestBehavior: $error',
    );
  }
  _expectCondition(
    invalidNameThrows,
    'Invalid byName lookup throws for SemanticsHitTestBehavior',
  );
