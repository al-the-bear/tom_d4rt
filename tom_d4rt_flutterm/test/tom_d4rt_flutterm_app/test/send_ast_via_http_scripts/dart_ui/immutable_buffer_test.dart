...
dynamic build(BuildContext context) {
  ...
  final mediumBytes = Uint8List.fromList(
    List<int>.generate(128, (index) => index % 256),
  );
  ...
  expectCheck(
    emptyFuture is Future<ui.ImmutableBuffer>,
    'emptyFuture should be Future<ImmutableBuffer>',
  );
  expectCheck(
    smallFuture is Future<ui.ImmutableBuffer>,
    'smallFuture should be Future<ImmutableBuffer>',
  );
  expectCheck(
    mediumFuture is Future<ui.ImmutableBuffer>,
    'mediumFuture should be Future<ImmutableBuffer>',
  );

  emptyFuture.then((buffer) {
    log(
      'empty buffer length=${buffer.length} disposed=${buffer.debugDisposed}',
    );
  ...
  smallFuture.then((buffer) {
    log(
      'small buffer length=${buffer.length} disposed=${buffer.debugDisposed}',
    );
  ...
  mediumFuture.then((buffer) {
    log(
      'medium buffer length=${buffer.length} disposed=${buffer.debugDisposed}',
    );
