...
dynamic build(BuildContext context) {
  ...
  _expectCondition(
    values.isNotEmpty,
    'ViewFocusDirection has at least one value',
  );
  _expectCondition(
  ...
  _expectCondition(
    ...
    'Index map covers all ViewFocusDirection values',
  );
  _expectCondition(
    names.toSet().length == names.length,
    'ViewFocusDirection names are unique',
  );
  _expectCondition(
    indexes.toSet().length == indexes.length,
    'ViewFocusDirection indexes are unique',
  );
  _expectCondition(indexes.first == 0, 'First ViewFocusDirection index is 0');
  _expectCondition(
    indexes.last == values.length - 1,
    'Last ViewFocusDirection index is values.length - 1',
  );
  ...
  _expectCondition(
    ViewFocusDirection.values.byName(first.name) == first,
    'byName resolves first ViewFocusDirection',
  );
  _expectCondition(
    ViewFocusDirection.values.byName(last.name) == last,
    'byName resolves last ViewFocusDirection',
  );
  _expectCondition(
    indexMap[first.index] == first,
    'Index map resolves first ViewFocusDirection',
  );
  _expectCondition(
    indexMap[last.index] == last,
    'Index map resolves last ViewFocusDirection',
  );
  _expectCondition(
    first == ViewFocusDirection.values[first.index],
    'First ViewFocusDirection round-trips from index',
  );
  _expectCondition(
    last == ViewFocusDirection.values[last.index],
    'Last ViewFocusDirection round-trips from index',
  );
  _expectCondition(
  ...
  _expectCondition(
    first == first,
    'Reflexive equality for first ViewFocusDirection',
  );
  _expectCondition(
    last == last,
    'Reflexive equality for last ViewFocusDirection',
  );
  ...
  _expectCondition(
    invalidNameThrows,
    'Invalid byName throws for ViewFocusDirection',
  );

  final sortedByIndex = [...values]..sort((a, b) => a.index.compareTo(b.index));
  _expectCondition(
    sortedByIndex.join('|') == values.join('|'),
    'Sorting by index preserves ViewFocusDirection order',
  );
