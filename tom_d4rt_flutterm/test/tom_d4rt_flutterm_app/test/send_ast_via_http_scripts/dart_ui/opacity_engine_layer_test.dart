...
dynamic build(BuildContext context) {
  ...
  _expect(
    layerA is ui.OpacityEngineLayer,
    'pushOpacity returns OpacityEngineLayer',
    logs,
  );
  assertionCount++;
  _expect(
    layerA.runtimeType.toString().contains('OpacityEngineLayer'),
    'runtime type includes OpacityEngineLayer',
    logs,
  );
  assertionCount++;
  builder.pop();
  ...
  final layerB = builder.pushOpacity(
    255,
    offset: const ui.Offset(10, 20),
    oldLayer: layerA,
  );
  _expect(
    layerB is ui.OpacityEngineLayer,
    'pushOpacity with oldLayer returns OpacityEngineLayer',
    logs,
  );
  assertionCount++;
  builder.pop();
  ...
  _expect(
    layerEdge is ui.OpacityEngineLayer,
    'edge alpha=0 still returns layer',
    logs,
  );
  assertionCount++;
  builder.pop();
  ...
  _expect(
    alphaValues.every((a) => a >= 0 && a <= 255),
    'alpha values are in valid range',
    logs,
  );
  assertionCount++;

  print(
    'layerA=$layerA layerB=$layerB layerEdge=$layerEdge alphas=$alphaValues',
  );
