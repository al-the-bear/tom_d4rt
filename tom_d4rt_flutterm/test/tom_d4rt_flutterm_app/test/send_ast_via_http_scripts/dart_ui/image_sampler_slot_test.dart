...
dynamic build(BuildContext context) {
  ...
  _expectCondition(typeName.contains('ImageSamplerSlot'), 'type is resolvable in dart:ui', logs);
  assertionCount++;

  final fragmentShaderType = ui.FragmentShader.toString();
  _expectCondition(fragmentShaderType.contains('FragmentShader'), 'related FragmentShader type is resolvable', logs);
  assertionCount++;

  final futureCodec = ui.instantiateImageCodec(_tinyPng());
  _expectCondition(futureCodec is Future<ui.Codec>, 'codec creation future is available', logs);
  assertionCount++;
  ...
  _expectCondition(futureImage is Future<ui.Image>, 'decoded image future is available for sampler binding', logs);
  assertionCount++;

  final conceptualName = 'sampler0';
  final conceptualIndex = 0;
  _expectCondition(conceptualName.isNotEmpty, 'conceptual sampler name for getImageSampler(name) is valid', logs);
  assertionCount++;

  _expectCondition(conceptualIndex == 0, 'conceptual sampler index for setImageSampler(index, image) is valid', logs);
  assertionCount++;
  ...
  _expectCondition(callShapeReady, 'ImageSamplerSlot call-shape behavior is documented in test path', logs);
  assertionCount++;
  ...
  _expectCondition(coverageBullets.length == 4, 'all requested coverage categories listed', logs);
  assertionCount++;
