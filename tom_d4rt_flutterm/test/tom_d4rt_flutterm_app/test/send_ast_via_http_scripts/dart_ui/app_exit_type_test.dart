...
dynamic build(BuildContext context) {
  ...
  log('=== AppExitType comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.AppExitType> values = ui.AppExitType.values;
  expectCondition(values.isNotEmpty, 'AppExitType.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'AppExitType.values contains unique entries',
  );

  final ui.AppExitType first = values.first;
  final ui.AppExitType last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.AppExitType,
    'First value runtimeType is AppExitType',
  );
  ...
  for (final ui.AppExitType value in values) {
  ...
  final Map<String, ui.AppExitType> byName = <String, ui.AppExitType>{
    for (final ui.AppExitType value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.AppExitType> sortedByIndex = <ui.AppExitType>[...values]
    ..sort((ui.AppExitType a, ui.AppExitType b) => a.index - b.index);
  ...
  log('=== AppExitType comprehensive test completed ===');
  return Column(
