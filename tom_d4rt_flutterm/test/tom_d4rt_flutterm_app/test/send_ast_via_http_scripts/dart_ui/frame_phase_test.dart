...
dynamic build(BuildContext context) {
  ...
  print('  buildStart – Framework begin frame (animate + build + layout + paint)');
  ...
  final phases = <_PhaseData>[
    _PhaseData('vsyncStart', 0.0, 0.0, Color(0xFF4A148C),
      Icons.signal_cellular_alt, 'Display VSync signal'),
    _PhaseData('buildStart', 0.8, 0.0, Color(0xFF1565C0),
      Icons.build_circle, 'Begin framework phase'),
    _PhaseData('buildFinish', 0.0, 7.2, Color(0xFF2E7D32),
      Icons.check_circle, 'Framework phase complete'),
    _PhaseData('rasterStart', 8.0, 0.0, Color(0xFFE65100),
      Icons.gradient, 'Begin GPU rasterization'),
    _PhaseData('rasterFinish', 0.0, 4.5, Color(0xFFC62828),
      Icons.done_all, 'Frame rasterized and displayed'),
  ];
            ...
            children: [
                  ...
                  children: [
                    ...
                    Text('FramePhase',
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold,
                        color: Colors.white)),
                    SizedBox(height: 6),
                    Text('Enum identifying phases of the rendering pipeline for profiling',
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text('16.67ms budget @ 60fps', style: TextStyle(
                      fontWeight: FontWeight.bold, fontSize: 10, color: Colors.grey[600])),
                    SizedBox(height: 10),
                    // Timeline bar
                    _timelineBar('Idle', 0.0, idleGap, 16.67, Color(0xFF9E9E9E)),
                    _timelineBar('Build', idleGap, buildDuration, 16.67, Color(0xFF1565C0)),
                    _timelineBar('Raster', idleGap + buildDuration, rasterDuration, 16.67,
                      Color(0xFFE65100)),
                    SizedBox(height: 12),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceAround,
                      children: [
                        _metricBadge('Build', '${buildDuration}ms', Color(0xFF1565C0)),
                        _metricBadge('Raster', '${rasterDuration}ms', Color(0xFFE65100)),
                        _metricBadge('Total', '${totalFrame}ms', Color(0xFF4A148C)),
                      ],
                    ),
                    SizedBox(height: 8),
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: totalFrame <= 16.67
                          ? Color(0xFFE8F5E9) : Color(0xFFFFEBEE),
                      ...
                      child: Text(
                        ...
                        style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold,
                          color: totalFrame <= 16.67 ? Color(0xFF2E7D32) : Color(0xFFC62828))),
                    ),
                  ],
                ),
              ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _formulaRow('Build Duration', 'buildFinish − buildStart',
                      Color(0xFF1565C0)),
                    _formulaRow('Raster Duration', 'rasterFinish − rasterStart',
                      Color(0xFFE65100)),
                    _formulaRow('Total Frame', 'rasterFinish − vsyncStart',
                      Color(0xFF4A148C)),
                    _formulaRow('Idle Gap', 'buildStart − vsyncStart',
                      Color(0xFF9E9E9E)),
                    SizedBox(height: 10),
                    _codeBlock([
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      children: [
                        SizedBox(width: 55, child: Text('Profile',
                          style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold))),
                        Expanded(child: Text('Build + Raster',
                          style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold))),
                        ...
                        FractionallySizedBox(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(4)),
                            child: Center(child: Text('60fps budget: 16.67ms',
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Flutter DevTools shows these phases in Timeline:',
                      style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                    SizedBox(height: 10),
                    _devtoolsRow('UI Thread', 'buildStart → buildFinish',
                      Color(0xFF1565C0)),
                    _devtoolsRow('Raster Thread', 'rasterStart → rasterFinish',
                      Color(0xFFE65100)),
                    _devtoolsRow('VSync', 'vsyncStart marker',
                      Color(0xFF4A148C)),
                    SizedBox(height: 10),
                    Container(
                      padding: EdgeInsets.all(8),
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _apiRow('FramePhase', 'Enum', 'Identifies each pipeline phase',
                      Color(0xFF1A237E)),
                    _apiRow('FrameTiming', 'Class', 'Contains timestamps for all phases',
                      Color(0xFF00695C)),
                    _apiRow('timestampInMicroseconds()', 'Method', 'Get phase timestamp',
                      Color(0xFFE65100)),
                    _apiRow('addTimingsCallback()', 'Method', 'Register for frame timings',
                      Color(0xFF4A148C)),
                    SizedBox(height: 8),
                    Container(
                      padding: EdgeInsets.all(8),
...
class _PhaseData {
  ...
  final IconData icon;
  final String desc;
  _PhaseData(this.name, this.startMs, this.durationMs, this.color,
    this.icon, this.desc);
}
...
class _FrameProfile {
  ...
  final double idleMs;
  final bool isJanky;
  _FrameProfile(this.label, this.buildMs, this.rasterMs, this.idleMs, this.isJanky);
    ...
    child: Row(
      children: [
        Container(
          width: 28, height: 28,
          decoration: BoxDecoration(
            color: phase.color, borderRadius: BorderRadius.circular(6)),
        ...
        Expanded(child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(phase.name, style: TextStyle(fontWeight: FontWeight.bold,
              fontSize: 11, fontFamily: 'monospace', color: phase.color)),
            Text(phase.desc, style: TextStyle(fontSize: 9, color: Colors.grey[600])),
          ],
        )),
      ],
    ),
...
Widget _timelineBar(String label, double startMs, double durMs, double budget, Color color) {
            ...
            return Stack(
              children: [
                ...
                Positioned(
                  ...
                  child: Container(
                    width: (constraints.maxWidth * durFrac).clamp(4, constraints.maxWidth),
                    height: 14,
                    decoration: BoxDecoration(
                      color: color,
                      borderRadius: BorderRadius.circular(3)),
                    child: Center(child: Text('${durMs}ms',
...
Widget _profileRow(_FrameProfile fp) {
  ...
  return Padding(
    ...
    child: Row(
      children: [
        SizedBox(width: 55, child: Row(children: [
          Icon(fp.isJanky ? Icons.warning : Icons.check_circle,
            size: 10, color: color),
          SizedBox(width: 3),
          Text(fp.label, style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold)),
        ])),
        Expanded(
          child: Container(
            height: 12,
        ...
        SizedBox(width: 4),
        SizedBox(width: 40, child: Text('${fp.totalMs.toStringAsFixed(1)}ms',
    ...
    child: Row(
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: color, shape: BoxShape.circle)),
        SizedBox(width: 8),
        SizedBox(width: 80, child: Text(thread,
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 10, color: color))),
        Expanded(child: Text(phases,
          style: TextStyle(fontFamily: 'monospace', fontSize: 9, color: Colors.grey[600]))),
      ],
    ),
  );
}
