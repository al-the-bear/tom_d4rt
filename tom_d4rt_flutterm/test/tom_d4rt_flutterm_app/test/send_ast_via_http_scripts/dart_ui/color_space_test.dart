...
dynamic build(BuildContext context) {
  ...
  log('=== ColorSpace comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<ui.ColorSpace> values = ui.ColorSpace.values;
  expectCondition(values.isNotEmpty, 'ColorSpace.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'ColorSpace.values contains unique entries',
  );

  final ui.ColorSpace first = values.first;
  final ui.ColorSpace last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == ui.ColorSpace,
    'First value runtimeType is ColorSpace',
  );
  ...
  for (final ui.ColorSpace value in values) {
  ...
  final Map<String, ui.ColorSpace> byName = <String, ui.ColorSpace>{
    for (final ui.ColorSpace value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<ui.ColorSpace> sortedByIndex = <ui.ColorSpace>[...values]
    ..sort((ui.ColorSpace a, ui.ColorSpace b) => a.index - b.index);
  ...
  log('=== ColorSpace comprehensive test completed ===');
  return Column(
