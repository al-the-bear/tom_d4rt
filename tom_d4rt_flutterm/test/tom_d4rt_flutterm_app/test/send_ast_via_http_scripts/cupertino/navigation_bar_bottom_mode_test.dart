// D4rt comprehensive test script: NavigationBarBottomMode (cupertino)
import 'package:flutter/cupertino.dart';

class NavigationBarBottomModeCaseStudy {
  NavigationBarBottomModeCaseStudy({
    required this.label,
  ...
  final String label;
  final List<NavigationBarBottomMode> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'NavigationBarBottomMode must expose at least one value');
    ...
    final byName = NavigationBarBottomMode.values.byName(samples.first.name);
  ...
  dynamic buildSummary(BuildContext context, List<String> logs) {
    ...
    return Column(
      ...
      children: [
        Text('NavigationBarBottomMode summary'),
        Text('label: $label'),
...
dynamic build(BuildContext context) {
  print('NavigationBarBottomMode test start');
  final values = NavigationBarBottomMode.values;
  final caseStudy = NavigationBarBottomModeCaseStudy(
    label: 'NavigationBarBottomMode-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'NavigationBarBottomMode must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('NavigationBarBottomMode test completed with ${logs.length} log lines');
