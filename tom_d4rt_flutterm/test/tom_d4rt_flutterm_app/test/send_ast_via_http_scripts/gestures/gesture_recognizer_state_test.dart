...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting GestureRecognizerState checks');

  final values = GestureRecognizerState.values;
  _check(
  ...
  final holders = values
      .map(
        (value) => _EnumHolder<GestureRecognizerState>(
          value: value,
          label: value.name,
        ),
      )
  ...
  final allRoundTrips = values
      .map((value) => GestureRecognizerState.values.byName(value.name) == value)
  ...
  try {
    GestureRecognizerState.values.byName('__GestureRecognizerState_invalid__');
  } catch (_) {
  ...
  final summary =
      'GestureRecognizerState summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
  ...
  return ListView(
    ...
    children: [
      const Text(
        'GestureRecognizerState Test Summary',
