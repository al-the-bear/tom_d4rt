...
dynamic build(BuildContext context) {
  ...
  final eventA = PointerCancelEvent(
    position: const Offset(10, 20),
    device: 3,
    pointer: 11,
  );
  final eventB = PointerCancelEvent(
    position: const Offset(-1, -4),
    device: 0,
    pointer: 0,
  );
  expectCondition(
    eventA is PointerEvent,
    'PointerCancelEvent is a PointerEvent',
    logs,
    counters,
  );
  expectCondition(
    eventA.position.dx == 10,
    'eventA dx recorded',
    logs,
    counters,
  );
  expectCondition(
    eventB.position.dy == -4,
    'eventB supports negative coordinates edge case',
    logs,
    counters,
  );
