...
dynamic build(BuildContext context) {
  ...
  final update = DragUpdateDetails(
    ...
    delta: const Offset(5, 15),
  );
  final end = DragEndDetails(
    velocity: const Velocity(pixelsPerSecond: Offset(120, 80)),
  );

  log(
    'down=${down.globalPosition} start=${start.globalPosition} update.delta=${update.delta}',
  );
  ...
  expectCheck(
    update.delta == Offset(5, 15),
    'DragUpdateDetails delta should match input',
  );
  expectCheck(
    end.velocity.pixelsPerSecond.dx == 120,
    'DragEndDetails velocity should match input',
  );
