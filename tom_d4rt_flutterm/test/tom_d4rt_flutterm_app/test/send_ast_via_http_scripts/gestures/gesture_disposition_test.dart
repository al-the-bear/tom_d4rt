...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting GestureDisposition checks');

  final values = GestureDisposition.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<GestureDisposition>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => GestureDisposition.values.byName(value.name) == value)
  ...
  try {
    GestureDisposition.values.byName('__GestureDisposition_invalid__');
  } catch (_) {
  ...
  final summary =
      'GestureDisposition summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
  ...
  return ListView(
    ...
    children: [
      const Text(
        'GestureDisposition Test Summary',
