...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting MultitouchDragStrategy checks');

  final values = MultitouchDragStrategy.values;
  _check(
  ...
  final holders = values
      .map(
        (value) => _EnumHolder<MultitouchDragStrategy>(
          value: value,
          label: value.name,
        ),
      )
  ...
  final allRoundTrips = values
      .map((value) => MultitouchDragStrategy.values.byName(value.name) == value)
  ...
  try {
    MultitouchDragStrategy.values.byName('__MultitouchDragStrategy_invalid__');
  } catch (_) {
  ...
  final summary =
      'MultitouchDragStrategy summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
  ...
  return ListView(
    ...
    children: [
      const Text(
        'MultitouchDragStrategy Test Summary',
