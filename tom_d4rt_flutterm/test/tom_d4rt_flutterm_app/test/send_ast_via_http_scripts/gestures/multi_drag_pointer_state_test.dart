// D4rt comprehensive test script for MultiDragPointerState
import 'package:flutter/gestures.dart';
import 'package:flutter/material.dart';

class _MultiDragPointerStateProbe extends MultiDragPointerState {
  _MultiDragPointerStateProbe(super.initialPosition);

  @override
  void checkForResolutionAfterMove() {
    print('checkForResolutionAfterMove: $initialPosition');
  }

  @override
  void accepted(GestureMultiDragStartCallback starter) {
    print('accepted called with starter: $starter');
  }

  @override
  void dispose() {
    print('dispose for pointer state at $initialPosition');
    super.dispose();
  }
}
...
dynamic build(BuildContext context) {
  const targetClassName = 'MultiDragPointerState';
  const testLabel = 'MultiDragPointerState';
  final logs = <String>[];
  ...
  final stateA = _MultiDragPointerStateProbe(const Offset(3, 4));
  final stateB = _MultiDragPointerStateProbe(const Offset(-12, 40));
  expectCondition(stateA is MultiDragPointerState, 'stateA is MultiDragPointerState subtype', logs, counters);
  expectCondition(stateB.initialPosition.dx == -12, 'edge case negative initial dx preserved', logs, counters);
  expectCondition(stateA.initialPosition == const Offset(3, 4), 'stateA initialPosition stored', logs, counters);
  stateA.checkForResolutionAfterMove();
  stateB.checkForResolutionAfterMove();
  stateA.dispose();
  stateB.dispose();
