// Comprehensive D4rt test script: VelocityDrag from gestures
      ...
      child: Card(
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              Text('D4rt gestures test: \$title', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive VelocityDrag script ===');
  ...
  detailLines.add('target=VelocityDrag');
  detailLines.add('package=gestures');
  ...
  const String targetTypeName = 'VelocityDrag';
  check(targetTypeName == 'VelocityDrag', 'Name matches');
  detailLines.add('category=gestures_velocity');
  detailLines.add('desc=Velocity tracking for drag gestures');
  // Test via VelocityTracker
  final VelocityTracker tracker = VelocityTracker.withKind(PointerDeviceKind.touch);
  check(tracker is VelocityTracker, 'Created VelocityTracker');
  detailLines.add('trackerType=${tracker.runtimeType}');
