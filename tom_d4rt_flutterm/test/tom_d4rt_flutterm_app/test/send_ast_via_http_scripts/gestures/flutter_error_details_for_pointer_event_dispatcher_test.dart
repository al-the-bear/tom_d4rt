...
dynamic build(BuildContext context) {
  print(
    'FlutterErrorDetailsForPointerEventDispatcher comprehensive test start',
  );
  final logs = <String>[];
  var pass = 0;
  var fail = 0;
  ...
  _check(
    logs: logs,
    label: 'details instantiated',
    condition: details is FlutterErrorDetailsForPointerEventDispatcher,
  );
  _check(
    logs: logs,
    label: 'exception preserved',
    condition: details.exception is StateError,
  );
  _check(
    logs: logs,
    label: 'library preserved',
    condition: details.library == 'gestures library',
  );
  _check(
    logs: logs,
    label: 'event preserved',
    condition: details.event == pointer,
  );
  _check(
    logs: logs,
    label: 'hitTestEntry optional null',
    condition: details.hitTestEntry == null,
  );
  ...
  _check(
    logs: logs,
    label: 'minimal constructor usage works',
    condition: basic is FlutterErrorDetailsForPointerEventDispatcher,
  );
  _check(
    logs: logs,
    label: 'minimal event null accepted',
    condition: basic.event == null,
  );

  final summary = details.toStringShort();
  _check(
    logs: logs,
    label: 'summary text available',
    condition: summary.isNotEmpty,
  );
  ...
  print(
    'FlutterErrorDetailsForPointerEventDispatcher comprehensive test finished: pass=$pass fail=$fail',
  );
