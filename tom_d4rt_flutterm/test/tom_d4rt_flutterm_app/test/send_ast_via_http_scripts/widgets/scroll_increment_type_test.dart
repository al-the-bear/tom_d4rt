// D4rt comprehensive test script: ScrollIncrementType from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('ScrollIncrementType test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== ScrollIncrementType comprehensive test start ==========');

  final List<ScrollIncrementType> values = ScrollIncrementType.values.toList(
    growable: false,
  );
  _expect(
    values.isNotEmpty,
    'ScrollIncrementType exposes at least one enum value',
  );
  _expect(
    values.length == ScrollIncrementType.values.length,
    'ScrollIncrementType.values list length is consistent',
  );

  final ScrollIncrementType first = values.first;
  final ScrollIncrementType last = values.last;

  print(_line('runtimeType', ScrollIncrementType));
  ...
  for (final value in values) {
    ...
    _expect(
      ScrollIncrementType.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = ScrollIncrementType.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    ScrollIncrementType.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<ScrollIncrementType> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<ScrollIncrementType> sorted = [...values]
  ...
  final Map<String, ScrollIncrementType> asMap = {
  ...
  final String summary =
      'ScrollIncrementType checks passed: ${values.length} values validated';
  print(summary);
  print('========== ScrollIncrementType comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('ScrollIncrementType Comprehensive Tests'),
