// D4rt comprehensive test script: RepeatMode from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('RepeatMode test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== RepeatMode comprehensive test start ==========');

  final List<RepeatMode> values = RepeatMode.values.toList(growable: false);
  _expect(values.isNotEmpty, 'RepeatMode exposes at least one enum value');
  _expect(
    values.length == RepeatMode.values.length,
    'RepeatMode.values list length is consistent',
  );

  final RepeatMode first = values.first;
  final RepeatMode last = values.last;

  print(_line('runtimeType', RepeatMode));
  ...
  for (final value in values) {
    ...
    _expect(
      RepeatMode.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = RepeatMode.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    RepeatMode.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<RepeatMode> reversed = values.reversed.toList(growable: false);
  ...
  final List<RepeatMode> sorted = [...values]
  ...
  final Map<String, RepeatMode> asMap = {
  ...
  final String summary =
      'RepeatMode checks passed: ${values.length} values validated';
  print(summary);
  print('========== RepeatMode comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('RepeatMode Comprehensive Tests'),
