// Comprehensive D4rt test script: ReorderableList from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive ReorderableList script ===');
  ...
  detailLines.add('target=ReorderableList');
  ...
  final Widget list = ReorderableList(
    itemBuilder: (BuildContext ctx, int i) => ListTile(key: ValueKey<int>(i), title: Text('Item \$i'), leading: const Icon(Icons.drag_handle)),
    itemCount: 5,
    onReorder: (int oldIndex, int newIndex) { print('reorder \$oldIndex -> \$newIndex'); },
  );
  check(list is ReorderableList, 'ReorderableList created');
  check(list is StatefulWidget, 'Is StatefulWidget');
