// Comprehensive D4rt test script: DoNothingAndStopPropagationIntent from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive DoNothingAndStopPropagationIntent script ===');
  ...
  detailLines.add('target=DoNothingAndStopPropagationIntent');
  ...
  const String targetTypeName = 'DoNothingAndStopPropagationIntent';
  check(targetTypeName.contains('StopPropagation'), 'Intent name includes StopPropagation');
  detailLines.add('description=Intent that does nothing and stops propagation');
  // Used with Actions framework to stop intent propagation
  final Widget actionsWidget = Actions(
    actions: <Type, Action<Intent>>{},
    child: const SizedBox.shrink(),
  );
  check(actionsWidget is Actions, 'Actions widget supports stop-propagation intents');
