...
// D4rt comprehensive test script for FocusHighlightMode
...
dynamic build(BuildContext context) {
  ...
  print('========== FocusHighlightMode comprehensive test ==========' );

  final List<FocusHighlightMode> values = FocusHighlightMode.values;
  expectCondition(values.isNotEmpty, 'FocusHighlightMode.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'FocusHighlightMode should provide multiple values', logs, counters);

  final FocusHighlightMode first = values.first;
  final FocusHighlightMode last = values.last;
  ...
  for (final FocusHighlightMode value in values) {
    ...
    final FocusHighlightMode byName = FocusHighlightMode.values.byName(value.name);
  ...
  final List<FocusHighlightMode> copied = List<FocusHighlightMode>.from(values);
  ...
  final List<FocusHighlightMode> reversed = values.reversed.toList(growable: false);
  ...
  final FocusHighlightMode edgeByName = FocusHighlightMode.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('FocusHighlightMode Comprehensive Tests'),
