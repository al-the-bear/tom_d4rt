// Comprehensive D4rt test script: PasteTextIntent from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive PasteTextIntent script ===');
  ...
  detailLines.add('target=PasteTextIntent');
  ...
  const String targetTypeName = 'PasteTextIntent';
  check(targetTypeName.contains('Paste'), 'Name includes Paste');
  detailLines.add('description=Intent to paste text from clipboard');
  final TextEditingController tec = TextEditingController(text: 'Paste test');
  final Widget textField = TextField(controller: tec);
  check(textField is TextField, 'TextField for paste intent created');
  tec.dispose();
