...
// D4rt comprehensive test script for AutovalidateMode
...
dynamic build(BuildContext context) {
  ...
  print('========== AutovalidateMode comprehensive test ==========' );

  final List<AutovalidateMode> values = AutovalidateMode.values;
  expectCondition(values.isNotEmpty, 'AutovalidateMode.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'AutovalidateMode should provide multiple values', logs, counters);

  final AutovalidateMode first = values.first;
  final AutovalidateMode last = values.last;
  ...
  for (final AutovalidateMode value in values) {
    ...
    final AutovalidateMode byName = AutovalidateMode.values.byName(value.name);
  ...
  final List<AutovalidateMode> copied = List<AutovalidateMode>.from(values);
  ...
  final List<AutovalidateMode> reversed = values.reversed.toList(growable: false);
  ...
  final AutovalidateMode edgeByName = AutovalidateMode.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('AutovalidateMode Comprehensive Tests'),
