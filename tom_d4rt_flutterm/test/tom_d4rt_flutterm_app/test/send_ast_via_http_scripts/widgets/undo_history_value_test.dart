// Comprehensive D4rt test script: UndoHistoryValue from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive UndoHistoryValue script ===');
  ...
  detailLines.add('target=UndoHistoryValue');
  ...
  const UndoHistoryValue val = UndoHistoryValue(canUndo: true, canRedo: false);
  check(val.canUndo == true, 'Can undo');
  check(val.canRedo == false, 'Cannot redo');
  const UndoHistoryValue empty = UndoHistoryValue.empty;
  check(empty.canUndo == false, 'Empty no undo');
  check(empty.canRedo == false, 'Empty no redo');
  detailLines.add('canUndo=true, canRedo=false');
