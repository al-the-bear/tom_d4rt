// D4rt test script: Comprehensive ViewportElementMixin coverage
...
dynamic build(BuildContext context) {
  print('--- ViewportElementMixin test start ---');
  final logs = <String>[];
  ...
  final viewportA = Viewport(
    offset: ViewportOffset.fixed(0.0),
    axisDirection: AxisDirection.down,
    slivers: const <Widget>[SliverToBoxAdapter(child: SizedBox(width: 12, height: 12))],
  );
  final elementA = viewportA.createElement();
  expectCondition(elementA is ViewportElementMixin, 'Viewport element mixes in ViewportElementMixin', logs, counters);
  expectCondition(elementA.runtimeType.toString().isNotEmpty, 'Viewport element type is readable', logs, counters);
  final viewportB = Viewport(
    offset: ViewportOffset.fixed(16.0),
    axisDirection: AxisDirection.down,
    cacheExtent: 120.0,
    slivers: const <Widget>[
      SliverToBoxAdapter(child: SizedBox(width: 8, height: 20)),
      SliverToBoxAdapter(child: SizedBox(width: 8, height: 24)),
    ],
  );
  final elementB = viewportB.createElement();
  expectCondition(elementB is ViewportElementMixin, 'Second element also uses the mixin', logs, counters);
  expectCondition(!identical(elementA, elementB), 'Different viewport instances create different elements', logs, counters);
  expectCondition(viewportB.children.length == 2, 'Second viewport has two slivers', logs, counters);
  final growable = <Element>[elementA, elementB];
  expectCondition(growable.length == 2, 'Two elements tracked in list', logs, counters);
  for (final item in growable) {
    expectCondition(item is ViewportElementMixin, 'Every tracked element has mixin', logs, counters);
  }
  expectCondition(viewportA.offset.pixels == 0.0, 'First viewport offset is fixed at zero', logs, counters);
  expectCondition(viewportB.offset.pixels == 16.0, 'Second viewport offset is fixed at 16', logs, counters);
  ...
  print('--- ViewportElementMixin test end ---');
  ...
  return Column(
    ...
    children: [
      summaryLine('ViewportElementMixin summary widget'),
      summaryLine('Title: ViewportElementMixin'),
