...
dynamic build(BuildContext context) {
  ...
  expectCondition(values.isNotEmpty, 'ContextMenuButtonType.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'ContextMenuButtonType should provide multiple values', logs, counters);
  ...
  for (final ContextMenuButtonType value in values) {
    ...
    final ContextMenuButtonType byName = ContextMenuButtonType.values.byName(value.name);
  ...
  final List<ContextMenuButtonType> copied = List<ContextMenuButtonType>.from(values);
  ...
  final List<ContextMenuButtonType> reversed = values.reversed.toList(growable: false);
  ...
  final ContextMenuButtonType edgeByName = ContextMenuButtonType.values.byName(edgeName);
