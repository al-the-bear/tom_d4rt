// Comprehensive D4rt test script: ReorderableDelayedDragStartListener from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive ReorderableDelayedDragStartListener script ===');
  ...
  detailLines.add('target=ReorderableDelayedDragStartListener');
  ...
  const String targetTypeName = 'ReorderableDelayedDragStartListener';
  check(targetTypeName.contains('Reorderable'), 'Name includes Reorderable');
  detailLines.add('description=Delayed drag start listener for reorderable lists');
  final Widget list = ReorderableList(itemBuilder: (BuildContext ctx, int i) => ListTile(key: ValueKey<int>(i), title: Text('Item \$i')), itemCount: 5, onReorder: (int a, int b) {});
  check(list is ReorderableList, 'ReorderableList created');
