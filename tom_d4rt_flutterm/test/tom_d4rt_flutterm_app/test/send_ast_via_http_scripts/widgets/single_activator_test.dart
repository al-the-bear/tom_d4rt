// Comprehensive D4rt test script: SingleActivator from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SingleActivator script ===');
  ...
  detailLines.add('target=SingleActivator');
  ...
  const String targetTypeName = 'SingleActivator';
  detailLines.add('category=widgets_shortcuts');
  detailLines.add('desc=Single key shortcut activator');
  // SingleActivator requires services import
  const SingleActivator activator = SingleActivator(LogicalKeyboardKey.keyA, control: true);
  check(activator is ShortcutActivator, 'Is ShortcutActivator');
  check(activator.trigger == LogicalKeyboardKey.keyA, 'Trigger key');
  check(activator.control == true, 'Control modifier');
  detailLines.add('trigger=keyA');
  detailLines.add('control=true');
