...
// D4rt comprehensive test script for HeroFlightDirection
...
dynamic build(BuildContext context) {
  ...
  print('========== HeroFlightDirection comprehensive test ==========' );

  final List<HeroFlightDirection> values = HeroFlightDirection.values;
  expectCondition(values.isNotEmpty, 'HeroFlightDirection.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'HeroFlightDirection should provide multiple values', logs, counters);

  final HeroFlightDirection first = values.first;
  final HeroFlightDirection last = values.last;
  ...
  for (final HeroFlightDirection value in values) {
    ...
    final HeroFlightDirection byName = HeroFlightDirection.values.byName(value.name);
  ...
  final List<HeroFlightDirection> copied = List<HeroFlightDirection>.from(values);
  ...
  final List<HeroFlightDirection> reversed = values.reversed.toList(growable: false);
  ...
  final HeroFlightDirection edgeByName = HeroFlightDirection.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('HeroFlightDirection Comprehensive Tests'),
