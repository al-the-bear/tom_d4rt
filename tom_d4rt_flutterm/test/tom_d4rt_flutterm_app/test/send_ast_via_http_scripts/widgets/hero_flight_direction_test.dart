...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    values.isNotEmpty,
    'HeroFlightDirection.values should not be empty',
    logs,
    counters,
  );
  expectCondition(
    values.length >= 2,
    'HeroFlightDirection should provide multiple values',
    logs,
    counters,
  );
  ...
  for (final HeroFlightDirection value in values) {
    ...
    final HeroFlightDirection byName = HeroFlightDirection.values.byName(
      value.name,
    );
    expectCondition(
      byName == value,
  ...
  final List<HeroFlightDirection> copied = List<HeroFlightDirection>.from(
    values,
  );
  expectCondition(
  ...
  final List<HeroFlightDirection> reversed = values.reversed.toList(
    growable: false,
  );
  expectCondition(
    reversed.first == last,
  ...
  final HeroFlightDirection edgeByName = HeroFlightDirection.values.byName(
    edgeName,
  );
  expectCondition(
