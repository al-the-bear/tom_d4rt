// D4rt comprehensive test script for WidgetsBindingObserver
...
class _WidgetsBindingObserverProbe with WidgetsBindingObserver {
  _WidgetsBindingObserverProbe(this.id);

  final String id;

  @override
  String toString() => 'Observer($id)';
}
...
dynamic build(BuildContext context) {
  const targetClassName = 'WidgetsBindingObserver';
  const testLabel = 'WidgetsBindingObserver';
  final logs = <String>[];
  ...
  final observerA = _WidgetsBindingObserverProbe('observer-A');
  final observerB = _WidgetsBindingObserverProbe('observer-B');
  final binding = WidgetsFlutterBinding.ensureInitialized();
  binding.addObserver(observerA);
  binding.removeObserver(observerA);
  binding.addObserver(observerB);
  binding.removeObserver(observerB);
  expectCondition(observerA is WidgetsBindingObserver, 'observerA implements WidgetsBindingObserver', logs, counters);
  expectCondition(observerB is WidgetsBindingObserver, 'observerB implements WidgetsBindingObserver', logs, counters);
  expectCondition(binding is WidgetsBinding, 'binding available for observer lifecycle', logs, counters);
  details['observerA'] = observerA.id;
  details['observerB'] = observerB.id;
