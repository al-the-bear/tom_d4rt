...
dynamic build(BuildContext context) {
  ...
  log('=== WidgetInspectorServiceExtensions comprehensive test start ===');
  ...
  expectCondition(
    values.isNotEmpty,
    'WidgetInspectorServiceExtensions.values is not empty',
  );
  expectCondition(
    values.toSet().length == values.length,
    'WidgetInspectorServiceExtensions.values contains unique entries',
  );

  final WidgetInspectorServiceExtensions first = values.first;
  final WidgetInspectorServiceExtensions last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == WidgetInspectorServiceExtensions,
    'First value runtimeType is WidgetInspectorServiceExtensions',
  );
  ...
  for (final WidgetInspectorServiceExtensions value in values) {
  ...
  final Map<String, WidgetInspectorServiceExtensions> byName =
      <String, WidgetInspectorServiceExtensions>{
        for (final WidgetInspectorServiceExtensions value in values)
          value.name: value,
      };
  expectCondition(
  ...
  const String missingName = '__missing_widget_inspector_service_extensions__';
  expectCondition(
    !byName.containsKey(missingName),
    'Invalid name is absent from lookup map',
  );
  final int outOfRangeIndex = values.length + 99;
  final WidgetInspectorServiceExtensions? safeLookup =
  ...
  final List<String> summary = <String>[
    'WidgetInspectorServiceExtensions summary',
    'count=${values.length}',
    'first=${first.name}',
    'last=${last.name}',
    'checksum=$checksum',
  ...
  log('=== WidgetInspectorServiceExtensions comprehensive test completed ===');
  return Column(
