...
Widget _child(Color color) =>
    ColoredBox(color: color, child: const SizedBox(width: 24, height: 24));
...
dynamic build(BuildContext context) {
  ...
  _expect(
    a.duration == const Duration(milliseconds: 200),
    'duration set on a',
    logs,
  );
  assertionCount++;
  _expect(
    (a.padding as EdgeInsets).left == 8,
    'padding all(8) applied to a',
    logs,
  );
  assertionCount++;
  ...
  assertionCount++;
  _expect(
    (b.padding as EdgeInsets).horizontal == 24,
    'horizontal padding aggregated for b',
    logs,
  );
  assertionCount++;
  _expect(
    (b.padding as EdgeInsets).vertical == 8,
    'vertical padding aggregated for b',
    logs,
  );
  assertionCount++;
  _expect(
    c.padding == EdgeInsets.zero,
    'edge case zero padding retained',
    logs,
  );
  assertionCount++;
  ...
  assertionCount++;
  _expect(
    widgets.every((w) => w.child != null),
    'all instances carry child widgets',
    logs,
  );
  assertionCount++;
