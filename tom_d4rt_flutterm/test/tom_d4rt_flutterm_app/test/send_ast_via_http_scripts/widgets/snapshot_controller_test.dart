// Comprehensive D4rt test script: SnapshotController from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SnapshotController script ===');
  ...
  detailLines.add('target=SnapshotController');
  ...
  const String targetTypeName = 'SnapshotController';
  detailLines.add('category=widgets_snapshot');
  detailLines.add('desc=Controls snapshot rasterization');
  final SnapshotController ctrl = SnapshotController(allowSnapshotting: true);
  check(ctrl is SnapshotController, 'Created');
  check(ctrl.allowSnapshotting == true, 'Allow=true');
  ctrl.allowSnapshotting = false;
  check(ctrl.allowSnapshotting == false, 'Allow=false');
  ctrl.dispose();
