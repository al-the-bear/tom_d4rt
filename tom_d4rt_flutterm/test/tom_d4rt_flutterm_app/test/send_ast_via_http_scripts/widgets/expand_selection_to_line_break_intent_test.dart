...
// D4rt test script: Tests ExpandSelectionToLineBreakIntent from widgets
...
dynamic build(BuildContext context) {
  ...
  print('ExpandSelectionToLineBreakIntent test executing');
  print('=' * 50);

  const ExpandSelectionToLineBreakIntent intentForward =
      ExpandSelectionToLineBreakIntent(forward: true);
  const ExpandSelectionToLineBreakIntent intentBackward =
  ...
  runCase('runtime type is correct', () {
    return intentForward.runtimeType == ExpandSelectionToLineBreakIntent;
  });

  runCase('two const forward instances are identical', () {
    const ExpandSelectionToLineBreakIntent other =
        ExpandSelectionToLineBreakIntent(forward: true);
  ...
  return Column(
    ...
    children: <Widget>[
      const Text('ExpandSelectionToLineBreakIntent Tests',
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
      const SizedBox(height: 8),
      Text('Passed: ${passed.length}'),
      Text('Failed: ${failed.length}'),
      Text(failed.isEmpty ? 'Status: PASS' : 'Status: FAIL'),
      const Text('ExpandSelectionToLineBreakIntent behavior checks completed'),
    ],
  );
}
