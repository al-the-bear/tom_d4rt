// D4rt comprehensive test script for SingleTickerProviderStateMixin
import 'package:flutter/material.dart';

class _SingleTickerProbeState extends State<StatefulWidget> with SingleTickerProviderStateMixin {
  _SingleTickerProbeState(this.id);
...
class _TickerProviderProbeState extends State<StatefulWidget> with TickerProviderStateMixin {
...
dynamic build(BuildContext context) {
  const targetClassName = 'SingleTickerProviderStateMixin';
  const testLabel = 'SingleTickerProviderStateMixin';
  final logs = <String>[];
  ...
  final stateA = _SingleTickerProbeState('single-A');
  final stateB = _SingleTickerProbeState('single-B');
  final tickerA = stateA.createTicker((_) {});
  final tickerB = stateB.createTicker((_) {});
  expectCondition(stateA is SingleTickerProviderStateMixin, 'stateA mixes in SingleTickerProviderStateMixin', logs, counters);
  expectCondition(tickerA.isActive == false, 'tickerA initially inactive', logs, counters);
  expectCondition(tickerB.isActive == false, 'tickerB initially inactive', logs, counters);
  tickerA.dispose();
  tickerB.dispose();
