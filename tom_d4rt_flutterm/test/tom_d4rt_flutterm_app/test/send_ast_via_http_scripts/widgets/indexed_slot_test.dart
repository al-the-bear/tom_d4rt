// Comprehensive D4rt test script: IndexedSlot from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive IndexedSlot script ===');
  ...
  detailLines.add('target=IndexedSlot');
  ...
  // IndexedSlot: slot with an index and a value
  check(true, 'IndexedSlot is used by the Element tree internally');
  detailLines.add('description=A slot with an index used during updateChildren');
  // Build a widget that produces multiple children to exercise IndexedSlot
  final Widget stack = Stack(children: <Widget>[
    Positioned(left: 0, child: Container(width: 50, height: 50, color: const Color(0xFFEF4444))),
    Positioned(left: 60, child: Container(width: 50, height: 50, color: const Color(0xFF3B82F6))),
  ]);
  check(stack is Stack, 'Stack with indexed children created');
