// Comprehensive D4rt test script: ReorderableListState from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive ReorderableListState script ===');
  ...
  detailLines.add('target=ReorderableListState');
  ...
  const String targetTypeName = 'ReorderableListState';
  check(targetTypeName.contains('State'), 'Name includes State');
  detailLines.add('description=State for ReorderableList');
  final Widget list = ReorderableList(itemBuilder: (BuildContext ctx, int i) => ListTile(key: ValueKey<int>(i), title: Text('R \$i')), itemCount: 4, onReorder: (int a, int b) {});
