// Comprehensive D4rt test script: LogicalKeySet from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive LogicalKeySet script ===');
  ...
  detailLines.add('target=LogicalKeySet');
  ...
  final LogicalKeySet singleKey = LogicalKeySet(LogicalKeyboardKey.escape);
  check(singleKey.keys.length == 1, 'Single key set has 1 key');
  final LogicalKeySet combo = LogicalKeySet(LogicalKeyboardKey.control, LogicalKeyboardKey.keyA);
  check(combo.keys.length == 2, 'Combo set has 2 keys');
  check(combo.keys.contains(LogicalKeyboardKey.control), 'Contains control');
  detailLines.add('singleKeyCount=1');
  detailLines.add('comboKeyCount=2');
