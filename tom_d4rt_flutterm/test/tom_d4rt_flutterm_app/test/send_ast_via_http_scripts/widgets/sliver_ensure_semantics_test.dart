// Comprehensive D4rt test script: SliverEnsureSemantics from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverEnsureSemantics script ===');
  ...
  detailLines.add('target=SliverEnsureSemantics');
  ...
  // SliverEnsureSemantics is an internal widget, test concept
  const String targetTypeName = 'SliverEnsureSemantics';
  detailLines.add('category=widgets_sliver');
  detailLines.add('desc=Ensures semantics nodes in sliver viewport');
  check(targetTypeName.contains('Semantics'), 'Name contains Semantics');
  // Test via Semantics wrapping a sliver-compatible widget
  final Semantics semSliver = Semantics(label: 'sliver content', child: SizedBox(height: 100));
  check(semSliver is Widget, 'Semantics widget created');
