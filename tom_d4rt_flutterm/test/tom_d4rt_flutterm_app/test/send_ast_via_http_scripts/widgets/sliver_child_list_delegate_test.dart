// Comprehensive D4rt test script: SliverChildListDelegate from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverChildListDelegate script ===');
  ...
  detailLines.add('target=SliverChildListDelegate');
  ...
  final SliverChildListDelegate delegate = SliverChildListDelegate(<Widget>[Container(height: 50, color: const Color(0xFF42A5F5)), Container(height: 70, color: const Color(0xFFEF5350)), Container(height: 60, color: const Color(0xFF66BB6A))]);
  check(delegate is SliverChildDelegate, 'Is SliverChildDelegate');
  check(delegate.children.length == 3, 'Has 3 children');
  final Widget? child = delegate.build(context, 1);
  check(child != null, 'Built child at 1');
  detailLines.add('childCount=3');
