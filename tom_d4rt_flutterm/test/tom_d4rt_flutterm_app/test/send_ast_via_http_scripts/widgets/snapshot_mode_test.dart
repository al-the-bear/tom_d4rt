// D4rt comprehensive test script: SnapshotMode from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('SnapshotMode test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== SnapshotMode comprehensive test start ==========');

  final List<SnapshotMode> values = SnapshotMode.values.toList(growable: false);
  _expect(values.isNotEmpty, 'SnapshotMode exposes at least one enum value');
  _expect(
    values.length == SnapshotMode.values.length,
    'SnapshotMode.values list length is consistent',
  );

  final SnapshotMode first = values.first;
  final SnapshotMode last = values.last;

  print(_line('runtimeType', SnapshotMode));
  ...
  for (final value in values) {
    ...
    _expect(
      SnapshotMode.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = SnapshotMode.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    SnapshotMode.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<SnapshotMode> reversed = values.reversed.toList(growable: false);
  ...
  final List<SnapshotMode> sorted = [...values]
  ...
  final Map<String, SnapshotMode> asMap = {
  ...
  final String summary =
      'SnapshotMode checks passed: ${values.length} values validated';
  print(summary);
  print('========== SnapshotMode comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('SnapshotMode Comprehensive Tests'),
