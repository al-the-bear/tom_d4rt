// Comprehensive D4rt test script: SliverChildBuilderDelegate from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverChildBuilderDelegate script ===');
  ...
  detailLines.add('target=SliverChildBuilderDelegate');
  ...
  final SliverChildBuilderDelegate delegate = SliverChildBuilderDelegate((BuildContext ctx, int index) { return Container(height: 50, color: Color(0xFF42A5F5 + index * 0x111111)); }, childCount: 10);
  check(delegate is SliverChildDelegate, 'Is SliverChildDelegate');
  check(delegate.childCount == 10, 'Count=10');
  final Widget? child = delegate.build(context, 0);
  check(child != null, 'Built child at 0');
  detailLines.add('childCount=10');
