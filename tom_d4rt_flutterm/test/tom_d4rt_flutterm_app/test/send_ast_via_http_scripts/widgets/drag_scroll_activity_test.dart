// Comprehensive D4rt test script: DragScrollActivity from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive DragScrollActivity script ===');
  ...
  detailLines.add('target=DragScrollActivity');
  ...
  const String targetTypeName = 'DragScrollActivity';
  check(targetTypeName == 'DragScrollActivity', 'Target type name set');
  detailLines.add('description=Scroll activity when user drags');
  final ScrollController sc = ScrollController();
  final Widget listView = ListView.builder(
    controller: sc,
    itemCount: 100,
    itemBuilder: (BuildContext ctx, int i) => Text('Item \$i'),
  );
  check(listView is ListView, 'ListView created (produces DragScrollActivity)');
  sc.dispose();
