// D4rt comprehensive test script: OverflowBarAlignment from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('OverflowBarAlignment test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== OverflowBarAlignment comprehensive test start ==========');

  final List<OverflowBarAlignment> values = OverflowBarAlignment.values.toList(
    growable: false,
  );
  _expect(
    values.isNotEmpty,
    'OverflowBarAlignment exposes at least one enum value',
  );
  _expect(
    values.length == OverflowBarAlignment.values.length,
    'OverflowBarAlignment.values list length is consistent',
  );

  final OverflowBarAlignment first = values.first;
  final OverflowBarAlignment last = values.last;

  print(_line('runtimeType', OverflowBarAlignment));
  ...
  for (final value in values) {
    ...
    _expect(
      OverflowBarAlignment.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = OverflowBarAlignment.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    OverflowBarAlignment.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<OverflowBarAlignment> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<OverflowBarAlignment> sorted = [...values]
  ...
  final Map<String, OverflowBarAlignment> asMap = {
  ...
  final String summary =
      'OverflowBarAlignment checks passed: ${values.length} values validated';
  print(summary);
  print('========== OverflowBarAlignment comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('OverflowBarAlignment Comprehensive Tests'),
