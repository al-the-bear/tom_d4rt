// Comprehensive D4rt test script: NestedScrollViewViewport from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive NestedScrollViewViewport script ===');
  ...
  detailLines.add('target=NestedScrollViewViewport');
  ...
  const String targetTypeName = 'NestedScrollViewViewport';
  check(targetTypeName.contains('Viewport'), 'Name includes Viewport');
  detailLines.add('description=Viewport for NestedScrollView');
  final Widget nested = NestedScrollView(
    headerSliverBuilder: (BuildContext ctx, bool b) => <Widget>[const SliverToBoxAdapter(child: Text('H'))],
    body: const Text('Body'),
  );
  check(nested is NestedScrollView, 'NestedScrollView (contains viewport) created');
