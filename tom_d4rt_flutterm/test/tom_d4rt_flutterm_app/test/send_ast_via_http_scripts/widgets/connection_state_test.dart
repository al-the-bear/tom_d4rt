...
// D4rt comprehensive test script for ConnectionState
...
dynamic build(BuildContext context) {
  ...
  print('========== ConnectionState comprehensive test ==========' );

  final List<ConnectionState> values = ConnectionState.values;
  expectCondition(values.isNotEmpty, 'ConnectionState.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'ConnectionState should provide multiple values', logs, counters);

  final ConnectionState first = values.first;
  final ConnectionState last = values.last;
  ...
  for (final ConnectionState value in values) {
    ...
    final ConnectionState byName = ConnectionState.values.byName(value.name);
  ...
  final List<ConnectionState> copied = List<ConnectionState>.from(values);
  ...
  final List<ConnectionState> reversed = values.reversed.toList(growable: false);
  ...
  final ConnectionState edgeByName = ConnectionState.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('ConnectionState Comprehensive Tests'),
