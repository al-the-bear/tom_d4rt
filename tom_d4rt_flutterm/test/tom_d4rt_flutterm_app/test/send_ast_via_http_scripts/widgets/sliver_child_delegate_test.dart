// Comprehensive D4rt test script: SliverChildDelegate from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverChildDelegate script ===');
  ...
  detailLines.add('target=SliverChildDelegate');
  ...
  const String targetTypeName = 'SliverChildDelegate';
  detailLines.add('category=widgets_sliver');
  detailLines.add('desc=Abstract delegate for sliver children');
  // Test via concrete subclass
  final SliverChildListDelegate listDel = SliverChildListDelegate(const <Widget>[SizedBox(height: 50), SizedBox(height: 60)]);
  check(listDel is SliverChildDelegate, 'Is SliverChildDelegate');
  final Widget? child = listDel.build(context, 0);
  check(child != null, 'Built child');
