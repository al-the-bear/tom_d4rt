// Comprehensive D4rt test script: ScrollView from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive ScrollView script ===');
  ...
  detailLines.add('target=ScrollView');
  ...
  // ScrollView is abstract - ListView is a concrete subclass
  final Widget list = ListView(children: List<Widget>.generate(10, (int i) => ListTile(title: Text('Item \$i'))));
  check(list is ListView, 'ListView created');
  check(list is ScrollView, 'ListView is ScrollView');
  final Widget grid = GridView.count(crossAxisCount: 2, children: const <Widget>[Text('A'), Text('B')]);
  check(grid is ScrollView, 'GridView is ScrollView');
  detailLines.add('scrollViewSubclasses=ListView,GridView');
