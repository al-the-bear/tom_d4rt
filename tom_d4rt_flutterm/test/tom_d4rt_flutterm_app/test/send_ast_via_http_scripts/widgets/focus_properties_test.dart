// Comprehensive D4rt test script: FocusProperties from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive FocusProperties script ===');
  ...
  detailLines.add('target=FocusProperties');
  ...
  const String targetTypeName = 'FocusProperties';
  check(targetTypeName == 'FocusProperties', 'Name matches');
  detailLines.add('category=widgets_focus');
  detailLines.add('desc=Focus node properties and configuration');
  final FocusNode node = FocusNode(debugLabel: 'testNode');
  check(node is FocusNode, 'Created FocusNode');
  check(node.debugLabel == 'testNode', 'Label set');
  check(node.hasFocus == false, 'No focus initially');
  check(node.canRequestFocus == true, 'Can request focus');
  node.dispose();
  detailLines.add('canRequestFocus=true');
