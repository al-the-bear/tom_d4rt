// Comprehensive D4rt test script: DisposableBuildContext from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive DisposableBuildContext script ===');
  ...
  detailLines.add('target=DisposableBuildContext');
  ...
  // DisposableBuildContext: build context that can be disposed
  const String targetTypeName = 'DisposableBuildContext';
  check(targetTypeName == 'DisposableBuildContext', 'Target type name set');
  detailLines.add('description=A BuildContext that tracks whether its State is mounted');
  // We test via ScrollController which uses DisposableBuildContext internally
  final ScrollController sc = ScrollController();
  check(sc is ScrollController, 'ScrollController created (uses DisposableBuildContext)');
  detailLines.add('scrollControllerCreated=true');
  sc.dispose();
  detailLines.add('scrollControllerDisposed=true');
