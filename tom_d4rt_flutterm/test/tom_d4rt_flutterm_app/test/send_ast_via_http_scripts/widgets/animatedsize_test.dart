...
dynamic build(BuildContext context) {
  ...
  _expect(
    a.duration == const Duration(milliseconds: 250),
    'constructor sets duration on a',
    logs,
  );
  assertionCount++;
  ...
  assertionCount++;
  _expect(
    b.alignment == Alignment.topLeft,
    'constructor sets alignment on b',
    logs,
  );
  assertionCount++;
  _expect(
    b.clipBehavior == Clip.hardEdge,
    'constructor sets clipBehavior on b',
    logs,
  );
  assertionCount++;
  _expect(
    c.reverseDuration == const Duration(milliseconds: 2),
    'constructor sets reverseDuration on c',
    logs,
  );
  assertionCount++;

  _expect(
    a.child != null && b.child != null && c.child != null,
    'all instances contain child widgets',
    logs,
  );
  assertionCount++;
  _expect(
    a.runtimeType.toString().contains('AnimatedSize'),
    'runtime type includes AnimatedSize',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    widgets.length == 3,
    'behavior path aggregates multiple instances',
    logs,
  );
  assertionCount++;

  print(
    'AnimatedSize instances created: ${widgets.map((w) => w.duration).toList()}',
  );
