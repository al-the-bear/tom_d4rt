// D4rt comprehensive test script: LiveTextInputStatus from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('LiveTextInputStatus test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== LiveTextInputStatus comprehensive test start ==========');

  final List<LiveTextInputStatus> values = LiveTextInputStatus.values.toList(
    growable: false,
  );
  _expect(
    values.isNotEmpty,
    'LiveTextInputStatus exposes at least one enum value',
  );
  _expect(
    values.length == LiveTextInputStatus.values.length,
    'LiveTextInputStatus.values list length is consistent',
  );

  final LiveTextInputStatus first = values.first;
  final LiveTextInputStatus last = values.last;

  print(_line('runtimeType', LiveTextInputStatus));
  ...
  for (final value in values) {
    ...
    _expect(
      LiveTextInputStatus.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = LiveTextInputStatus.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    LiveTextInputStatus.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<LiveTextInputStatus> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<LiveTextInputStatus> sorted = [...values]
  ...
  final Map<String, LiveTextInputStatus> asMap = {
  ...
  final String summary =
      'LiveTextInputStatus checks passed: ${values.length} values validated';
  print(summary);
  print('========== LiveTextInputStatus comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('LiveTextInputStatus Comprehensive Tests'),
