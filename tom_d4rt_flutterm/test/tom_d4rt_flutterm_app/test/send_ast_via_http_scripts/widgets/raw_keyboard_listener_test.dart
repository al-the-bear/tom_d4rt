// Comprehensive D4rt test script: RawKeyboardListener from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive RawKeyboardListener script ===');
  ...
  detailLines.add('target=RawKeyboardListener');
  ...
  final Widget listener = RawKeyboardListener(
    focusNode: fn,
    onKey: (RawKeyEvent event) { print('key=' + event.runtimeType.toString()); },
    child: const Text('Keyboard'),
  );
  check(listener is RawKeyboardListener, 'RawKeyboardListener created');
  check(listener is StatefulWidget, 'Is StatefulWidget');
  fn.dispose();
