// Comprehensive D4rt test script: FocusAttachment from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive FocusAttachment script ===');
  ...
  detailLines.add('target=FocusAttachment');
  ...
  const String targetTypeName = 'FocusAttachment';
  check(targetTypeName == 'FocusAttachment', 'Target type name set');
  detailLines.add('description=Attachment point for FocusNode');
  final FocusNode node = FocusNode(debugLabel: 'test-node');
  check(node.debugLabel == 'test-node', 'FocusNode debug label set');
  detailLines.add('focusNodeLabel=' + (node.debugLabel ?? 'null'));
  node.dispose();
