// Comprehensive D4rt test script: DualTransitionBuilder from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive DualTransitionBuilder script ===');
  ...
  detailLines.add('target=DualTransitionBuilder');
  ...
  // DualTransitionBuilder: animates between forward/reverse child
  final AnimationController anim = AnimationController(
    duration: const Duration(milliseconds: 300),
    vsync: const TestVSync(),
  );
  final Widget dtb = DualTransitionBuilder(
    animation: anim,
    forwardBuilder: (BuildContext ctx, Animation<double> a, Widget? child) {
      return FadeTransition(opacity: a, child: child);
    },
    reverseBuilder: (BuildContext ctx, Animation<double> a, Widget? child) {
      return FadeTransition(opacity: a, child: child);
    },
    child: const Text('Dual transition'),
  );
  check(dtb is DualTransitionBuilder, 'DualTransitionBuilder created');
  anim.dispose();
