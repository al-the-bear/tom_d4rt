// Comprehensive D4rt test script: GestureDetectorAdv from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive GestureDetectorAdv script ===');
  ...
  detailLines.add('target=GestureDetectorAdv');
  ...
  const String targetTypeName = 'GestureDetectorAdv';
  detailLines.add('category=widgets_gesture');
  detailLines.add('desc=Advanced gesture detector usage');
  int tapCount = 0;
  final GestureDetector gd = GestureDetector(onTap: () { tapCount++; }, onDoubleTap: () { tapCount += 2; }, onLongPress: () { tapCount += 10; }, child: Container(width: 100, height: 100, color: const Color(0xFF66BB6A)));
  check(gd is Widget, 'Is Widget');
  check(gd.onTap != null, 'Has onTap');
  check(gd.onDoubleTap != null, 'Has onDoubleTap');
  check(gd.onLongPress != null, 'Has onLongPress');
  detailLines.add('gestures=tap,doubleTap,longPress');
