...
dynamic build(BuildContext context) {
  ...
  log('=== TraversalDirection comprehensive test start ===');
  log('BuildContext type: ${context.runtimeType}');

  final List<TraversalDirection> values = TraversalDirection.values;
  expectCondition(values.isNotEmpty, 'TraversalDirection.values is not empty');
  expectCondition(
    values.toSet().length == values.length,
    'TraversalDirection.values contains unique entries',
  );

  final TraversalDirection first = values.first;
  final TraversalDirection last = values.last;

  log('Constructor-like checks (enum canonical instances)');
  expectCondition(
    first.runtimeType == TraversalDirection,
    'First value runtimeType is TraversalDirection',
  );
  ...
  for (final TraversalDirection value in values) {
  ...
  final Map<String, TraversalDirection> byName = <String, TraversalDirection>{
    for (final TraversalDirection value in values) value.name: value,
  };
  expectCondition(
  ...
  final List<TraversalDirection> sortedByIndex = <TraversalDirection>[...values]
    ..sort((TraversalDirection a, TraversalDirection b) => a.index - b.index);
  ...
  log('=== TraversalDirection comprehensive test completed ===');
  return Column(
