// Comprehensive D4rt test script: Widget from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive Widget script ===');
  ...
  const String targetTypeName = 'Widget';
  detailLines.add('category=widgets_core');
  detailLines.add('desc=Base Widget class coverage');
  // SizedBox is a concrete Widget
  const SizedBox sb = SizedBox(width: 50, height: 50);
  check(sb is Widget, 'SizedBox is Widget');
  check(sb.key == null, 'No key by default');
  const SizedBox keyed = SizedBox(key: ValueKey<String>('k'), width: 10, height: 10);
  check(keyed.key != null, 'Has key');
  detailLines.add('widgetExamples=SizedBox');
