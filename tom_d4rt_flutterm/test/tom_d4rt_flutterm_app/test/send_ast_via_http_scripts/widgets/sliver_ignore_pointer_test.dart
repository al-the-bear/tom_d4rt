// Comprehensive D4rt test script: SliverIgnorePointer from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverIgnorePointer script ===');
  ...
  detailLines.add('target=SliverIgnorePointer');
  ...
  const SliverIgnorePointer ignore = SliverIgnorePointer(sliver: SliverToBoxAdapter(child: SizedBox(height: 80)));
  check(ignore is Widget, 'Is Widget');
  check(ignore.ignoring == true, 'Default ignoring=true');
  const SliverIgnorePointer notIgnore = SliverIgnorePointer(ignoring: false, sliver: SliverToBoxAdapter(child: SizedBox(height: 80)));
  check(notIgnore.ignoring == false, 'ignoring=false');
  detailLines.add('ignoring=true');
