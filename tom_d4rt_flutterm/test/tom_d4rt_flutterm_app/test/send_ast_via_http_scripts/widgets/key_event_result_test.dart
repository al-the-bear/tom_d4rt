...
// D4rt comprehensive test script for KeyEventResult
...
dynamic build(BuildContext context) {
  ...
  print('========== KeyEventResult comprehensive test ==========' );

  final List<KeyEventResult> values = KeyEventResult.values;
  expectCondition(values.isNotEmpty, 'KeyEventResult.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'KeyEventResult should provide multiple values', logs, counters);

  final KeyEventResult first = values.first;
  final KeyEventResult last = values.last;
  ...
  for (final KeyEventResult value in values) {
    ...
    final KeyEventResult byName = KeyEventResult.values.byName(value.name);
  ...
  final List<KeyEventResult> copied = List<KeyEventResult>.from(values);
  ...
  final List<KeyEventResult> reversed = values.reversed.toList(growable: false);
  ...
  final KeyEventResult edgeByName = KeyEventResult.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('KeyEventResult Comprehensive Tests'),
