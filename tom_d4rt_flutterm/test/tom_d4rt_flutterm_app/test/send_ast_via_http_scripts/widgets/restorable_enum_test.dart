// Comprehensive D4rt test script: RestorableEnum from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive RestorableEnum script ===');
  ...
  detailLines.add('target=RestorableEnum');
  ...
  const String targetTypeName = 'RestorableEnum';
  check(targetTypeName.contains('Enum'), 'Name includes Enum');
  detailLines.add('description=Restorable enum value');
  final RestorableInt ri = RestorableInt(42);
  check(ri.value == 42, 'RestorableInt (similar pattern) works');
  ri.dispose();
