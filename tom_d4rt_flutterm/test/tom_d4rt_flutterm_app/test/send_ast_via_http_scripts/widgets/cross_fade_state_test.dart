...
// D4rt comprehensive test script for CrossFadeState
...
dynamic build(BuildContext context) {
  ...
  print('========== CrossFadeState comprehensive test ==========' );

  final List<CrossFadeState> values = CrossFadeState.values;
  expectCondition(values.isNotEmpty, 'CrossFadeState.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'CrossFadeState should provide multiple values', logs, counters);

  final CrossFadeState first = values.first;
  final CrossFadeState last = values.last;
  ...
  for (final CrossFadeState value in values) {
    ...
    final CrossFadeState byName = CrossFadeState.values.byName(value.name);
  ...
  final List<CrossFadeState> copied = List<CrossFadeState>.from(values);
  ...
  final List<CrossFadeState> reversed = values.reversed.toList(growable: false);
  ...
  final CrossFadeState edgeByName = CrossFadeState.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('CrossFadeState Comprehensive Tests'),
