...
// D4rt test script: Tests ExtendSelectionToLineBreakIntent from widgets
...
dynamic build(BuildContext context) {
  ...
  print('ExtendSelectionToLineBreakIntent test executing');
  print('=' * 50);

  const ExtendSelectionToLineBreakIntent intentForwardCollapse =
      ExtendSelectionToLineBreakIntent(forward: true, collapseSelection: true);
  const ExtendSelectionToLineBreakIntent intentBackwardExtend =
  ...
  runCase('runtime type is correct', () {
    return intentForwardCollapse.runtimeType == ExtendSelectionToLineBreakIntent;
  });

  runCase('two const instances with same params are identical', () {
    const ExtendSelectionToLineBreakIntent other =
        ExtendSelectionToLineBreakIntent(forward: true, collapseSelection: true);
  ...
  return Column(
    ...
    children: <Widget>[
      const Text('ExtendSelectionToLineBreakIntent Tests',
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
      const SizedBox(height: 8),
      Text('Passed: ${passed.length}'),
      Text('Failed: ${failed.length}'),
      Text(failed.isEmpty ? 'Status: PASS' : 'Status: FAIL'),
      const Text('ExtendSelectionToLineBreakIntent behavior checks completed'),
    ],
  );
}
