// Comprehensive D4rt test script: LabeledGlobalKey from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive LabeledGlobalKey script ===');
  ...
  detailLines.add('target=LabeledGlobalKey');
  ...
  final GlobalKey gk = LabeledGlobalKey<State>('test-label');
  check(gk is GlobalKey, 'LabeledGlobalKey is a GlobalKey');
  check(gk.toString().contains('test-label'), 'Label appears in toString');
  detailLines.add('label=test-label');
  final GlobalKey gk2 = LabeledGlobalKey<State>('test-label');
  check(gk != gk2, 'Two LabeledGlobalKeys with same label are distinct');
