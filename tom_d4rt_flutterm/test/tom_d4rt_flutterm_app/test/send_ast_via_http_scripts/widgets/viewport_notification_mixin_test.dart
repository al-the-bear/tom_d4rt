...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    overscrollA is ViewportNotificationMixin,
    'Overscroll notification includes ViewportNotificationMixin',
    logs,
    counters,
  );
  expectCondition(
    overscrollB is ViewportNotificationMixin,
    'Second overscroll notification includes mixin',
    logs,
    counters,
  );
  expectCondition(
    overscrollA.depth == 0,
    'Initial depth is zero for overscrollA',
    logs,
    counters,
  );
  expectCondition(
    overscrollB.depth == 0,
    'Initial depth is zero for overscrollB',
    logs,
    counters,
  );
  ...
  expectCondition(
    details.any((e) => e.contains('depth')),
    'Description includes depth text',
    logs,
    counters,
  );
  final metrics = const FixedScrollMetrics(
    minScrollExtent: 0,
    maxScrollExtent: 500,
    pixels: 120,
  ...
  expectCondition(
    scrollStart is ViewportNotificationMixin,
    'ScrollStartNotification also has mixin',
    logs,
    counters,
  );
  expectCondition(
    scrollStart.depth == 0,
    'ScrollStart depth starts at zero',
    logs,
    counters,
  );
  final scrollEnd = ScrollEndNotification(metrics: metrics, context: null);
  expectCondition(
    scrollEnd is ViewportNotificationMixin,
    'ScrollEndNotification has mixin',
    logs,
    counters,
  );
  expectCondition(
    scrollEnd.depth == 0,
    'ScrollEnd depth starts at zero',
    logs,
    counters,
  );
  for (final n in <Notification>[
    overscrollA,
    overscrollB,
    scrollStart,
    scrollEnd,
    ...
    expectCondition(
      ...
      'Notification has string output',
      logs,
      counters,
    );
  }
  expectCondition(
    metrics.pixels == 120,
    'Metrics exposes pixel offset',
    logs,
    counters,
  );
  expectCondition(
    metrics.maxScrollExtent > metrics.minScrollExtent,
    'Metrics bounds are valid',
    logs,
    counters,
  );
  expectCondition(
