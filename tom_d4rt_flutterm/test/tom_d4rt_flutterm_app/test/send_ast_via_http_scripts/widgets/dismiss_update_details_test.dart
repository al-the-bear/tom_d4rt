// Comprehensive D4rt test script: DismissUpdateDetails from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive DismissUpdateDetails script ===');
  ...
  detailLines.add('target=DismissUpdateDetails');
  ...
  // DismissUpdateDetails: details about an ongoing dismiss gesture
  const String targetTypeName = 'DismissUpdateDetails';
  check(targetTypeName == 'DismissUpdateDetails', 'Target type name set');
  detailLines.add('description=Holds details about dismiss gesture progress');
  final Widget dismissible = Dismissible(
    key: const ValueKey<String>('details-test'),
    onUpdate: (DismissUpdateDetails details) {
      print('progress=' + details.progress.toString());
    },
    child: const Text('Dismiss details test'),
  );
  check(dismissible is Dismissible, 'Dismissible with onUpdate created');
