...
// D4rt comprehensive test script for FocusHighlightStrategy
...
dynamic build(BuildContext context) {
  ...
  print('========== FocusHighlightStrategy comprehensive test ==========' );

  final List<FocusHighlightStrategy> values = FocusHighlightStrategy.values;
  expectCondition(values.isNotEmpty, 'FocusHighlightStrategy.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'FocusHighlightStrategy should provide multiple values', logs, counters);

  final FocusHighlightStrategy first = values.first;
  final FocusHighlightStrategy last = values.last;
  ...
  for (final FocusHighlightStrategy value in values) {
    ...
    final FocusHighlightStrategy byName = FocusHighlightStrategy.values.byName(value.name);
  ...
  final List<FocusHighlightStrategy> copied = List<FocusHighlightStrategy>.from(values);
  ...
  final List<FocusHighlightStrategy> reversed = values.reversed.toList(growable: false);
  ...
  final FocusHighlightStrategy edgeByName = FocusHighlightStrategy.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('FocusHighlightStrategy Comprehensive Tests'),
