...
// D4rt comprehensive test script for BannerLocation
...
dynamic build(BuildContext context) {
  ...
  print('========== BannerLocation comprehensive test ==========' );

  final List<BannerLocation> values = BannerLocation.values;
  expectCondition(values.isNotEmpty, 'BannerLocation.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'BannerLocation should provide multiple values', logs, counters);

  final BannerLocation first = values.first;
  final BannerLocation last = values.last;
  ...
  for (final BannerLocation value in values) {
    ...
    final BannerLocation byName = BannerLocation.values.byName(value.name);
  ...
  final List<BannerLocation> copied = List<BannerLocation>.from(values);
  ...
  final List<BannerLocation> reversed = values.reversed.toList(growable: false);
  ...
  final BannerLocation edgeByName = BannerLocation.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('BannerLocation Comprehensive Tests'),
