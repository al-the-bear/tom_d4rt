// Comprehensive D4rt test script: IconThemeData from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive IconThemeData script ===');
  ...
  detailLines.add('target=IconThemeData');
  ...
  // IconThemeData: defines icon theme properties
  const IconThemeData themeData = IconThemeData(
    color: Color(0xFF3B82F6),
    size: 24.0,
    opacity: 0.8,
  );
  check(themeData.color == const Color(0xFF3B82F6), 'Color set');
  check(themeData.size == 24.0, 'Size is 24.0');
  check(themeData.opacity == 0.8, 'Opacity is 0.8');
  detailLines.add('color=' + themeData.color.toString());
  detailLines.add('size=' + themeData.size.toString());
  // Test merge / copyWith
  final IconThemeData merged = themeData.copyWith(size: 32.0);
  check(merged.size == 32.0, 'Merged size is 32.0');
  check(merged.color == themeData.color, 'Merged color preserved');
