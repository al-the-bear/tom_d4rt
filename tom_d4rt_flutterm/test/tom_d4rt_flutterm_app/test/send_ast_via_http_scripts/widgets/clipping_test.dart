// Comprehensive D4rt test script: Clipping from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive Clipping script ===');
  ...
  detailLines.add('target=Clipping');
  ...
  // Tests various clipping widgets
  const String targetTypeName = 'Clipping';
  final ClipRect clipRect = ClipRect(child: Container(width: 100, height: 100, color: const Color(0xFF42A5F5)));
  check(clipRect is Widget, 'ClipRect is Widget');
  final ClipRRect clipRRect = ClipRRect(borderRadius: BorderRadius.circular(16), child: Container(width: 100, height: 100, color: const Color(0xFFEF5350)));
  check(clipRRect is Widget, 'ClipRRect is Widget');
  const ClipOval clipOval = ClipOval(child: SizedBox(width: 80, height: 80));
  check(clipOval is Widget, 'ClipOval is Widget');
  detailLines.add('clipTypes=ClipRect,ClipRRect,ClipOval');
