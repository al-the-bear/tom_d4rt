// Comprehensive D4rt test script: NestedScrollViewState from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive NestedScrollViewState script ===');
  ...
  detailLines.add('target=NestedScrollViewState');
  ...
  const String targetTypeName = 'NestedScrollViewState';
  check(targetTypeName.contains('State'), 'Name includes State');
  detailLines.add('description=State for NestedScrollView');
  final Widget nested = NestedScrollView(
    headerSliverBuilder: (BuildContext ctx, bool innerBoxIsScrolled) => <Widget>[const SliverAppBar(title: Text('Header'))],
    body: const Center(child: Text('Body')),
  );
  check(nested is NestedScrollView, 'NestedScrollView created');
