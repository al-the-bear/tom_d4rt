// Comprehensive D4rt test script: Dismissible from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive Dismissible script ===');
  ...
  detailLines.add('target=Dismissible');
  ...
  final ValueKey<String> dKey = const ValueKey<String>('item-1');
  bool dismissed = false;
  final Widget dismissible = Dismissible(
    key: dKey,
    onDismissed: (DismissDirection dir) { dismissed = true; },
    background: Container(color: const Color(0xFFEF4444)),
    child: const ListTile(title: Text('Swipe me')),
  );
  check(dismissible is Dismissible, 'Dismissible instance created');
  check(dismissible.key == dKey, 'Dismissible key assigned');
  check(!dismissed, 'Not yet dismissed initially');
  detailLines.add('dismissDirection=horizontal (default)');
  // Test with vertical direction
  final Widget vDismissible = Dismissible(
    key: const ValueKey<String>('item-v'),
    direction: DismissDirection.vertical,
    onDismissed: (_) {},
    child: const Text('Vertical dismiss'),
  );
  check(vDismissible is Dismissible, 'Vertical Dismissible created');
  detailLines.add('verticalDismissible=created');
