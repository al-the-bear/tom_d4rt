// D4rt comprehensive test script for SlottedMultiChildRenderObjectWidgetMixin
...
class _SlottedMultiChildRenderObjectWidgetMixinProbe
    implements SlottedMultiChildRenderObjectWidgetMixin {
  _SlottedMultiChildRenderObjectWidgetMixinProbe(
    this.label, {
    required this.seed,
  });
...
Map<String, dynamic> _runProbeCase({
  ...
  required _SlottedMultiChildRenderObjectWidgetMixinProbe probe,
...
dynamic build(BuildContext context) {
  print(
    '=== SlottedMultiChildRenderObjectWidgetMixin comprehensive script start ===',
  );

  final _SlottedMultiChildRenderObjectWidgetMixinProbe primary =
      _SlottedMultiChildRenderObjectWidgetMixinProbe('primary', seed: 7);
  final _SlottedMultiChildRenderObjectWidgetMixinProbe edge =
      _SlottedMultiChildRenderObjectWidgetMixinProbe('', seed: -3);
  final _SlottedMultiChildRenderObjectWidgetMixinProbe large =
      _SlottedMultiChildRenderObjectWidgetMixinProbe('large-case', seed: 999);

  _assertTrue(primary is Object, "primary object existence");
  _assertTrue(
    primary.runtimeType.toString().contains("Probe"),
    "runtime type naming",
  );
  ...
  _assertTrue(
    caseResults.any((Map<String, dynamic> m) => m["even"] == true),
    "at least one even score",
  );
  _assertTrue(
    caseResults.any((Map<String, dynamic> m) => (m["score"] as int) < 0) ==
        false,
    "scores non-negative after increment",
  );

  final List<String> mergedLogs = <String>[
    ...primary.logs,
    ...edge.logs,
    ...large.logs,
  ];
  ...
  _assertTrue(
    mergedLogs.any((String value) => value.contains("edge")),
    "edge marker exists",
  );
  ...
  _assertTrue(
    stats["totalLogs"]! >= stats["primaryLogs"]!,
    "total logs >= primary logs",
  );
  _assertTrue(
    stats.values.every((int value) => value >= 1),
    "each stats bucket >= 1",
  );
  ...
  final List<String> scoreLines = caseResults
      .map(
        (Map<String, dynamic> result) =>
            "${result["case"]}:${result["score"]}:${result["even"]}",
      )
      .toList(growable: false);

  _assertEquals(scoreLines.length, 3, "score line count");
  _assertTrue(
    scoreLines.join("|").contains("standard"),
    "score lines include standard",
  );
  print(
    '=== SlottedMultiChildRenderObjectWidgetMixin comprehensive script done ===',
  );
  ...
  return Directionality(
    ...
    child: Column(
      ...
      children: <Widget>[
        const Text('SlottedMultiChildRenderObjectWidgetMixin Summary'),
