// Comprehensive D4rt test script: RawRadio from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive RawRadio script ===');
  ...
  detailLines.add('target=RawRadio');
  ...
  const String targetTypeName = 'RawRadio';
  check(targetTypeName.contains('Radio'), 'Name includes Radio');
  detailLines.add('description=Low-level radio button');
  int? selected = 1;
  final Widget radio = Radio<int>(value: 1, groupValue: selected, onChanged: (int? v) { selected = v; });
  check(radio is Radio, 'Radio widget created');
  final Widget radio2 = Radio<int>(value: 2, groupValue: selected, onChanged: (int? v) { selected = v; });
  check(radio2 is Radio, 'Second radio widget created');
