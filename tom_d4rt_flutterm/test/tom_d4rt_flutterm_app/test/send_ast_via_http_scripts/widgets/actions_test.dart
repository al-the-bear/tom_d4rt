...
dynamic build(BuildContext context) {
  ...
  _expect(
    actionsMap.containsKey(_SaveIntent),
    'actions map contains _SaveIntent key',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    result == 'saved',
    'CallbackAction returns expected invoke result',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    dispatcher.runtimeType == ActionDispatcher,
    'ActionDispatcher can be instantiated',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    actionsWidget.actions.length == 1,
    'Actions widget stores provided action map',
    logs,
  );
  assertionCount++;
  _expect(
    actionsWidget.dispatcher == dispatcher,
    'Actions widget stores provided dispatcher',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    unknown == null,
    'edge case unknown intent returns null action',
    logs,
  );
  assertionCount++;
