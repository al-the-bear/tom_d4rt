// Comprehensive D4rt test script: SliverAnimatedGrid from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverAnimatedGrid script ===');
  ...
  detailLines.add('target=SliverAnimatedGrid');
  ...
  final GlobalKey<SliverAnimatedGridState> key = GlobalKey<SliverAnimatedGridState>();
  final SliverAnimatedGrid grid = SliverAnimatedGrid(key: key, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3), itemBuilder: (BuildContext ctx, int index, Animation<double> anim) { return ScaleTransition(scale: anim, child: Container(color: const Color(0xFF42A5F5))); }, initialItemCount: 6);
  check(grid is Widget, 'Is Widget');
  check(grid.initialItemCount == 6, 'Initial count');
  detailLines.add('crossAxisCount=3');
  detailLines.add('initialItems=6');
