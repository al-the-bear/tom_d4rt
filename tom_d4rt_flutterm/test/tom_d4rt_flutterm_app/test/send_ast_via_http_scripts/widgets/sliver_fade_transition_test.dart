// Comprehensive D4rt test script: SliverFadeTransition from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive SliverFadeTransition script ===');
  ...
  detailLines.add('target=SliverFadeTransition');
  ...
  final AnimationController ctrl = AnimationController(value: 0.7, vsync: const TestVSyncAll());
  final SliverFadeTransition fade = SliverFadeTransition(opacity: ctrl, sliver: const SliverToBoxAdapter(child: SizedBox(height: 100)));
  check(fade is Widget, 'Is Widget');
  check(ctrl.value == 0.7, 'Opacity=0.7');
  detailLines.add('opacity=0.7');
  ctrl.dispose();
