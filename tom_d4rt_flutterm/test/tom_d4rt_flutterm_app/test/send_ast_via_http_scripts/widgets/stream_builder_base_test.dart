// Comprehensive D4rt test script: StreamBuilderBase from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive StreamBuilderBase script ===');
  ...
  detailLines.add('target=StreamBuilderBase');
  ...
  const String targetTypeName = 'StreamBuilderBase';
  detailLines.add('category=widgets_async');
  detailLines.add('desc=Base class for stream-based builders');
  // StreamBuilder extends StreamBuilderBase
  final StreamBuilder<int> sb = StreamBuilder<int>(stream: Stream<int>.fromIterable(<int>[1, 2, 3]), builder: (BuildContext ctx, AsyncSnapshot<int> snap) { return Text('val=\${snap.data}'); });
  check(sb is Widget, 'StreamBuilder is Widget');
  detailLines.add('streamItems=3');
