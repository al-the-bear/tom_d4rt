// Comprehensive D4rt test script: IOSSystemContextMenuItemSelectAll from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive IOSSystemContextMenuItemSelectAll script ===');
  ...
  detailLines.add('target=IOSSystemContextMenuItemSelectAll');
  ...
  const String targetTypeName = 'IOSSystemContextMenuItemSelectAll';
  check(targetTypeName.startsWith('IOS'), 'Is iOS context menu item');
  detailLines.add('variant=SelectAll');
  detailLines.add('description=iOS system context menu item (SelectAll)');
  detailLines.add('platform=iOS-only');
  // iOS items are platform-specific; test menu structure
  final Widget menu = PopupMenuButton<String>(
    itemBuilder: (BuildContext ctx) => <PopupMenuEntry<String>>[
      const PopupMenuItem<String>(value: 'action', child: Text('Context action')),
    ],
    child: const Text('Menu'),
  );
  check(menu is PopupMenuButton, 'PopupMenuButton as iOS menu proxy created');
