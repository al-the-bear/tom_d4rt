...
dynamic build(BuildContext context) {
  const testId = 'widgets.navigatorstate';
  log('=== start $testId ===');
  ...
  log('nav runtimeType=${nav.runtimeType}');
  log('maybeNav runtimeType=${maybeNav?.runtimeType}');
  log('rootNav runtimeType=${rootNav.runtimeType}');

  expectCheck(maybeNav != null, 'Navigator.maybeOf should resolve in this harness');
  expectCheck(identical(nav, maybeNav), 'Navigator.of and maybeOf should return same state');
  expectCheck(nav.mounted, 'NavigatorState should be mounted');

  final canPop = nav.canPop();
  final overlay = nav.overlay;
  final focusNode = nav.focusNode;
  final contextRef = nav.context;

  log('canPop=$canPop overlay=$overlay focusNode=$focusNode');
  log('navigator context runtimeType=${contextRef.runtimeType}');

  expectCheck(overlay != null, 'overlay should be available');
  expectCheck(focusNode != null, 'focusNode should be available');
  expectCheck(Navigator.defaultRouteName == '/', 'default route name should be /');
