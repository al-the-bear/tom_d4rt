// Comprehensive D4rt test script: ScrollAwareImageProvider from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive ScrollAwareImageProvider script ===');
  ...
  detailLines.add('target=ScrollAwareImageProvider');
  ...
  const String targetTypeName = 'ScrollAwareImageProvider';
  check(targetTypeName.contains('Image'), 'Name includes Image');
  detailLines.add('description=Image provider aware of scroll position');
  final Widget img = Image.network('https://example.com/img.png', errorBuilder: (BuildContext c, Object e, StackTrace? s) => const Icon(Icons.image));
  check(img is Image, 'Image widget created');
