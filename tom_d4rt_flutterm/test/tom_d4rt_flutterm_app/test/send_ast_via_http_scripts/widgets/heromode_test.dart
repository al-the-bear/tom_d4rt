...
dynamic build(BuildContext context) {
  ...
  final enabled = HeroMode(enabled: true, child: _heroChild('hero-enabled'));
  final disabled = HeroMode(enabled: false, child: _heroChild('hero-disabled'));
  ...
  assertionCount++;
  _expect(
    toggleCases.any((m) => m.enabled) && toggleCases.any((m) => !m.enabled),
    'both enabled states covered',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    edge.child is SizedBox,
    'edge case allows non-Hero child safely',
    logs,
  );
  assertionCount++;
