// D4rt comprehensive test script: ScrollPositionAlignmentPolicy from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('ScrollPositionAlignmentPolicy test failed: $message');
  }
  print('✅ $message');
}
...
List<String> _collectDiagnostics(List<ScrollPositionAlignmentPolicy> values) {
  final names = values.map((v) => v.name).toList(growable: false);
  final indices = values.map((v) => v.index).toList(growable: false);
  return <String>[
    _line('enum', 'ScrollPositionAlignmentPolicy'),
...
dynamic build(BuildContext context) {
  print(
    '========== ScrollPositionAlignmentPolicy comprehensive test start ==========',
  );

  final List<ScrollPositionAlignmentPolicy> values =
      ScrollPositionAlignmentPolicy.values.toList(growable: false);
  _expect(
    values.isNotEmpty,
    'ScrollPositionAlignmentPolicy exposes at least one enum value',
  );
  _expect(
    values.length == ScrollPositionAlignmentPolicy.values.length,
    'ScrollPositionAlignmentPolicy.values list length is consistent',
  );

  final ScrollPositionAlignmentPolicy first = values.first;
  final ScrollPositionAlignmentPolicy last = values.last;

  print(_line('runtimeType', ScrollPositionAlignmentPolicy));
  ...
  for (final value in values) {
    ...
    _expect(
      ScrollPositionAlignmentPolicy.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = ScrollPositionAlignmentPolicy.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    ScrollPositionAlignmentPolicy.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<ScrollPositionAlignmentPolicy> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<ScrollPositionAlignmentPolicy> sorted = [...values]
  ...
  final Map<String, ScrollPositionAlignmentPolicy> asMap = {
  ...
  final String summary =
      'ScrollPositionAlignmentPolicy checks passed: ${values.length} values validated';
  print(summary);
  print(
    '========== ScrollPositionAlignmentPolicy comprehensive test end ==========',
  );
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('ScrollPositionAlignmentPolicy Comprehensive Tests'),
