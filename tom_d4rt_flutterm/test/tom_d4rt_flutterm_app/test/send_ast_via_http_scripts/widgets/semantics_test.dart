// Comprehensive D4rt test script: Semantics from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive Semantics script ===');
  ...
  detailLines.add('target=Semantics');
  ...
  final Semantics sem = Semantics(label: 'Test Button', button: true, enabled: true, child: SizedBox(width: 80, height: 40));
  check(sem is Widget, 'Is Widget');
  check(sem.properties.label == 'Test Button', 'Label');
  check(sem.properties.button == true, 'Is button');
  check(sem.properties.enabled == true, 'Enabled');
  final Semantics heading = Semantics(header: true, label: 'Title', child: SizedBox());
  check(heading.properties.header == true, 'Is header');
  detailLines.add('label=Test Button');
  detailLines.add('button=true');
