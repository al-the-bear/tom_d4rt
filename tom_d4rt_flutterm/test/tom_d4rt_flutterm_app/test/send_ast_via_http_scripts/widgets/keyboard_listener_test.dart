// Comprehensive D4rt test script: KeyboardListener from widgets
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive KeyboardListener script ===');
  ...
  detailLines.add('target=KeyboardListener');
  ...
  // KeyboardListener: listens for raw keyboard events
  final FocusNode focusNode = FocusNode();
  final Widget listener = KeyboardListener(
    focusNode: focusNode,
    onKeyEvent: (KeyEvent event) {
      print('Key event: ' + event.runtimeType.toString());
    },
    child: const Text('Press a key'),
  );
  check(listener is KeyboardListener, 'KeyboardListener created');
  check(listener is Widget, 'KeyboardListener is a Widget');
  detailLines.add('focusNodeLabel=' + (focusNode.debugLabel ?? 'null'));
  focusNode.dispose();
