...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    values.isNotEmpty,
    'DiagonalDragBehavior.values should not be empty',
    logs,
    counters,
  );
  expectCondition(
    values.length >= 2,
    'DiagonalDragBehavior should provide multiple values',
    logs,
    counters,
  );
  ...
  for (final DiagonalDragBehavior value in values) {
    ...
    final DiagonalDragBehavior byName = DiagonalDragBehavior.values.byName(
      value.name,
    );
    expectCondition(
      byName == value,
  ...
  final List<DiagonalDragBehavior> copied = List<DiagonalDragBehavior>.from(
    values,
  );
  expectCondition(
  ...
  final List<DiagonalDragBehavior> reversed = values.reversed.toList(
    growable: false,
  );
  expectCondition(
    reversed.first == last,
  ...
  final DiagonalDragBehavior edgeByName = DiagonalDragBehavior.values.byName(
    edgeName,
  );
  expectCondition(
