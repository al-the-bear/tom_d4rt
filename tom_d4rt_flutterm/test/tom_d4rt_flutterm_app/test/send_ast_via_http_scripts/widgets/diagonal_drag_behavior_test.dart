...
// D4rt comprehensive test script for DiagonalDragBehavior
...
dynamic build(BuildContext context) {
  ...
  print('========== DiagonalDragBehavior comprehensive test ==========' );

  final List<DiagonalDragBehavior> values = DiagonalDragBehavior.values;
  expectCondition(values.isNotEmpty, 'DiagonalDragBehavior.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'DiagonalDragBehavior should provide multiple values', logs, counters);

  final DiagonalDragBehavior first = values.first;
  final DiagonalDragBehavior last = values.last;
  ...
  for (final DiagonalDragBehavior value in values) {
    ...
    final DiagonalDragBehavior byName = DiagonalDragBehavior.values.byName(value.name);
  ...
  final List<DiagonalDragBehavior> copied = List<DiagonalDragBehavior>.from(values);
  ...
  final List<DiagonalDragBehavior> reversed = values.reversed.toList(growable: false);
  ...
  final DiagonalDragBehavior edgeByName = DiagonalDragBehavior.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('DiagonalDragBehavior Comprehensive Tests'),
