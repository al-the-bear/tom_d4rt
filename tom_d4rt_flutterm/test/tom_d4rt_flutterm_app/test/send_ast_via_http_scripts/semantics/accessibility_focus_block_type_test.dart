// D4rt test script: Comprehensive AccessibilityFocusBlockType coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(AccessibilityFocusBlockType value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = AccessibilityFocusBlockType.values;
  ...
  for (final value in values) {
    ...
    final byName = AccessibilityFocusBlockType.values.byName(value.name);
    assert(identical(byName, value));
    assert(AccessibilityFocusBlockType.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <AccessibilityFocusBlockType, String>{
    for (final value in values) value: 'layout:${value.name}',
  };
  ...
  try {
    AccessibilityFocusBlockType.values.byName('__not_an_accessibility_focus_block_type__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => AccessibilityFocusBlockType.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('AccessibilityFocusBlockType Tests'),
