// D4rt test script: Comprehensive Assertiveness coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = Assertiveness.values.byName(value.name);
    assert(identical(byName, value));
    assert(Assertiveness.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <Assertiveness, String>{
  ...
  try {
    Assertiveness.values.byName('__not_an_assertiveness__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => Assertiveness.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('Assertiveness Tests'),
