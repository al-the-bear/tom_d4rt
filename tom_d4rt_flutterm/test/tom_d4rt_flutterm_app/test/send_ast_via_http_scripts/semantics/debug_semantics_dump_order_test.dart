// D4rt test script: Comprehensive DebugSemanticsDumpOrder coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(DebugSemanticsDumpOrder value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = DebugSemanticsDumpOrder.values;
  ...
  for (final value in values) {
    ...
    final byName = DebugSemanticsDumpOrder.values.byName(value.name);
    assert(identical(byName, value));
    assert(DebugSemanticsDumpOrder.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <DebugSemanticsDumpOrder, String>{
  ...
  try {
    DebugSemanticsDumpOrder.values.byName('__not_a_debug_semantics_dump_order__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => DebugSemanticsDumpOrder.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('DebugSemanticsDumpOrder Tests'),
