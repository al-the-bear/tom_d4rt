...
class Class {
  ...
  Class copyWith({int? id, String? label}) =>
      Class(id ?? this.id, label ?? this.label);
...
dynamic build(BuildContext context) {
  ...
  assertionCount++;
  _expect(
    copy.id == sample.id && copy.label == 'secondary',
  ...
  assertionCount++;
  _expect(
    sample.toString().contains('Class('),
    'toString includes class name',
    logs,
  );
  assertionCount++;
