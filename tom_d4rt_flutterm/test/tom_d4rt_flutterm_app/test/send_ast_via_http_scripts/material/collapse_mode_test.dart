// D4rt comprehensive test script: CollapseMode (material)
...
class CollapseModeCaseStudy {
  CollapseModeCaseStudy({
    required this.label,
  ...
  final String label;
  final List<CollapseMode> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'CollapseMode must expose at least one value');
    ...
    final byName = CollapseMode.values.byName(samples.first.name);
...
dynamic build(BuildContext context) {
  print('CollapseMode test start');
  final values = CollapseMode.values;
  final caseStudy = CollapseModeCaseStudy(
    label: 'CollapseMode-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'CollapseMode must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('CollapseMode test completed with ${logs.length} log lines');
