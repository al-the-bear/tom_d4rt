...
dynamic build(BuildContext context) {
  ...
  log(
    'Created DesktopTextSelectionControls instances for equality/runtime checks',
  );
  _expect(
    controls.runtimeType.toString().contains('DesktopTextSelectionControls'),
    'constructs DesktopTextSelectionControls',
    logs,
  );
  assertionCount++;
  _expect(
    second.runtimeType == controls.runtimeType,
    'multiple instances share runtime type',
    logs,
  );
  assertionCount++;
  _expect(
    !identical(controls, second),
    'edge case: separate instances are not identical',
    logs,
  );
  assertionCount++;
  _expect(
    controls.hashCode >= 0 || controls.hashCode < 0,
    'hashCode can be read for property probe',
    logs,
  );
  assertionCount++;
  _expect(
    controls.toString().isNotEmpty,
