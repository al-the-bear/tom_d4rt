// D4rt test script: Comprehensive tests for DesktopTextSelectionControls
...
dynamic build(BuildContext context) {
  ...
  print('=== DesktopTextSelectionControls comprehensive D4rt test start ===');
  _expect(context.runtimeType.toString().isNotEmpty, 'context runtimeType is available', logs);
  assertionCount++;

  final controls = DesktopTextSelectionControls();
  final second = DesktopTextSelectionControls();
  log('Created DesktopTextSelectionControls instances for equality/runtime checks');
  _expect(controls.runtimeType.toString().contains('DesktopTextSelectionControls'), 'constructs DesktopTextSelectionControls', logs);
  assertionCount++;
  _expect(second.runtimeType == controls.runtimeType, 'multiple instances share runtime type', logs);
  assertionCount++;
  _expect(!identical(controls, second), 'edge case: separate instances are not identical', logs);
  assertionCount++;
  _expect(controls.hashCode >= 0 || controls.hashCode < 0, 'hashCode can be read for property probe', logs);
  assertionCount++;
  _expect(controls.toString().isNotEmpty, 'string representation is non-empty', logs);
  assertionCount++;
  ...
  print('=== DesktopTextSelectionControls comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DesktopTextSelectionControls Summary'),
