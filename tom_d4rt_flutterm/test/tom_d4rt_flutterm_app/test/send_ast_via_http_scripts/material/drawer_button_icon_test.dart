// D4rt test script: Comprehensive tests for DrawerButtonIcon
...
dynamic build(BuildContext context) {
  ...
  print('=== DrawerButtonIcon comprehensive D4rt test start ===');
  _expect(context.runtimeType.toString().isNotEmpty, 'context runtimeType is available', logs);
  assertionCount++;

  const icon = DrawerButtonIcon();
  const icon2 = DrawerButtonIcon();
  log('Instantiated DrawerButtonIcon constants for runtime and identity checks');
  _expect(icon.runtimeType.toString().contains('DrawerButtonIcon'), 'constructs DrawerButtonIcon', logs);
  assertionCount++;
  _expect(icon.key == null, 'default key property is null', logs);
  assertionCount++;
  _expect(icon.hashCode >= 0 || icon.hashCode < 0, 'hashCode property is available', logs);
  assertionCount++;
  _expect(icon2.runtimeType == icon.runtimeType, 'second const instance has same runtime type', logs);
  assertionCount++;
  _expect(icon.toString().isNotEmpty, 'string representation is non-empty', logs);
  assertionCount++;
  ...
  print('=== DrawerButtonIcon comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DrawerButtonIcon Summary'),
