// D4rt test script: Comprehensive tests for DesktopTextSelectionToolbar
...
dynamic build(BuildContext context) {
  ...
  print('=== DesktopTextSelectionToolbar comprehensive D4rt test start ===');
  _expect(context.runtimeType.toString().isNotEmpty, 'context runtimeType is available', logs);
  assertionCount++;

  final toolbar = DesktopTextSelectionToolbar(
    anchor: const Offset(20, 20),
    children: const [Text('Copy'), Text('Paste')],
  );
  final singleActionToolbar = DesktopTextSelectionToolbar(
    anchor: const Offset(5, 5),
    children: const [Text('Only one')],
  );
  log('Built DesktopTextSelectionToolbar variants with different anchors && children');
  _expect(toolbar.runtimeType.toString().contains('DesktopTextSelectionToolbar'), 'constructs DesktopTextSelectionToolbar', logs);
  assertionCount++;
  _expect(toolbar.children.length == 2, 'toolbar keeps two action children', logs);
  assertionCount++;
  _expect(singleActionToolbar.children.length == 1, 'edge case: single action toolbar supported', logs);
  assertionCount++;
  _expect(toolbar.anchor.dx == 20 && toolbar.anchor.dy == 20, 'anchor property retains provided offset', logs);
  assertionCount++;
  _expect(toolbar.children.first.runtimeType.toString().contains('Text'), 'toolbar children remain widget instances', logs);
  assertionCount++;
  ...
  print('=== DesktopTextSelectionToolbar comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DesktopTextSelectionToolbar Summary'),
