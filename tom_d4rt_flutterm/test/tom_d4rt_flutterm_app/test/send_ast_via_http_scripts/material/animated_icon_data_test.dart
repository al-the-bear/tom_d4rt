// D4rt test script: Comprehensive tests for AnimatedIconData
import 'package:flutter/material.dart';

void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    logs.add('FAIL: ' + message);
    throw StateError('AnimatedIconData assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== AnimatedIconData comprehensive test start ===');
  final logs = <String>[];
  var assertionCount = 0;


  final AnimatedIconData iconData = AnimatedIcons.add_event;
  _expect(iconData is AnimatedIconData, 'uses AnimatedIconData constant', logs);
  assertionCount++;
  final AnimatedIconData iconData2 = AnimatedIcons.close_menu;
  _expect(iconData2 is AnimatedIconData, 'uses second AnimatedIconData constant', logs);
  assertionCount++;
  _expect(iconData != iconData2, 'different constants represent different animations', logs);
  assertionCount++;
  final animatedIcon = AnimatedIcon(icon: iconData, progress: const AlwaysStoppedAnimation<double>(0.5));
  _expect(animatedIcon.icon == iconData, 'AnimatedIcon stores AnimatedIconData', logs);
  assertionCount++;
  ...
  print('=== AnimatedIconData comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('AnimatedIconData Tests'),
