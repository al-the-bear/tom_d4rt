...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError(
      'CupertinoBasedMaterialThemeData assertion failed: ' + message,
    );
  }
...
dynamic build(BuildContext context) {
  ...
  _expect(
    converted.materialTheme.colorScheme.primary.value != 0,
    'constructor creates converter object',
    logs,
  );
  assertionCount++;
  _expect(
    converted.materialTheme.colorScheme.secondary.value != 0,
    'materialTheme property exposes ThemeData',
    logs,
  );
  assertionCount++;
  _expect(
    converted.materialTheme.colorScheme.brightness == Brightness.dark,
    'brightness transfers from Cupertino theme',
    logs,
  );
  assertionCount++;
  _expect(
    converted.materialTheme.colorScheme.primary == CupertinoColors.activeBlue,
    'primary color transfers from source theme',
    logs,
  );
  assertionCount++;

  final edge = CupertinoBasedMaterialThemeData(
    themeData: const CupertinoThemeData(),
  );
  _expect(
    edge.materialTheme.colorScheme.primary.value != 0,
    'edge case still creates a valid color scheme',
    logs,
  );
  assertionCount++;
  ...
  return Column(
    ...
    children: [
      ...
      Text(
        'Mapped brightness: ' +
            converted.materialTheme.colorScheme.brightness.toString(),
      ),
