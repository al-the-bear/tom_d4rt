// D4rt test script: Comprehensive tests for DropdownMenuFormField
...
dynamic build(BuildContext context) {
  ...
  print('=== DropdownMenuFormField comprehensive D4rt test start ===');
  _expect(
  ...
  final field = DropdownMenuFormField<int>(
    initialSelection: 2,
  ...
  final emptyField = DropdownMenuFormField<int>(dropdownMenuEntries: const []);
  log(
    'Constructed DropdownMenuFormField variants with populated and empty entries',
  );
  _expect(
    field.runtimeType.toString().contains('DropdownMenuFormField<int>'),
    'constructs DropdownMenuFormField',
    logs,
  );
  assertionCount++;
  _expect(field.key == null, 'default key is null', logs);
  assertionCount++;
  _expect(
    field.hashCode >= 0 || field.hashCode < 0,
    'hashCode property can be read',
    logs,
  );
  assertionCount++;
  _expect(
    emptyField.runtimeType == field.runtimeType,
    'edge case: empty entries keep same runtime type',
    logs,
  );
  assertionCount++;
  _expect(
    emptyField.toString().isNotEmpty,
    'string representation remains available',
    logs,
  );
  assertionCount++;
  ...
  print('=== DropdownMenuFormField comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DropdownMenuFormField Summary'),
