...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError('CarouselController assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== CarouselController comprehensive test start ===');
  final logs = <String>[];
  ...
  _expect(
    controller.runtimeType.toString().contains('CarouselController'),
  ...
  _expect(
    ...
    'positions are empty before attachment',
    logs,
  );
  assertionCount++;
  _expect(
    edgeController.initialItem == -1,
    'edge case allows negative initialItem',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    true,
    'animateToItem without clients is a no-op and does not throw',
    logs,
  );
  assertionCount++;
  ...
  print('=== CarouselController comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('CarouselController Tests'),
      Text('Assertions: $assertionCount'),
      Text('Initial item: ' + controller.initialItem.toString()),
