// D4rt test script: Comprehensive tests for BottomAppBarThemeData
...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError('BottomAppBarThemeData assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== BottomAppBarThemeData comprehensive test start ===');
  final logs = <String>[];
  var assertionCount = 0;

  const data = BottomAppBarThemeData(
    color: Colors.black,
    elevation: 6.0,
    height: 64.0,
    shape: CircularNotchedRectangle(),
  );
  _expect(data.color == Colors.black, 'stores color', logs);
  assertionCount++;
  _expect(data.elevation == 6.0, 'stores elevation', logs);
  assertionCount++;
  _expect(data.height == 64.0, 'stores height', logs);
  assertionCount++;
  _expect(data.shape is CircularNotchedRectangle, 'stores shape', logs);
  assertionCount++;
  ...
  print('=== BottomAppBarThemeData comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('BottomAppBarThemeData Tests'),
