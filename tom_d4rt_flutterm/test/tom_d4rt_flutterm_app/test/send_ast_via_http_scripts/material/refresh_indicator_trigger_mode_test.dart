...
Map<String, dynamic> _analyzeRefreshIndicatorTriggerMode() {
  ...
  assert(
    values.isNotEmpty,
    'RefreshIndicatorTriggerMode should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'RefreshIndicatorTriggerMode: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for RefreshIndicatorTriggerMode',
    );

    final fromName = RefreshIndicatorTriggerMode.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for RefreshIndicatorTriggerMode',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for RefreshIndicatorTriggerMode',
    );
  }
  ...
  assert(
    ...
    'RefreshIndicatorTriggerMode enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'RefreshIndicatorTriggerMode enum indexes must be unique',
  );
  ...
  assert(
    ...
    'RefreshIndicatorTriggerMode enum indexes should be contiguous and zero-based',
  );

  var invalidNameThrows = false;
  try {
    RefreshIndicatorTriggerMode.values.byName(
      '__invalid_refreshindicatortriggermode__',
    );
  } catch (error) {
    invalidNameThrows = true;
    print(
      'RefreshIndicatorTriggerMode: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'RefreshIndicatorTriggerMode.values.byName should throw for invalid names',
  );
  ...
  print(
    'RefreshIndicatorTriggerMode: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'RefreshIndicatorTriggerMode has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'RefreshIndicatorTriggerMode summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'RefreshIndicatorTriggerMode must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'RefreshIndicatorTriggerMode order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'RefreshIndicatorTriggerMode invalid-name edge case must throw',
  );
