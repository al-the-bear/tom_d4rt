...
dynamic build(BuildContext context) {
  ...
  _expect(
    controller.length == 3,
    'TabController constructor sets length',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    edgeController.length == 1,
    'edge controller supports single tab',
    logs,
  );
  assertionCount++;

  print(
    'controller.index=${controller.index} tabs=${tabs.map((t) => t.text).toList()}',
  );
