...
// D4rt test script: Comprehensive tests for ExpansionPanelRadio from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: ExpansionPanelRadio ===');

  final symbolName = 'ExpansionPanelRadio';
  final fileName = 'expansion_panel_radio_test.dart';
  final logs = <String>[];
  ...
  Widget header(BuildContext ctx, bool expanded) {
    return Text('expanded=$expanded');
  }

  final panel = ExpansionPanelRadio(
    value: 'panel-1',
    headerBuilder: header,
    body: const SizedBox(height: 12),
    canTapOnHeader: true,
  );
  final panelEdge = ExpansionPanelRadio(
    value: 42,
    headerBuilder: header,
    body: const SizedBox.shrink(),
  );
  check(panel.value == 'panel-1', 'panel value preserved');
  check(panel.canTapOnHeader, 'panel can tap on header');
  check(panelEdge.value == 42, 'edge panel value preserved');
  check(panel.body.toString().contains('SizedBox'), 'panel body is preserved');
  symbolDetails.add('panel=$panel');
