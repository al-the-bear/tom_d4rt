...
Map<String, dynamic> _analyzeShowValueIndicator() {
  ...
  assert(
    values.isNotEmpty,
    'ShowValueIndicator should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'ShowValueIndicator: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for ShowValueIndicator',
    );

    final fromName = ShowValueIndicator.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for ShowValueIndicator',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for ShowValueIndicator',
    );
  }
  ...
  assert(
    ...
    'ShowValueIndicator enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'ShowValueIndicator enum indexes must be unique',
  );
  ...
  assert(
    ...
    'ShowValueIndicator enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'ShowValueIndicator: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'ShowValueIndicator.values.byName should throw for invalid names',
  );
  ...
  print(
    'ShowValueIndicator: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'ShowValueIndicator has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'ShowValueIndicator summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'ShowValueIndicator must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'ShowValueIndicator order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'ShowValueIndicator invalid-name edge case must throw',
  );
