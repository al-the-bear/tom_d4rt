...
Map<String, dynamic> _analyzeThemeMode() {
  ...
  for (final value in values) {
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for ThemeMode',
    );

    final fromName = ThemeMode.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for ThemeMode',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for ThemeMode',
    );
  }
  ...
  assert(
    ...
    'ThemeMode enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'ThemeMode enum indexes must be unique',
  );
  ...
  assert(
    ...
    'ThemeMode enum indexes should be contiguous and zero-based',
  );
  ...
  assert(
    invalidNameThrows,
    'ThemeMode.values.byName should throw for invalid names',
  );
  ...
  print(
    'ThemeMode: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'ThemeMode has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'ThemeMode summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'ThemeMode must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'ThemeMode order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'ThemeMode invalid-name edge case must throw',
  );
