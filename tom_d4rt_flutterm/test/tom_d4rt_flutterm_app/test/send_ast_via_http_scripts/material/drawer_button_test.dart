// D4rt test script: Comprehensive tests for DrawerButton
...
dynamic build(BuildContext context) {
  ...
  print('=== DrawerButton comprehensive D4rt test start ===');
  _expect(
  ...
  const button = DrawerButton();
  const buttonWithKey = DrawerButton(key: ValueKey<String>('drawer-btn'));
  log('Constructed DrawerButton variants with && without key');
  _expect(
    button.runtimeType.toString().contains('DrawerButton'),
    'constructs DrawerButton',
    logs,
  );
  assertionCount++;
  _expect(button.key == null, 'default key is null', logs);
  assertionCount++;
  _expect(buttonWithKey.key != null, 'custom key is preserved', logs);
  assertionCount++;
  _expect(
    buttonWithKey.toString().contains('DrawerButton'),
    'string output mentions DrawerButton',
    logs,
  );
  assertionCount++;
  _expect(
    buttonWithKey.runtimeType == button.runtimeType,
    'runtime type remains DrawerButton',
    logs,
  );
  assertionCount++;
  ...
  print('=== DrawerButton comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DrawerButton Summary'),
