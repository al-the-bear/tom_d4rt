...
Map<String, dynamic> _analyzeNavigationRailLabelType() {
  ...
  assert(
    values.isNotEmpty,
    'NavigationRailLabelType should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'NavigationRailLabelType: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for NavigationRailLabelType',
    );

    final fromName = NavigationRailLabelType.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for NavigationRailLabelType',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for NavigationRailLabelType',
    );
  }
  ...
  assert(
    ...
    'NavigationRailLabelType enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'NavigationRailLabelType enum indexes must be unique',
  );
  ...
  assert(
    ...
    'NavigationRailLabelType enum indexes should be contiguous and zero-based',
  );

  var invalidNameThrows = false;
  try {
    NavigationRailLabelType.values.byName(
      '__invalid_navigationraillabeltype__',
    );
  } catch (error) {
    invalidNameThrows = true;
    print(
      'NavigationRailLabelType: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'NavigationRailLabelType.values.byName should throw for invalid names',
  );
  ...
  print(
    'NavigationRailLabelType: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'NavigationRailLabelType has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'NavigationRailLabelType summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'NavigationRailLabelType must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'NavigationRailLabelType order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'NavigationRailLabelType invalid-name edge case must throw',
  );
