...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting SnackBarClosedReason checks');

  final values = SnackBarClosedReason.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<SnackBarClosedReason>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => SnackBarClosedReason.values.byName(value.name) == value)
  ...
  try {
    SnackBarClosedReason.values.byName('__SnackBarClosedReason_invalid__');
  } catch (_) {
  ...
  final summary =
      'SnackBarClosedReason summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
  ...
  return ListView(
    ...
    children: [
      const Text(
        'SnackBarClosedReason Test Summary',
