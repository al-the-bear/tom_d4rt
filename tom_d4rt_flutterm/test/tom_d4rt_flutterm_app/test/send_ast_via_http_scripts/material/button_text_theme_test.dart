// D4rt comprehensive test script: ButtonTextTheme (material)
import 'package:flutter/material.dart';

class ButtonTextThemeCaseStudy {
  ButtonTextThemeCaseStudy({
    required this.label,
  ...
  final String label;
  final List<ButtonTextTheme> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'ButtonTextTheme must expose at least one value');
    ...
    final byName = ButtonTextTheme.values.byName(samples.first.name);
...
dynamic build(BuildContext context) {
  print('ButtonTextTheme test start');
  final values = ButtonTextTheme.values;
  final caseStudy = ButtonTextThemeCaseStudy(
    label: 'ButtonTextTheme-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'ButtonTextTheme must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('ButtonTextTheme test completed with ${logs.length} log lines');
