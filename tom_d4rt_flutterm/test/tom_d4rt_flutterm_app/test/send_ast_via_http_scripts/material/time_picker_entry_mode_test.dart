...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    values.isNotEmpty,
    'TimePickerEntryMode.values should not be empty',
    logs,
    counters,
  );
  expectCondition(
    values.length >= 2,
    'TimePickerEntryMode should provide multiple values',
    logs,
    counters,
  );
  ...
  for (final TimePickerEntryMode value in values) {
    ...
    final TimePickerEntryMode byName = TimePickerEntryMode.values.byName(
      value.name,
    );
    expectCondition(
      byName == value,
  ...
  final List<TimePickerEntryMode> copied = List<TimePickerEntryMode>.from(
    values,
  );
  expectCondition(
  ...
  final List<TimePickerEntryMode> reversed = values.reversed.toList(
    growable: false,
  );
  expectCondition(
    reversed.first == last,
  ...
  final TimePickerEntryMode edgeByName = TimePickerEntryMode.values.byName(
    edgeName,
  );
  expectCondition(
