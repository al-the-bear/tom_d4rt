...
dynamic build(BuildContext context) {
  ...
  _expect(
    ...
    'constructor creates ControlsDetails',
    logs,
  );
  assertionCount++;
  _expect(
    details.currentStep == 1 && details.stepIndex == 1,
    'constructor stores step values',
    logs,
  );
  assertionCount++;
  _expect(
    details.isActive,
    'isActive is true when currentStep == stepIndex',
    logs,
  );
  assertionCount++;
  _expect(
    edge.isActive == false,
    'edge case marks inactive when indices differ',
    logs,
  );
  assertionCount++;
  _expect(
    edge.onStepContinue == null && edge.onStepCancel == null,
