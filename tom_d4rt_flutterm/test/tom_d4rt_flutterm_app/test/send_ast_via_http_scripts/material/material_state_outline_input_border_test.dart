...
dynamic build(BuildContext context) {
  ...
  const MaterialStateOutlineInputBorder statefulBorder =
      MaterialStateOutlineInputBorder.resolveWith(_resolveOutline);
  final InputBorder defaultBorder = statefulBorder.resolve(<MaterialState>{});
  final InputBorder pressedBorder = statefulBorder.resolve(<MaterialState>{
    MaterialState.pressed,
  });

  check(
    statefulBorder.runtimeType.toString().isNotEmpty,
    'Stateful border runtimeType is readable',
  );
  check(
    defaultBorder is OutlineInputBorder,
    'Default resolved border is OutlineInputBorder',
  );
  check(
    pressedBorder is OutlineInputBorder,
    'Pressed resolved border is OutlineInputBorder',
  );
  check(
    (pressedBorder as OutlineInputBorder).borderSide.width == 2,
    'Pressed edge case uses thicker border',
  );
  check(
    (defaultBorder as OutlineInputBorder).borderSide.width == 1,
