...
// D4rt test script: Comprehensive tests for DropdownMenuItem from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: DropdownMenuItem ===');

  final symbolName = 'DropdownMenuItem';
  final fileName = 'dropdown_menu_item_test.dart';
  final logs = <String>[];
  ...
  const primary = DropdownMenuItem<int?>(
    value: 7,
    enabled: true,
    child: Text('primary'),
  );
  const disabled = DropdownMenuItem<int?>(
    value: null,
    enabled: false,
    child: SizedBox.shrink(),
  );
  check(primary.value == 7, 'primary value is retained');
  check(primary.enabled, 'primary item is enabled');
  check(disabled.enabled == false, 'disabled item toggles state');
  check(primary.child.runtimeType == Text, 'primary child keeps text widget');
  check(
    disabled.child.runtimeType == SizedBox,
    'disabled child supports edge widget',
  );
  symbolDetails.add('runtimeType=${primary.runtimeType}');
  symbolDetails.add('disabledValue=${disabled.value}');
