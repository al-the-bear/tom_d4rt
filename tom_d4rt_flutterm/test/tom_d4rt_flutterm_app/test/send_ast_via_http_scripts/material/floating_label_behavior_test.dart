...
Map<String, dynamic> _analyzeFloatingLabelBehavior() {
  ...
  assert(
    values.isNotEmpty,
    'FloatingLabelBehavior should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'FloatingLabelBehavior: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for FloatingLabelBehavior',
    );

    final fromName = FloatingLabelBehavior.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for FloatingLabelBehavior',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for FloatingLabelBehavior',
    );
  }
  ...
  assert(
    ...
    'FloatingLabelBehavior enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'FloatingLabelBehavior enum indexes must be unique',
  );
  ...
  assert(
    ...
    'FloatingLabelBehavior enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'FloatingLabelBehavior: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'FloatingLabelBehavior.values.byName should throw for invalid names',
  );
  ...
  print(
    'FloatingLabelBehavior: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'FloatingLabelBehavior has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'FloatingLabelBehavior summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'FloatingLabelBehavior must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'FloatingLabelBehavior order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'FloatingLabelBehavior invalid-name edge case must throw',
  );
