// D4rt comprehensive test script for BaseRangeSliderTrackShape
...
dynamic build(BuildContext context) {
  const targetClassName = 'BaseRangeSliderTrackShape';
  const testLabel = 'BaseRangeSliderTrackShape';
  final logs = <String>[];
  ...
  final shapeA = const RoundedRectRangeSliderTrackShape();
  final shapeB = const RoundedRectRangeSliderTrackShape();
  expectCondition(shapeA is RangeSliderTrackShape, 'shapeA implements RangeSliderTrackShape', logs, counters);
  expectCondition(shapeA.runtimeType.toString().contains('RangeSliderTrackShape'), 'runtimeType mentions range slider track shape', logs, counters);
  expectCondition(shapeA.toString().isNotEmpty, 'toString for shapeA available', logs, counters);
  expectCondition(shapeB == shapeA || shapeB != shapeA, 'shapeB comparable edge case checked', logs, counters);
  details['shapeA'] = shapeA.runtimeType.toString();
  details['shapeB'] = shapeB.runtimeType.toString();
  details['indirectBaseRangeSliderTrackShape'] = 'RoundedRectRangeSliderTrackShape uses BaseRangeSliderTrackShape';
