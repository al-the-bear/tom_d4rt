// D4rt comprehensive test script: DrawerAlignment (material)
import 'package:flutter/material.dart';

class DrawerAlignmentCaseStudy {
  DrawerAlignmentCaseStudy({
    required this.label,
  ...
  final String label;
  final List<DrawerAlignment> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'DrawerAlignment must expose at least one value');
    ...
    final byName = DrawerAlignment.values.byName(samples.first.name);
...
dynamic build(BuildContext context) {
  print('DrawerAlignment test start');
  final values = DrawerAlignment.values;
  final caseStudy = DrawerAlignmentCaseStudy(
    label: 'DrawerAlignment-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'DrawerAlignment must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('DrawerAlignment test completed with ${logs.length} log lines');
