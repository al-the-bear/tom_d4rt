...
Map<String, dynamic> _analyzeRefreshIndicatorStatus() {
  ...
  assert(
    values.isNotEmpty,
    'RefreshIndicatorStatus should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'RefreshIndicatorStatus: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for RefreshIndicatorStatus',
    );

    final fromName = RefreshIndicatorStatus.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for RefreshIndicatorStatus',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for RefreshIndicatorStatus',
    );
  }
  ...
  assert(
    ...
    'RefreshIndicatorStatus enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'RefreshIndicatorStatus enum indexes must be unique',
  );
  ...
  assert(
    ...
    'RefreshIndicatorStatus enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'RefreshIndicatorStatus: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'RefreshIndicatorStatus.values.byName should throw for invalid names',
  );
  ...
  print(
    'RefreshIndicatorStatus: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'RefreshIndicatorStatus has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'RefreshIndicatorStatus summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'RefreshIndicatorStatus must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'RefreshIndicatorStatus order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'RefreshIndicatorStatus invalid-name edge case must throw',
  );
