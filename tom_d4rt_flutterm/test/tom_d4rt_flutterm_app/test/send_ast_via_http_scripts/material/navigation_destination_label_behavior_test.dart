...
List<String> _collectValueNames(
  List<NavigationDestinationLabelBehavior> values,
) {
  final names = <String>[];
...
Map<String, dynamic> _analyzeNavigationDestinationLabelBehavior() {
  ...
  assert(
    values.isNotEmpty,
    'NavigationDestinationLabelBehavior should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'NavigationDestinationLabelBehavior: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for NavigationDestinationLabelBehavior',
    );

    final fromName = NavigationDestinationLabelBehavior.values.byName(
      value.name,
    );
    assert(
      fromName == value,
      'byName must resolve the same value for NavigationDestinationLabelBehavior',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for NavigationDestinationLabelBehavior',
    );
  }
  ...
  assert(
    ...
    'NavigationDestinationLabelBehavior enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'NavigationDestinationLabelBehavior enum indexes must be unique',
  );
  ...
  assert(
    ...
    'NavigationDestinationLabelBehavior enum indexes should be contiguous and zero-based',
  );

  var invalidNameThrows = false;
  try {
    NavigationDestinationLabelBehavior.values.byName(
      '__invalid_navigationdestinationlabelbehavior__',
    );
  } catch (error) {
    invalidNameThrows = true;
    print(
      'NavigationDestinationLabelBehavior: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'NavigationDestinationLabelBehavior.values.byName should throw for invalid names',
  );
  ...
  print(
    'NavigationDestinationLabelBehavior: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'NavigationDestinationLabelBehavior has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'NavigationDestinationLabelBehavior summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'NavigationDestinationLabelBehavior must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'NavigationDestinationLabelBehavior order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'NavigationDestinationLabelBehavior invalid-name edge case must throw',
  );
