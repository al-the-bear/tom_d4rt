// D4rt test script: Comprehensive tests for DisabledChipAttributes
...
dynamic build(BuildContext context) {
  ...
  print('=== DisabledChipAttributes comprehensive D4rt test start ===');
  _expect(context.runtimeType.toString().isNotEmpty, 'context runtimeType is available', logs);
  assertionCount++;

  final enabled = FilterChip(label: const Text('Enabled'), selected: false, onSelected: (_) {});
  final disabled = const FilterChip(label: Text('Disabled'), selected: false, onSelected: null);
  final enabledAttrs = enabled as DisabledChipAttributes;
  final disabledAttrs = disabled as DisabledChipAttributes;
  log('Evaluated DisabledChipAttributes using enabled/disabled FilterChip instances');
  _expect(enabledAttrs.runtimeType.toString().contains('FilterChip'), 'attribute runtime matches FilterChip implementation', logs);
  assertionCount++;
  _expect(enabledAttrs.isEnabled, 'enabled chip reports isEnabled true', logs);
  assertionCount++;
  _expect(!disabledAttrs.isEnabled, 'edge case: disabled chip reports isEnabled false', logs);
  assertionCount++;
  _expect(enabled.runtimeType == disabled.runtimeType, 'enabled/disabled states keep same runtime type', logs);
  assertionCount++;
  ...
  print('=== DisabledChipAttributes comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DisabledChipAttributes Summary'),
