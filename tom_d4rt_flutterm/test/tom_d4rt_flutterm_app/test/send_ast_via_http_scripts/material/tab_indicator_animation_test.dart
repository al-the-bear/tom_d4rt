...
Map<String, dynamic> _analyzeTabIndicatorAnimation() {
  ...
  assert(
    values.isNotEmpty,
    'TabIndicatorAnimation should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'TabIndicatorAnimation: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for TabIndicatorAnimation',
    );

    final fromName = TabIndicatorAnimation.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for TabIndicatorAnimation',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for TabIndicatorAnimation',
    );
  }
  ...
  assert(
    ...
    'TabIndicatorAnimation enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'TabIndicatorAnimation enum indexes must be unique',
  );
  ...
  assert(
    ...
    'TabIndicatorAnimation enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'TabIndicatorAnimation: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'TabIndicatorAnimation.values.byName should throw for invalid names',
  );
  ...
  print(
    'TabIndicatorAnimation: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'TabIndicatorAnimation has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'TabIndicatorAnimation summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'TabIndicatorAnimation must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'TabIndicatorAnimation order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'TabIndicatorAnimation invalid-name edge case must throw',
  );
