...
dynamic build(BuildContext context) {
  ...
  _expect(
    state.runtimeType.toString().contains('DrawerControllerState'),
    'constructs DrawerControllerState',
    logs,
  );
  assertionCount++;
  _expect(
    !state.mounted,
    'edge case: newly created State is not mounted',
    logs,
  );
  assertionCount++;
  _expect(
    state.hashCode >= 0 || state.hashCode < 0,
    'state hashCode can be read',
    logs,
  );
  assertionCount++;
  _expect(
    state.toString().isNotEmpty,
    'state string representation is non-empty',
    logs,
  );
  assertionCount++;
  _expect(
    second.runtimeType == state.runtimeType,
    'second instance has same runtime type',
    logs,
  );
  assertionCount++;
