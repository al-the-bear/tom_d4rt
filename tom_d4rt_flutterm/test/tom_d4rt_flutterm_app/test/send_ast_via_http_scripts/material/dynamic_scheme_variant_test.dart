// D4rt comprehensive test script: DynamicSchemeVariant (material)
import 'package:flutter/material.dart';

class DynamicSchemeVariantCaseStudy {
  DynamicSchemeVariantCaseStudy({
    required this.label,
  ...
  final String label;
  final List<DynamicSchemeVariant> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'DynamicSchemeVariant must expose at least one value');
    ...
    final byName = DynamicSchemeVariant.values.byName(samples.first.name);
  ...
  dynamic buildSummary(BuildContext context, List<String> logs) {
    ...
    return Column(
      ...
      children: [
        Text('DynamicSchemeVariant summary'),
        Text('label: $label'),
...
dynamic build(BuildContext context) {
  print('DynamicSchemeVariant test start');
  final values = DynamicSchemeVariant.values;
  final caseStudy = DynamicSchemeVariantCaseStudy(
    label: 'DynamicSchemeVariant-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'DynamicSchemeVariant must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('DynamicSchemeVariant test completed with ${logs.length} log lines');
