...
// D4rt test script: Comprehensive tests for FlexibleSpaceBarSettings from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: FlexibleSpaceBarSettings ===');

  final symbolName = 'FlexibleSpaceBarSettings';
  final fileName = 'flexible_space_bar_settings_test.dart';
  final logs = <String>[];
  ...
  final settings = FlexibleSpaceBarSettings(
    toolbarOpacity: 0.9,
    minExtent: 56,
    maxExtent: 240,
    currentExtent: 144,
  ...
  final edgeSettings = FlexibleSpaceBarSettings(
    toolbarOpacity: 0,
    minExtent: 56,
    maxExtent: 56,
    currentExtent: 56,
  ...
  check(settings.maxExtent > settings.minExtent, 'max extent exceeds min extent');
  check(settings.currentExtent >= settings.minExtent, 'current extent above min extent');
  check(edgeSettings.maxExtent == edgeSettings.minExtent, 'edge supports equal extents');
  check(settings.child.toString().contains('SizedBox'), 'settings child is preserved');
  symbolDetails.add('settings=$settings');
