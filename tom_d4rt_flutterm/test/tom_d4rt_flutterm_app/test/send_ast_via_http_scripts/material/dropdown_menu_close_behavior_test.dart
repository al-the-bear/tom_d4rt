...
class DropdownMenuCloseBehaviorCaseStudy {
  ...
  List<String> run() {
    ...
    assert(
      samples.isNotEmpty,
      'DropdownMenuCloseBehavior must expose at least one value',
    );
...
dynamic build(BuildContext context) {
  ...
  assert(
    values.length >= 1,
    'DropdownMenuCloseBehavior must have at least one enum value',
  );
  ...
  print(
    'DropdownMenuCloseBehavior test completed with ${logs.length} log lines',
  );
