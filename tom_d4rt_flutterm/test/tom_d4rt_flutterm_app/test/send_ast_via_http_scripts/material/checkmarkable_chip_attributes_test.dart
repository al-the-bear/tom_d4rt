...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError(
      'CheckmarkableChipAttributes assertion failed: ' + message,
    );
  }
...
dynamic build(BuildContext context) {
  ...
  _expect(
    (chip as CheckmarkableChipAttributes).showCheckmark == true,
    'FilterChip exposes checkmark attributes',
    logs,
  );
  assertionCount++;
  final attrs = chip as CheckmarkableChipAttributes;
  _expect(
    attrs.showCheckmark == true,
    'showCheckmark is exposed via interface',
    logs,
  );
  assertionCount++;
  _expect(
    attrs.checkmarkColor == Colors.green,
    'checkmarkColor is exposed via interface',
    logs,
  );
  assertionCount++;
  final edgeAttrs = edgeChip as CheckmarkableChipAttributes;
  _expect(
    edgeAttrs.showCheckmark == false,
    'edge case supports hidden checkmark',
    logs,
  );
  assertionCount++;
  _expect(
    edgeAttrs.checkmarkColor == null,
    'edge case supports null checkmark color',
    logs,
  );
  assertionCount++;
