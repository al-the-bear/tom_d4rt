// D4rt comprehensive test script: DatePickerMode (material)
...
class DatePickerModeCaseStudy {
  DatePickerModeCaseStudy({
    required this.label,
  ...
  final String label;
  final List<DatePickerMode> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'DatePickerMode must expose at least one value');
    ...
    final byName = DatePickerMode.values.byName(samples.first.name);
...
dynamic build(BuildContext context) {
  print('DatePickerMode test start');
  final values = DatePickerMode.values;
  final caseStudy = DatePickerModeCaseStudy(
    label: 'DatePickerMode-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'DatePickerMode must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('DatePickerMode test completed with ${logs.length} log lines');
