...
// D4rt comprehensive test script for TimeOfDayFormat
...
dynamic build(BuildContext context) {
  ...
  print('========== TimeOfDayFormat comprehensive test ==========' );

  final List<TimeOfDayFormat> values = TimeOfDayFormat.values;
  expectCondition(values.isNotEmpty, 'TimeOfDayFormat.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'TimeOfDayFormat should provide multiple values', logs, counters);

  final TimeOfDayFormat first = values.first;
  final TimeOfDayFormat last = values.last;
  ...
  for (final TimeOfDayFormat value in values) {
    ...
    final TimeOfDayFormat byName = TimeOfDayFormat.values.byName(value.name);
  ...
  final List<TimeOfDayFormat> copied = List<TimeOfDayFormat>.from(values);
  ...
  final List<TimeOfDayFormat> reversed = values.reversed.toList(growable: false);
  ...
  final TimeOfDayFormat edgeByName = TimeOfDayFormat.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('TimeOfDayFormat Comprehensive Tests'),
