...
Map<String, dynamic> _analyzeScriptCategory() {
  ...
  assert(
    values.isNotEmpty,
    'ScriptCategory should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'ScriptCategory: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for ScriptCategory',
    );

    final fromName = ScriptCategory.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for ScriptCategory',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for ScriptCategory',
    );
  }
  ...
  assert(
    ...
    'ScriptCategory enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'ScriptCategory enum indexes must be unique',
  );
  ...
  assert(
    ...
    'ScriptCategory enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'ScriptCategory: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'ScriptCategory.values.byName should throw for invalid names',
  );
  ...
  print(
    'ScriptCategory: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'ScriptCategory has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'ScriptCategory summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'ScriptCategory must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'ScriptCategory order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'ScriptCategory invalid-name edge case must throw',
  );
