...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting SnackBarBehavior checks');

  final values = SnackBarBehavior.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<SnackBarBehavior>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => SnackBarBehavior.values.byName(value.name) == value)
  ...
  try {
    SnackBarBehavior.values.byName('__SnackBarBehavior_invalid__');
  } catch (_) {
  ...
  final summary =
      'SnackBarBehavior summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
