...
Map<String, dynamic> _analyzeListTileControlAffinity() {
  ...
  assert(
    values.isNotEmpty,
    'ListTileControlAffinity should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'ListTileControlAffinity: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for ListTileControlAffinity',
    );

    final fromName = ListTileControlAffinity.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for ListTileControlAffinity',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for ListTileControlAffinity',
    );
  }
  ...
  assert(
    ...
    'ListTileControlAffinity enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'ListTileControlAffinity enum indexes must be unique',
  );
  ...
  assert(
    ...
    'ListTileControlAffinity enum indexes should be contiguous and zero-based',
  );

  var invalidNameThrows = false;
  try {
    ListTileControlAffinity.values.byName(
      '__invalid_listtilecontrolaffinity__',
    );
  } catch (error) {
    invalidNameThrows = true;
    print(
      'ListTileControlAffinity: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'ListTileControlAffinity.values.byName should throw for invalid names',
  );
  ...
  print(
    'ListTileControlAffinity: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'ListTileControlAffinity has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'ListTileControlAffinity summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'ListTileControlAffinity must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'ListTileControlAffinity order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'ListTileControlAffinity invalid-name edge case must throw',
  );
