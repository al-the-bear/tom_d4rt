...
// D4rt test script: Comprehensive tests for ElevationOverlay from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: ElevationOverlay ===');

  final symbolName = 'ElevationOverlay';
  final fileName = 'elevation_overlay_test.dart';
  final logs = <String>[];
  ...
  final overlayColor = ElevationOverlay.applyOverlay(context, base, 6);
  final blended = ElevationOverlay.colorWithOverlay(base, Colors.white, 10);
  check(overlayColor != base, 'overlay modifies color at elevation');
  check(blended != base, 'colorWithOverlay modifies color');
  check(
    (overlayColor.a * 255).round().clamp(0, 255) == 255,
    'overlay keeps opaque alpha',
  );
  check(
    (blended.a * 255).round().clamp(0, 255) == 255,
    'blended keeps opaque alpha',
  );
  symbolDetails.add('overlay=${overlayColor.toARGB32().toRadixString(16)}');
  symbolDetails.add('blended=${blended.toARGB32().toRadixString(16)}');
