...
dynamic build(BuildContext context) {
  ...
  log(
    'Wrapped active && disabled DropdownButton with DropdownButtonHideUnderline',
  );
  _expect(
    wrapped.runtimeType.toString().contains('DropdownButtonHideUnderline'),
    'constructs DropdownButtonHideUnderline',
    logs,
  );
  assertionCount++;
  _expect(
    wrapped.child.runtimeType.toString().contains('DropdownButton<int>'),
    'keeps DropdownButton child',
    logs,
  );
  assertionCount++;
  _expect(
    disabled.child.runtimeType.toString().contains('DropdownButton<int>'),
    'edge case: disabled dropdown still wraps',
    logs,
  );
  assertionCount++;
  _expect(
    wrapped.runtimeType == disabled.runtimeType,
    'runtime type stable across child state',
    logs,
  );
  assertionCount++;
