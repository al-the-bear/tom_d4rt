// D4rt comprehensive test script for BaseSliderTrackShape
...
dynamic build(BuildContext context) {
  const targetClassName = 'BaseSliderTrackShape';
  const testLabel = 'BaseSliderTrackShape';
  final logs = <String>[];
  ...
  final shapeA = const RoundedRectSliderTrackShape();
  final shapeB = const RectangularSliderTrackShape();
  expectCondition(shapeA is SliderTrackShape, 'shapeA implements SliderTrackShape', logs, counters);
  expectCondition(shapeB is SliderTrackShape, 'shapeB implements SliderTrackShape', logs, counters);
  expectCondition(shapeA.runtimeType.toString().isNotEmpty, 'shapeA runtimeType available', logs, counters);
  expectCondition(shapeB.runtimeType.toString().isNotEmpty, 'shapeB runtimeType available', logs, counters);
  details['shapeA'] = shapeA.runtimeType.toString();
  details['shapeB'] = shapeB.runtimeType.toString();
  details['indirectBaseSliderTrackShape'] = 'RoundedRectSliderTrackShape/RectangularSliderTrackShape use BaseSliderTrackShape';
