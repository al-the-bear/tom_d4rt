// D4rt test script: Comprehensive tests for DeletableChipAttributes
...
dynamic build(BuildContext context) {
  ...
  print('=== DeletableChipAttributes comprehensive D4rt test start ===');
  _expect(
  ...
  final widget = InputChip(
    label: const Text('Deletable'),
    onDeleted: () {
      log('delete callback invoked');
    },
  );
  final attrs = widget as DeletableChipAttributes;
  final nonDeletable = InputChip(label: const Text('No delete'));
  final nonDeletableAttrs = nonDeletable as DeletableChipAttributes;
  log('Casted InputChip instances to DeletableChipAttributes');
  _expect(
    attrs.runtimeType.toString().contains('InputChip'),
    'attribute implementation uses InputChip runtime',
    logs,
  );
  assertionCount++;
  _expect(
    attrs.onDeleted != null,
    'onDeleted is exposed by DeletableChipAttributes',
    logs,
  );
  assertionCount++;
  _expect(
    nonDeletableAttrs.onDeleted == null,
    'edge case: onDeleted can be null',
    logs,
  );
  assertionCount++;
  _expect(
    widget.runtimeType.toString().contains('InputChip'),
    'runtime type remains InputChip',
    logs,
  );
  assertionCount++;
  ...
  print('=== DeletableChipAttributes comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DeletableChipAttributes Summary'),
