...
dynamic build(BuildContext context) {
  ...
  final widget = InputChip(
    ...
    onDeleted: () { log('delete callback invoked'); },
  );
  ...
  _expect(attrs.runtimeType.toString().contains('InputChip'), 'attribute implementation uses InputChip runtime', logs);
  assertionCount++;
  _expect(attrs.onDeleted != null, 'onDeleted is exposed by DeletableChipAttributes', logs);
  assertionCount++;
  _expect(nonDeletableAttrs.onDeleted == null, 'edge case: onDeleted can be null', logs);
  assertionCount++;
  _expect(widget.runtimeType.toString().contains('InputChip'), 'runtime type remains InputChip', logs);
  assertionCount++;
