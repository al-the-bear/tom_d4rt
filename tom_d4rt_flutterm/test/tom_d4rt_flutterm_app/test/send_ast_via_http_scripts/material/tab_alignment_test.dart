...
Map<String, dynamic> _analyzeTabAlignment() {
  ...
  assert(
    values.isNotEmpty,
    'TabAlignment should expose at least one enum value',
  );
  ...
  for (final value in values) {
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for TabAlignment',
    );

    final fromName = TabAlignment.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for TabAlignment',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for TabAlignment',
    );
  }
  ...
  assert(
    ...
    'TabAlignment enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'TabAlignment enum indexes must be unique',
  );
  ...
  assert(
    ...
    'TabAlignment enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'TabAlignment: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'TabAlignment.values.byName should throw for invalid names',
  );
  ...
  print(
    'TabAlignment: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'TabAlignment has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'TabAlignment summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'TabAlignment must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'TabAlignment order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'TabAlignment invalid-name edge case must throw',
  );
