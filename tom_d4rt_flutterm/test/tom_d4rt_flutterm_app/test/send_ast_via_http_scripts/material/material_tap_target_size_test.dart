...
Map<String, dynamic> _analyzeMaterialTapTargetSize() {
  ...
  assert(
    values.isNotEmpty,
    'MaterialTapTargetSize should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'MaterialTapTargetSize: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for MaterialTapTargetSize',
    );

    final fromName = MaterialTapTargetSize.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for MaterialTapTargetSize',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for MaterialTapTargetSize',
    );
  }
  ...
  assert(
    ...
    'MaterialTapTargetSize enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'MaterialTapTargetSize enum indexes must be unique',
  );
  ...
  assert(
    ...
    'MaterialTapTargetSize enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'MaterialTapTargetSize: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'MaterialTapTargetSize.values.byName should throw for invalid names',
  );
  ...
  print(
    'MaterialTapTargetSize: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'MaterialTapTargetSize has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'MaterialTapTargetSize summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'MaterialTapTargetSize must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'MaterialTapTargetSize order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'MaterialTapTargetSize invalid-name edge case must throw',
  );
