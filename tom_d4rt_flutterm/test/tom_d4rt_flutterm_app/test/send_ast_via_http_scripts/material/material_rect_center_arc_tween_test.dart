// Comprehensive D4rt test script: MaterialRectCenterArcTween from material
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive MaterialRectCenterArcTween script ===');
  ...
  detailLines.add('target=MaterialRectCenterArcTween');
  detailLines.add('package=material');
  detailLines.add('buildContextType=' + context.runtimeType.toString());

  final MaterialRectCenterArcTween tween = MaterialRectCenterArcTween(
    begin: const Rect.fromLTWH(0, 0, 12, 12),
    end: const Rect.fromLTWH(30, 20, 18, 18),
  );
  final Rect atStart = tween.lerp(0);
  final Rect atMiddle = tween.lerp(0.5);
  final Rect atEnd = tween.lerp(1);

  check(atStart == const Rect.fromLTWH(0, 0, 12, 12), 'lerp(0) returns begin');
  check(atEnd == const Rect.fromLTWH(30, 20, 18, 18), 'lerp(1) returns end');
  check(atMiddle.center.dx >= atStart.center.dx, 'Middle center progresses on x-axis');
  check(atMiddle.center.dy >= atStart.center.dy, 'Middle center progresses on y-axis');
  check(tween.centerArc != null, 'Center arc tween is available');
