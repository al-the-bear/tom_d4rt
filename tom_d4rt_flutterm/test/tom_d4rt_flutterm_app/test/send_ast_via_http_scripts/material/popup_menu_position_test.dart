...
Map<String, dynamic> _analyzePopupMenuPosition() {
  ...
  assert(
    values.isNotEmpty,
    'PopupMenuPosition should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'PopupMenuPosition: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for PopupMenuPosition',
    );

    final fromName = PopupMenuPosition.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for PopupMenuPosition',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for PopupMenuPosition',
    );
  }
  ...
  assert(
    ...
    'PopupMenuPosition enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'PopupMenuPosition enum indexes must be unique',
  );
  ...
  assert(
    ...
    'PopupMenuPosition enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'PopupMenuPosition: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'PopupMenuPosition.values.byName should throw for invalid names',
  );
  ...
  print(
    'PopupMenuPosition: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'PopupMenuPosition has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'PopupMenuPosition summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'PopupMenuPosition must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'PopupMenuPosition order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'PopupMenuPosition invalid-name edge case must throw',
  );
