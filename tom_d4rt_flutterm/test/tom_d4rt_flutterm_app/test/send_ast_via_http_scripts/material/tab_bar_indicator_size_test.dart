...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting TabBarIndicatorSize checks');

  final values = TabBarIndicatorSize.values;
  _check(
  ...
  final holders = values
      .map(
        (value) =>
            _EnumHolder<TabBarIndicatorSize>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => TabBarIndicatorSize.values.byName(value.name) == value)
  ...
  try {
    TabBarIndicatorSize.values.byName('__TabBarIndicatorSize_invalid__');
  } catch (_) {
  ...
  final summary =
      'TabBarIndicatorSize summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
  ...
  return ListView(
    ...
    children: [
      const Text(
        'TabBarIndicatorSize Test Summary',
