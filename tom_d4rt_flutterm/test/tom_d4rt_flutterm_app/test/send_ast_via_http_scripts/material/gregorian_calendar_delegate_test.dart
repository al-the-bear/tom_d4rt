...
// D4rt test script: Comprehensive tests for GregorianCalendarDelegate from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: GregorianCalendarDelegate ===');

  final symbolName = 'GregorianCalendarDelegate';
  final fileName = 'gregorian_calendar_delegate_test.dart';
  final logs = <String>[];
  ...
  const delegateA = GregorianCalendarDelegate();
  const delegateB = GregorianCalendarDelegate();
  final now = DateTime.now();
  final dateOnly = delegateA.dateOnly(now);
  final next = delegateA.addDaysToDate(dateOnly, 1);
  check(
    delegateA.runtimeType == GregorianCalendarDelegate,
    'delegate type matches symbol',
  );
  check(
    delegateB.runtimeType == GregorianCalendarDelegate,
    'second delegate type matches symbol',
  );
  check(dateOnly.hour == 0, 'dateOnly normalizes to midnight');
  check(next.isAfter(dateOnly), 'addDaysToDate moves date forward');
  symbolDetails.add('dateOnly=$dateOnly');
  symbolDetails.add('next=$next');
