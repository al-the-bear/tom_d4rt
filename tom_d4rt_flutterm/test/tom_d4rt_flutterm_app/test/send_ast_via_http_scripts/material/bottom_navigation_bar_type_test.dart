// D4rt comprehensive test script: BottomNavigationBarType (material)
import 'package:flutter/material.dart';

class BottomNavigationBarTypeCaseStudy {
  BottomNavigationBarTypeCaseStudy({
    required this.label,
  ...
  final String label;
  final List<BottomNavigationBarType> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'BottomNavigationBarType must expose at least one value');
    ...
    final byName = BottomNavigationBarType.values.byName(samples.first.name);
  ...
  dynamic buildSummary(BuildContext context, List<String> logs) {
    ...
    return Column(
      ...
      children: [
        Text('BottomNavigationBarType summary'),
        Text('label: $label'),
...
dynamic build(BuildContext context) {
  print('BottomNavigationBarType test start');
  final values = BottomNavigationBarType.values;
  final caseStudy = BottomNavigationBarTypeCaseStudy(
    label: 'BottomNavigationBarType-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'BottomNavigationBarType must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('BottomNavigationBarType test completed with ${logs.length} log lines');
