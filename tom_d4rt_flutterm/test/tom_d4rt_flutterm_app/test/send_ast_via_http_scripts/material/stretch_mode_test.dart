...
Map<String, dynamic> _analyzeStretchMode() {
  ...
  assert(
    values.isNotEmpty,
    'StretchMode should expose at least one enum value',
  );
  ...
  for (final value in values) {
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for StretchMode',
    );

    final fromName = StretchMode.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for StretchMode',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for StretchMode',
    );
  }
  ...
  assert(
    ...
    'StretchMode enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'StretchMode enum indexes must be unique',
  );
  ...
  assert(
    ...
    'StretchMode enum indexes should be contiguous and zero-based',
  );
  ...
  assert(
    invalidNameThrows,
    'StretchMode.values.byName should throw for invalid names',
  );
  ...
  print(
    'StretchMode: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'StretchMode has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'StretchMode summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'StretchMode must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'StretchMode order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'StretchMode invalid-name edge case must throw',
  );
