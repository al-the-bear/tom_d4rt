// D4rt test script: Comprehensive tests for ButtonBar
...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError('ButtonBar assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== ButtonBar comprehensive test start ===');
  final logs = <String>[];
  ...
  _expect(bar is ButtonBar, 'instantiates ButtonBar', logs);
  assertionCount++;
  _expect(bar.alignment == MainAxisAlignment.center, 'stores alignment', logs);
  assertionCount++;
  _expect(bar.children.length == 2, 'stores children list', logs);
  assertionCount++;
  _expect(bar.toStringShort().contains('ButtonBar'), 'string representation available', logs);
  assertionCount++;
  ...
  print('=== ButtonBar comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('ButtonBar Tests'),
