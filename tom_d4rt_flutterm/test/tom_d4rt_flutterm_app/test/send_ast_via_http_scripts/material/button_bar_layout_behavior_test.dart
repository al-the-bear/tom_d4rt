// D4rt comprehensive test script: ButtonBarLayoutBehavior (material)
...
class ButtonBarLayoutBehaviorCaseStudy {
  ButtonBarLayoutBehaviorCaseStudy({
    required this.label,
  ...
  final String label;
  final List<ButtonBarLayoutBehavior> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'ButtonBarLayoutBehavior must expose at least one value');
    ...
    final byName = ButtonBarLayoutBehavior.values.byName(samples.first.name);
  ...
  dynamic buildSummary(BuildContext context, List<String> logs) {
    ...
    return Column(
      ...
      children: [
        Text('ButtonBarLayoutBehavior summary'),
        Text('label: $label'),
...
dynamic build(BuildContext context) {
  print('ButtonBarLayoutBehavior test start');
  final values = ButtonBarLayoutBehavior.values;
  final caseStudy = ButtonBarLayoutBehaviorCaseStudy(
    label: 'ButtonBarLayoutBehavior-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'ButtonBarLayoutBehavior must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('ButtonBarLayoutBehavior test completed with ${logs.length} log lines');
