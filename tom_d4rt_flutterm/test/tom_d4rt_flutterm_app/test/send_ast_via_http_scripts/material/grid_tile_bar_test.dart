...
// D4rt test script: Comprehensive tests for GridTileBar from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: GridTileBar ===');

  final symbolName = 'GridTileBar';
  final fileName = 'grid_tile_bar_test.dart';
  final logs = <String>[];
  ...
  const compact = GridTileBar(title: Text('Compact title'));
  const rich = GridTileBar(
  ...
  check(compact.title.runtimeType == Text, 'compact has title');
  check(rich.subtitle.runtimeType == Text, 'rich has subtitle');
  check(rich.leading.runtimeType == Icon, 'rich has leading icon');
  check(rich.trailing.runtimeType == Icon, 'rich has trailing icon');
  check(rich.backgroundColor != null, 'rich has background color');
  symbolDetails.add('compact=$compact');
  symbolDetails.add('rich=$rich');
