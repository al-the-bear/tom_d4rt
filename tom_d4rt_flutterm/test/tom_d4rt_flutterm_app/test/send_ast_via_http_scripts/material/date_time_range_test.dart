// D4rt test script: Comprehensive tests for DateTimeRange
...
dynamic build(BuildContext context) {
  ...
  print('=== DateTimeRange comprehensive D4rt test start ===');
  _expect(context.runtimeType.toString().isNotEmpty, 'context runtimeType is available', logs);
  assertionCount++;

  final range = DateTimeRange(start: DateTime(2024, 1, 1), end: DateTime(2024, 1, 8));
  final singleDay = DateTimeRange(start: DateTime(2024, 2, 14), end: DateTime(2024, 2, 14));
  log('Created DateTimeRange probes for standard && single-day intervals');
  _expect(range.runtimeType.toString().contains('DateTimeRange'), 'constructs DateTimeRange', logs);
  assertionCount++;
  _expect(range.duration.inDays == 7, 'duration reflects 7 day span', logs);
  assertionCount++;
  _expect(range.start.isBefore(range.end), 'start before end in standard range', logs);
  assertionCount++;
  _expect(singleDay.duration.inDays == 0, 'edge case supports zero-duration range', logs);
  assertionCount++;
  _expect(range.start.year == 2024, 'range start property is accessible', logs);
  assertionCount++;
  ...
  print('=== DateTimeRange comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DateTimeRange Summary'),
