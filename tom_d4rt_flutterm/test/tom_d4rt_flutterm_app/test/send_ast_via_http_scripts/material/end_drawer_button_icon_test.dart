...
// D4rt test script: Comprehensive tests for EndDrawerButtonIcon from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: EndDrawerButtonIcon ===');

  final symbolName = 'EndDrawerButtonIcon';
  final fileName = 'end_drawer_button_icon_test.dart';
  final logs = <String>[];
  ...
  const iconA = EndDrawerButtonIcon();
  const iconB = EndDrawerButtonIcon();
  check(iconA.runtimeType == EndDrawerButtonIcon, 'icon A type is correct');
  check(iconB.runtimeType == EndDrawerButtonIcon, 'icon B type is correct');
  check(identical(iconA, iconB), 'const widget canonicalization works');
  check(iconA.toStringShort() == iconB.toStringShort(), 'short strings match');
  symbolDetails.add('iconA=$iconA');
