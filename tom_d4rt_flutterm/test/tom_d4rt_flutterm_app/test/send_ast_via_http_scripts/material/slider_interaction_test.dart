...
Map<String, dynamic> _analyzeSliderInteraction() {
  ...
  assert(
    values.isNotEmpty,
    'SliderInteraction should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'SliderInteraction: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for SliderInteraction',
    );

    final fromName = SliderInteraction.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for SliderInteraction',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for SliderInteraction',
    );
  }
  ...
  assert(
    ...
    'SliderInteraction enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'SliderInteraction enum indexes must be unique',
  );
  ...
  assert(
    ...
    'SliderInteraction enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'SliderInteraction: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'SliderInteraction.values.byName should throw for invalid names',
  );
  ...
  print(
    'SliderInteraction: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'SliderInteraction has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'SliderInteraction summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'SliderInteraction must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'SliderInteraction order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'SliderInteraction invalid-name edge case must throw',
  );
