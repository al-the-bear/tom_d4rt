// D4rt test script: Comprehensive tests for DialogRoute
...
dynamic build(BuildContext context) {
  ...
  print('=== DialogRoute comprehensive D4rt test start ===');
  _expect(context.runtimeType.toString().isNotEmpty, 'context runtimeType is available', logs);
  assertionCount++;

  final route = DialogRoute<void>(
    context: context,
    builder: (_) => const AlertDialog(title: Text('Probe')),
  );
  log('Constructed DialogRoute with AlertDialog builder');
  _expect(route.runtimeType.toString().contains('DialogRoute'), 'constructs DialogRoute with builder', logs);
  assertionCount++;
  _expect(route.settings.name == null, 'default route settings name is null', logs);
  assertionCount++;
  _expect(route.barrierDismissible, 'dialog route is dismissible by default', logs);
  assertionCount++;
  _expect(route.barrierColor != null, 'dialog route exposes barrier color', logs);
  assertionCount++;
  _expect(route.toString().contains('DialogRoute'), 'string representation confirms DialogRoute behavior', logs);
  assertionCount++;
  ...
  print('=== DialogRoute comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DialogRoute Summary'),
