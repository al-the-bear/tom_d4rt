// D4rt test script: Comprehensive tests for DataTableTheme
...
dynamic build(BuildContext context) {
  ...
  print('=== DataTableTheme comprehensive D4rt test start ===');
  _expect(
  ...
  const themed = DataTableTheme(
    data: DataTableThemeData(dataRowMinHeight: 30, dataRowMaxHeight: 60),
    child: SizedBox(width: 10, height: 10),
  );
  final inherited = DataTableTheme.of(context);
  log('Built DataTableTheme && resolved inherited DataTableThemeData');
  _expect(
    themed.runtimeType.toString().contains('DataTableTheme'),
    'constructs DataTableTheme widget',
    logs,
  );
  assertionCount++;
  _expect(
    themed.data.dataRowMinHeight == 30,
    'theme exposes configured min row height',
    logs,
  );
  assertionCount++;
  _expect(
    themed.data.dataRowMaxHeight == 60,
    'theme exposes configured max row height',
    logs,
  );
  assertionCount++;
  _expect(
    themed.child.runtimeType.toString().contains('SizedBox'),
    'theme keeps provided child widget',
    logs,
  );
  assertionCount++;
  _expect(
    inherited.runtimeType.toString().contains('DataTableThemeData'),
    'DataTableTheme.of returns theme data',
    logs,
  );
  assertionCount++;
  ...
  print('=== DataTableTheme comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DataTableTheme Summary'),
