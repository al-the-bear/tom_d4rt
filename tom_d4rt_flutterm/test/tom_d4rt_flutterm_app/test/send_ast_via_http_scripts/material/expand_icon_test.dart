...
// D4rt test script: Comprehensive tests for ExpandIcon from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: ExpandIcon ===');

  final symbolName = 'ExpandIcon';
  final fileName = 'expand_icon_test.dart';
  final logs = <String>[];
  ...
  final expand = ExpandIcon(
    isExpanded: false,
    onPressed: (bool value) {
      log('ExpandIcon callback value=$value');
    },
  );
  final collapsed = ExpandIcon(
    isExpanded: true,
    onPressed: (bool value) {
      log('ExpandIcon second callback value=$value');
    },
  );
  check(expand.isExpanded == false, 'expand icon starts collapsed');
  check(collapsed.isExpanded, 'second icon starts expanded');
  check(expand.onPressed != null, 'expand callback exists');
  check(collapsed.onPressed != null, 'second callback exists');
  symbolDetails.add('expand=$expand');
