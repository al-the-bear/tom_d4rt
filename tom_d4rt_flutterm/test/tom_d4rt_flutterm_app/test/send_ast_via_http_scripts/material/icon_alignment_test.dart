...
Map<String, dynamic> _analyzeIconAlignment() {
  ...
  assert(
    values.isNotEmpty,
    'IconAlignment should expose at least one enum value',
  );
  ...
  for (final value in values) {
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for IconAlignment',
    );

    final fromName = IconAlignment.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for IconAlignment',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for IconAlignment',
    );
  }
  ...
  assert(
    ...
    'IconAlignment enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'IconAlignment enum indexes must be unique',
  );
  ...
  assert(
    ...
    'IconAlignment enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'IconAlignment: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'IconAlignment.values.byName should throw for invalid names',
  );
  ...
  print(
    'IconAlignment: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'IconAlignment has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'IconAlignment summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'IconAlignment must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'IconAlignment order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'IconAlignment invalid-name edge case must throw',
  );
