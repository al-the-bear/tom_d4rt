...
// D4rt comprehensive test script for Thumb
...
dynamic build(BuildContext context) {
  ...
  print('========== Thumb comprehensive test ==========' );

  final List<Thumb> values = Thumb.values;
  expectCondition(values.isNotEmpty, 'Thumb.values should not be empty', logs, counters);
  expectCondition(values.length >= 2, 'Thumb should provide multiple values', logs, counters);

  final Thumb first = values.first;
  final Thumb last = values.last;
  ...
  for (final Thumb value in values) {
    ...
    final Thumb byName = Thumb.values.byName(value.name);
  ...
  final List<Thumb> copied = List<Thumb>.from(values);
  ...
  final List<Thumb> reversed = values.reversed.toList(growable: false);
  ...
  final Thumb edgeByName = Thumb.values.byName(edgeName);
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        Text('Thumb Comprehensive Tests'),
