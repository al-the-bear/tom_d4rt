...
dynamic build(BuildContext context) {
  ...
  _expect(
    dialog.runtimeType.toString().contains('DateRangePickerDialog'),
    'constructs DateRangePickerDialog',
    logs,
  );
  assertionCount++;
  _expect(
    selected.start.isBefore(selected.end),
    'edge case: initial range start is before end',
    logs,
  );
  assertionCount++;
  _expect(
    !firstDate.isAfter(lastDate),
    'edge case: firstDate is not after lastDate',
    logs,
  );
  assertionCount++;
  _expect(
    dialog.toString().contains('DateRangePickerDialog'),
    'string form identifies dialog type',
    logs,
  );
  assertionCount++;
  _expect(
    ctor.runtimeType.toString().isNotEmpty,
    'constructor tear-off is available',
    logs,
  );
  assertionCount++;
