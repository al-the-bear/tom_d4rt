...
Map<String, dynamic> _analyzeListTileTitleAlignment() {
  ...
  assert(
    values.isNotEmpty,
    'ListTileTitleAlignment should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'ListTileTitleAlignment: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for ListTileTitleAlignment',
    );

    final fromName = ListTileTitleAlignment.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for ListTileTitleAlignment',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for ListTileTitleAlignment',
    );
  }
  ...
  assert(
    ...
    'ListTileTitleAlignment enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'ListTileTitleAlignment enum indexes must be unique',
  );
  ...
  assert(
    ...
    'ListTileTitleAlignment enum indexes should be contiguous and zero-based',
  );
  ...
  try {
    ...
    print(
      'ListTileTitleAlignment: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'ListTileTitleAlignment.values.byName should throw for invalid names',
  );
  ...
  print(
    'ListTileTitleAlignment: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'ListTileTitleAlignment has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'ListTileTitleAlignment summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'ListTileTitleAlignment must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'ListTileTitleAlignment order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'ListTileTitleAlignment invalid-name edge case must throw',
  );
