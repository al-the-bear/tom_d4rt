...
Map<String, dynamic> _analyzeHourFormat() {
  ...
  for (final value in values) {
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for HourFormat',
    );

    final fromName = HourFormat.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for HourFormat',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for HourFormat',
    );
  }
  ...
  assert(
    ...
    'HourFormat enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'HourFormat enum indexes must be unique',
  );
  ...
  assert(
    ...
    'HourFormat enum indexes should be contiguous and zero-based',
  );
  ...
  assert(
    invalidNameThrows,
    'HourFormat.values.byName should throw for invalid names',
  );
  ...
  print(
    'HourFormat: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'HourFormat has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'HourFormat summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'HourFormat must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'HourFormat order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'HourFormat invalid-name edge case must throw',
  );
