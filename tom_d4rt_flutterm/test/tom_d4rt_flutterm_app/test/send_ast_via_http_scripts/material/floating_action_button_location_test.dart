...
// D4rt test script: Comprehensive tests for FloatingActionButtonLocation from material
import 'package:flutter/material.dart';

dynamic build(BuildContext context) {
  print('=== D4rt material test start: FloatingActionButtonLocation ===');

  final symbolName = 'FloatingActionButtonLocation';
  final fileName = 'floating_action_button_location_test.dart';
  final logs = <String>[];
  ...
  const locationA = FloatingActionButtonLocation.endFloat;
  const locationB = FloatingActionButtonLocation.startDocked;
  const locationEdge = FloatingActionButtonLocation.centerFloat;
  check(
    locationA.runtimeType.toString().isNotEmpty,
    'endFloat runtime type available',
  );
  check(
    locationB.runtimeType.toString().isNotEmpty,
    'startDocked runtime type available',
  );
  check(
    locationEdge.runtimeType.toString().isNotEmpty,
    'centerFloat runtime type available',
  );
  check(locationA != locationB, 'different presets differ');
  symbolDetails.add('locationA=$locationA');
  symbolDetails.add('locationB=$locationB');
  symbolDetails.add('locationEdge=$locationEdge');
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Checklist sample (${checklist.length}):',
                style: Theme.of(context).textTheme.labelLarge,
              ),
