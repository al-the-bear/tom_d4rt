...
dynamic build(BuildContext context) {
  ...
  logs.add('Starting MaterialType checks');

  final values = MaterialType.values;
  _check(
  ...
  final holders = values
      .map(
        (value) => _EnumHolder<MaterialType>(value: value, label: value.name),
      )
  ...
  final allRoundTrips = values
      .map((value) => MaterialType.values.byName(value.name) == value)
  ...
  try {
    MaterialType.values.byName('__MaterialType_invalid__');
  } catch (_) {
  ...
  final summary =
      'MaterialType summary: checks=${stats.checks}, failures=${stats.failures}, values=${values.length}';
