// D4rt test script: Comprehensive tests for BottomNavigationBarTheme
...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError('BottomNavigationBarTheme assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== BottomNavigationBarTheme comprehensive test start ===');
  final logs = <String>[];
  var assertionCount = 0;


  final themeData = const BottomNavigationBarThemeData(selectedItemColor: Colors.purple);
  final theme = BottomNavigationBarTheme(
    data: themeData,
    child: const SizedBox(width: 20, height: 20),
  );
  _expect(theme is BottomNavigationBarTheme, 'instantiates BottomNavigationBarTheme', logs);
  assertionCount++;
  _expect(theme.data.selectedItemColor == Colors.purple, 'stores theme data', logs);
  assertionCount++;
  _expect(theme.child is SizedBox, 'stores child widget', logs);
  assertionCount++;
  _expect(theme.toStringShort().contains('BottomNavigationBarTheme'), 'string representation available', logs);
  assertionCount++;
  ...
  print('=== BottomNavigationBarTheme comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('BottomNavigationBarTheme Tests'),
