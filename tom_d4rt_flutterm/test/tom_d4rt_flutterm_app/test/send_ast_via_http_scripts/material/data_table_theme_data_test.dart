// D4rt test script: Comprehensive tests for DataTableThemeData
...
dynamic build(BuildContext context) {
  ...
  print('=== DataTableThemeData comprehensive D4rt test start ===');
  _expect(
  ...
  const baseTheme = DataTableThemeData();
  final denseTheme = baseTheme.copyWith(
    dataRowMinHeight: 24,
    dataRowMaxHeight: 48,
    headingRowHeight: 52,
    dividerThickness: 1.5,
  );
  log('Created default && dense DataTableThemeData instances');
  _expect(
    baseTheme.runtimeType.toString().contains('DataTableThemeData'),
    'default constructor creates expected runtime type',
    logs,
  );
  assertionCount++;
  _expect(
    denseTheme.dataRowMinHeight == 24,
    'copyWith updates dataRowMinHeight',
    logs,
  );
  assertionCount++;
  _expect(
    denseTheme.dataRowMaxHeight == 48,
    'copyWith updates dataRowMaxHeight',
    logs,
  );
  assertionCount++;
  _expect(
    denseTheme.headingRowHeight == 52,
    'copyWith updates heading row height',
    logs,
  );
  assertionCount++;
  _expect(
    denseTheme.dividerThickness == 1.5,
    'copyWith updates divider thickness',
    logs,
  );
  assertionCount++;
  ...
  print('=== DataTableThemeData comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DataTableThemeData Summary'),
