...
dynamic build(BuildContext context) {
  ...
  _expect(
    dateOnly.year == 2026 && dateOnly.month == 3 && dateOnly.day == 14,
    'dateOnly keeps Y/M/D',
    logs,
  );
  assertionCount++;
  _expect(
    dateOnly.hour == 0 && dateOnly.minute == 0,
    'dateOnly resets time to midnight',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    !DateUtils.isSameDay(a, b),
    'isSameDay false across day boundary',
    logs,
  );
  assertionCount++;
  _expect(
    DateUtils.isSameMonth(a, b),
    'isSameMonth true within same month/year',
    logs,
  );
  assertionCount++;

  final delta = DateUtils.monthDelta(
    DateTime(2024, 1, 1),
    DateTime(2025, 3, 1),
  );
  ...
  _expect(
    plusMonths.year == 2024 && plusMonths.month == 3 && plusMonths.day == 1,
    'addMonthsToMonthDate normalizes to month start',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    plusDays.year == 2025 && plusDays.month == 1 && plusDays.day == 1,
    'addDaysToDate crosses year boundary',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    leapDays == 29 && nonLeapDays == 28,
    'getDaysInMonth handles leap year',
    logs,
  );
  assertionCount++;
  ...
  print(
    'dateOnly=$dateOnly delta=$delta plusMonths=$plusMonths plusDays=$plusDays leap/nonLeap=$leapDays/$nonLeapDays',
  );
