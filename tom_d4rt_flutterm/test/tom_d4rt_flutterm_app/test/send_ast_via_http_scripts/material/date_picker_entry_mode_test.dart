// D4rt comprehensive test script: DatePickerEntryMode (material)
...
class DatePickerEntryModeCaseStudy {
  DatePickerEntryModeCaseStudy({
    required this.label,
  ...
  final String label;
  final List<DatePickerEntryMode> samples;
  ...
  List<String> run() {
    ...
    assert(samples.isNotEmpty, 'DatePickerEntryMode must expose at least one value');
    ...
    final byName = DatePickerEntryMode.values.byName(samples.first.name);
  ...
  dynamic buildSummary(BuildContext context, List<String> logs) {
    ...
    return Column(
      ...
      children: [
        Text('DatePickerEntryMode summary'),
        Text('label: $label'),
...
dynamic build(BuildContext context) {
  print('DatePickerEntryMode test start');
  final values = DatePickerEntryMode.values;
  final caseStudy = DatePickerEntryModeCaseStudy(
    label: 'DatePickerEntryMode-case-study',
    samples: values,
  ...
  assert(values.length >= 1, 'DatePickerEntryMode must have at least one enum value');

  final containsValueLog = logs.any((entry) => entry.startsWith('value:'));
  assert(containsValueLog, 'Value logs are required');

  print('DatePickerEntryMode test completed with ${logs.length} log lines');
