...
class BottomNavigationBarLandscapeLayoutCaseStudy {
  ...
  List<String> run() {
    ...
    assert(
      samples.isNotEmpty,
      'BottomNavigationBarLandscapeLayout must expose at least one value',
    );
    ...
    final byName = BottomNavigationBarLandscapeLayout.values.byName(
      samples.first.name,
    );
...
dynamic build(BuildContext context) {
  ...
  assert(
    values.length >= 1,
    'BottomNavigationBarLandscapeLayout must have at least one enum value',
  );
  ...
  print(
    'BottomNavigationBarLandscapeLayout test completed with ${logs.length} log lines',
  );
