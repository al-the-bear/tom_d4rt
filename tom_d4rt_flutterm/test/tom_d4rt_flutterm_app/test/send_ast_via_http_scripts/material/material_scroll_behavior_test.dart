...
dynamic build(BuildContext context) {
  ...
  final Widget probeWidget = ListView(
    children: const <Widget>[SizedBox(height: 20)],
  );
  final ScrollPhysics physics = behavior.getScrollPhysics(context);

  check(
    behavior.runtimeType.toString().contains('MaterialScrollBehavior'),
    'Runtime type mentions MaterialScrollBehavior',
  );
  check(dragDevices.isNotEmpty, 'dragDevices is not empty');
  check(
    dragDevices.contains(PointerDeviceKind.touch),
    'touch dragging is supported',
  );
  check(
    probeWidget.runtimeType.toString().contains('ListView'),
    'Probe widget is a ListView',
  );
  check(
    physics.runtimeType.toString().isNotEmpty,
    'getScrollPhysics returns concrete physics',
  );
