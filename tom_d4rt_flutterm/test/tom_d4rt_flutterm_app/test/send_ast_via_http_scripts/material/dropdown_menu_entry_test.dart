// D4rt test script: Comprehensive tests for DropdownMenuEntry
...
dynamic build(BuildContext context) {
  ...
  print('=== DropdownMenuEntry comprehensive D4rt test start ===');
  _expect(
  ...
  const entry = DropdownMenuEntry<int>(value: 7, label: 'Seven');
  const disabledEntry = DropdownMenuEntry<int>(
    value: 8,
    label: 'Eight',
    enabled: false,
  );
  log('Created enabled && disabled DropdownMenuEntry values');
  _expect(
    entry.runtimeType.toString().contains('DropdownMenuEntry<int>'),
    'constructs generic DropdownMenuEntry',
    logs,
  );
  assertionCount++;
  _expect(entry.value == 7, 'value property stores generic value', logs);
  assertionCount++;
  _expect(entry.label == 'Seven', 'label property stores display string', logs);
  assertionCount++;
  _expect(!disabledEntry.enabled, 'edge case: disabled entry supported', logs);
  assertionCount++;
  _expect(disabledEntry.value == 8, 'disabled entry keeps value', logs);
  assertionCount++;
  ...
  print('=== DropdownMenuEntry comprehensive D4rt test end ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('DropdownMenuEntry Summary'),
