...
dynamic build(BuildContext context) {
  ...
  _expect(
    style.enableAnimation != null,
    'constructor creates ChipAnimationStyle',
    logs,
  );
  assertionCount++;
  _expect(
    style.enableAnimation?.duration == const Duration(milliseconds: 100),
    'stores enable animation',
    logs,
  );
  assertionCount++;
  _expect(
    style.selectAnimation?.duration == const Duration(milliseconds: 200),
    'stores select animation',
    logs,
  );
  assertionCount++;
  _expect(
    style.avatarDrawerAnimation?.duration == const Duration(milliseconds: 150),
    'stores avatar drawer animation',
    logs,
  );
  assertionCount++;
  _expect(
    style.deleteDrawerAnimation?.duration == const Duration(milliseconds: 170),
    'stores delete drawer animation',
    logs,
  );
  assertionCount++;
  _expect(
    edge.enableAnimation == null && edge.selectAnimation == null,
    'edge case supports null defaults',
    logs,
  );
  assertionCount++;
