...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError('ButtonBarTheme assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== ButtonBarTheme comprehensive test start ===');
  final logs = <String>[];
  var assertionCount = 0;

  const baseData = ButtonBarThemeData(alignment: MainAxisAlignment.center);
  ...
  _expect(
    inherited.hashCode.toString().isNotEmpty,
    'ButtonBarTheme.of resolves data',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    themed.updateShouldNotify(updated),
    'updateShouldNotify reacts to data change',
    logs,
  );
  assertionCount++;
  ...
  print('=== ButtonBarTheme comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('ButtonBarTheme Tests'),
      Text('Assertions: $assertionCount'),
      Text('Inherited hash: ' + ButtonBarTheme.of(context).hashCode.toString()),
