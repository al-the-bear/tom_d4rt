// D4rt test script: Comprehensive tests for BottomNavigationBarThemeData
...
void _expect(bool condition, String message, List<String> logs) {
  if (!condition) {
    ...
    throw StateError('BottomNavigationBarThemeData assertion failed: ' + message);
  }
  logs.add('PASS: ' + message);
}

dynamic build(BuildContext context) {
  print('=== BottomNavigationBarThemeData comprehensive test start ===');
  final logs = <String>[];
  ...
  _expect(data.backgroundColor == Colors.black, 'stores backgroundColor', logs);
  assertionCount++;
  _expect(data.selectedItemColor == Colors.blue, 'stores selectedItemColor', logs);
  assertionCount++;
  _expect(data.unselectedItemColor == Colors.grey, 'stores unselectedItemColor', logs);
  assertionCount++;
  _expect(data.showSelectedLabels == true, 'stores showSelectedLabels', logs);
  assertionCount++;
  ...
  print('=== BottomNavigationBarThemeData comprehensive test complete ===');

  return Column(
    mainAxisSize: MainAxisSize.min,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('BottomNavigationBarThemeData Tests'),
