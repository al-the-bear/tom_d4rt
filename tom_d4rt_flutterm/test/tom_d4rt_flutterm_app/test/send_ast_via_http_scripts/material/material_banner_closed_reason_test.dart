...
Map<String, dynamic> _analyzeMaterialBannerClosedReason() {
  ...
  assert(
    values.isNotEmpty,
    'MaterialBannerClosedReason should expose at least one enum value',
  );
  ...
  for (final value in values) {
    print(
      'MaterialBannerClosedReason: value=${value.name} index=${value.index} raw=$value',
    );
    ...
    assert(
      fromIndex == value,
      'Index lookup must resolve the same value for MaterialBannerClosedReason',
    );

    final fromName = MaterialBannerClosedReason.values.byName(value.name);
    assert(
      fromName == value,
      'byName must resolve the same value for MaterialBannerClosedReason',
    );
    ...
    assert(
      ...
      'toString should contain the enum name for MaterialBannerClosedReason',
    );
  }
  ...
  assert(
    ...
    'MaterialBannerClosedReason enum names must be unique',
  );
  assert(
    uniqueIndexes.length == indexes.length,
    'MaterialBannerClosedReason enum indexes must be unique',
  );
  ...
  assert(
    ...
    'MaterialBannerClosedReason enum indexes should be contiguous and zero-based',
  );

  var invalidNameThrows = false;
  try {
    MaterialBannerClosedReason.values.byName(
      '__invalid_materialbannerclosedreason__',
    );
  } catch (error) {
    invalidNameThrows = true;
    print(
      'MaterialBannerClosedReason: expected edge-case failure for invalid name -> $error',
    );
  }
  assert(
    invalidNameThrows,
    'MaterialBannerClosedReason.values.byName should throw for invalid names',
  );
  ...
  print(
    'MaterialBannerClosedReason: first=$first middle=$middle last=$last total=${values.length}',
  );
  ...
  return <String, dynamic>{
    ...
    'constructorCoverage':
        'MaterialBannerClosedReason has implicit enum constructors; instance creation is validated via values traversal',
...
dynamic build(BuildContext context) {
  ...
  assert(
    ...
    'MaterialBannerClosedReason summary must expose integer valueCount',
  );
  assert(
    (summary['valueCount'] as int) > 0,
    'MaterialBannerClosedReason must have one or more values',
  );
  assert(
    summary['orderChecksPassed'] == true,
    'MaterialBannerClosedReason order checks must pass',
  );
  assert(
    summary['invalidNameThrows'] == true,
    'MaterialBannerClosedReason invalid-name edge case must throw',
  );
