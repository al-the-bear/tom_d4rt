// D4rt test script: Comprehensive KeyboardSide coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = KeyboardSide.values.byName(value.name);
    assert(identical(byName, value));
    assert(KeyboardSide.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <KeyboardSide, String>{
  ...
  try {
    KeyboardSide.values.byName('__not_a_keyboard_side__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => KeyboardSide.values.byName(value.name))
