// D4rt test script: Comprehensive tests for SystemSoundType
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SystemSoundType: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SystemSoundType comprehensive test start ===');

  final values = SystemSoundType.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SystemSoundType.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = SystemSoundType.values.byName(value.name);
    _expect(
  ...
  try {
    SystemSoundType.values.byName('__definitely_not_a_systemsoundtype__');
  } catch (error) {
  ...
  print('=== SystemSoundType comprehensive test completed ===');
  return Column(
