// D4rt test script: Comprehensive DeviceOrientation coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(DeviceOrientation value) => '${value.name}#${value.index}';
  ...
  for (final value in values) {
    ...
    final byName = DeviceOrientation.values.byName(value.name);
    assert(identical(byName, value));
    assert(DeviceOrientation.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <DeviceOrientation, String>{
  ...
  try {
    DeviceOrientation.values.byName('__not_a_device_orientation__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => DeviceOrientation.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('DeviceOrientation Tests'),
