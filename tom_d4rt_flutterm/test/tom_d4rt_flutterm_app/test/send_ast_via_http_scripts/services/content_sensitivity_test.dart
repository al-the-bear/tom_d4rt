// D4rt test script: Comprehensive ContentSensitivity coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = ContentSensitivity.values.byName(value.name);
    assert(identical(byName, value));
    assert(ContentSensitivity.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <ContentSensitivity, String>{
  ...
  try {
    ContentSensitivity.values.byName('__not_a_content_sensitivity__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => ContentSensitivity.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('ContentSensitivity Tests'),
