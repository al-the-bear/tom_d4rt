// D4rt test script: Comprehensive KeyboardLockMode coverage.
...
dynamic build(BuildContext context) {
  ...
  for (final value in values) {
    ...
    final byName = KeyboardLockMode.values.byName(value.name);
    assert(identical(byName, value));
    assert(KeyboardLockMode.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <KeyboardLockMode, String>{
  ...
  try {
    KeyboardLockMode.values.byName('__not_a_keyboard_lock_mode__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => KeyboardLockMode.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('KeyboardLockMode Tests'),
