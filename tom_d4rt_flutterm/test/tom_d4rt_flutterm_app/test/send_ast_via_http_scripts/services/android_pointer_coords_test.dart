...
dynamic build(BuildContext context) {
  ...
  try {
    final coords1 = AndroidPointerCoords(
      ...
      toolMinor: 10.0, touchMajor: 15.0, touchMinor: 15.0, x: 0.0, y: 0.0,
    );
    assert(coords1.orientation == 0.0);
    print('Orientation at 0: ${coords1.orientation}');

    final coords2 = AndroidPointerCoords(
      orientation: 1.5708, pressure: 1.0, size: 0.5, toolMajor: 10.0,
      toolMinor: 10.0, touchMajor: 15.0, touchMinor: 15.0, x: 0.0, y: 0.0,
    );
  ...
  try {
    ...
    final lightCoords = AndroidPointerCoords(
      orientation: 0.0, pressure: 0.1, size: 0.3, toolMajor: 5.0,
      toolMinor: 5.0, touchMajor: 8.0, touchMinor: 8.0, x: 50.0, y: 50.0,
    );
    ...
    final mediumCoords = AndroidPointerCoords(
      orientation: 0.0, pressure: 0.5, size: 0.5, toolMajor: 10.0,
      toolMinor: 10.0, touchMajor: 15.0, touchMinor: 15.0, x: 100.0, y: 100.0,
    );
    ...
    final fullCoords = AndroidPointerCoords(
      orientation: 0.0, pressure: 1.0, size: 0.8, toolMajor: 15.0,
      toolMinor: 15.0, touchMajor: 20.0, touchMinor: 20.0, x: 150.0, y: 150.0,
    );
  ...
  try {
    final coords = AndroidPointerCoords(
      orientation: 0.0, pressure: 1.0, size: 0.5, toolMajor: 20.0,
      toolMinor: 15.0, touchMajor: 25.0, touchMinor: 20.0, x: 200.0, y: 200.0,
    );
  ...
  try {
    final coords = AndroidPointerCoords(
      orientation: 0.0, pressure: 1.0, size: 0.6, toolMajor: 12.0,
      toolMinor: 10.0, touchMajor: 18.0, touchMinor: 14.0, x: 250.0, y: 250.0,
    );
  ...
  try {
    final smallTouch = AndroidPointerCoords(
      orientation: 0.0, pressure: 0.5, size: 0.2, toolMajor: 5.0,
      toolMinor: 5.0, touchMajor: 8.0, touchMinor: 8.0, x: 300.0, y: 300.0,
    );
    assert(smallTouch.size == 0.2);
    print('Small touch size: ${smallTouch.size}');

    final largeTouch = AndroidPointerCoords(
      orientation: 0.0, pressure: 1.0, size: 0.9, toolMajor: 25.0,
      toolMinor: 25.0, touchMajor: 30.0, touchMinor: 30.0, x: 350.0, y: 350.0,
    );
  ...
  try {
    ...
    final largeCoords = AndroidPointerCoords(
      orientation: 0.0, pressure: 1.0, size: 0.5, toolMajor: 10.0,
      toolMinor: 10.0, touchMajor: 15.0, touchMinor: 15.0, x: 2560.0, y: 1440.0,
    );
  ...
  try {
    final coordsList = <AndroidPointerCoords>[
      ...
      AndroidPointerCoords(
        ...
        toolMinor: 8.0, touchMajor: 12.0, touchMinor: 12.0, x: 300.0, y: 300.0,
      ),
      AndroidPointerCoords(
        orientation: -0.5, pressure: 0.9, size: 0.45, toolMajor: 9.0,
        toolMinor: 9.0, touchMajor: 14.0, touchMinor: 14.0, x: 500.0, y: 200.0,
      ),
    ];
  ...
  return Column(
    ...
    children: [
      Text('AndroidPointerCoords Test Results',
