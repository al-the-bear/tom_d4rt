// D4rt test script: Comprehensive tests for ModifierKey
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for ModifierKey: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== ModifierKey comprehensive test start ===');

  final values = ModifierKey.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('ModifierKey.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = ModifierKey.values.byName(value.name);
    _expect(
  ...
  try {
    ModifierKey.values.byName('__definitely_not_a_modifierkey__');
  } catch (error) {
  ...
  print('=== ModifierKey comprehensive test completed ===');
  return Column(
