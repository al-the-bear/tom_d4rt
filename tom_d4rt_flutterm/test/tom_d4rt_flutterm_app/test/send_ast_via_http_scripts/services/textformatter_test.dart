...
dynamic build(BuildContext context) {
  ...
  _expect(
    digitsResult.text == '123',
    'digitsOnly removes non-digit characters',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    limited.text.length == 4,
    'LengthLimitingTextInputFormatter enforces max length',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    upper.text == 'ABZ',
    'custom formatter transforms text to uppercase',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    !noSpaces.text.contains(' '),
    'deny formatter removes whitespace',
    logs,
  );
  assertionCount++;

  _expect(
    digitsResult.selection.baseOffset >= 0,
    'formatted value keeps valid selection offset',
    logs,
  );
  assertionCount++;
