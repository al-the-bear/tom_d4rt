// D4rt test script: Comprehensive tests for TextCapitalization
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for TextCapitalization: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== TextCapitalization comprehensive test start ===');

  final values = TextCapitalization.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('TextCapitalization.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = TextCapitalization.values.byName(value.name);
    _expect(
  ...
  try {
    TextCapitalization.values.byName('__definitely_not_a_textcapitalization__');
  } catch (error) {
  ...
  print('=== TextCapitalization comprehensive test completed ===');
  return Column(
