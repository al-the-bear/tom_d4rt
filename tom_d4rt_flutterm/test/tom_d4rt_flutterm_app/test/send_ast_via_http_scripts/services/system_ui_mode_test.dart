// D4rt test script: Comprehensive tests for SystemUiMode
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SystemUiMode: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SystemUiMode comprehensive test start ===');

  final values = SystemUiMode.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SystemUiMode.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = SystemUiMode.values.byName(value.name);
    _expect(
  ...
  try {
    SystemUiMode.values.byName('__definitely_not_a_systemuimode__');
  } catch (error) {
  ...
  print('=== SystemUiMode comprehensive test completed ===');
  return Column(
