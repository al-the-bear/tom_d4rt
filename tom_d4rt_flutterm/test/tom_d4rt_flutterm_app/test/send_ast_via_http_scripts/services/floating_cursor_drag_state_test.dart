// D4rt test script: Comprehensive FloatingCursorDragState coverage.
...
dynamic build(BuildContext context) {
  ...
  String describe(FloatingCursorDragState value) => '${value.name}#${value.index}';

  log('--- constructor semantics ---');
  log('Enum instances are canonical singletons created by Dart enum constructors.');

  final values = FloatingCursorDragState.values;
  ...
  for (final value in values) {
    ...
    final byName = FloatingCursorDragState.values.byName(value.name);
    assert(identical(byName, value));
    assert(FloatingCursorDragState.values[value.index] == value);
  }

  log('--- behavior matrix ---');
  final behaviorMatrix = <FloatingCursorDragState, String>{
  ...
  try {
    FloatingCursorDragState.values.byName('__not_a_floating_cursor_drag_state__');
  } catch (error) {
  ...
  final roundTripStable = values
      .map((value) => FloatingCursorDragState.values.byName(value.name))
  ...
  return Column(
    ...
    children: [
      const Text('FloatingCursorDragState Tests'),
