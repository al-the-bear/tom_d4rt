// D4rt test script: Comprehensive tests for SwipeEdge
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SwipeEdge: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SwipeEdge comprehensive test start ===');

  final values = SwipeEdge.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SwipeEdge.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = SwipeEdge.values.byName(value.name);
    _expect(
  ...
  try {
    SwipeEdge.values.byName('__definitely_not_a_swipeedge__');
  } catch (error) {
  ...
  print('=== SwipeEdge comprehensive test completed ===');
  return Column(
