...
class _SingleChildLayoutProxy extends SingleChildLayoutDelegate {
  ...
  @override
  bool shouldRelayout(covariant _SingleChildLayoutProxy oldDelegate) =>
...
dynamic build(BuildContext context) {
  ...
  final childConstraints = delegateA.getConstraintsForChild(
    const BoxConstraints.tightFor(width: 100, height: 40),
  );
  _expect(
    !childConstraints.hasTightWidth && !childConstraints.hasTightHeight,
    'constraints are loosened for child',
    logs,
  );
  assertionCount++;

  final centered = delegateA.getPositionForChild(
    const Size(100, 60),
    const Size(20, 10),
  );
  _expect(
    centered.dx == 40 && centered.dy == 25,
    'center anchor computes centered offset',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    ...
    'same anchor does not relayout',
    logs,
  );
  assertionCount++;
  _expect(
    delegateA.shouldRelayout(delegateC),
    'different anchor triggers relayout',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    widget.delegate == delegateA,
    'CustomSingleChildLayout stores delegate',
    logs,
  );
  assertionCount++;

  final edge = delegateC.getPositionForChild(
    const Size(10, 10),
    const Size(20, 20),
  );
  _expect(
    edge.dx <= 0 && edge.dy <= 0,
    'edge case larger child yields non-positive offset',
    logs,
  );
  assertionCount++;
