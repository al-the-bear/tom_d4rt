...
dynamic build(BuildContext context) {
  ...
  _expect(v0 == 2, 'transform at 0 is begin', logs);
  assertionCount++;
  _expect(v1 == 9, 'transform at 1 is end', logs);
  assertionCount++;
  ...
  _expect(
    quarter <= half && half <= threeQuarter,
    'step progression is monotonic',
    logs,
  );
  assertionCount++;
  _expect(
    quarter is int && half is int && threeQuarter is int,
    'step outputs are integers',
    logs,
  );
  assertionCount++;

  final down = StepTween(begin: 9, end: 2);
  _expect(down.transform(0.0) == 9, 'descending tween begin works', logs);
  assertionCount++;
  _expect(down.transform(1.0) == 2, 'descending tween end works', logs);
  assertionCount++;
