...
dynamic build(BuildContext context) {
  ...
  _expect(
    start.dx >= 0 && start.dy >= 0,
    'transform(0.0) returns valid start sample',
    logs,
  );
  assertionCount++;
  _expect(
    end.dx <= 1.0 + 1e-9 && end.dy <= 1.0 + 1e-9,
    'transform(1.0) returns valid end sample',
    logs,
  );
  assertionCount++;
  _expect(
    middle.dx >= 0 && middle.dx <= 1,
    'middle sample x remains in expected range',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    (preMid.dx - middle.dx).abs() < 0.2,
    'precomputed spline behavior is close to lazy spline',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    samples.isNotEmpty,
    'generateSamples returns at least one sample',
    logs,
  );
  assertionCount++;
  _expect(
    samples.first.t >= 0 && samples.last.t <= 1,
    'generated samples remain in parameter range',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    edgeMid.dx >= 0 && edgeMid.dx <= 1,
    'edge case tension=1.0 remains evaluable',
    logs,
  );
  assertionCount++;
