...
dynamic build(BuildContext context) {
  ...
  for (final t in samples) {
    ...
    longValues.add(l);
    log(
      't=$t default=${d.toStringAsFixed(6)} short=${s.toStringAsFixed(6)} long=${l.toStringAsFixed(6)}',
    );
  }

  expectCheck(defaultValues.first == 0.0, 'ElasticOutCurve(0) should be 0');
  expectCheck(
    (defaultValues.last - 1.0).abs() < 0.0000001,
    'ElasticOutCurve(1) should be ~1',
  );
  expectCheck(
    shortValues.length == samples.length,
    'short-period sample size should match input',
  );
  expectCheck(
    longValues.length == samples.length,
    'long-period sample size should match input',
  );
