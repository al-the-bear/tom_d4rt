// Comprehensive D4rt test script: AnimationLocalStatusListenersMixin from animation
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive AnimationLocalStatusListenersMixin script ===');
  ...
  detailLines.add('target=AnimationLocalStatusListenersMixin');
  ...
  const String targetTypeName = 'AnimationLocalStatusListenersMixin';
  check(targetTypeName.contains('Status'), 'Status in name');
  detailLines.add('category=animation_mixin');
  detailLines.add('desc=Manages local animation status listeners');
  ...
  ctrl.addStatusListener((AnimationStatus s) { lastStatus = s; });
  check(ctrl.status == AnimationStatus.dismissed, 'Initial status dismissed');
  ctrl.dispose();
