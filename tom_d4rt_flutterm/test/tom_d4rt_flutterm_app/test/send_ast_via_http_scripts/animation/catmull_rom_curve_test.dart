// Comprehensive D4rt test script: CatmullRomCurve from animation
...
dynamic build(BuildContext context) {
  print('=== Running comprehensive CatmullRomCurve script ===');
  ...
  detailLines.add('target=CatmullRomCurve');
  ...
  final CatmullRomCurve curve = CatmullRomCurve(const <Offset>[Offset(0.2, 0.3), Offset(0.5, 0.8), Offset(0.8, 0.4)]);
  check(curve is Curve, 'Is Curve');
  final double mid = curve.transform(0.5);
  check(mid >= 0.0 && mid <= 1.0, 'Mid-point in range');
  check(curve.transform(0.0) >= 0.0, 'Start in range');
  check(curve.transform(1.0) >= 0.0, 'End in range');
  detailLines.add('controlPoints=3');
