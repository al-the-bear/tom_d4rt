...
dynamic build(BuildContext context) {
  ...
  assertionCount++;
  _expect(
    sampleA.value == Offset(0.2, 0.4),
    'constructor stores 2D value',
    logs,
  );
  assertionCount++;
  _expect(
    sampleB.t > sampleA.t,
    'samples can represent increasing parameter values',
    logs,
  );
  assertionCount++;
  ...
  assertionCount++;
  _expect(
    asText.contains('0.20') || asText.contains('0.2'),
    'toString contains x coordinate',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    interpolated.dx > sampleA.value.dx,
    'behavior: midpoint interpolation increases dx',
    logs,
  );
  assertionCount++;
  _expect(
    interpolated.dy > sampleA.value.dy,
    'behavior: midpoint interpolation increases dy',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    edgeSample.value == Offset.zero,
    'edge case supports zero sample',
    logs,
  );
  assertionCount++;
