...
dynamic build(BuildContext context) {
  ...
  _expect(
    base.begin == 0.0 && base.end == 10.0,
    'base tween constructor values retained',
    logs,
  );
  assertionCount++;
  _expect(
    reverse.parent == base,
    'ReverseTween parent references base tween',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    sequenceValues.first >= sequenceValues.last,
    'sequence is descending for increasing t',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    chained.transform(0.2).isFinite,
    'chained animatable produces finite value',
    logs,
  );
  assertionCount++;
  ...
  _expect(
    intAt0 == 5 && intAt1 == 1,
    'edge generic case with int tween works',
    logs,
  );
  assertionCount++;
