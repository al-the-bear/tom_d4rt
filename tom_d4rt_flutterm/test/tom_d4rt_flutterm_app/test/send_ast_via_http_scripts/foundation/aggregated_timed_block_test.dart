...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    blockA.name == 'layout',
    'BlockA name is stored',
    logs,
    counters,
  );
  expectCondition(
    blockA.duration == 1.5,
    'BlockA duration is stored',
    logs,
    counters,
  );
  ...
  expectCondition(
    blockB.name == 'paint',
    'BlockB name is stored',
    logs,
    counters,
  );
  expectCondition(
    blockB.duration == 2.0,
    'BlockB duration is stored',
    logs,
    counters,
  );
  expectCondition(blockB.count == 4, 'BlockB count is stored', logs, counters);
  expectCondition(
    blockA.toString().contains('AggregatedTimedBlock'),
  ...
  expectCondition(
    blocks.length == 2,
    'Two aggregated blocks collected',
    logs,
    counters,
  );
  ...
  expectCondition(
    edge.duration == 0,
    'Edge duration of zero is supported',
    logs,
    counters,
  );
  expectCondition(
    edge.count == 0,
    'Edge count of zero is supported',
    logs,
    counters,
  );
  expectCondition(
