...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'Factory<int> instantiated',
    condition: intFactory is Factory<int>,
  );
  _check(
    logs: logs,
    label: 'type reports int',
    condition: intFactory.type == int,
  );
  ...
  _check(
    logs: logs,
    label: 'constructor returns expected value',
    condition: first == 42 && second == 42,
  );
  _check(
    logs: logs,
    label: 'constructor callable multiple times',
    condition: createCount == 2,
  );

  final stringFactory = Factory<String>(() => 'flutter');
  _check(
    logs: logs,
    label: 'Factory<String> type',
    condition: stringFactory.type == String,
  );
  _check(
    logs: logs,
    label: 'Factory<String> toString includes type',
    condition: stringFactory.toString().contains('String'),
  );
  ...
  _check(
    logs: logs,
    label: 'Factory creates map object',
    condition: object['x'] == 1 && object['y'] == 2,
  );
  ...
  _check(
    logs: logs,
    label: 'edge empty list supported',
    condition: edgeList.isEmpty,
  );
