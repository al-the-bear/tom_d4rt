...
dynamic build(BuildContext context) {
  ...
  log(
    'frame class=${frame.className} method=${frame.method} source=${frame.source}',
  );
  ...
  expectCheck(
    frame.column == 12,
    'frame column should match constructor value',
  );
  expectCheck(
    frame.className == 'App',
    'frame className should match constructor value',
  );

  final stackText =
      '#0 Root.render (package:myapp/root.dart:10:2)\n#1 App.build (package:myapp/src/main.dart:42:12)\n#2 WidgetTester.pump (package:flutter_test/src/widget_tester.dart:200:7)';
  final parsed = StackFrame.fromStackString(stackText);
  log('parsed count=${parsed.length}');
  expectCheck(
    parsed.length >= 2,
    'parsed frames should include at least two entries',
  );

  final first = parsed.first;
  log(
    'first parsed class=${first.className} method=${first.method} line=${first.line}',
  );
  expectCheck(
    first.className != null,
    'first parsed className should not be null',
  );
