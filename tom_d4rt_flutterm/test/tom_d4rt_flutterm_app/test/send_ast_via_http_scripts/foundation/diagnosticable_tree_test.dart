...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    treeA is DiagnosticableTree,
    'Custom node extends DiagnosticableTree',
    logs,
    counters,
  );
  expectCondition(
    treeA.name == 'alpha',
    'Name property is stored',
    logs,
    counters,
  );
  expectCondition(treeB.value == 2, 'Value property is stored', logs, counters);
  expectCondition(
    treeA.toStringShort().contains('alpha'),
    'toStringShort includes node name',
    logs,
    counters,
  );
  ...
  expectCondition(
    deep.contains('alpha'),
    'Diagnostics deep output includes node data',
    logs,
    counters,
  );
  ...
  expectCondition(
    children.length == 3,
    'Three diagnostic child nodes created',
    logs,
    counters,
  );
  for (final child in children) {
    expectCondition(
      child is DiagnosticableTree,
      'Child is DiagnosticableTree',
      logs,
      counters,
    );
    expectCondition(
      child.toStringShort().contains('child-'),
      'Child short string uses expected prefix',
      logs,
      counters,
    );
  }
  ...
  expectCondition(
    propertyBuilder.properties.length >= 2,
    'debugFillProperties writes multiple fields',
    logs,
    counters,
  );
  expectCondition(
