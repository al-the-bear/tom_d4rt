...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    filter is StackFilter,
    'RepetitiveStackFrameFilter is a StackFilter',
    logs,
    counters,
  );
  expectCondition(
    filter.numFrames == 1,
    'Filter reports one pattern frame',
    logs,
    counters,
  );
  expectCondition(
    filter.replacement.contains('filtered'),
    'Replacement text is stored',
    logs,
    counters,
  );
  ...
  expectCondition(
    reasons.isEmpty,
    'Filtering empty frame list keeps reasons empty',
    logs,
    counters,
  );
  ...
  expectCondition(
    filter2.numFrames == 2,
    'Second filter tracks two frames',
    logs,
    counters,
  );
  expectCondition(
    filter2.replacement.startsWith('['),
    'Second replacement has bracketed marker',
    logs,
    counters,
  );
  ...
  for (final f in filters) {
    expectCondition(
      f.runtimeType.toString().contains('Filter'),
      'Runtime type references filter',
      logs,
      counters,
    );
  }
