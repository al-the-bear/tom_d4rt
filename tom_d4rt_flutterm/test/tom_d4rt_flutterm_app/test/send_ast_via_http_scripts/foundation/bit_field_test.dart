...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'BitField<int> instantiated',
    condition: field is BitField<int>,
  );
  ...
  _check(
    logs: logs,
    label: 'filled constructor works',
    condition: filled is BitField<int>,
  );
  _check(
    logs: logs,
    label: 'filled false keeps bit clear',
    condition: !filled[1],
  );
  ...
  final toggledCount = List<int>.generate(
    8,
    (i) => i,
  ).where((i) => filled[i]).length;
  _check(
    logs: logs,
    label: 'exactly four bits set',
    condition: toggledCount == 4,
  );
  ...
  _check(
    logs: logs,
    label: 'upper edge index is writable',
    condition: field[edgeIndex],
  );
