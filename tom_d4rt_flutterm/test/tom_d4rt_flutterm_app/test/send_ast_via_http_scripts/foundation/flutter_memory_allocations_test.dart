...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'instance is singleton object',
    condition: identical(allocations, FlutterMemoryAllocations.instance),
  );
  ...
  _check(
    logs: logs,
    label: 'listener add call does not throw',
    condition: true,
  );
  ...
  _check(
    logs: logs,
    label: 'listener count non-negative',
    condition: seen >= 0,
  );
  ...
  if (captured != null) {
    ...
    _check(
      logs: logs,
      label: 'captured event serializes',
      condition: map.isNotEmpty,
    );
  } else {
    _check(
      logs: logs,
      label: 'event capture optional based on runtime mode',
      condition: !kFlutterMemoryAllocationsEnabled,
    );
  }
  ...
  _check(
    logs: logs,
    label: 'listener remove call does not throw',
    condition: true,
  );
  ...
  _check(
    logs: logs,
    label: 'ObjectDisposed toMap has eventType',
    condition: disposedMap.values.first['eventType'] == 'disposed',
  );

  final createdMap = event.toMap();
  _check(
    logs: logs,
    label: 'ObjectCreated toMap has library',
    condition:
        createdMap.values.first['libraryName'] ==
        'package:flutter/foundation.dart',
  );

  _check(
    logs: logs,
    label: 'hasListeners is bool',
    condition: allocations.hasListeners is bool,
  );
  ...
  print(
    'FlutterMemoryAllocations comprehensive test finished: pass=$pass fail=$fail',
  );
