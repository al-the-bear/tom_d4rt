...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'StringProperty instantiated',
    condition: base is StringProperty,
  );
  _check(
    logs: logs,
    label: 'base output has name',
    condition: base.toString().contains('title'),
  );
  _check(
    logs: logs,
    label: 'showName false hides key',
    condition: !hiddenName.toString().contains('title:'),
  );
  _check(
    logs: logs,
    label: 'quoted includes quotes',
    condition: quoted.toString().contains('"text"'),
  );
  _check(
    logs: logs,
    label: 'unquoted excludes quotes',
    condition: unquoted.toString().contains('text'),
  );
  _check(
    logs: logs,
    label: 'ifEmpty applied',
    condition: emptyFallback.toString().contains('EMPTY'),
  );
  _check(
    logs: logs,
    label: 'ifNull applied',
    condition: nullFallback.toString().contains('NULL'),
  );

  final level = StringProperty('level', 'ok', level: DiagnosticLevel.info);
  _check(
    logs: logs,
    label: 'level preserved',
    condition: level.level == DiagnosticLevel.info,
  );

  final compare =
      StringProperty('cmp', 'v').toString() ==
      StringProperty('cmp', 'v').toString();
