...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'ObjectCreated is ObjectEvent',
    condition: created is ObjectEvent,
  );
  _check(
    logs: logs,
    label: 'ObjectDisposed is ObjectEvent',
    condition: disposed is ObjectEvent,
  );
  ...
  _check(
    logs: logs,
    label: 'create map has tracked key',
    condition: createMap.containsKey(tracked),
  );
  _check(
    logs: logs,
    label: 'dispose map has tracked key',
    condition: disposeMap.containsKey(tracked),
  );
  ...
  _check(
    logs: logs,
    label: 'create payload eventType created',
    condition: createPayload['eventType'] == 'created',
  );
  _check(
    logs: logs,
    label: 'create payload contains className',
    condition: createPayload['className'] == 'RenderBox',
  );
  _check(
    logs: logs,
    label: 'create payload contains libraryName',
    condition: createPayload['libraryName'] == 'package:flutter/rendering.dart',
  );
  _check(
    logs: logs,
    label: 'dispose payload eventType disposed',
    condition: disposePayload['eventType'] == 'disposed',
  );
  ...
  _check(
    logs: logs,
    label: 'second ObjectCreated also valid',
    condition: another.toMap().isNotEmpty,
  );

  final identityOk =
      identical(created.object, tracked) && identical(disposed.object, tracked);
