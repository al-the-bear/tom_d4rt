...
dynamic build(BuildContext context) {
  ...
  _check(
    logs: logs,
    label: 'CachingIterable instantiated',
    condition: iterable is CachingIterable<int>,
  );
  ...
  _check(
    logs: logs,
    label: 'map propagates values',
    condition: mapped.join(',') == '2,4,6,8,10,12',
  );

  final filtered = iterable.where((e) => e.isEven).toList();
  _check(
    logs: logs,
    label: 'where returns evens',
    condition: filtered.join(',') == '2,4,6',
  );

  final expanded = iterable.expand((e) => [e, -e]).take(4).toList();
  _check(
    logs: logs,
    label: 'expand works with caching',
    condition: expanded.join(',') == '1,-1,2,-2',
  );
  ...
  _check(
    logs: logs,
    label: 'source not iterated excessively',
    condition: sourceMoves == 6,
  );
