...
dynamic build(BuildContext context) {
  ...
  final nullValue = PercentProperty(
    'progressNull',
    null,
  ...
  _check(
    logs: logs,
    label: 'PercentProperty zero created',
    condition: zero is PercentProperty,
  );
  _check(
    logs: logs,
    label: 'PercentProperty half created',
    condition: half is PercentProperty,
  );
  _check(
    logs: logs,
    label: 'PercentProperty full created',
    condition: full is PercentProperty,
  );
  _check(
    logs: logs,
    label: 'name is preserved',
    condition: zero.name == 'progressZero',
  );
  ...
  _check(
    logs: logs,
    label: '0 renders as percent',
    condition: textZero.contains('%'),
  );
  _check(
    logs: logs,
    label: '50 renders as percent',
    condition: textHalf.contains('%'),
  );
  _check(
    logs: logs,
    label: '100 renders as percent',
    condition: textFull.contains('%'),
  );
  _check(
    logs: logs,
    label: 'ifNull renders custom text',
    condition: textNull.contains('indeterminate'),
  );
  _check(
    logs: logs,
    label: 'over-one still renders',
    condition: textOver.isNotEmpty,
  );
  _check(
    logs: logs,
    label: 'under-zero still renders',
    condition: textUnder.isNotEmpty,
  );
  ...
  _check(
    logs: logs,
    label: 'builder collected 4 props',
    condition: builder.properties.length == 4,
  );
