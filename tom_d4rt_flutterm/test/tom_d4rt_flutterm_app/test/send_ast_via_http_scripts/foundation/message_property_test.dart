...
dynamic build(BuildContext context) {
  ...
  final warning = MessageProperty(
    'warning',
    'latency increased',
    level: DiagnosticLevel.warning,
  );
  final error = MessageProperty(
    'error',
    'connection failed',
  ...
  expectCondition(
    info.name == 'status',
    'Name is stored for info property',
    logs,
    counters,
  );
  expectCondition(
    warning.level == DiagnosticLevel.warning,
    'Warning level is preserved',
    logs,
    counters,
  );
  expectCondition(
    error.level == DiagnosticLevel.error,
    'Error level is preserved',
    logs,
    counters,
  );
  expectCondition(
    info.toString().contains('all systems operational'),
    'Info toString includes message',
    logs,
    counters,
  );
  expectCondition(
    warning.toString().contains('latency increased'),
    'Warning toString includes message',
    logs,
    counters,
  );
  expectCondition(
  ...
  expectCondition(
    ...
    'Diagnostics output is produced',
    logs,
    counters,
  );
  ...
  expectCondition(
    nodes.length == 3,
    'Three MessageProperty instances created',
    logs,
    counters,
  );
  for (final node in nodes) {
    expectCondition(
      node.name.startsWith('step-'),
      'Node name has expected prefix',
      logs,
      counters,
    );
    expectCondition(
      node.toString().isNotEmpty,
      'Node has non-empty string output',
      logs,
      counters,
    );
  }
  expectCondition(
    ...
    'defaultValue remains null for MessageProperty',
    logs,
    counters,
  );
  expectCondition(
