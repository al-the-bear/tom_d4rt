...
dynamic build(BuildContext context) {
  ...
  _expectCondition(indexMap.length == values.length, 'Index map covers all TargetPlatform values');
  _expectCondition(names.toSet().length == names.length, 'TargetPlatform names are unique');
  _expectCondition(indexes.toSet().length == indexes.length, 'TargetPlatform indexes are unique');
  _expectCondition(indexes.first == 0, 'First TargetPlatform index is 0');
  _expectCondition(indexes.last == values.length - 1, 'Last TargetPlatform index is values.length - 1');
  ...
  _expectCondition(TargetPlatform.values.byName(first.name) == first, 'byName resolves first TargetPlatform');
  _expectCondition(TargetPlatform.values.byName(last.name) == last, 'byName resolves last TargetPlatform');
  _expectCondition(indexMap[first.index] == first, 'Index map resolves first TargetPlatform');
  _expectCondition(indexMap[last.index] == last, 'Index map resolves last TargetPlatform');
  _expectCondition(first == TargetPlatform.values[first.index], 'First TargetPlatform round-trips from index');
  _expectCondition(last == TargetPlatform.values[last.index], 'Last TargetPlatform round-trips from index');
  ...
  _expectCondition(first == first, 'Reflexive equality for first TargetPlatform');
  ...
  _expectCondition(invalidNameThrows, 'Invalid byName throws for TargetPlatform');

  final sortedByIndex = [...values]..sort((a, b) => a.index.compareTo(b.index));
  _expectCondition(sortedByIndex.join('|') == values.join('|'), 'Sorting by index preserves TargetPlatform order');
