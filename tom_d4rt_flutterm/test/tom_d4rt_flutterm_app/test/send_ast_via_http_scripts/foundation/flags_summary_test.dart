...
dynamic build(BuildContext context) {
  ...
  final summary = FlagsSummary<Object>(
    'callbacks',
    <String, Object?>{'onTap': () {}, 'onLongPress': null, 'onHover': () {}},
    ifEmpty: 'none',
    showName: true,
    showSeparator: true,
  );
  ...
  _check(
    logs: logs,
    label: 'FlagsSummary instantiated',
    condition: summary is FlagsSummary<Object>,
  );
  _check(
    logs: logs,
    label: 'name preserved',
    condition: summary.name == 'callbacks',
  );

  final text = summary.toString();
  _check(
    logs: logs,
    label: 'non-null keys reported',
    condition: text.contains('onTap') && text.contains('onHover'),
  );
  _check(
    logs: logs,
    label: 'null key omitted',
    condition: !text.contains('onLongPress'),
  );

  final empty = FlagsSummary<Object>(
    'empty',
    <String, Object?>{},
    ifEmpty: 'empty-state',
  );
  ...
  _check(
    logs: logs,
    label: 'ifEmpty text reported',
    condition: emptyText.contains('empty-state'),
  );

  final hidden = FlagsSummary<Object>('hidden', <String, Object?>{});
  _check(
    logs: logs,
    label: 'empty without ifEmpty is hidden level',
    condition: hidden.level == DiagnosticLevel.hidden,
  );

  final level = FlagsSummary<Object>('level', <String, Object?>{
    'a': 1,
  }, level: DiagnosticLevel.info);
  _check(
    logs: logs,
    label: 'explicit level accepted',
    condition: level.level == DiagnosticLevel.info,
  );

  final jsonMap = summary.toJsonMap(const DiagnosticsSerializationDelegate());
  _check(
    logs: logs,
    label: 'json export includes values',
    condition: jsonMap.containsKey('values'),
  );
