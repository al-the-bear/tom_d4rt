// D4rt comprehensive test script for RepetitiveStackFrameFilter
...
dynamic build(BuildContext context) {
  const targetClassName = 'RepetitiveStackFrameFilter';
  const testLabel = 'RepetitiveStackFrameFilter';
  final logs = <String>[];
  ...
  const filterA = RepetitiveStackFrameFilter(
    frames: <PartialStackFrame>[
      PartialStackFrame(package: 'package:flutter', className: 'Element', method: 'rebuild'),
    ],
    replacement: '[framework repetition]',
  );
  const filterB = RepetitiveStackFrameFilter(
    frames: <PartialStackFrame>[
  ...
  filterA.filter(frames, reasons);
  expectCondition(filterA is StackFilter, 'filterA is StackFilter', logs, counters);
  expectCondition(filterB.numFrames == 2, 'filterB stores two pattern frames', logs, counters);
  expectCondition(frames.isEmpty && reasons.isEmpty, 'filtering empty frame list keeps lists empty', logs, counters);
  details['replacementA'] = filterA.replacement;
  details['replacementB'] = filterB.replacement;
