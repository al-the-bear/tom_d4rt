...
class SingletonFlutterWindowCaseStudy {
  ...
  List<String> run() {
    ...
    assert(
      ...
      'Device pixel ratio should be positive',
    );
    assert(
      window.physicalSize.width >= 0,
      'Physical width must be non-negative',
    );
    assert(
      window.physicalSize.height >= 0,
      'Physical height must be non-negative',
    );

    logs.add('window-type:${window.runtimeType}');
    logs.add('dpr:${window.devicePixelRatio}');
    logs.add(
      'physical:${window.physicalSize.width}x${window.physicalSize.height}',
    );
...
dynamic build(BuildContext context) {
  ...
  assert(
    logs.any((line) => line.startsWith('window-type:')),
    'Expected window type log',
  );
  assert(
