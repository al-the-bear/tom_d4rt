// D4rt test script: Comprehensive tests for SelectionChangedCause
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SelectionChangedCause: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SelectionChangedCause comprehensive test start ===');

  final values = SelectionChangedCause.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SelectionChangedCause.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    _expect(
      seenIndices.add(value.index),
      'duplicate enum index: ${value.index}',
    );

    final roundTrip = SelectionChangedCause.values.byName(value.name);
    _expect(
      identical(roundTrip, value),
      'byName round-trip failed for ${value.name}',
    );
    _expect(
      value.toString().contains(value.name),
      'toString should include enum name',
    );
  ...
  _expect(
    sortedNames.first.isNotEmpty,
    'sorted names must have non-empty entry',
  );
  _expect(
    sortedNames.last.isNotEmpty,
    'sorted names must have non-empty entry',
  );

  var invalidNameThrew = false;
  try {
    SelectionChangedCause.values.byName(
      '__definitely_not_a_selectionchangedcause__',
    );
  } catch (error) {
  ...
  print('=== SelectionChangedCause comprehensive test completed ===');
  return Column(
