// D4rt test script: Comprehensive tests for SmartDashesType
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SmartDashesType: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SmartDashesType comprehensive test start ===');

  final values = SmartDashesType.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SmartDashesType.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = SmartDashesType.values.byName(value.name);
    _expect(
  ...
  try {
    SmartDashesType.values.byName('__definitely_not_a_smartdashestype__');
  } catch (error) {
  ...
  print('=== SmartDashesType comprehensive test completed ===');
  return Column(
