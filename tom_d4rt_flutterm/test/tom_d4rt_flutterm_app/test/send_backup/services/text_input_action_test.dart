// D4rt test script: Comprehensive tests for TextInputAction
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for TextInputAction: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== TextInputAction comprehensive test start ===');

  final values = TextInputAction.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('TextInputAction.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = TextInputAction.values.byName(value.name);
    _expect(
  ...
  try {
    TextInputAction.values.byName('__definitely_not_a_textinputaction__');
  } catch (error) {
  ...
  print('=== TextInputAction comprehensive test completed ===');
  return Column(
