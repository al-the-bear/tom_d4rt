// D4rt test script: Comprehensive tests for SystemUiOverlay
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SystemUiOverlay: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SystemUiOverlay comprehensive test start ===');

  final values = SystemUiOverlay.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SystemUiOverlay.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = SystemUiOverlay.values.byName(value.name);
    _expect(
  ...
  try {
    SystemUiOverlay.values.byName('__definitely_not_a_systemuioverlay__');
  } catch (error) {
  ...
  print('=== SystemUiOverlay comprehensive test completed ===');
  return Column(
