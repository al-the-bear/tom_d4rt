// D4rt test script: Comprehensive tests for KeyDataTransitMode
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for KeyDataTransitMode: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== KeyDataTransitMode comprehensive test start ===');

  final values = KeyDataTransitMode.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('KeyDataTransitMode.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = KeyDataTransitMode.values.byName(value.name);
    _expect(
  ...
  try {
    KeyDataTransitMode.values.byName('__definitely_not_a_keydatatransitmode__');
  } catch (error) {
  ...
  print('=== KeyDataTransitMode comprehensive test completed ===');
  return Column(
