// D4rt test script: Comprehensive tests for SmartQuotesType
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for SmartQuotesType: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== SmartQuotesType comprehensive test start ===');

  final values = SmartQuotesType.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('SmartQuotesType.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = SmartQuotesType.values.byName(value.name);
    _expect(
  ...
  try {
    SmartQuotesType.values.byName('__definitely_not_a_smartquotestype__');
  } catch (error) {
  ...
  print('=== SmartQuotesType comprehensive test completed ===');
  return Column(
