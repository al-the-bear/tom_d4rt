...
dynamic build(BuildContext context) {
  ...
  _expect(tooltip.message == 'Hello Tooltip', 'stores tooltip message', logs);
  assertionCount++;
  _expect(
    tooltip.waitDuration == const Duration(milliseconds: 120),
    'stores wait duration',
    logs,
  );
  assertionCount++;
  _expect(
    tooltip.showDuration == const Duration(milliseconds: 900),
    'stores show duration',
    logs,
  );
  assertionCount++;
  _expect(tooltip.preferBelow == false, 'stores placement preference', logs);
  assertionCount++;
  _expect(
  ...
  _expect(
    defaultTooltip.message == 'Default',
    'supports minimal constructor',
    logs,
  );
  assertionCount++;

  final asText = tooltip.toStringShort();
  _expect(asText.isNotEmpty, 'toStringShort available', logs);
  assertionCount++;
