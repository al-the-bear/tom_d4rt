// D4rt comprehensive test script: SelectableRegionSelectionStatus from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('SelectableRegionSelectionStatus test failed: $message');
  }
  print('✅ $message');
}
...
List<String> _collectDiagnostics(List<SelectableRegionSelectionStatus> values) {
  final names = values.map((v) => v.name).toList(growable: false);
  final indices = values.map((v) => v.index).toList(growable: false);
  return <String>[
    _line('enum', 'SelectableRegionSelectionStatus'),
...
dynamic build(BuildContext context) {
  print(
    '========== SelectableRegionSelectionStatus comprehensive test start ==========',
  );

  final List<SelectableRegionSelectionStatus> values =
      SelectableRegionSelectionStatus.values.toList(growable: false);
  _expect(
    values.isNotEmpty,
    'SelectableRegionSelectionStatus exposes at least one enum value',
  );
  _expect(
    values.length == SelectableRegionSelectionStatus.values.length,
    'SelectableRegionSelectionStatus.values list length is consistent',
  );

  final SelectableRegionSelectionStatus first = values.first;
  final SelectableRegionSelectionStatus last = values.last;

  print(_line('runtimeType', SelectableRegionSelectionStatus));
  ...
  _expect(
    uniqueIndices.length == indices.length,
    'all enum indices are unique',
  );
  _expect(indices.first == 0, 'first enum index starts at zero');

  for (final value in values) {
    print(
      'Inspecting value => name=${value.name}, index=${value.index}, raw=$value',
    );
    _expect(
      SelectableRegionSelectionStatus.values[value.index] == value,
      'index lookup round-trip works for $value',
    );
    _expect(
      value.toString().contains('.'),
      'toString has qualified enum format for $value',
    );
  }

  for (final name in names) {
    final parsed = SelectableRegionSelectionStatus.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    SelectableRegionSelectionStatus.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<SelectableRegionSelectionStatus> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<SelectableRegionSelectionStatus> sorted = [...values]
    ..sort((a, b) => a.index.compareTo(b.index));
  _expect(sorted.length == values.length, 'sorted list preserves all entries');
  for (var i = 0; i < sorted.length; i++) {
    _expect(
      sorted[i] == values[i],
      'sorted order matches declaration order at index $i',
    );
  }

  final Map<String, SelectableRegionSelectionStatus> asMap = {
  ...
  final String summary =
      'SelectableRegionSelectionStatus checks passed: ${values.length} values validated';
  print(summary);
  print(
    '========== SelectableRegionSelectionStatus comprehensive test end ==========',
  );
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('SelectableRegionSelectionStatus Comprehensive Tests'),
