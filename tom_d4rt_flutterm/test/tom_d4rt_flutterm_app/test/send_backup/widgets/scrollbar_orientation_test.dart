// D4rt comprehensive test script: ScrollbarOrientation from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('ScrollbarOrientation test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== ScrollbarOrientation comprehensive test start ==========');

  final List<ScrollbarOrientation> values = ScrollbarOrientation.values.toList(
    growable: false,
  );
  _expect(
    values.isNotEmpty,
    'ScrollbarOrientation exposes at least one enum value',
  );
  _expect(
    values.length == ScrollbarOrientation.values.length,
    'ScrollbarOrientation.values list length is consistent',
  );

  final ScrollbarOrientation first = values.first;
  final ScrollbarOrientation last = values.last;

  print(_line('runtimeType', ScrollbarOrientation));
  ...
  for (final value in values) {
    ...
    _expect(
      ScrollbarOrientation.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = ScrollbarOrientation.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    ScrollbarOrientation.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<ScrollbarOrientation> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<ScrollbarOrientation> sorted = [...values]
  ...
  final Map<String, ScrollbarOrientation> asMap = {
  ...
  final String summary =
      'ScrollbarOrientation checks passed: ${values.length} values validated';
  print(summary);
  print('========== ScrollbarOrientation comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('ScrollbarOrientation Comprehensive Tests'),
