...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    values.isNotEmpty,
    'FocusHighlightStrategy.values should not be empty',
    logs,
    counters,
  );
  expectCondition(
    values.length >= 2,
    'FocusHighlightStrategy should provide multiple values',
    logs,
    counters,
  );
  ...
  for (final FocusHighlightStrategy value in values) {
    ...
    final FocusHighlightStrategy byName = FocusHighlightStrategy.values.byName(
      value.name,
    );
    expectCondition(
      byName == value,
  ...
  final List<FocusHighlightStrategy> copied = List<FocusHighlightStrategy>.from(
    values,
  );
  expectCondition(
  ...
  final List<FocusHighlightStrategy> reversed = values.reversed.toList(
    growable: false,
  );
  expectCondition(
    reversed.first == last,
  ...
  final FocusHighlightStrategy edgeByName = FocusHighlightStrategy.values
      .byName(edgeName);
  expectCondition(
