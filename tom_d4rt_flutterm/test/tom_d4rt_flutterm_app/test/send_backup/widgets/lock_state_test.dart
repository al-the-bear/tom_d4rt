// D4rt comprehensive test script: LockState from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('LockState test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== LockState comprehensive test start ==========');

  final List<LockState> values = LockState.values.toList(growable: false);
  _expect(values.isNotEmpty, 'LockState exposes at least one enum value');
  _expect(
    values.length == LockState.values.length,
    'LockState.values list length is consistent',
  );

  final LockState first = values.first;
  final LockState last = values.last;

  print(_line('runtimeType', LockState));
  ...
  for (final value in values) {
    ...
    _expect(
      LockState.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = LockState.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    LockState.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<LockState> reversed = values.reversed.toList(growable: false);
  ...
  final List<LockState> sorted = [...values]
  ...
  final Map<String, LockState> asMap = {
  ...
  final String summary =
      'LockState checks passed: ${values.length} values validated';
  print(summary);
  print('========== LockState comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('LockState Comprehensive Tests'),
