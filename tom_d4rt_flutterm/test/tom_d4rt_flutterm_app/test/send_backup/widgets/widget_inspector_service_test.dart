...
dynamic build(BuildContext context) {
  ...
  expectCondition(
    ...
    'WidgetInspectorService.instance is stable singleton',
    logs,
    counters,
  );
  expectCondition(
    serviceA.runtimeType.toString().isNotEmpty,
    'Service runtime type is available',
    logs,
    counters,
  );
  ...
  expectCondition(
    ...
    'Service can generate an id for an object',
    logs,
    counters,
  );
  final resolved = serviceA.toObject(id, objectGroup);
  expectCondition(
    identical(resolved, sampleText),
    'Generated id resolves back to object',
    logs,
    counters,
  );
  ...
  expectCondition(
    missing == null,
    'Missing id resolves to null',
    logs,
    counters,
  );
  var callbackHit = 0;
  serviceA.selectionChangedCallback = () {
    callbackHit += 1;
  };
  expectCondition(
    serviceA.selectionChangedCallback != null,
    'Selection callback can be assigned',
    logs,
    counters,
  );
  ...
  expectCondition(
    serviceA.selectionChangedCallback == null,
    'Selection callback can be cleared',
    logs,
    counters,
  );
  for (var i = 0; i < 6; i++) {
