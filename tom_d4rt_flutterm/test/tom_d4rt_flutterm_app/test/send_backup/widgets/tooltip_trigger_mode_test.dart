// D4rt test script: Comprehensive tests for TooltipTriggerMode
...
void _expect(bool condition, String message) {
  if (!condition) {
    throw StateError('Assertion failed for TooltipTriggerMode: $message');
  }
}
...
dynamic build(BuildContext context) {
  print('=== TooltipTriggerMode comprehensive test start ===');

  final values = TooltipTriggerMode.values;
  _expect(values.isNotEmpty, 'values must not be empty');
  print('TooltipTriggerMode.values.length = ${values.length}');
  ...
  for (final value in values) {
    ...
    final roundTrip = TooltipTriggerMode.values.byName(value.name);
    _expect(
  ...
  try {
    TooltipTriggerMode.values.byName('__definitely_not_a_tooltiptriggermode__');
  } catch (error) {
  ...
  print('=== TooltipTriggerMode comprehensive test completed ===');
  return Column(
