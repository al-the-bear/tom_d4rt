// D4rt comprehensive test script: RouteInformationReportingType from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('RouteInformationReportingType test failed: $message');
  }
  print('✅ $message');
}
...
List<String> _collectDiagnostics(List<RouteInformationReportingType> values) {
  final names = values.map((v) => v.name).toList(growable: false);
  final indices = values.map((v) => v.index).toList(growable: false);
  return <String>[
    _line('enum', 'RouteInformationReportingType'),
...
dynamic build(BuildContext context) {
  print(
    '========== RouteInformationReportingType comprehensive test start ==========',
  );

  final List<RouteInformationReportingType> values =
      RouteInformationReportingType.values.toList(growable: false);
  _expect(
    values.isNotEmpty,
    'RouteInformationReportingType exposes at least one enum value',
  );
  _expect(
    values.length == RouteInformationReportingType.values.length,
    'RouteInformationReportingType.values list length is consistent',
  );

  final RouteInformationReportingType first = values.first;
  final RouteInformationReportingType last = values.last;

  print(_line('runtimeType', RouteInformationReportingType));
  ...
  for (final value in values) {
    ...
    _expect(
      RouteInformationReportingType.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = RouteInformationReportingType.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    RouteInformationReportingType.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<RouteInformationReportingType> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<RouteInformationReportingType> sorted = [...values]
  ...
  final Map<String, RouteInformationReportingType> asMap = {
  ...
  final String summary =
      'RouteInformationReportingType checks passed: ${values.length} values validated';
  print(summary);
  print(
    '========== RouteInformationReportingType comprehensive test end ==========',
  );
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('RouteInformationReportingType Comprehensive Tests'),
