// D4rt comprehensive test script: PanAxis from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('PanAxis test failed: $message');
  }
  print('✅ $message');
}
...
List<String> _collectDiagnostics(List<PanAxis> values) {
...
dynamic build(BuildContext context) {
  print('========== PanAxis comprehensive test start ==========');

  final List<PanAxis> values = PanAxis.values.toList(growable: false);
  _expect(values.isNotEmpty, 'PanAxis exposes at least one enum value');
  _expect(
    values.length == PanAxis.values.length,
    'PanAxis.values list length is consistent',
  );

  final PanAxis first = values.first;
  final PanAxis last = values.last;

  print(_line('runtimeType', PanAxis));
  ...
  for (final value in values) {
    ...
    _expect(
      PanAxis.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = PanAxis.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    PanAxis.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<PanAxis> reversed = values.reversed.toList(growable: false);
  ...
  final List<PanAxis> sorted = [...values]
  ...
  final Map<String, PanAxis> asMap = {
  ...
  final String summary =
      'PanAxis checks passed: ${values.length} values validated';
  print(summary);
  print('========== PanAxis comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('PanAxis Comprehensive Tests'),
