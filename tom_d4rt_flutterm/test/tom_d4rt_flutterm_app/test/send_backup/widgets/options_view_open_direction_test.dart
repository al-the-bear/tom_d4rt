// D4rt comprehensive test script: OptionsViewOpenDirection from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('OptionsViewOpenDirection test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print(
    '========== OptionsViewOpenDirection comprehensive test start ==========',
  );

  final List<OptionsViewOpenDirection> values = OptionsViewOpenDirection.values
      .toList(growable: false);
  _expect(
    values.isNotEmpty,
    'OptionsViewOpenDirection exposes at least one enum value',
  );
  _expect(
    values.length == OptionsViewOpenDirection.values.length,
    'OptionsViewOpenDirection.values list length is consistent',
  );

  final OptionsViewOpenDirection first = values.first;
  final OptionsViewOpenDirection last = values.last;

  print(_line('runtimeType', OptionsViewOpenDirection));
  ...
  for (final value in values) {
    ...
    _expect(
      OptionsViewOpenDirection.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = OptionsViewOpenDirection.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    OptionsViewOpenDirection.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<OptionsViewOpenDirection> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<OptionsViewOpenDirection> sorted = [...values]
  ...
  final Map<String, OptionsViewOpenDirection> asMap = {
  ...
  final String summary =
      'OptionsViewOpenDirection checks passed: ${values.length} values validated';
  print(summary);
  print(
    '========== OptionsViewOpenDirection comprehensive test end ==========',
  );
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('OptionsViewOpenDirection Comprehensive Tests'),
