// D4rt comprehensive test script: PlatformProvidedMenuItemType from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('PlatformProvidedMenuItemType test failed: $message');
  }
  print('✅ $message');
}
...
List<String> _collectDiagnostics(List<PlatformProvidedMenuItemType> values) {
  final names = values.map((v) => v.name).toList(growable: false);
  final indices = values.map((v) => v.index).toList(growable: false);
  return <String>[
    _line('enum', 'PlatformProvidedMenuItemType'),
...
dynamic build(BuildContext context) {
  print(
    '========== PlatformProvidedMenuItemType comprehensive test start ==========',
  );

  final List<PlatformProvidedMenuItemType> values = PlatformProvidedMenuItemType
      .values
      .toList(growable: false);
  _expect(
    values.isNotEmpty,
    'PlatformProvidedMenuItemType exposes at least one enum value',
  );
  _expect(
    values.length == PlatformProvidedMenuItemType.values.length,
    'PlatformProvidedMenuItemType.values list length is consistent',
  );

  final PlatformProvidedMenuItemType first = values.first;
  final PlatformProvidedMenuItemType last = values.last;

  print(_line('runtimeType', PlatformProvidedMenuItemType));
  ...
  for (final value in values) {
    ...
    _expect(
      PlatformProvidedMenuItemType.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = PlatformProvidedMenuItemType.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    PlatformProvidedMenuItemType.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<PlatformProvidedMenuItemType> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<PlatformProvidedMenuItemType> sorted = [...values]
  ...
  final Map<String, PlatformProvidedMenuItemType> asMap = {
  ...
  final String summary =
      'PlatformProvidedMenuItemType checks passed: ${values.length} values validated';
  print(summary);
  print(
    '========== PlatformProvidedMenuItemType comprehensive test end ==========',
  );
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('PlatformProvidedMenuItemType Comprehensive Tests'),
