// D4rt comprehensive test script: RoutePopDisposition from widgets
...
void _expect(bool condition, String message) {
  if (!condition) {
    print('❌ $message');
    throw StateError('RoutePopDisposition test failed: $message');
  }
  print('✅ $message');
}
...
dynamic build(BuildContext context) {
  print('========== RoutePopDisposition comprehensive test start ==========');

  final List<RoutePopDisposition> values = RoutePopDisposition.values.toList(
    growable: false,
  );
  _expect(
    values.isNotEmpty,
    'RoutePopDisposition exposes at least one enum value',
  );
  _expect(
    values.length == RoutePopDisposition.values.length,
    'RoutePopDisposition.values list length is consistent',
  );

  final RoutePopDisposition first = values.first;
  final RoutePopDisposition last = values.last;

  print(_line('runtimeType', RoutePopDisposition));
  ...
  for (final value in values) {
    ...
    _expect(
      RoutePopDisposition.values[value.index] == value,
  ...
  for (final name in names) {
    final parsed = RoutePopDisposition.values.byName(name);
    _expect(parsed.name == name, 'byName resolves exact name "$name"');
  }

  bool byNameThrows = false;
  try {
    RoutePopDisposition.values.byName('__invalid_enum_name__');
  } catch (error) {
    byNameThrows = true;
  ...
  final List<RoutePopDisposition> reversed = values.reversed.toList(
    growable: false,
  );
  ...
  final List<RoutePopDisposition> sorted = [...values]
  ...
  final Map<String, RoutePopDisposition> asMap = {
  ...
  final String summary =
      'RoutePopDisposition checks passed: ${values.length} values validated';
  print(summary);
  print('========== RoutePopDisposition comprehensive test end ==========');
  ...
  return Directionality(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Text('RoutePopDisposition Comprehensive Tests'),
