    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.6/E24 (ast) + §2.D contention
          // (test): TimeoutException 30s — cold-start contention.
          // 715-line / 22 KB script builds in ~1.3 s. Same family
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.6/E28 (ast) + §2.D contention
          // (test): TimeoutException 30s — cold-start contention.
          // 1803-line / 57 KB script builds in ~1.8 s. Same family
          // as the §1.3/§1.4/§1.5 E-series: 50 s leaves 10 s of
          // headroom under the 60 s dart-test wrapper. Applied
          // symmetrically with the ast variant.
          httpBuildTimeout: const Duration(seconds: 50),
