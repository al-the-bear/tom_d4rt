...
void main() {
  ...
  group('Section 2 - Bridge Generator Issues (80)', () {
    ...
    // 1944 TODO C.133 (2026-06-01): historical 20260523-1056 §S/E1
    // cold-start-contention wrapper REMOVED. TEST sibling of C.128
    // (AST). Same inline `httpBuildTimeout: 50s` + outer `Timeout:
    // 60s` shape as the AST sibling. Script runs in ~2.6 s under
    // isolated retest (httpMs=2397, totalMs=2623, frameworkErrors=0,
    // sourceChars=60302 — 60 KB / 1521-line). First pre-fix retest
    // hit U31; retry #1 PASSED clean — standard U31 retry protocol.
    // Defaults apply, matching the AST sibling C.128.
    ...
    // 1944 TODO C.134 (2026-06-01): historical 20260524-2003 §6/T4
    // cold-start-contention wrapper REMOVED. TEST sibling of C.129
    // (AST). Same inline `httpBuildTimeout: 50s` + outer `Timeout:
    // 60s` shape as the AST sibling C.129 and the C.133 sibling.
    // Script runs in ~2.6 s under isolated retest (httpMs=2425,
    // totalMs=2638, frameworkErrors=0, sourceChars=71483 — 71 KB
    // matching the AST sibling C.129 baseline). Defaults apply.
