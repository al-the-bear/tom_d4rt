...
void main() {
  ...
  group('material/', () {
    ...
    test('dynamic_scheme_variant_test.dart', () async {
      // 1944 TODO C.70 (2026-05-31): historical 20260523-1056 §1.5/E18
      // (ast) + §2.D contention (test) cold-start-contention wrapper
      // REMOVED. Script runs in ~4.1 s under normal load
      // (httpMs=4144, sourceChars=58318 — 58 KB / 1697-line script).
      // Slower than typical but still ~26 s of headroom under 30 s.
      final result = await SendTestRunner.send(
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.5/E22 (ast) + §2.D contention
          // (test): Transport failure 25s — cold-start contention.
          // 1380-line / 48 KB script builds in ~1.5 s. Same family
          // as the §1.3/§1.4 E-series: 50 s leaves 10 s of headroom
          // under the 60 s dart-test wrapper. Applied symmetrically
          // with the ast variant.
          httpBuildTimeout: const Duration(seconds: 50),
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.5/E23 (ast) + §2.D contention
          // (test): TimeoutException 30s — cold-start contention.
          // 1206-line / 36 KB script builds in ~1.5 s. Same family
          // as the §1.3/§1.4 E-series: 50 s leaves 10 s of headroom
          // under the 60 s dart-test wrapper. Applied symmetrically
          // with the ast variant.
          httpBuildTimeout: const Duration(seconds: 50),
