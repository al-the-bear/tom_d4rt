...
void main() {
  ...
  group('dart_ui/', () {
    ...
    // isolation. Bisect on the ast variant reproduced 3× passing runs
    // (totalMs 2266 / 2461 / 2426, frameworkErrors=0). The original D1
    // "wedge" diagnosis from the 20260427 Linux baseline was a
    // misattribution of U25-family cold-start cascade contention to a
    // permanent test-app destabilisation. Lifted the skip and replaced
    // with the standard `httpBuildTimeout: 50 s` caller-side cap; the
    // ast variant's full hardly_1 suite run verified no cascade
    // (+204 ~1 -0). Applied symmetrically to flutter_test.
    test(
    ...
    test(
      'uniform_vec2_slot_test.dart',
      () async {
        final result = await SendTestRunner.send(
          'dart_ui/uniform_vec2_slot_test.dart',
          // 20260523-1056 baseline §1.4/E13 (ast) + §2.D contention
          // (test): TimeoutException 30s — cold-start contention.
          // 2156-line / 68 KB script builds in ~1.7 s. Same family
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.4/E15 (ast) + §2.D contention
          // (test): TimeoutException 30s — cold-start contention.
          // 2326-line / 72 KB script builds in ~1.8 s. Same family
          // as the §1.3 E-series: 50 s leaves 10 s of headroom
          // under the 60 s dart-test wrapper. Applied symmetrically
          // with the ast variant.
          httpBuildTimeout: const Duration(seconds: 50),
