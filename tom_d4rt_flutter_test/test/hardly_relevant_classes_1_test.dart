...
void main() {
  ...
  group('dart_ui/', () {
    ...
    test('class_test.dart', () async {
      // 1944 TODO C.56 (2026-05-31): historical 20260523-1056 §1.4/E11
      // (ast) + §2.D contention (test) cold-start-contention wrapper
      // REMOVED. Script runs in ~2.2 s under normal load
      // (httpMs=2193, sourceChars=108674 — 109 KB / 3275-line script).
      final result = await SendTestRunner.send(
    ...
    test('image_sampler_slot_test.dart', () async {
      // 1944 TODO C.57 (2026-05-31): historical 20260525 §6.1/D1
      // "wedge" misattribution wrapper REMOVED. Script runs in
      // ~2.2 s under normal load (httpMs=2162, sourceChars=28842).
      final result = await SendTestRunner.send(
    ...
    test('uniform_vec2_slot_test.dart', () async {
      // 1944 TODO C.59 (2026-05-31): historical 20260523-1056 §1.4/E13
      // (ast) + §2.D contention (test) cold-start-contention wrapper
      // REMOVED. Script runs in ~1.7 s under normal load
      // (httpMs=1735, sourceChars=68472 — 68 KB / 2156-line script).
      final result = await SendTestRunner.send(
  ...
  group('foundation/', () {
    ...
    test('diagnostics_serialization_delegate_test.dart', () async {
      // 1944 TODO C.60 (2026-05-31): historical 20260523-1056 §1.4/E14
      // (ast) + §2.D contention (test) cold-start-contention wrapper
      // REMOVED. Script runs in ~2.2 s under normal load
      // (httpMs=2243, sourceChars=69817 — 70 KB / 2260-line script).
      final result = await SendTestRunner.send(
    ...
    test('object_event_test.dart', () async {
      // 1944 TODO C.61 (2026-05-31): historical 20260523-1056 §1.4/E15
      // (ast) + §2.D contention (test) cold-start-contention wrapper
      // REMOVED. Script runs in ~1.8 s under normal load
      // (httpMs=1816, sourceChars=72291 — 72 KB / 2326-line script).
      final result = await SendTestRunner.send(
  ...
  group('gestures/', () {
    ...
    test('least_squares_solver_test.dart', () async {
      // 1944 TODO C.62 (2026-05-31): historical 20260518-1449 Step 9
      // full-suite-contention wrapper REMOVED. Script runs in ~4.5 s
      // under normal load (httpMs=4547, sourceChars=81488 — 81 KB /
      // 2337-line script). Slower than the average §C.iv entry but
      // still ~25 s of headroom under 30 s.
      final result = await SendTestRunner.send(
