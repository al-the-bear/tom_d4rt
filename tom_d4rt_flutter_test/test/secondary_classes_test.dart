    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E2 (ast) + §2.C contention
          // (test): under parallel-driver contention the /build for
          // this 1399-line, 52 KB script can exceed the default 25 s
          // HTTP cap on the first request after the test app cold-
          // start (measured 25001 ms cold vs 1661 ms warm — a 15×
          // gap). Serial isolated re-runs complete in ~1.6 s. 50 s
          // leaves 10 s of headroom under the 60 s dart-test wrapper.
          // Applied symmetrically with the ast variant to prevent
          // future contention-induced failures on either project.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
      },
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E7 (ast) + §2.C contention
          // (test): TimeoutException after 30s — cold-start
          // contention. This 1716-line / 54 KB script takes ~1.7 s
          // warm in both variants. Same family as E1/E2/E4/E6: 50 s
          // leaves 10 s of headroom under the 60 s dart-test wrapper.
          // Applied symmetrically with the ast variant.
          httpBuildTimeout: const Duration(seconds: 50),
        );
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E8 (ast) + §2.C contention
          // (test): TimeoutException after 30s — cold-start
          // contention. 1456-line / 54 KB script takes ~1.3 s in this
          // variant. Same family as E1/E2/E4/E6/E7: 50 s leaves 10 s
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E9 (ast) + §2.C contention
          // (test): TimeoutException after 30s — cold-start
          // contention. 1096-line / 40 KB script takes ~1.6 s.
          // Same family as E1/E2/E4/E6/E7/E8: 50 s leaves 10 s of
