...
- [x] **fixed** 22. `widgets/cliprrect_test.dart` (2 events) — layout audit. **Done — current state is `frameworkErrors=0` on both `tom_d4rt_flutter_ast` and `tom_d4rt_flutter_test`; the original 2 events were captured on Linux at 1920×1080 (`linux/runner/my_application.cc` default size) and cannot be reproduced on the current macOS 800×600 viewport.** Two `Row(mainAxisAlignment: spaceEvenly)` patterns with fixed-size children were the most likely sources: Section 8's custom-clipper comparison (`Row` with two 220×110 `_WaveClipper` cards) and Section 10's `ClipRRect`/`ClipOval`/`ClipPath` family comparison (`Row` with three 120×120 `syntheticImage` Columns). On a narrow Linux render path these could overflow horizontally; the `Row` would not reflow because all children are fixed-size. **Fix:** converted both `Row(mainAxisAlignment: spaceEvenly)` calls to `Wrap(alignment: WrapAlignment.spaceEvenly, spacing: …, runSpacing: …)` — on the macOS 800-px viewport the items still display side-by-side (identical visual); on narrower viewports they reflow to the next line instead of asserting. Test-script-only change → rule (a), individual retest only. Pre-fix: `frameworkErrors=0` on macOS (Linux baseline reported 2 events but cannot be reproduced); post-fix: `frameworkErrors=0` on both projects (verified on `tom_d4rt_flutter_ast` essential_classes and `tom_d4rt_flutter_test` essential_classes; one transient cold-start timeout on the source-based variant cleared on retry).
