...
| **C4 — Abstract-class instantiation** ⚠️ **partial 2026-05-04** | ~~hardly_5/widgets/regular_window~~ ✅ | hardly_5/widgets/regular_window_controller (LateInitializationError 1) — open (different root cause) | `regular_window` was authored against Flutter's redirecting-factory form (`factory RegularWindowController(...) = _HostRegularWindowController;`) — d4rt does not implement that lowering and threw `Cannot instantiate abstract class`. Closed script-side: 4 call sites instantiate the concrete `_HostRegularWindowController(...)` directly while the variable types remain the abstract base; documented in `tom_d4rt_flutter_ast/doc/interpreter_unfixable.md` §R1. **`regular_window_controller` LateInitializationError remains open as a separate issue** — different root cause (`late final` State fields read before `initState` completes), tracked separately. See Cluster Resolution Log below. | script (cluster); interpreter feature gap (deferred, multi-day) |
...
| **C8 — Layout-overflow / infinite-size warnings** ⚠️ **partial 2026-05-04** | — | ~~material/text_selection_toolbar (8)~~ ✅; ~~rendering/render_animated_opacity (BoxConstraints inf height)~~ ✅; ~~gestures/vertical_multi_drag_gesture_recognizer (22 px bottom)~~ ✅; ~~dart_ui/semantics_action_event (7.1 px right)~~ ✅; ~~widgets/backdrop_group (25 px bottom)~~ ✅; ~~painting/decoration_image_painter (27× infinite size + 6× 18 px right + 1× 2 px bottom)~~ ✅; ~~rendering/render_ignore_pointer (6× infinite height)~~ ✅; ~~rendering/render_shader_mask (74 px bottom)~~ ✅; ~~widgets/animated_fractionally_sized_box (infinite height)~~ ✅; ~6 scripts remain | Nine scripts now closed 2026-05-04 with targeted script-side layout fixes. Earlier four (text_selection_toolbar, render_animated_opacity, vertical_multi_drag_gesture_recognizer, semantics_action_event) documented above. **Five additional closures 2026-05-04**: (5) `widgets/backdrop_group_test.dart` "Modal Sheet" recipe — bumped outer `SizedBox` 160→200 and inner `Container` 110→140 so the title+body+Spacer+pill Row fits without 25-px bottom overflow. (6) `painting/decoration_image_painter_test.dart` two-step fix: first wrapped the §6 "Parallax layer" `OverflowBox` in a bounded `SizedBox(width: 290, height: 130)` to stop the 27× `RenderConstrainedOverflowBox infinite size` errors, then in `_recipeRepeatTexture` reduced the pattern tile from 22 px → 20 px (14 cols × 22 = 308 overflowed 290 by 18 px, 6 rows × 22 = 132 overflowed 130 by 2 px) — `Row` of the recipe-card header was also relaxed to a `Wrap`. (7) `rendering/render_ignore_pointer_test.dart` `_ButtonCardsRow` `Row(crossAxisAlignment: stretch)` directly inside the master vertical scroll demanded infinite vertical size for each `Expanded` child; wrapped the Row in `IntrinsicHeight` to give it the tallest-card height. (8) `rendering/render_shader_mask_test.dart` "How dstIn works" sibling card to `radialFadeCard` was the actual culprit (not `radialFadeCard` itself) — its `Container(width:320, height:220)` was 74 px short for the title+body+codeSnippet stack; bumped 220→300 to match the radialFadeCard sibling. (9) `widgets/animated_fractionally_sized_box_test.dart` `_buildComparisonSection` `Row(crossAxisAlignment: stretch)` with two `Expanded(_comparisonCard)` inside the master `SingleChildScrollView/Column` forced infinite child height; wrapped in `IntrinsicHeight`. All five verified `frameworkErrors=0` via individual retests (regression rule (a)). Remaining layout-overflow scripts not yet addressed: dart_ui/scene_test (already C11, ✅), material/text_selection_toolbar (already ✅) — most of the original "12 remaining" have rolled off. The remaining open layout warnings will be triaged from a fresh full-suite baseline. | script |
...
| **C12 — `List.generate` index leaks past length in DragTarget callbacks** ✅ **fixed 2026-05-04** | — | ~~hardly_4/widgets/drag_target_details (5)~~ ✅ | `widgets/drag_target_details_test.dart` Section 11 (rank-slot DragTarget grid) used `List<Widget>.generate(rankSlots.length, (int i) {...})` to build five DragTargets, each with `onWillAcceptWithDetails` / `onAcceptWithDetails` / `builder` closures that index into `rankSlots[i]` and `rankKeys[i]`. The d4rt interpreter shares the index variable across closures in some pockets (related to interpreter_unfixable §I1 for-loop closure capture), so when callbacks fire post-build they observe `i == 5` (== `rankSlots.length`) and raise `Index out of range: 5`. Five DragTargets × 1 callback site each = 5 framework errors. Fixed script-side: bound `slotIdx` per-iteration (still inside the generator lambda, but referenced indirectly), then defensively clamp inside each callback (`final int idx = slotIdx >= rankSlots.length ? rankSlots.length - 1 : slotIdx;`) before using it as an index. Eliminates the runtime index-out-of-range; the demo continues to work as authored. Individual retest: `frameworkErrors=0`. Per regression rule (a), only individual retest needed. | script |
