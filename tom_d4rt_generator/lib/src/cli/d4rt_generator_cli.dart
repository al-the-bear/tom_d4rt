...
Future<void> _generateDartscriptFile(String dartscriptPath, BridgeConfig config,
  ...
  for (final module in config.modules) {
    ...
    final registrationImport = module.barrelImport ??
        'package:${config.name}/${config.name}.dart';
    buffer.writeln(
        '    ${module.name}_bridges.${capitalizedName}Bridge.registerBridges(');
    buffer.writeln('      d4rt,');
  ...
  for (final module in config.modules) {
    final capitalizedName = toPascalCase(module.name);
    buffer.writeln(
        '      ...${module.name}_bridges.${capitalizedName}Bridge.bridgeClasses(),');
  }
