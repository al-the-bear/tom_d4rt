...
import 'gen120_part_declared_extension_test.dart'
    show writePartExtensionFixture;
...
void main() {
  ...
  group('scd11: directory mode filters and dedupes extensions', () {
    test(
      ...
      () async {
        ...
        expect(
          ...
          hasLength(1),
          reason:
              'Emitted into two files, each with a complete definition, '
        ...
        expect(
          ...
          reason:
              'the surviving copy must be the parent library: a part file '
    ...
    test('G-SCD11-2: the extension is registered once within that file '
      ...
      expect(
        ...
        1,
        reason:
            'a duplicate element in a List literal is legal Dart, so '
      ...
      expect(
        ...
        1,
        reason:
            'a duplicate map key is equal_keys_in_map, and Dart keeps '
            'the last entry — deciding the source URI by source order',
      );
    });

    test('G-SCD11-3: excludeSourcePatterns applies to extensions in directory '
        'mode [2026-09-12] (PASS)', () async {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'the exclusion filter sat in the single-file branch only, so '
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          contains("'ZomLevel'"),
          reason:
              'parent_lib.dart declares enum ZomLevel; emitting the '
              'extension into a separate group would drop it',
        );
      },
    );

    test('G-SCD11-6: single-file mode honours the exclusion for the same '
        'part-declared extension [2026-09-12] (PASS)', () async {
