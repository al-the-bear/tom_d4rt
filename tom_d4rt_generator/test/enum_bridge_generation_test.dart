...
void main() {
  ...
  setUpAll(() {
    ...
    tempOutputDir = Directory.systemTemp
        .createTempSync('bridge_gen_test_')
        .path;
  });
  ...
  group('Enum Bridge Generation', () {
    ...
    test(
      'G-ENM-13: Generates bridgedEnums method. [2026-02-10 06:37] (PASS)',
      () {
        expect(
          generatedCode,
          contains('static List<BridgedEnumDefinition> bridgedEnums()'),
        );
      },
    );

    test(
      'G-ENM-14: Generates BridgedEnumDefinition with correct type parameter. [2026-02-10 06:37] (PASS)',
      () {
        // Types are prefixed with $<pkgname>_<N> since source imports use direct import aliases
        expect(
          generatedCode,
          contains(r'BridgedEnumDefinition<$test_package_1.SimpleStatus>'),
        );
        expect(
          generatedCode,
          contains(r'BridgedEnumDefinition<$test_package_1.Color>'),
        );
        expect(
          generatedCode,
          contains(r'BridgedEnumDefinition<$test_package_1.Priority>'),
        );
      },
    );

    test(
      'G-ENM-15: Generates enum values reference. [2026-02-10 06:37] (PASS)',
      () {
        // Types are prefixed with $<pkgname>_<N> since source imports use direct import aliases
        expect(
          generatedCode,
          contains(r'values: $test_package_1.SimpleStatus.values'),
        );
        expect(
          generatedCode,
          contains(r'values: $test_package_1.Color.values'),
        );
        expect(
          generatedCode,
          contains(r'values: $test_package_1.Priority.values'),
        );
      },
    );

    test(
      'G-ENM-16: Generates registerBridgedEnum calls in registerBridges. [2026-02-10 06:37] (PASS)',
      () {
        // The call includes sourceUri for deduplication support
        expect(
          generatedCode,
          contains(
            'interpreter.registerBridgedEnum(enumDef, importPath, sourceUri:',
          ),
        );
      },
    );
    ...
    test(
      'G-ENM-1: Generates enumNames property. [2026-02-10 06:37] (PASS)',
      () {
    ...
    test(
      'G-ENM-2: Generates class bridges that reference enums. [2026-02-10 06:37] (PASS)',
      () {
    ...
    test(
      'G-ENM-3: Generates getter adapters for enhanced enum fields. [2026-02-10 06:37] (PASS)',
      () {
        // Priority has computed getters: 'level' and 'isHighPriority'
        expect(generatedCode, contains("'level': (visitor, target) =>"));
        expect(
          generatedCode,
          contains(r'(target as $test_package_1.Priority).level'),
        );
        expect(
          generatedCode,
          contains("'isHighPriority': (visitor, target) =>"),
        );
        expect(
          generatedCode,
          contains(r'(target as $test_package_1.Priority).isHighPriority'),
        );
      },
    );

    test(
      'G-ENM-4: Generates getter adapters for Planet fields. [2026-02-10 06:37] (PASS)',
      () {
        ...
        expect(
          generatedCode,
          contains(r'(target as $test_package_1.Planet).mass'),
        );
        expect(
          generatedCode,
          contains(r'(target as $test_package_1.Planet).radius'),
        );
        expect(
          generatedCode,
          contains("'surfaceGravity': (visitor, target) =>"),
        );
      },
    );

    test(
      'G-ENM-5: Generates method adapters for enhanced enum methods. [2026-02-10 06:37] (PASS)',
      () {
    ...
    test(
      'G-ENM-6: Does NOT generate getters for simple enums. [2026-02-10 06:37] (PASS)',
      () {
        ...
        final nextDef = generatedCode.indexOf(
  ...
  group('Enum Bridge Generation - Edge Cases', () {
    test(
      'G-ENM-7: Handles file with no enums. [2026-02-10 06:37] (PASS)',
      () async {
...
''');
    ...
    test(
      'G-ENM-8: Handles file with only private enums. [2026-02-10 06:37] (PASS)',
      () async {
        // Create a temp file with only private enums
        final privateEnumFile = File(
          p.join(tempOutputDir, 'private_enums.dart'),
        );
...
''');
    ...
    test(
      'G-ENM-STATIC-1: Emits staticMethods block for enum static methods (issue #2).',
      () async {
        final staticMethodFile = File(
          p.join(tempOutputDir, 'static_method_enum.dart'),
        );
...
''');
        ...
        expect(
          code,
          contains('staticMethods: {'),
          reason: 'static enum methods must be emitted (issue #2)',
        );
        expect(
          code,
          contains("'fromString': (visitor, positional, named, typeArgs) {"),
        );
        expect(
          code,
          contains('PageFormat.fromString'),
    ...
    test(
      'G-ENM-9: Handles single-value enum. [2026-02-10 06:37] (PASS)',
      () async {
        final singleValueFile = File(
          p.join(tempOutputDir, 'single_value.dart'),
        );
