...
void main() {
  ...
  setUpAll(() {
    ...
    tempOutputDir =
        Directory.systemTemp.createTempSync('bridge_flutter_patterns_').path;
  });
  ...
  group('Flutter Pattern Generation', () {
    ...
    setUpAll(() async {
      ...
      final sourceFile =
          p.join(testFixturesDir, 'flutter_patterns_source.dart');
      final outputFile =
          p.join(tempOutputDir, 'flutter_patterns_bridges.dart');
    ...
    group('RC-1: Non-wrappable defaults', () {
      test(
        ...
        expect(
          generatedCode,
          isNot(contains("getOptionalNamedArg<\$test_package_1.SimpleValue?>")),
          reason:
      ...
      test(
        ...
        expect(
          generatedCode,
          isNot(contains("getOptionalNamedArg<\$test_package_1.BoxSideLike?>")),
          reason:
              'Non-wrappable defaults should not fall back to nullable type',
        );
      });
    ...
    group('RC-1b: Non-wrappable defaults exceeding threshold', () {
      ...
      test(
        ...
        final hasProperHandling = generatedCode.contains(
    ...
    group('RC-5: Typed collection defaults', () {
      test(
        ...
        expect(
            generatedCode, contains("name: 'WidgetWithCollectionDefaults'"));
        ...
        expect(
          ...
          reason:
              'Typed empty list default should have resolved type prefix',
        );
      });
    ...
    group('RC-2: Callback named params', () {
      test(
        ...
        final bareNonNullableNamedBool =
            RegExp(r'\{\s*(?!required\s)bool\s+allowUpscaling\s*[,}]');
        expect(
          generatedCode,
