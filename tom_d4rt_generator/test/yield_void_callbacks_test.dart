...
void main() {
  ...
  group('B.14 yieldVoidCallbacks generator switch', () {
    ...
    test('G-B14-1: default (off) emits no yield — synchronous void wrappers. '
        '[2026-06-07] (PASS)', () {
      expect(
        generatedOff,
        isNot(contains(_yieldExpr)),
        reason:
            'With the switch off no void callback may carry the 1 ms '
    ...
    test('G-B14-2: on ⇒ void callbacks become async with the 1 ms yield. '
        '[2026-06-07] (PASS)', () {
      expect(
        generatedOn,
        contains(_yieldExpr),
    ...
    test('G-B14-3: on ⇒ non-void callbacks are left untouched (no async, no '
      ...
      expect(
        ...
        reason:
            'non-void callbacks must not be async-wrapped — their '
    ...
    test('G-B14-4: non-void wrapper fragments are byte-identical across the '
      ...
      expect(
        generatedOn,
        contains(
          '() async { D4.callInterpreterCallback(visitor!, '
      ...
      expect(
        generatedOff,
        contains(
          '() { D4.callInterpreterCallback(visitor!, onReadyRaw, '
          '[]); }',
        ),
  ...
  group('B.14 BridgeConfig plumbing', () {
    test('G-B14-5: yieldVoidCallbacks defaults false and round-trips through '
        'json/copyWith. [2026-06-07] (PASS)', () {
      const base = BridgeConfig(name: 'pkg', modules: []);
      expect(
        base.yieldVoidCallbacks,
        isFalse,
