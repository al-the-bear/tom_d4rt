/// SCE190. `AstgenTestSetup.exclusive` is what keeps the two suites that
...
void main() {
  ...
  test('F-SCE190-1: concurrent holders never overlap, and the lock is '
    ...
    var inside = 0;
    var maxInside = 0;
    Future<int> holder(int id) => AstgenTestSetup.exclusive(lockPath, () async {
      inside++;
    ...
    File(lockPath)
      ...
      ..setLastModifiedSync(
        DateTime.now().subtract(
          AstgenTestSetup.staleLockAfter + const Duration(minutes: 1),
        ),
      );
    expect(await AstgenTestSetup.exclusive(lockPath, () async => 'ran'), 'ran');

    await expectLater(
      AstgenTestSetup.exclusive<void>(
        lockPath,
