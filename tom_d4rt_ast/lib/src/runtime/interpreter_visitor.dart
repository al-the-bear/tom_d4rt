...
class InterpreterVisitor extends GeneralizingSAstVisitor<Object?> {
  ...
  Object? visitAsExpression(SAsExpression node) {
    ...
    final typeDesc = _castTypeDescription(node.type);
  ...
  /// suffix.
  Object? _tryCast(Object? value, SAstNode? typeNode) {
  ...
  @override
  Object? visitSymbolLiteral(SSymbolLiteral node) {
    // `#foo.bar.baz` is ONE library-qualified symbol named 'foo.bar.baz', not a
    // member access on `#foo`. `tom_ast_generator` already joins the analyzer's
    // component tokens with `.` into `SSymbolLiteral.value`, so this side just
    // reads the finished name. Operator symbols (`#+`, `#[]`) arrive as a single
    // component and need no special handling.
    //
  ...
  Object? visitBinaryExpression(SBinaryExpression node) {
    ...
    if (leftOperandValue is InterpretedExtensionTypeInstance) {
      ...
      if (operatorMethod != null) {
        Logger.debug(
          "[SBinaryExpression] Found extension-type operator '$operatorName' on ${leftOperandValue.extensionType.name}. Calling...",
        );
        try {
          return operatorMethod.bind(leftOperandValue).call(this, [
            rightOperandValue,
  ...
  Object? visitMethodInvocation(SMethodInvocation node) {
    ...
    if (node.target == null) {
      ...
      // Handle static method call OR NAMED CONSTRUCTOR call
        ...
        // script must write `<package>.Name`. Picking whichever same-name
  ...
  Object? visitListLiteral(SListLiteral node) {
    ...
    for (final element in node.elements) {
      ...
      // is rebuilt from the top with the resolved value in place.
  ...
  /// deliberately discarded, but a discarded value and an unfinished one are
  /// not the same thing.
  Object? _executeCascadeMethodInvocation(
    Object? targetValue,
    SMethodInvocation node,
  ) {
    if (targetValue == null) {
  ...
  Object? _executeCascadePropertyAccess(
    ...
    if (interpreted != null) {
      ...
      // Fall through to the bridge path on miss.
    }
    if (toBridgedInstance(targetValue).$2) {
      ...
      // Preserve legacy "return whatever `.get` returns" for pure
      // InterpretedInstance misses.
  ...
  /// Mirror of tom_d4rt/interpreter_visitor.dart::_cascadeInterpretedTarget.
  /// If [targetValue] is an [InterpretedInstance] directly, returns it. If it
  /// is a [D4InterpretedProxy] that wraps one, returns the wrapped instance.
  /// Returns `null` otherwise — callers should fall through to the bridge
  /// path.
  ...
  Object? _executeCascadeAssignment(
    ...
    if (lhs is SSimpleIdentifier) {
      ...
      // Set the value on the target — InterpretedInstance side first, then
      // bridge fallback.
      var assigned = false;
      if (interpreted != null) {
      ...
      // 3. Set the new value — InterpretedInstance first, bridge fallback.
      var assigned = false;
      if (interpretedP != null) {
  ...
  AppliedRuntimeType? _resolveAppliedReturnType(
    ...
    try {
      final baseType = _resolveTypeAnnotationWithEnvironment(
        returnTypeNode,
        env,
      );
      final declaredArgs = [
    ...
    switch (operatorType) {
      ...
      case '-': // Unary minus (negation)
        ...
        if (operand is num) {
          ...
          // DFUB9: unary operator `-` declared on an extension type. The
          // declaration is a zero-arg `operator -()`; the binary `-` (one arg)
          // lives under the same `-` key, so only the prefix site may bind it
  ...
  bool catchClauseMatches(SCatchClause clause, Object? thrownValue) {
    // No `on Type`: matches anything.
    if (clause.exceptionType == null) return true;

    final typeNode = clause.exceptionType!;
    final targetCatchTypeName = typeNode is SNamedType
        ? typeNode.name!.name
        : '$typeNode';
  ...
  SCatchClause? selectCatchClause(STryStatement node, Object? thrownValue) {
  ...
  RuntimeType _resolveTypeAnnotationWithEnvironment(
    ...
    if (typeNode is SNamedType) {
      ...
      for (final field in typeNode.namedFields) {
        named[field.name?.name ?? ''] = _resolveTypeAnnotationWithEnvironment(
          field.type,
          env,
        );
      }
  ...
  void _matchAndBind(SAstNode pattern, Object? value, Environment environment) {
    ...
    if (pattern is SDeclaredVariablePattern) {
      ...
      try {
        ...
        if (Logger.isDebug) {
          Logger.debug(
            "[_matchAndBind] SLogicalOrPattern: left operand matched",
          );
        }
      ...
      if (Logger.isDebug) {
        Logger.debug(
          "[_matchAndBind] SLogicalAndPattern: both operands matched",
        );
      }
    ...
    } else if (pattern is SParenthesizedPattern) {
    ...
    } else if (pattern is SNullCheckPattern) {
      ...
      _matchAndBind(pattern.pattern, value, environment);
    } else if (pattern is SNullAssertPattern) {
  ...
  Object? visitImportDirective(SImportDirective node) {
    ...
    if (importUri.hasScheme) {
      ...
      // matching upstream. The downstream module loader decides whether such a
      // URI is actually loadable.
...
///
/// Produced when a script writes `BridgedClass.new` (Dart 2.15+ unnamed
/// constructor tear-off) or `BridgedClass.namedCtor`. Invoking the
/// wrapper calls the underlying [BridgedConstructorCallable] adapter
/// with the supplied arguments and returns the resulting native object.
///
/// Mirrors [_NamedConstructorTearOff] but for bridged classes — both
/// support the same tear-off semantics on either side of the bridge.
