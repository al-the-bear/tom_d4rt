...
///
/// THE VERSIONER IS NOT THE PROBLEM — THE INSTALLED BINARY IS. SCD82 inferred
/// an emitter defect; SCE124 measured it and the inference was wrong. The
/// template in `tom_build_kit`'s `versioner_executor.dart` already writes the
/// two long getters pre-wrapped, and a stamp generated by running that source
/// is format-clean. The `buildkit` on this machine's `~/.tom/bin` is **1.7.1,
/// built 2026-02-24**, four minors behind the 1.11.0 source, and it is the one
/// that emits the unwrapped form. Measured 2026-09-22 on the same package,
/// both ways round: binary `1 changed`, source `0 changed`.
///
/// So the exclusion stays for a reason that has nothing to do with the
/// versioner's code: until every machine's `buildkit` binary is current, a
/// regenerated stamp is unformatted wherever the old one runs. That is a
/// toolchain-distribution chore — `buildkit` opts out of being compiled by a
/// running buildkit (`skip-compile-with-buildkit: true`) and is built by the
/// bootstrap script — and it is outside this repo. sce124_aimn carries the
/// diagnosis.
///
/// DO NOT DELETE THIS NAME AFTER SEEING THE FOUR COMMITTED STAMPS PASS. They
/// pass because somebody formatted them after generating, which is the fight
/// this exclusion exists to avoid, not evidence that it is over.
///
/// `version.g.dart` is the SAME versioner's other output and is excluded for
/// the same reason. It was missed when this set was written because the two
/// names look unrelated; the shared header (`// Generated by versioner at …`)
/// is what identifies them as one class of file. Every copy in the repo is
/// gitignored, so for these the stated rationale above does not even apply —
