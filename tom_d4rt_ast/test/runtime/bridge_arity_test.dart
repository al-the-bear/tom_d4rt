...
void main() {
  group('SCB28 — describeArityError recognises a short argument list', () {
    test('F-SCB28-AST-1: empty list, slot 0 — singular wording [2026-09-03]',
        () {
      final args = <Object?>[];
      final error = _errorFrom(() => args[0]);
      expect(
        D4.describeArityError(error, args, 'UriData.parse'),
        equals('UriData.parse expects at least 1 positional argument, '
            'but was called with 0.'),
      );
    });

    test(
        'F-SCB28-AST-2: one argument, slot 2 — reports the slot reached, '
        'plural wording [2026-09-03]', () {
      final args = <Object?>['a'];
      final error = _errorFrom(() => args[2]);
      expect(
        D4.describeArityError(error, args, 'String.replaceRange'),
        equals('String.replaceRange expects at least 3 positional arguments, '
    ...
    test('F-SCB28-AST-3: the member description is passed through verbatim '
      ...
      final error = _errorFrom(() => args[0]);
      final message =
          D4.describeArityError(error, args, 'MyClass.myMember')!;
  ...
  group('SCB28 — describeArityError stays silent on everything else', () {
    test(
        'F-SCB28-AST-4: a native range error from inside the call passes '
        'through [2026-09-03]', () {
    ...
    test(
        'F-SCB28-AST-5: an out-of-range read on a list of a different length '
    ...
    test(
        'F-SCB28-AST-7: a RangeError whose invalid value is already in range '
        'passes through [2026-09-03]', () {
    ...
    test(
        'F-SCB28-AST-8: a correctly-sized call never reaches the helper at '
