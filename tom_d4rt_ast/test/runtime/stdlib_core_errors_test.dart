...
void main() {
  ...
  group('SC5/AST: catchable dart:core error types', () {
    test(
        'F-SC5-AST-1: all seven error types are registered against their SDK '
      ...
      for (final entry in expected.entries) {
        expect(bridgeOf(entry.key).nativeType, entry.value,
            reason: entry.key);
      }
    });

    test(
        'F-SC5-AST-2: every new bridge carries an isAssignable predicate that '
    ...
    test(
        'F-SC5-AST-3: the private VM subclasses route to their public bridge '
        '[2026-07-27]', () {
      ...
      expect(
          bridgeOf('AssertionError').nativeNames, contains('_AssertionError'));
    });

    test(
        'F-SC5-AST-4: the dart:core error hierarchy is in the supertype '
        'registry [2026-07-27]', () {
      // Bridges are registered flat, so this registry is the only thing that
      // can answer `indexError is RangeError`.
      expect(BridgedClass.transitiveSupertypeNames('IndexError'),
          containsAll(<String>['RangeError', 'ArgumentError', 'Error']));
      expect(BridgedClass.transitiveSupertypeNames('RangeError'),
          containsAll(<String>['ArgumentError', 'Error']));
      expect(BridgedClass.transitiveSupertypeNames('UnimplementedError'),
      ...
      for (final name in const [
        ...
        expect(BridgedClass.transitiveSupertypeNames(name), contains('Error'),
            reason: name);
      }
    });

    test('F-SC5-AST-5: isSubtypeOf follows the registered chain [2026-07-27]',
        () {
    ...
    test('F-SC5-AST-6: the constructors build real SDK errors [2026-07-27]', () {
      expect(
          bridgeOf('IndexError').constructors['withLength']!(
              visitor, [5, 2], {'name': 'idx'}),
          isA<IndexError>());
      expect(
          bridgeOf('ConcurrentModificationError').constructors['']!(
              visitor, [
            [1, 2]
          ], {}),
          isA<ConcurrentModificationError>());
      expect(bridgeOf('TypeError').constructors['']!(visitor, [], {}),
          isA<TypeError>());
      expect(
          bridgeOf('AssertionError').constructors['']!(visitor, ['boom'], {}),
          isA<AssertionError>());
      expect(bridgeOf('StackOverflowError').constructors['']!(visitor, [], {}),
          isA<StackOverflowError>());
      expect(bridgeOf('OutOfMemoryError').constructors['']!(visitor, [], {}),
          isA<OutOfMemoryError>());
      expect(
          bridgeOf('NoSuchMethodError').constructors['withInvocation']!(
              visitor, [1, Invocation.method(#foo, const [])], {}),
          isA<NoSuchMethodError>());
    });

    test('F-SC5-AST-7: the getters read through to the SDK value [2026-07-27]',
        () {
      ...
      expect(
          bridgeOf('AssertionError')
              .getters['message']!(visitor, AssertionError('boom')),
          'boom');
      final modified = [1, 2, 3];
      expect(
          bridgeOf('ConcurrentModificationError')
              .getters['modifiedObject']!(visitor, ConcurrentModificationError(modified)),
          same(modified));
    });

    test(
        'F-SC5-AST-8: adding the new bridges leaves the pre-existing error '
