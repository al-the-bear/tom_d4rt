...
void main() {
  ...
  group('SC8: BytesBuilder typed_data bridge', () {
    test('F-SC8-AST-1: is registered under the name BytesBuilder [2026-07-27]',
        () {
    ...
    test('F-SC8-AST-2: routes both private implementations [2026-07-27]', () {
      ...
      expect(
          BytesBuilder(copy: false).runtimeType.toString(), '_BytesBuilder');
    });

    test('F-SC8-AST-3: declares isAssignable over both flavours [2026-07-27]',
        () {
    ...
    test('F-SC8-AST-4: the constructor honours the copy: flag [2026-07-27]', () {
      final bridge = builderBridge();
    ...
    test('F-SC8-AST-5: the constructor rejects bad arguments [2026-07-27]', () {
      ...
      expect(() => bridge.constructors['']!(visitor, [true], {}),
          throwsA(isA<RuntimeD4rtException>()));
      expect(() => bridge.constructors['']!(visitor, [], {'copy': 1}),
          throwsA(isA<RuntimeD4rtException>()));
    });
    ...
    test('F-SC8-AST-6: addByte and add accumulate in order [2026-07-27]', () {
      ...
      bridge.methods['add']!(visitor, builder, [
        [2, 3]
      ], {}, []);
      ...
      expect(bridge.methods['toBytes']!(visitor, builder, [], {}, []),
    ...
    test('F-SC8-AST-7: takeBytes drains but toBytes does not [2026-07-27]', () {
      ...
      expect(bridge.methods['toBytes']!(visitor, builder, [], {}, []),
          orderedEquals(<int>[1, 2, 3]));
      expect(bridge.getters['length']!(visitor, builder), 3);
      expect(bridge.methods['takeBytes']!(visitor, builder, [], {}, []),
          orderedEquals(<int>[1, 2, 3]));
    ...
    test('F-SC8-AST-9: the members reject bad arguments [2026-07-27]', () {
      final bridge = builderBridge();
      final builder = BytesBuilder();
      expect(() => bridge.methods['addByte']!(visitor, builder, ['x'], {}, []),
          throwsA(isA<RuntimeD4rtException>()));
      expect(() => bridge.methods['add']!(visitor, builder, [42], {}, []),
          throwsA(isA<RuntimeD4rtException>()));
    ...
    test('F-SC8-AST-10: the copy: false flavour works past construction '
      ...
      final bridge = builderBridge();
      final builder =
          bridge.constructors['']!(visitor, [], {'copy': false}) as BytesBuilder;
