...
void main() {
  ...
  group('IMP-OPT-6: Phase-1 registration is O(matched)', () {
    test(
      ...
      final uris =
          List.generate(5, (i) => 'package:lib$i/lib$i.dart');
      for (var i = 0; i < uris.length; i++) {
        runner.registerBridgedClass(marker('Class$i'), uris[i],
            sourceUri: uris[i]);
      }
      ...
      for (var i = 0; i < uris.length; i++) {
        expect(runner.bridgedClasses[uris[i]]!.keys, ['Class$i'],
      ...
      expect(runner.bridgedClasses.containsKey('package:none/none.dart'),
          isFalse);
      ...
      runner.registerBridgedClass(
          marker('Extra3'), 'package:lib3/lib3.dart',
          sourceUri: 'package:lib3/lib3.dart');
      expect(runner.bridgedClasses['package:lib3/lib3.dart']!.length, 2);
      for (var i = 0; i < uris.length; i++) {
        if (i == 3) continue;
        expect(runner.bridgedClasses[uris[i]]!.length, sizesBefore[uris[i]],
    ...
    test(
        ...
        () {
      final runner = D4rtRunner();
      runner.registerBridgedClass(marker('AClass'), 'package:a/a.dart',
