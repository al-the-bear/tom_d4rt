...
///     ahead of them.
///
/// Twin of `tom_d4rt/test/scd137_typedef_arity_test.dart`, minus its two
/// SCRIPT-level cases. That is a decision, not an omission: this line has no
/// analyzer, so a script here is a hand-built `SAstNode` tree, and
/// `scd136_closure_vs_bridged_typedef_test.dart`'s F-SCD136-AST-4 already
/// builds one and establishes that this line's declared-parameter path reaches
/// `FunctionRuntimeType.isSubtypeOf`. The arity rule is a refinement INSIDE
/// that same method, and `runtime_interfaces.dart` is code-identical across the
/// twins (SCC92 does not baseline it), so a second bundle would re-measure the
/// plumbing rather than the rule.
library;
...
void main() {
  group('SCD137/AST: a bridged typedef checks positional arity', () {
    test(
      'F-SCD137-AST-1: a matching closure is accepted — 1-arg against ValueChanged',
      () {
        expect(
          _closure(
            required: 1,
    ...
    test(
      'F-SCD137-AST-2: a closure that cannot be invoked is refused — this is the '
    ...
    test('F-SCD137-AST-3: optional positionals count toward what a closure can '
        'accept', () {
    ...
    test(
      'F-SCD137-AST-4: only the typedef\'s REQUIRED count is checked, never its '
      'maximum',
      () {
    ...
    test(
      'F-SCD137-AST-5: no arity, no opinion — an un-annotated bridge behaves '
    ...
    test('F-SCD137-AST-6: return types are never consulted', () {
    ...
    test(
      'F-SCD137-AST-7: a bridge that is not a Function is still refused',
      () {
