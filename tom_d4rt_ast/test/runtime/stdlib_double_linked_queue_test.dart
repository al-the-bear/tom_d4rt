...
void main() {
  ...
  group('SC7: DoubleLinkedQueue collection bridge', () {
    ...
    test(
      'F-SC7-AST-2: declares isAssignable, unlike the abstract bridges [2026-07-27]',
      () {
    ...
    test(
      'F-SC7-AST-3: constructs from the three constructors [2026-07-27]',
      () {
        final bridge = queueBridge();
        expect(
          bridge.constructors['']!(visitor, [], {}),
          isA<DoubleLinkedQueue>(),
        );
        expect(
    ...
    test(
      'F-SC7-AST-4: the constructors reject a non-Iterable [2026-07-27]',
      () {
        final bridge = queueBridge();
        expect(
          () => bridge.constructors['from']!(visitor, [42], {}),
          throwsA(isA<RuntimeD4rtException>()),
        );
        expect(
          () => bridge.constructors['of']!(visitor, [42], {}),
          throwsA(isA<RuntimeD4rtException>()),
        );
        expect(
          () => bridge.constructors['']!(visitor, [1], {}),
          throwsA(isA<RuntimeD4rtException>()),
        );
      },
    );
  ...
  group('SC7: queue supertype hierarchy', () {
    test(
      'F-SC7-AST-7: the DoubleLinkedQueue -> Queue -> Iterable chain is registered [2026-07-27]',
      () {
        ...
        expect(
          BridgedClass.transitiveSupertypeNames('DoubleLinkedQueue'),
    ...
    test(
      'F-SC7-AST-8: the ListQueue edges were registered too [2026-07-27]',
      () {
        ...
        expect(
          BridgedClass.transitiveSupertypeNames('ListQueue'),
          containsAll(<String>['Queue', 'Iterable']),
        );
        expect(
          BridgedClass.transitiveSupertypeNames('Queue'),
          contains('Iterable'),
        );
      },
    );
    ...
    test(
      'F-SC7-AST-9: DoubleLinkedQueue and ListQueue stay distinct [2026-07-27]',
      () {
        ...
        expect(
          queueBridge().isSubtypeOf(env.findBridgedClassByName('ListQueue')!),
          isFalse,
        );
      },
    );
  ...
  group('SC7: DoubleLinkedQueueEntry bridge', () {
    test(
      'F-SC7-AST-10: is registered and routes the private element type [2026-07-27]',
      () {
        ...
        final resolved = env.getRuntimeType(
          DoubleLinkedQueue<dynamic>.from([1]).firstEntry(),
        );
    ...
    test(
      'F-SC7-AST-11: firstEntry and lastEntry navigate the chain [2026-07-27]',
      () {
    ...
    test(
      'F-SC7-AST-12: append and prepend splice into the queue [2026-07-27]',
      () {
    ...
    test(
      'F-SC7-AST-13: remove unlinks and element is settable [2026-07-27]',
      () {
        final queue = DoubleLinkedQueue<dynamic>.from([1, 2, 3]);
        final entries = entryBridge();
        final first = queueBridge().methods['firstEntry']!(
    ...
    test('F-SC7-AST-14: a standalone entry is constructible [2026-07-27]', () {
      ...
      expect(
        () => entries.constructors['']!(visitor, [], {}),
        throwsA(isA<RuntimeD4rtException>()),
      );
    });

    test(
      'F-SC7-AST-15: forEachEntry visits every entry, empty yields null [2026-07-27]',
      () {
        ...
        queueBridge().methods['forEachEntry']!(
          ...
          [
            NativeFunction(
              (_, positional, _, _) {
                seen.add(
                  entries.getters['element']!(visitor, positional.first!),
                );
                return null;
              },
              arity: 1,
