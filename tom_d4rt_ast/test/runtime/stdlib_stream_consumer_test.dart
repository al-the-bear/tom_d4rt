...
void main() {
  ...
  group('SC4: StreamConsumer async bridge', () {
    test(
      'F-SC4-AST-1: is registered under the name StreamConsumer [2026-07-27]',
      () {
    ...
    test(
      'F-SC4-AST-2: exposes exactly the two interface members [2026-07-27]',
      () {
        final bridge = env.findBridgedClassByName('StreamConsumer')!;
        expect(
          bridge.methods.keys,
          containsAll(<String>['addStream', 'close']),
        );
    ...
    test(
      'F-SC4-AST-3: declares no isAssignable, so dispatch is untouched [2026-07-27]',
      () {
        ...
        expect(
          env.findBridgedClassByName('StreamConsumer')!.isAssignable,
          isNull,
        );
      },
    );
    ...
    test(
      'F-SC4-AST-4: addStream forwards to the native consumer [2026-07-27]',
      () async {
        final bridge = env.findBridgedClassByName('StreamConsumer')!;
    ...
    test(
      'F-SC4-AST-5: addStream rejects a non-Stream argument [2026-07-27]',
      () {
        final bridge = env.findBridgedClassByName('StreamConsumer')!;
        final controller = StreamController<int>();
  ...
  group('SC4: StreamSink routing and supertype edges', () {
    test(
      'F-SC4-AST-6: the sink hierarchy is registered for isSubtypeOf [2026-07-27]',
      () {
    ...
    test(
      'F-SC4-AST-7: StreamSink claims the controller sink wrapper [2026-07-27]',
      () {
    ...
    test(
      'F-SC4-AST-9: a sink resolves to StreamSink, and that bridge is a StreamConsumer [2026-07-27]',
      () {
