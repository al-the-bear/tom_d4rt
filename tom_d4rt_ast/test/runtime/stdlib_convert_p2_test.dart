...
void main() {
  ...
  group('SC9: JsonUtf8Encoder convert bridge', () {
    test('F-SC9-AST-1: is registered under the name JsonUtf8Encoder [2026-07-27]',
        () {
    ...
    test('F-SC9-AST-2: needs no nativeNames — the class is concrete [2026-07-27]',
        () {
    ...
    test('F-SC9-AST-3: the constructor reads its three optionals by position [2026-07-27]',
        () {
    ...
    test('F-SC9-AST-4: the constructor rejects mistyped arguments [2026-07-27]',
        () {
      final ctor = bridge('JsonUtf8Encoder').constructors['']!;
      expect(() => ctor(visitor, [42], {}), throwsA(isA<RuntimeD4rtException>()));
      expect(
          () => ctor(visitor, [null, 'x'], {}), throwsA(isA<RuntimeD4rtException>()));
      expect(() => ctor(visitor, [null, null, 'big'], {}),
          throwsA(isA<RuntimeD4rtException>()));
    });

    test('F-SC9-AST-5: convert produces UTF-8 JSON bytes [2026-07-27]', () {
      final result = bridge('JsonUtf8Encoder').methods['convert']!(
          visitor, JsonUtf8Encoder(), [
        {'a': 1}
      ], {}, null);
    ...
    test('F-SC9-AST-6: JsonEncoder.fuse(Utf8Encoder()) lands on this bridge [2026-07-27]',
        () {
  ...
  group('SC9: ClosableStringSink convert bridge', () {
    test('F-SC9-AST-7: is registered and routes its private impl [2026-07-27]',
        () {
      ...
      expect(
        ClosableStringSink.fromStringSink(StringBuffer(), () {})
            .runtimeType
            .toString(),
        '_ClosableStringSink',
      );
    ...
    test('F-SC9-AST-8: the write-through adapters reach the backing sink [2026-07-27]',
        () {
      ...
      var closed = false;
      final sink = ClosableStringSink.fromStringSink(buffer, () => closed = true);
      b.methods['write']!(visitor, sink, ['hi'], {}, null);
      b.methods['writeCharCode']!(visitor, sink, [33], {}, null);
      b.methods['writeAll']!(visitor, sink, [
        ['a', 'b'],
        '-'
      ], {}, null);
    ...
    test('F-SC9-AST-15: fromStringSink validates both arguments [2026-07-27]',
        () {
      final ctor = bridge('ClosableStringSink').constructors['fromStringSink']!;
      expect(() => ctor(visitor, [42, null], {}),
          throwsA(isA<RuntimeD4rtException>()));
      ...
      expect(() => ctor(visitor, [StringBuffer(), 7], {}),
          throwsA(isA<RuntimeD4rtException>()));
    });

    test('F-SC9-AST-9: the StringSink surface is declared, not inherited [2026-07-27]',
        () {
      ...
      expect(
        ...
        containsAll(<String>['write', 'writeln', 'writeCharCode', 'writeAll']),
      );
    });
  });

  group('SC9: chunked-conversion sinks, newly registered', () {
    test('F-SC9-AST-10: both formerly-dead bridges are now reachable [2026-07-27]',
        () {
    ...
    test('F-SC9-AST-11: asStringSink is the idiomatic ClosableStringSink route [2026-07-27]',
        () {
    ...
    test('F-SC9-AST-12: the root claims assignability over both subtypes [2026-07-27]',
        () {
    ...
    test('F-SC9-AST-13: supertype edges let the resolver keep the subtype [2026-07-27]',
        () {
      ...
      expect(BridgedClass.transitiveSupertypeNames('StringConversionSink'),
          contains('ChunkedConversionSink'));
      expect(BridgedClass.transitiveSupertypeNames('ByteConversionSink'),
          contains('ChunkedConversionSink'));
      expect(BridgedClass.transitiveSupertypeNames('ClosableStringSink'),
          contains('StringSink'));
    });

    test('F-SC9-AST-14: the byte sink carries its own predicate and names [2026-07-27]',
        () {
      ...
      expect(b.isAssignable!(ByteConversionSink.withCallback((_) {})), isTrue);
      expect(b.isAssignable!(StringConversionSink.withCallback((_) {})), isFalse);
      expect(b.nativeNames, containsAll(<String>['_ByteCallbackSink', '_ByteAdapterSink']));
    });
  });
}
