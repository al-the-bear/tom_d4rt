...
void main() {
  ...
  group('SC6: StreamView async bridge', () {
    test(
      'F-SC6-AST-1: is registered under the name StreamView [2026-07-27]',
      () {
    ...
    test(
      'F-SC6-AST-3: the constructor rejects a non-Stream argument [2026-07-27]',
      () {
        final bridge = env.findBridgedClassByName('StreamView')!;
        expect(
    ...
    test(
      'F-SC6-AST-4: declares no isAssignable, so dispatch is untouched [2026-07-27]',
      () {
    ...
    test(
      'F-SC6-AST-5: StreamView instances route to the Stream bridge [2026-07-27]',
      () {
    ...
    test(
      'F-SC6-AST-6: the StreamView -> Stream edge is registered [2026-07-27]',
      () {
        expect(
  ...
  group('SC6: AsyncError async bridge', () {
    test(
      ...
      () {
        ...
        expect(
          bridge.getters.keys,
          containsAll(<String>['error', 'stackTrace']),
        );
    ...
    test(
      'F-SC6-AST-8: declares isAssignable, unlike the abstract bridges [2026-07-27]',
      () {
        ...
        expect(
          bridge.isAssignable!(AsyncError('boom', StackTrace.current)),
          isTrue,
        );
    ...
    test(
      'F-SC6-AST-9: the constructor accepts one or two arguments [2026-07-27]',
      () {
        ...
        final bridge = env.findBridgedClassByName('AsyncError')!;
        expect(
          (bridge.constructors['']!(visitor, ['boom'], {}) as AsyncError).error,
          'boom',
        );
        final st = StackTrace.current;
        final two =
            bridge.constructors['']!(visitor, ['boom', st], {}) as AsyncError;
    ...
    test(
      'F-SC6-AST-11: the AsyncError -> Error edge is registered [2026-07-27]',
      () {
  ...
  group('SC6: StreamTransformerBase async bridge', () {
    test(
      'F-SC6-AST-12: is registered under the name StreamTransformerBase [2026-07-27]',
      () {
    ...
    test(
      'F-SC6-AST-13: exposes a default constructor so super() resolves [2026-07-27]',
      () {
    ...
    test(
      'F-SC6-AST-14: declares no isAssignable, so dispatch is untouched [2026-07-27]',
      () {
        // Abstract, and a supertype of everything `StreamTransformer` owns —
        // a predicate here would contest that bridge's dispatch.
        expect(
          env.findBridgedClassByName('StreamTransformerBase')!.isAssignable,
          isNull,
        );
      },
    );
    ...
    test(
      'F-SC6-AST-15: the StreamTransformerBase -> StreamTransformer edge is registered [2026-07-27]',
      () {
    ...
    test(
      'F-SC6-AST-16: Stream.transform accepts a native transformer [2026-07-27]',
      () async {
        ...
        final doubler = StreamTransformer<int, int>.fromBind(
          (s) => s.map((e) => e * 2),
        );
        final transformed =
    ...
    test(
      'F-SC6-AST-17: Stream.transform rejects a non-transformer [2026-07-27]',
      () {
        final bridge = env.findBridgedClassByName('Stream')!;
        expect(
          () => bridge.methods['transform']!(
