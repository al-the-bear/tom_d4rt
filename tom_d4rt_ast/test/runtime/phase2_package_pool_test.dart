...
void main() {
  ...
  group('IMP-OPT-7: providePackage + process-global pool', () {
    ...
    test(
      'IMP-OPT-7c: freshly-provided defs land in the pool under the name',
      () {
        final runner = D4rtRunner();
        ...
        expect(
          D4rtRunner.debugPooledClassCount('pkg_c'),
          2,
    ...
    test('IMP-OPT-7d: unmigrated callers route to the <default> package', () {
      ...
      expect(
        runner.bridgedClasses['package:d/d.dart']!.containsKey('DClass'),
        isTrue,
      );
    });

    test('IMP-OPT-7e: providePackage(true) closes the registration context so '
      ...
      second.registerBridgedClass(
        marker('Stray'),
        'package:stray/stray.dart',
        sourceUri: 'package:stray/stray.dart',
      );
      expect(
        D4rtRunner.debugPooledClassCount('pkg_e'),
        1,
        reason: 'pkg_e keeps only its own class',
      );
      expect(
        D4rtRunner.debugPooledClassCount('<default>'),
        1,
