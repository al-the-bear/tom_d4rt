...
void main() {
  ...
  SStatement throwString(String message) => SExpressionStatement(
    ...
    expression: SThrowExpression(
      offset: 0,
      length: 0,
      expression: SSimpleStringLiteral(offset: 0, length: 0, value: message),
    ),
  );
  ...
  SStatement unawaitedFutureError(String message) => SExpressionStatement(
    ...
    expression: SMethodInvocation(
      ...
      argumentList: SArgumentList(
        offset: 0,
        length: 0,
        arguments: [SSimpleStringLiteral(offset: 0, length: 0, value: message)],
      ),
    ),
  );

  group('SCC23/AST: the embedder hook', () {
    test('F-SCC23-AST-1: onUncaughtError is null by default and installing one '
        'leaves an ordinary result untouched [2026-09-04]', () {
      expect(
        D4rtRunner().onUncaughtError,
        isNull,
    ...
    test('F-SCC23-AST-2: a synchronous script error still throws out of '
      ...
      expect(
        () => runner.executeBundle(bundleOf([throwString('boom')])),
        throwsA(anything),
      );
      expect(
        escapes,
        isEmpty,
        reason:
            'errors the caller can already observe are not routed to '
    ...
    test(
      ...
      'chain reaches the hook, unwrapped [2026-09-04]',
      () async {
        final escapes = <Object>[];
        ...
        expect(
          ...
          'escaped',
          reason:
              'the interpreter-internal wrapper is removed before the '
        ...
        expect(
          ...
          isEmpty,
          reason:
              'a hook *contains* the error — that is what makes it usable '
              'as a sandbox boundary by a host running untrusted script',
        );
      },
    );

    test('F-SCC23-AST-4: with no hook the escape still reaches the enclosing '
      ...
      expect(
        ...
        hasLength(1),
        reason:
            'the pre-existing routing is untouched when no hook is set; '
            'this is the property F-SCB9-14 depends on',
      );
    });

    test(
      'F-SCC23-AST-5: a hook that throws is not lost [2026-09-04]',
      () async {
        final zoneErrors = <Object>[];
        ...
        expect(
          ...
          isA<StateError>(),
          reason:
              "an embedder's hook is ordinary code and can be wrong; "
