...
void main() {
  ...
  group('cupertino/', () {
    test('class_test.dart', () async {
      // 1944 TODO C.46 (2026-05-31): historical 20260523-1056 §1.4/E10
      // cold-start-contention wrapper REMOVED. Script runs in ~3.1 s
      // under normal load (httpMs=3112, bundleJsonBytes=861264 —
      // 861 KB bundle / 70 KB / 1723-line script).
      final result = await SendTestRunner.send(
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.4/E13: TimeoutException 30s
          // — cold-start contention. This 2156-line / 68 KB script
          // (849 KB bundle) builds in ~1.8 s in both variants. Same
          // family as the §1.3 E-series and E10-E12: 50 s leaves
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.4/E14: TimeoutException 30s
          // — cold-start contention. This 2260-line / 70 KB script
          // (837 KB bundle) builds in ~2.1 s in both variants. Same
          // family as the §1.3 E-series and E10-E13: 50 s leaves
          // 10 s of headroom under the 60 s dart-test wrapper.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.4/E17: Transport failure 25s
          // — cold-start contention. This 2178-line / 71 KB script
          // (765 KB bundle) builds in ~1.5 s in both variants. Same
          // family as the §1.3 E-series and E10-E15: 50 s leaves
          // 10 s of headroom under the 60 s dart-test wrapper.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
