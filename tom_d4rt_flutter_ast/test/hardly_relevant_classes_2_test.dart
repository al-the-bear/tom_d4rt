...
void main() {
  ...
  group('material/', () {
    ...
    test('hour_format_test.dart', () async {
      // 1944 TODO C.65 (2026-05-31): historical 20260523-1056 §1.5/E19
      // cold-start-contention wrapper REMOVED. Script runs in ~2.3 s
      // under normal load (httpMs=2313, bundleJsonBytes=702083 —
      // 702 KB bundle / 49 KB / 1664-line script).
      final result = await SendTestRunner.send(
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.5/E20: TimeoutException 30s
          // — cold-start contention. This 1734-line / 58 KB script
          // (576 KB bundle) builds in ~1.5 s in both variants. Same
          // family as the §1.3/§1.4 E-series and E18/E19: 50 s
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.5/E21: TimeoutException 30s
          // — cold-start contention. This 1331-line / 49 KB script
          // (448 KB bundle) builds in ~1.8 s in both variants. Same
          // family as the §1.3/§1.4 E-series and E18/E19/E20: 50 s
          // leaves 10 s of headroom under the 60 s dart-test wrapper.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
    ...
    test(
      'widget_state_input_border_test.dart',
      () async {
        final result = await SendTestRunner.send(
          'material/widget_state_input_border_test.dart',
          // 20260523-1056 baseline §1.5/E22: Transport failure 25s
          // — cold-start contention. This 1380-line / 48 KB script
          // (558 KB bundle) builds in ~1.6 s in both variants. Same
