#!/usr/bin/env bash
#
# run_test_profiler.sh — drive the analyzer-free (AST) D4rt Flutter corpus
# against an ALREADY-RUNNING test app, for a manual profiling session.
# macOS / Linux (bash).
#
...
# tom_d4rt_ast (see start_test_profiler.sh header). With it off the run still
# works; you just won't see the per-phase init breakdown lines.
#
...
if [ ! -s "$OUT/profile.txt" ]; then
  ...
  echo "==       Flip it to true in tom_d4rt_ast + rebuild the app to capture init-path spans."
fi
exit "$rc"
