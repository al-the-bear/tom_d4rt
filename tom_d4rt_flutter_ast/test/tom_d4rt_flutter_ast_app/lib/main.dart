...
import 'package:tom_d4rt_ast/d4rt.dart' show IsolatePermission;
...
class _D4rtTestPageState extends State<D4rtTestPage>
  ...
  final FlutterD4rt _d4rt = _grantedInterpreter();

  /// The interpreter the harness drives, with the capabilities the CORPUS
  /// needs granted explicitly.
  ///
  /// SCE166 — `dart:isolate` is permission-gated on this line as of now; it
  /// was not before, which is the divergence that todo closed. This tree is
  /// the one that ships inside Flutter apps executing downloaded bundles, so
  /// it is the last place an ungated capability belongs — and the source twin
  /// has granted the same thing since scd139.
  ///
  /// `dart_ui/isolate_name_server_test.dart` is the corpus script that needs
  /// it: `ReceivePort` + `IsolateNameServer`, entirely within one isolate.
  /// Without the grant it dies with
  ///
  ///     Access to dart:isolate requires IsolatePermission.
  ///
  /// Granting here is correct rather than a workaround: a harness whose entire
  /// job is executing corpus scripts should declare the capabilities the
  /// corpus needs, explicitly and in one place.
  static FlutterD4rt _grantedInterpreter() {
    final d4rt = FlutterD4rt();
    d4rt.interpreter.grant(IsolatePermission.any);
    return d4rt;
  }
  ...
  /// `TOM_D4RT_AST_TEST_PORT` env var at startup (matching
  /// `SendTestRunner.portEnvVar`); falls back to 4247. Lets the runner
  ...
  /// (warm-parent + stdlib registration) off the first frame.
  ///
  /// Scheduled via [WidgetsBinding.addPostFrameCallback] so it never blocks
  /// initial paint: the first frame renders, then [FlutterD4rt.warmup] runs
  /// before the harness has finished bringing up the HTTP server and sending
  /// its first `/build`. The elapsed time is logged as a `[D4rtApp][warmup]`
  /// metric so the off-frame move is measurable in the test logs; compare it
  /// against the first `[BUILD_METRIC]` `interpMs`, which now excludes this
  /// one-time cost.
  ...
  void _handleFlutterError(FlutterErrorDetails details) {
    ...
    if (_capturingFrameworkErrors) {
      ...
      const ignoredPatterns = [
        ...
        // × 5 + timeout) with the suppression off found exactly ONE
        // affected script: `cupertino/restorable_cupertino_tab_controller_test.dart`
        // with 55 hits, all originating from the per-tab-item Column
        // inside the script's `_MiniTabBar` helper. The Column's
        // children (Icon size 18 + SizedBox 2 + Text fontSize 10)
        // request ≈ 34 logical pixels because Flutter's default font
        // line-height factor for fontSize 10 rounds the rendered Text
        // height to ≈ 14 px (not 10); available inner-Column space
        // was 33.5 ((tabBarHeight 50 − 0.5 top border) − 8 margin − 8
        // padding). Script-side fix bumped `_kTabBarHeight` from
        // 50.0 to 51.0 — raises available inner-Column space to 34.5,
        // eliminating the banner without visibly changing the tab-bar
        // dimensions. Post-fix isolated retests on AST + TEST both
        // returned `frameworkErrors=0`. The framework's 0.500-pixel
        // overflow detection itself remains the correct signal — it
        // catches legitimate layout-sum-vs-parent-height mismatches
        // that future scripts could regress on. If a future script
