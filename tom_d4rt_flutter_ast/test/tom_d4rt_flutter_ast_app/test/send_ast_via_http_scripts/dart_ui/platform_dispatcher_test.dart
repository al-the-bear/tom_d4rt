...
class _PlatformDispatcherDemoPage extends StatefulWidget {
  ...
  @override
  State<_PlatformDispatcherDemoPage> createState() =>
      _PlatformDispatcherDemoPageState();
}

class _PlatformDispatcherDemoPageState
    extends State<_PlatformDispatcherDemoPage> {
  ...
  void _captureSnapshot() {
    ...
    final _DispatcherSnapshot snap = _DispatcherSnapshot(
      ...
      localesLabel: _dispatcher.locales
          .map((ui.Locale l) => l.toString())
          .join(', '),
  ...
  void _runProbes() {
    ...
    probe(
      'PlatformDispatcher singleton is stable',
      identical(_dispatcher, ui.PlatformDispatcher.instance),
    );
    probe(
      'views iterable is available',
      _dispatcher.views.runtimeType.toString().contains('Iterable'),
    );
    probe(
      'displays iterable is available',
      _dispatcher.displays.runtimeType.toString().contains('Iterable'),
    );
    probe(
      'locale language code exists',
      _dispatcher.locale.languageCode.isNotEmpty,
    );
    probe('locales list is non-empty', _dispatcher.locales.isNotEmpty);
    probe(
      'textScaleFactor is greater than zero',
      _dispatcher.textScaleFactor > 0,
    );
    probe(
      'semanticsEnabled is bool',
      _dispatcher.semanticsEnabled == true ||
          _dispatcher.semanticsEnabled == false,
    );
    probe(
      'alwaysUse24HourFormat is bool',
      _dispatcher.alwaysUse24HourFormat == true ||
          _dispatcher.alwaysUse24HourFormat == false,
    );
    probe(
      'platformBrightness string contains Brightness',
      _dispatcher.platformBrightness.toString().contains('Brightness'),
    );
    probe(
      'summary text can be formed',
      '${_passed.length + _failed.length} checks'.endsWith('checks'),
    );

    _log(
      'Runtime probes executed: ${_passed.length} pass, ${_failed.length} fail.',
    );
    setState(() {});
  }
      ...
      child: Row(
        children: <Widget>[
          card(
            ...
            'Singleton access to app-wide platform integration.',
            Icons.public,
            const Color(0xFF1D4ED8),
          ),
          card(
            'View and display graph',
            'Track view count, display count, geometry, and density.',
            Icons.dashboard_customize,
            const Color(0xFF047857),
          ),
          card(
            'Locale and semantics',
            'Observe locale stacks, text scale, and semantics flags.',
            Icons.translate,
            const Color(0xFF7C3AED),
          ),
          card(
            'Operational telemetry',
            'Capture snapshots over time and compare runtime drift.',
            Icons.timeline,
  ...
  Widget _controlPanel() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          const Text(
            'Dispatcher controls and snapshot tools',
          ...
          Slider(
            ...
            max: 1.6,
            divisions: 90,
            onChanged: (double v) => setState(() => _nodeScale = v),
          ),
          ...
          Wrap(
            ...
            children: <Widget>[
              ...
              FilterChip(
                ...
                onSelected: (bool v) =>
                    setState(() => _showAccessibilityFocus = v),
              ),
            ],
          ),
          const SizedBox(height: 8),
  ...
  Widget _currentStatePanel() {
    ...
    final List<_FactRow> rows = <_FactRow>[
      ...
      _FactRow(
        'Locales',
        _dispatcher.locales.map((ui.Locale l) => l.toString()).join(', '),
      ),
      _FactRow('Text Scale', _dispatcher.textScaleFactor.toStringAsFixed(2)),
      _FactRow(
        'Platform Brightness',
        _dispatcher.platformBrightness.toString(),
      ),
      ...
      _FactRow(
        'Implicit View',
        _dispatcher.implicitView == null ? 'null' : 'present',
      ),
    ];
  ...
  Widget _topologyPanel() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          const Text(
            'View/display topology visualization',
            style: TextStyle(fontWeight: FontWeight.w700),
          ),
          const SizedBox(height: 8),
          SizedBox(
  ...
  Widget _viewsPanel() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          const Text(
            'FlutterView detail table',
            style: TextStyle(fontWeight: FontWeight.w700),
          ),
          const SizedBox(height: 8),
          if (views.isEmpty)
            const Text(
              'No views reported by PlatformDispatcher.',
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'id=${view.viewId} | size=${view.physicalSize.width.toStringAsFixed(0)}x${view.physicalSize.height.toStringAsFixed(0)}',
                    ),
                    Text(
                      'dpr=${view.devicePixelRatio.toStringAsFixed(2)} | display=${view.display.toString()}',
                    ),
                    Text(
                      'padding=${view.padding} | viewPadding=${view.viewPadding}',
                    ),
                    Text(
                      'viewInsets=${view.viewInsets} | systemGestureInsets=${view.systemGestureInsets}',
                    ),
                  ],
                ),
              );
        ...
        children: <Widget>[
          ...
          SizedBox(
            ...
            child: _timeline.isEmpty
                ? const Center(
                    child: Text(
                      'No snapshots yet. Use Capture Snapshot to build history.',
                      style: TextStyle(
                        fontSize: 12.2,
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Text(
                              '$time | views=${s.viewCount}, displays=${s.displayCount}, textScale=${s.textScaleFactor.toStringAsFixed(2)}',
                              style: const TextStyle(
                                fontSize: 12.2,
                                fontWeight: FontWeight.w600,
                              ),
                            ...
                            Text(
                              'locale=${s.localeLabel} | semantics=${s.semanticsEnabled} | 24h=${s.alwaysUse24HourFormat}',
                              style: const TextStyle(fontSize: 11.8),
                            ),
                            Text(
                              'brightness=${s.platformBrightness} | implicitView=${s.implicitViewPresent}',
                              style: const TextStyle(fontSize: 11.8),
                            ),
                            Text(
                            ...
                            Text(
                              'frame=${s.frameDataSummary}',
                              style: const TextStyle(fontSize: 11.8),
                            ),
                            if (_showAccessibilityFocus)
                              Text(
                                'locales=${s.localesLabel}',
                                style: const TextStyle(fontSize: 11.8),
                              ),
                          ],
                        ),
  ...
  Widget _guidancePanel() {
    final List<_FactRow> tips = <_FactRow>[
      const _FactRow(
        ...
        'Read PlatformDispatcher values at runtime instead of hard-coding environment assumptions.',
      ),
      const _FactRow(
        'Capture drift over time',
        'Snapshot timeline helps compare locale/text-scale/semantics changes across runs.',
      ),
      const _FactRow(
        'Treat views as dynamic',
        'Multi-view environments can modify available FlutterView entries.',
      ),
      const _FactRow(
        'Bridge UI policy carefully',
        'Use accessibility and 24-hour format signals to adapt UI behavior appropriately.',
      ),
    ];
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16),
    ...
    return Scaffold(
      ...
      body: ListView(
        children: <Widget>[
          ...
          _section(
            '1) Concept overview',
            'Core responsibilities of PlatformDispatcher.',
            Icons.menu_book,
            const Color(0xFF1D4ED8),
          ),
          _conceptCards(),
          _section(
            '2) Control studio',
            'Capture and compare platform snapshots.',
            Icons.tune,
            const Color(0xFF7C3AED),
          ),
          _controlPanel(),
          _section(
            '3) Current state',
            'Inspect immediate global runtime values.',
            Icons.list_alt,
            const Color(0xFF047857),
          ),
          _currentStatePanel(),
          _section(
            '4) Topology map',
            'Visualize active views and display relationships.',
            Icons.account_tree,
            const Color(0xFFB45309),
          ),
          _topologyPanel(),
          _section(
            '5) View details',
            'Inspect FlutterView geometry and insets.',
            Icons.view_quilt,
          ...
          _section(
            '6) Snapshot timeline',
            'Track runtime changes chronologically.',
            Icons.timeline,
            const Color(0xFF0EA5E9),
          ),
          _timelinePanel(),
          _section(
            '7) Probes and guidance',
            'Validate behavior and apply best practices.',
            Icons.fact_check,
            const Color(0xFF166534),
          ),
          _probePanel(),
...
class _TopologyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect root = Rect.fromCenter(
      center: Offset(size.width * 0.5, 46),
      width: nodeW,
      height: nodeH,
    );
    final Rect displays = Rect.fromCenter(
      center: Offset(size.width * 0.25, 148),
      width: nodeW,
      height: nodeH,
    );
    final Rect views = Rect.fromCenter(
      center: Offset(size.width * 0.75, 148),
      width: nodeW,
      height: nodeH,
    );
    ...
    void drawNode(Rect r, String text, Color c) {
      canvas.drawRRect(
        ...
        Paint()
          ..color = c.withAlpha((130 + 90 * pulse * pulseStrength).toInt()),
      );
    ...
    canvas.drawLine(
      root.bottomCenter,
      Offset(displays.center.dx, displays.top),
      link,
    );
    canvas.drawLine(
      root.bottomCenter,
      Offset(views.center.dx, views.top),
      link,
    );
