...
const List<_FaqItem> _faqItems = <_FaqItem>[
  _FaqItem(
    ...
    answer:
        'Use it when visuals depend on custom canvas drawing, procedural patterns, or direct path control.',
  ),
  _FaqItem(
    question: 'How do I animate a CustomPainter?',
    answer:
        'Pass a Listenable to repaint and read animation values inside paint().',
  ),
  _FaqItem(
    question: 'Should all paint values be recomputed every frame?',
    answer:
        'Only recompute what changes; cache expensive path data when possible.',
  ),
  _FaqItem(
    question: 'Can I combine background and foreground painters?',
    answer:
        'Yes, CustomPaint supports both and this is useful for overlays and guides.',
  ),
  _FaqItem(
...
enum _PainterMode { geometry, signal, pattern, radial, pathLab }
...
class _RenderCustomPaintStudio extends StatefulWidget {
  ...
  @override
  State<_RenderCustomPaintStudio> createState() =>
      _RenderCustomPaintStudioState();
}

class _RenderCustomPaintStudioState extends State<_RenderCustomPaintStudio>
  ...
  _PaintSnapshot _snapshot = const _PaintSnapshot(
    mode: 'geometry',
    complexity: 0,
    layers: 0,
  ...
  void _reset() {
    setState(() {
      ...
      _snapshot = const _PaintSnapshot(
        mode: 'geometry',
        complexity: 0,
        layers: 0,
        pointer: Offset(0.5, 0.5),
  ...
  List<_MetricEntry> _metrics() {
    return <_MetricEntry>[
      _MetricEntry(
        label: 'Mode',
        value: _mode.name,
        note: 'Active painter scenario.',
        icon: Icons.brush_outlined,
      ),
      _MetricEntry(
        label: 'Theme',
        value: _themePresets[_themeIndex].name,
        note: 'Color and brightness preset.',
        icon: Icons.palette_outlined,
      ),
      _MetricEntry(
        label: 'Canvas',
        value:
            '${_canvasWidth.toStringAsFixed(0)} x ${_canvasHeight.toStringAsFixed(0)}',
        note: 'Painting surface dimensions.',
        icon: Icons.crop_square_outlined,
      ),
      _MetricEntry(
        label: 'Stroke Scale',
        value: _strokeScale.toStringAsFixed(2),
        note: 'Line thickness multiplier.',
        icon: Icons.line_weight_outlined,
      ),
      _MetricEntry(
        label: 'Amplitude',
        value: _amplitude.toStringAsFixed(2),
        note: 'Signal wave height.',
        icon: Icons.show_chart_outlined,
      ),
      _MetricEntry(
        label: 'Frequency',
        value: _frequency.toStringAsFixed(2),
        note: 'Signal oscillation intensity.',
        icon: Icons.tune_outlined,
      ),
      _MetricEntry(
        label: 'Blend',
        value: _blend.toStringAsFixed(2),
        note: 'Foreground blend strength.',
        icon: Icons.filter_b_and_w_outlined,
      ),
      _MetricEntry(
        label: 'Roundness',
        value: _shapeRoundness.toStringAsFixed(2),
        note: 'Corner smoothing for geometry mode.',
        icon: Icons.rounded_corner_outlined,
      ),
      _MetricEntry(
        label: 'Path Warp',
        value: _pathWarp.toStringAsFixed(2),
        note: 'Path deformation strength.',
        icon: Icons.timeline_outlined,
      ),
      _MetricEntry(
        label: 'Noise',
        value: _noise.toStringAsFixed(2),
        note: 'Pattern variation intensity.',
        icon: Icons.grain_outlined,
      ),
      _MetricEntry(
        label: 'Guide Opacity',
        value: _guideOpacity.toStringAsFixed(2),
        note: 'Grid and axis overlay opacity.',
        icon: Icons.grid_on_outlined,
      ),
      _MetricEntry(
        label: 'Color Shift',
        value: _colorShift.toStringAsFixed(2),
        note: 'Palette hue offset over time.',
        icon: Icons.gradient_outlined,
      ),
      _MetricEntry(
        label: 'Mode Switches',
        value: '$_modeSwitches',
        note: 'Number of mode transitions.',
      ...
      _MetricEntry(
        label: 'Stage Taps',
        value: '$_tapCount',
        note: 'Pointer probes placed on stage.',
      ...
      _MetricEntry(
        label: 'Snapshot Complexity',
        value: '${_snapshot.complexity}',
        note: 'Current painter complexity estimate.',
        icon: Icons.analytics_outlined,
      ),
      _MetricEntry(
        label: 'Snapshot Layers',
        value: '${_snapshot.layers}',
        note: 'Estimated paint layers used.',
          ...
          children: <Widget>[
            Wrap(
              ...
              children: <Widget>[
                ...
                Text(
                  'RenderCustomPaint Painter Playground',
                  style: TextStyle(
              ...
              children: _PainterMode.values.map((_PainterMode mode) {
                return FilterChip(
                  ...
                  onSelected: (_) {
                    ...
                    _addTimeline(
                      'Mode',
                      'Painter mode switched to ${mode.name}.',
                    );
                  },
                );
              }).toList(),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Tune surface size, paint math, blend dynamics, and debugging overlays.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 10),
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Canvas Width',
                  'Set width to ${v.toStringAsFixed(0)}.',
                );
              },
            ),
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Canvas Height',
                  'Set height to ${v.toStringAsFixed(0)}.',
                );
              },
            ),
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Stroke Scale',
                  'Set stroke scale to ${v.toStringAsFixed(2)}.',
                );
              },
            ),
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Amplitude',
                  'Set amplitude to ${v.toStringAsFixed(2)}.',
                );
              },
            ),
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Frequency',
                  'Set frequency to ${v.toStringAsFixed(2)}.',
                );
              },
            ),
            _sliderRow(
            ...
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Roundness',
                  'Set roundness to ${v.toStringAsFixed(2)}.',
                );
              },
            ),
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Path Warp',
                  'Set path warp to ${v.toStringAsFixed(2)}.',
                );
              },
            ),
            _sliderRow(
            ...
            _sliderRow(
              ...
              onChangeEnd: (double v) {
                ...
                _addTimeline(
                  'Color Shift',
                  'Set color shift to ${v.toStringAsFixed(2)}.',
                );
              },
            ),
            Wrap(
              ...
              children: <Widget>[
                CheckboxMenuButton(
                  ...
                  onChanged: (bool? v) {
                    ...
                    _addTimeline(
                      'Animation',
                      next ? 'Animation enabled.' : 'Animation paused.',
                    );
                  },
                ...
                CheckboxMenuButton(
                  value: _showAxes,
                  onChanged: (bool? v) => _setToggle('axes', v),
                  child: const Text('Show axes'),
                ),
                CheckboxMenuButton(
                  value: _showForeground,
                  onChanged: (bool? v) => _setToggle('foreground', v),
                  child: const Text('Show foreground painter'),
                ),
                CheckboxMenuButton(
                  value: _showProbe,
                ...
                CheckboxMenuButton(
                  ...
                  child: const Text('Show diagnostics panel'),
                ),
                CheckboxMenuButton(
                  value: _showGuide,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Interactive CustomPaint stage with background scene and optional foreground overlays.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            ...
            Center(
              child: GestureDetector(
                onTapDown: (TapDownDetails details) {
                  ...
                  _addTimeline(
                    'Probe',
                    'Probe moved to (${probe.dx.toStringAsFixed(2)}, ${probe.dy.toStringAsFixed(2)}).',
                  );
                },
                child: SizedBox(
                  width: _canvasWidth,
                  ...
                  child: CustomPaint(
                    painter: _BackgroundScenePainter(
                      config: config,
                      onSnapshot: _setSnapshot,
                    ),
                    foregroundPainter: _showForeground
                        ? _ForegroundGuidePainter(config: config)
                        : null,
            ...
            if (_showLegend)
              Wrap(
                ...
                children: <Widget>[
                  ...
                  _legendChip(
                    'blend ${_blend.toStringAsFixed(2)}',
                    scheme.tertiary,
                  ),
                  _legendChip(
                    'freq ${_frequency.toStringAsFixed(2)}',
                    scheme.primary,
                  ),
                  _legendChip(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Small previews for each painter mode to compare composition style and complexity.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 12),
            LayoutBuilder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ...
                    return InkWell(
                      ...
                      onTap: () {
                        ...
                        _addTimeline(
                          'Mode Pick',
                          'Selected ${mode.name} from gallery.',
                        );
                      },
                      child: DecoratedBox(
                        decoration: BoxDecoration(
                              ...
                              Expanded(
                                child: ClipRRect(
                                  ...
                                  child: CustomPaint(
                                    painter: _BackgroundScenePainter(
                                      config: config,
                                      onSnapshot: _ignoreSnapshot,
                                    ),
                                    foregroundPainter: _showForeground
                                        ? _ForegroundGuidePainter(
                                            config: config,
                                          )
                                        : null,
                                    child: const SizedBox.expand(),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'How painter layering differs from pure widget composition and decoration-only styling.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 12),
            LayoutBuilder(
              builder: (BuildContext context, BoxConstraints constraints) {
                ...
                final Widget customPaintCard = _comparisonCard(
                  ...
                  title: 'CustomPaint',
                  subtitle:
                      'Canvas-level control over paths, blending, and draw order.',
                ...
                final Widget decoratedBoxCard = _comparisonCard(
                  ...
                  title: 'DecoratedBox',
                  subtitle:
                      'Great for static background decoration with lower complexity.',
                ...
                return Row(
                  children: <Widget>[
                    Expanded(child: customPaintCard),
                    const SizedBox(width: 10),
                    Expanded(child: decoratedBoxCard),
                    const SizedBox(width: 10),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'theme=${_themePresets[_themeIndex].id} mode=${_mode.name} phase=$_phase',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'canvas=${_canvasWidth.toStringAsFixed(0)}x${_canvasHeight.toStringAsFixed(0)} stroke=${_strokeScale.toStringAsFixed(2)}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'noise=${_noise.toStringAsFixed(2)} amp=${_amplitude.toStringAsFixed(2)} freq=${_frequency.toStringAsFixed(2)} blend=${_blend.toStringAsFixed(2)}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'warp=${_pathWarp.toStringAsFixed(2)} round=${_shapeRoundness.toStringAsFixed(2)} guide=${_guideOpacity.toStringAsFixed(2)} shift=${_colorShift.toStringAsFixed(2)}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'toggles grid=$_showGrid axes=$_showAxes fg=$_showForeground probe=$_showProbe legend=$_showLegend',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'switches mode=$_modeSwitches theme=$_themeSwitches controls=$_controlEdits taps=$_tapCount',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'snapshot mode=${_snapshot.mode} complexity=${_snapshot.complexity} layers=${_snapshot.layers} pointer=(${_snapshot.pointer.dx.toStringAsFixed(2)}, ${_snapshot.pointer.dy.toStringAsFixed(2)})',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Interaction history for mode changes, control updates, and probe placements.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 10),
...
class _BackgroundScenePainter extends CustomPainter {
    ...
    final Paint base = Paint()
      ..shader = LinearGradient(
        colors: <Color>[
          HSVColor.fromAHSV(
            1,
            180 + (config.colorShift * 120),
            0.65,
            0.74,
          ).toColor(),
          HSVColor.fromAHSV(
            1,
            260 + (config.colorShift * 120),
            0.55,
            0.78,
          ).toColor(),
          HSVColor.fromAHSV(
            1,
            20 + (config.colorShift * 100),
            0.60,
            0.82,
          ).toColor(),
        ],
  ...
  void _paintGeometry(Canvas canvas, Size size) {
    ...
    final double radius =
        math.min(size.width, size.height) * (0.14 + (config.amplitude * 0.22));
  ...
  void _paintSignal(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= points; i += 1) {
      ...
      final double yA =
          chart.center.dy +
          math.sin(
                (t * math.pi * 2 * config.frequency) +
                    (config.progress * math.pi * 2),
              ) *
              chart.height *
              0.36 *
              config.amplitude;
      final double yB =
          chart.center.dy +
          math.cos(
                (t * math.pi * 2 * (config.frequency + 0.35)) -
                    (config.progress * math.pi * 2),
              ) *
              chart.height *
              0.24 *
              (0.4 + config.amplitude * 0.6);
  ...
  void _paintPattern(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < count; i += 1) {
      ...
      final double x =
          (math.sin((t * 91) + (config.progress * math.pi * 2)) * 0.5 + 0.5) *
          size.width;
      final double y =
          (math.cos((t * 73) - (config.progress * math.pi * 2)) * 0.5 + 0.5) *
          size.height;
  ...
  void _paintRadial(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 7; i += 1) {
      ...
      final double radius =
          maxRadius * factor * (0.55 + (config.amplitude * 0.45));
      final Paint ring = Paint()
        ..style = PaintingStyle.stroke
        ..strokeWidth = (1.0 + (i * 0.9)) * config.strokeScale
        ..color = Colors.white.withValues(
          alpha: (0.32 - (factor * 0.20)).clamp(0.08, 0.35),
        );
  ...
  void paint(Canvas canvas, Size size) {
    if (config.showGrid) {
      final Paint grid = Paint()
        ..color = Colors.white.withValues(
          alpha: (0.25 * config.guideOpacity).clamp(0.04, 0.25),
        )
        ..strokeWidth = 1;
    ...
    if (config.showAxes) {
      final Paint axis = Paint()
        ..color = Colors.white.withValues(
          alpha: (0.6 * config.guideOpacity).clamp(0.10, 0.60),
        )
        ..strokeWidth = 1.3 * config.strokeScale;
    ...
    if (config.showProbe) {
      final Offset p = Offset(
        size.width * config.probe.dx,
        size.height * config.probe.dy,
      );
      final Paint ring = Paint()
