  ...
  child: Row(
    ...
    children: [
      ...
      Expanded(
        child: Text(
          t,
          style: const TextStyle(fontSize: 12.5, color: _txSteel, height: 1.4),
        ),
      ),
    ],
  ),
...
Widget _txPropCard(String name, String type, String desc, Color c) => Container(
  width: double.infinity,
  padding: const EdgeInsets.all(10),
...
Widget _txBanner() => Container(
  ...
  child: Column(
    children: [
      ...
      Text(
        'A RenderBox that displays a platform texture',
        textAlign: TextAlign.center,
        style: TextStyle(
          fontSize: 13,
    ...
    child: Column(
      children: [
        _txPropCard(
          'textureId',
          'int',
          'Engine-assigned ID identifying the native texture',
          _txBlue,
        ),
        _txPropCard(
          'freeze',
          'bool',
          'When true, paints the last frame instead of updating',
          _txOrange,
        ),
        _txPropCard(
          'filterQuality',
          'FilterQuality',
          'Scaling quality: none, low, medium, high',
          _txPurple,
        ),
      ],
    ),
  ...
  Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 10),
        _txQualRow('none', 'Nearest-neighbor', 'Fastest, pixelated', _txRed),
        _txQualRow('low', 'Bilinear', 'Default, good balance', _txOrange),
        _txQualRow(
          'medium',
          'Bilinear + mipmaps',
          'Smoother minification',
          _txBlue,
        ),
        _txQualRow('high', 'Bicubic', 'Smoothest, most expensive', _txGreen),
      ],
    ),
  ),
  _txCode(
    ...
    children: [
      ...
      ...List.generate(
        ...
        (i) => Container(
          ...
          decoration: BoxDecoration(
            color: i < perf ? _txGreen : _txSteel.withValues(alpha: 0.1),
      ...
      ...List.generate(
        ...
        (i) => Container(
          ...
          margin: const EdgeInsets.only(right: 2),
          decoration: BoxDecoration(
            color: i < qual ? _txBlue : _txSteel.withValues(alpha: 0.1),
            borderRadius: BorderRadius.circular(2),
          ),
        ),
      ),
...
Widget _txUseCase(IconData icon, String title, String desc, Color c) => Padding(
  padding: const EdgeInsets.symmetric(vertical: 4),
  child: Row(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
...
List<Widget> _txVsHybrid() => [
  ...
  Container(
    ...
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _txVsRow('Aspect', 'Texture (TextureBox)', 'Hybrid', isHeader: true),
        _txVsRow(
          'Rendering',
          'Offscreen, composited',
          'Native view in hierarchy',
        ),
        _txVsRow('Performance', 'Faster compositing', 'Slightly slower'),
        _txVsRow('Touch', 'Requires forwarding', 'Native touch handling'),
        _txVsRow(
          'Z-ordering',
          'Limited (always behind Flutter)',
          'Full native z-order',
        ),
        _txVsRow('Platform', 'Android + iOS + Web', 'Android + iOS'),
        _txVsRow(
          'Use case',
          'Video, camera, games',
    ...
    child: Row(
      ...
      children: [
        ...
        Expanded(
          child: Text(
            tex,
            style: style.copyWith(color: isHeader ? _txSteel : _txBlue),
          ),
        ),
        Expanded(
          child: Text(
            hyb,
            style: style.copyWith(color: isHeader ? _txSteel : _txOrange),
          ),
        ),
      ],
    ),
  ...
  Container(
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(
        colors: [_txSteel.withValues(alpha: 0.08), _txIce],
        begin: Alignment.topLeft,
        end: Alignment.bottomRight,
      ),
      borderRadius: BorderRadius.circular(12),
    ...
    child: Column(
      ...
      children: [
        ...
        _txSumPt(
          'textureId',
          'Integer ID linking to engine-registered '
              'native texture',
        ),
        _txSumPt('freeze', 'Boolean to pause updates and show last frame'),
        _txSumPt(
          'filterQuality',
          'Controls scaling interpolation '
              '(none/low/medium/high)',
        ),
        _txSumPt(
          'TextureLayer',
          'Paints via compositing layer, not '
              'canvas draw calls',
        ),
        _txSumPt(
          'sizedByParent',
          'Always fills parent constraints — '
              'wrap with SizedBox/AspectRatio',
        ),
        _txSumPt(
          'Texture widget',
          'Widget-level wrapper — preferred '
              'over using TextureBox directly',
        ),
        _txSumPt(
          'Use cases',
          'Video, camera, maps, WebView, games, '
              'any native surface',
        ),
      ],
    ),
