...
const List<_ThemePreset> _themePresets = <_ThemePreset>[
  _ThemePreset(
    ...
    name: 'Lagoon Lab',
    description:
        'Balanced profile for studying child overflow against parent constraints.',
  ...
  _ThemePreset(
    ...
    name: 'Ember Desk',
    description:
        'Warm profile for clip and edge intersection visibility checks.',
  ...
  _ThemePreset(
    ...
    name: 'Ocean Scope',
    description:
        'Cool profile for constraint geometry and alignment diagnostics.',
...
const List<_ScenarioLane> _scenarioLanes = <_ScenarioLane>[
  _ScenarioLane(
    ...
    subtitle:
        'Interactive parent-vs-child constraints with explicit overflow behavior.',
  ),
  _ScenarioLane(
    id: 'alignment',
    title: 'Alignment Explorer',
    subtitle:
        'Directional and anchored overflow behavior using alignment controls.',
  ),
  _ScenarioLane(
    id: 'fit',
    title: 'Fit and Limits',
    subtitle:
        'Custom overflow fit modes and min/max combinations across varying parent bounds.',
  ),
  _ScenarioLane(
    id: 'compare',
    title: 'Widget Comparison',
    subtitle:
        'OverflowBox vs SizedOverflowBox vs regular constrained layout behavior.',
  ),
  _ScenarioLane(
    id: 'ops',
    title: 'Ops Diagnostics',
    subtitle:
        'Metrics, snapshots, and timeline for reproducible overflow investigations.',
  ),
];
...
const List<_FaqItem> _faqItems = <_FaqItem>[
  _FaqItem(
    ...
    answer:
        'It lays out a child with adjusted constraints that can exceed the parent, enabling controlled visual overflow.',
  ),
  _FaqItem(
    question: 'Is overflow always visible?',
    answer:
        'Only if an ancestor does not clip the child; clipping strategy determines whether escaped regions are shown.',
  ),
  _FaqItem(
    question: 'How is it different from SizedOverflowBox?',
    answer:
        'SizedOverflowBox reports a fixed size while allowing a differently sized child; OverflowBox adjusts child constraints directly.',
  ),
  _FaqItem(
    question: 'When should I avoid it?',
    answer:
        'Avoid it for accidental layout fixes; prefer explicit constraints unless overflow is truly part of the design.',
  ),
  _FaqItem(
    question: 'How can I debug overflow intent?',
    answer:
        'Use overlays showing parent bounds, child bounds, and active clip settings to validate behavior quickly.',
  ),
];
...
const List<_DemoTileModel> _demoTiles = <_DemoTileModel>[
  _DemoTileModel(
    ...
    note: 'Center overflow with mild expansion',
  ),
  _DemoTileModel(
    label: 'Beacon',
    colorA: Color(0xFF22C55E),
    colorB: Color(0xFF16A34A),
    note: 'Right-anchored escape behavior',
  ),
  _DemoTileModel(
    label: 'Cinder',
    colorA: Color(0xFFF59E0B),
    colorB: Color(0xFFEF4444),
    note: 'Top spill with aggressive width',
  ),
  _DemoTileModel(
    label: 'Iris',
    colorA: Color(0xFF8B5CF6),
    colorB: Color(0xFF6366F1),
    note: 'Diagonal overflow under clip',
  ),
  _DemoTileModel(
    label: 'Rose',
    colorA: Color(0xFFF43F5E),
    colorB: Color(0xFFFB7185),
    note: 'Bottom offset with narrow parent',
  ),
  _DemoTileModel(
    label: 'Slate',
    colorA: Color(0xFF475569),
    colorB: Color(0xFF1E293B),
    note: 'Constrained limits stress-test',
  ),
];
...
enum _OverflowFitMode { loose, balanced, tight }
...
class _RenderConstrainedOverflowBoxStudio extends StatefulWidget {
  ...
  @override
  State<_RenderConstrainedOverflowBoxStudio> createState() =>
      _RenderConstrainedOverflowBoxStudioState();
}

class _RenderConstrainedOverflowBoxStudioState
    extends State<_RenderConstrainedOverflowBoxStudio>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _pushTimeline(
        'Init',
        'RenderConstrainedOverflowBox Constraint Escape Lab initialized.',
      );
    });
  }
  ...
  Size _sceneChildSize() {
    ...
    switch (_scene) {
      ...
      case _OverflowScene.leftSpill:
        return Size(
          _childWidth * 1.12 * animScale,
          (_childHeight * 0.86) * animScale,
        );
      case _OverflowScene.rightSpill:
        return Size(
          (_childWidth * 1.08) * animScale,
          (_childHeight * 0.92) * animScale,
        );
      case _OverflowScene.topSpill:
        return Size(
          (_childWidth * 0.92) * animScale,
          (_childHeight * 1.24) * animScale,
        );
      case _OverflowScene.bottomSpill:
        return Size(
          (_childWidth * 0.94) * animScale,
          (_childHeight * 1.18) * animScale,
        );
      case _OverflowScene.diagonal:
        return Size(
          (_childWidth * 1.18) * animScale,
          (_childHeight * 1.16) * animScale,
        );
      case _OverflowScene.dual:
        return Size(
          (_childWidth * 1.26) * animScale,
          (_childHeight * 1.12) * animScale,
        );
    }
  }
  ...
  List<_MetricEntry> _metrics() {
    ...
    return <_MetricEntry>[
      _MetricEntry(
        label: 'Scenario',
        value: _scenarioLanes[_scenarioIndex].title,
      ...
      _MetricEntry(
        label: 'Scene',
        value: _scene.name,
        note: 'Current overflow scene model.',
        icon: Icons.animation_outlined,
      ),
      _MetricEntry(
        label: 'Fit Mode',
        value: _fitMode.name,
        note: 'Effective scaling profile for min/max overrides.',
        icon: Icons.fit_screen_outlined,
      ),
      _MetricEntry(
        label: 'Alignment',
        value: _alignmentOptions
            .firstWhere((_AlignmentOption o) => o.value == _alignment)
            .label,
        note: 'Base alignment selection.',
      ...
      _MetricEntry(
        ...
        note: 'Parent container dimensions.',
        icon: Icons.crop_square_outlined,
      ),
      _MetricEntry(
        label: 'Child Size',
        value:
            '${child.width.toStringAsFixed(0)} x ${child.height.toStringAsFixed(0)}',
        note: 'Effective child dimensions in scene.',
        icon: Icons.open_in_full_outlined,
      ),
      _MetricEntry(
        label: 'Min Width',
        value: _useMinWidth ? _minWidth.toStringAsFixed(0) : 'off',
        note: 'minWidth override state.',
        icon: Icons.horizontal_rule_outlined,
      ),
      _MetricEntry(
        label: 'Max Width',
        value: _useMaxWidth ? _maxWidth.toStringAsFixed(0) : 'off',
        note: 'maxWidth override state.',
        icon: Icons.width_normal_outlined,
      ),
      _MetricEntry(
        label: 'Min Height',
        value: _useMinHeight ? _minHeight.toStringAsFixed(0) : 'off',
        note: 'minHeight override state.',
        icon: Icons.vertical_align_top_outlined,
      ),
      _MetricEntry(
        label: 'Max Height',
        value: _useMaxHeight ? _maxHeight.toStringAsFixed(0) : 'off',
        note: 'maxHeight override state.',
        icon: Icons.height_outlined,
      ),
      _MetricEntry(
        label: 'Clip Parent',
        value: _showClip ? 'enabled' : 'disabled',
        note: 'Parent clipping visibility mode.',
        icon: Icons.content_cut_outlined,
      ),
      _MetricEntry(
        label: 'Scene Switches',
        value: '$_sceneSwitchCount',
        note: 'Scene mode changes.',
        icon: Icons.swap_horiz_outlined,
      ),
      _MetricEntry(
        label: 'Fit Switches',
        value: '$_fitSwitchCount',
        note: 'Overflow fit mode changes.',
        icon: Icons.change_circle_outlined,
      ),
      _MetricEntry(
        label: 'Alignment Switches',
        value: '$_alignmentSwitchCount',
        note: 'Alignment changes.',
        icon: Icons.align_horizontal_left_outlined,
      ),
      _MetricEntry(
        label: 'Hero Taps',
        value: '$_heroTapCount',
        note: 'Primary stage interaction count.',
        icon: Icons.touch_app_outlined,
      ),
      _MetricEntry(
        label: 'Tile Taps',
        value: '$_tileTapCount',
        note: 'Gallery interaction count.',
        icon: Icons.grid_view_outlined,
      ),
      _MetricEntry(
        label: 'Phase',
        value: _phase,
        note: 'Current interaction phase label.',
        icon: Icons.flag_outlined,
      ),
      _MetricEntry(
        label: 'Last Tile',
        value: _lastTile,
        note: 'Most recently selected gallery tile.',
          ...
          children: <Widget>[
            Wrap(
              ...
              children: <Widget>[
                ...
                Text(
                  'RenderConstrainedOverflowBox Constraint Escape Lab',
                  style: TextStyle(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              _scenarioLanes[_scenarioIndex].subtitle,
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Tune parent and child geometry, overflow fit strategy, and optional min/max limit overrides.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
              ...
              children: _OverflowFitMode.values.map((_OverflowFitMode fit) {
                return ChoiceChip(
                  ...
                  onSelected: (_) {
                    ...
                    _pushTimeline(
                      'Fit',
                      'Overflow fit mode changed to ${fit.name}.',
                    );
                  },
                );
              }).toList(),
                ...
                Expanded(
                  child: DropdownButtonFormField<Alignment>(
                    ...
                    items: _alignmentOptions
                        .map(
                          (_AlignmentOption opt) => DropdownMenuItem<Alignment>(
                            value: opt.value,
                    ...
                    onChanged: (Alignment? next) {
                      ...
                      _pushTimeline(
                        'Alignment',
                        'Alignment changed to ${_alignmentOptions.firstWhere((_AlignmentOption o) => o.value == next).label}.',
                      );
                    },
                  ),
                ),
            ...
            _sliderRow(
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'Parent Width',
                'Parent width set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'Parent Height',
                'Parent height set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'Child Width',
                'Child width set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'Child Height',
                'Child height set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            const SizedBox(height: 8),
            Wrap(
              ...
              children: <Widget>[
                CheckboxMenuButton(
                  value: _useMinWidth,
                  onChanged: (bool? v) =>
                      setState(() => _useMinWidth = v ?? false),
                  child: const Text('Enable minWidth'),
                ),
                CheckboxMenuButton(
                  value: _useMaxWidth,
                  onChanged: (bool? v) =>
                      setState(() => _useMaxWidth = v ?? false),
                  child: const Text('Enable maxWidth'),
                ),
                CheckboxMenuButton(
                  value: _useMinHeight,
                  onChanged: (bool? v) =>
                      setState(() => _useMinHeight = v ?? false),
                  child: const Text('Enable minHeight'),
                ),
                CheckboxMenuButton(
                  value: _useMaxHeight,
                  onChanged: (bool? v) =>
                      setState(() => _useMaxHeight = v ?? false),
                  child: const Text('Enable maxHeight'),
                ),
              ],
            ),
            _sliderRow(
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'minWidth',
                'minWidth set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'maxWidth',
                'maxWidth set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'minHeight',
                'minHeight set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'maxHeight',
                'maxHeight set to ${v.toStringAsFixed(0)}.',
              ),
            ),
            _sliderRow(
              scheme: scheme,
              ...
              onChangeEnd: (double v) => _pushTimeline(
                'Backdrop',
                'Overlay density set to ${v.toStringAsFixed(2)}.',
              ),
            ),
            const SizedBox(height: 8),
            Wrap(
              ...
              children: <Widget>[
                ...
                CheckboxMenuButton(
                  ...
                  child: const Text('Show bounds overlays'),
                ),
                CheckboxMenuButton(
                  value: _showClip,
                  onChanged: (bool? v) =>
                      setState(() => _showClip = v ?? false),
                  child: const Text('Clip parent'),
                ),
                CheckboxMenuButton(
                  value: _animateChild,
                  onChanged: (bool? v) =>
                      setState(() => _animateChild = v ?? false),
                  child: const Text('Animate child size'),
                ),
                CheckboxMenuButton(
            ...
            Center(
              child: GestureDetector(
                onTap: () {
                  ...
                  _pushTimeline(
                    'Hero Tap',
                    'Hero stage tapped in ${_scene.name} scene.',
                  );
                },
                child: OverflowBox(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Primary visual board for parent constraints, child overflow, alignment, and clip decisions.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 12),
            Center(child: stage),
            ...
            Wrap(
              ...
              children: <Widget>[
                ...
                _pill(
                  'parent: ${_parentWidth.toStringAsFixed(0)}x${_parentHeight.toStringAsFixed(0)}',
                ),
                _pill(
                  'child: ${childSize.width.toStringAsFixed(0)}x${childSize.height.toStringAsFixed(0)}',
                ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Multiple illustrative use cases of intentional overflow under different scene presets.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 12),
            LayoutBuilder(
  ...
  Widget _sceneTile(ColorScheme scheme, _DemoTileModel tile, int index) {
    final _OverflowScene localScene =
        _OverflowScene.values[index % _OverflowScene.values.length];
    final Alignment localAlignment =
        _alignmentOptions[index % _alignmentOptions.length].value;
            ...
            Expanded(
              child: GestureDetector(
                onTap: () {
                  ...
                  _pushTimeline(
                    'Tile Tap',
                    'Tapped gallery tile ${tile.label} (${localScene.name}).',
                  );
                },
                child: DecoratedBox(
                  decoration: BoxDecoration(
                    ...
                    children: <Widget>[
                      Center(
                        child: OverflowBox(
                          ...
                          minWidth: _useMinWidth
                              ? (_minWidth * 0.6) * _fitScale()
                              : null,
                          maxWidth: _useMaxWidth
                              ? (_maxWidth * 0.7) * _fitScale()
                              : null,
                          minHeight: _useMinHeight
                              ? (_minHeight * 0.6) * _fitScale()
                              : null,
                          maxHeight: _useMaxHeight
                              ? (_maxHeight * 0.7) * _fitScale()
                              : null,
                          child: Container(
                        ...
                        IgnorePointer(
                          child: CustomPaint(
                            painter: _BoundsPainter(
                              ...
                              childColor: scheme.primary.withValues(
                                alpha: 0.70,
                              ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Contrast intentional overflow behavior with neighboring constraint widgets.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 12),
            LayoutBuilder(
              builder: (BuildContext context, BoxConstraints constraints) {
                ...
                final Widget overflow = _comparisonCard(
                  ...
                  title: 'OverflowBox',
                  subtitle:
                      'Child can exceed parent constraints via min/max and fit.',
                  color: const Color(0xFF0F766E),
                  child: OverflowBox(
                    alignment: _sceneAlignment(),
                    minWidth: _useMinWidth
                        ? (_minWidth * 0.5) * _fitScale()
                        : null,
                    maxWidth: _useMaxWidth
                        ? (_maxWidth * 0.6) * _fitScale()
                        : null,
                    minHeight: _useMinHeight
                        ? (_minHeight * 0.5) * _fitScale()
                        : null,
                    maxHeight: _useMaxHeight
                        ? (_maxHeight * 0.6) * _fitScale()
                        : null,
                    child: Container(
                      width: 190,
                      height: 130,
                ...
                final Widget sizedOverflow = _comparisonCard(
                  ...
                  subtitle:
                      'Reports fixed size but allows child to paint larger.',
                ...
                final Widget constrained = _comparisonCard(
                  ...
                  subtitle:
                      'Strictly applies constraints without overflow escape behavior.',
                ...
                return Row(
                  children: <Widget>[
                    Expanded(child: overflow),
                    const SizedBox(width: 10),
                    Expanded(child: sizedOverflow),
                    const SizedBox(width: 10),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'theme=${_themePresets[_themeIndex].id} scenario=${_scenarioLanes[_scenarioIndex].id} scene=${_scene.name}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'fitMode=${_fitMode.name} alignment=${_alignmentOptions.firstWhere((_AlignmentOption o) => o.value == _alignment).label}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'parent=${_parentWidth.toStringAsFixed(0)}x${_parentHeight.toStringAsFixed(0)} child=${child.width.toStringAsFixed(0)}x${child.height.toStringAsFixed(0)}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'minW=${_useMinWidth ? _minWidth.toStringAsFixed(0) : 'off'} maxW=${_useMaxWidth ? _maxWidth.toStringAsFixed(0) : 'off'} minH=${_useMinHeight ? _minHeight.toStringAsFixed(0) : 'off'} maxH=${_useMaxHeight ? _maxHeight.toStringAsFixed(0) : 'off'}',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'clip=$_showClip bounds=$_showBounds grid=$_showGrid animateChild=$_animateChild',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'sceneSwitch=$_sceneSwitchCount fitSwitch=$_fitSwitchCount alignSwitch=$_alignmentSwitchCount themeSwitch=$_themeSwitchCount',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            Text(
              'heroTap=$_heroTapCount tileTap=$_tileTapCount phase=$_phase lastTile=$_lastTile',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 8),
            Text(
              'Chronological log of scene changes, fit toggles, and interaction events.',
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            const SizedBox(height: 10),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 8),
                Text(
                  'Child size ${width.toStringAsFixed(0)} x ${height.toStringAsFixed(0)}',
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint wash = Paint()
      ..color = Colors.black.withValues(alpha: overlayDensity * 0.40);
  ...
  bool shouldRepaint(covariant _GridPainter oldDelegate) {
    return oldDelegate.progress != progress ||
        oldDelegate.overlayDensity != overlayDensity;
  }
}
...
class _BoundsPainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _BoundsPainter oldDelegate) {
    return oldDelegate.parentColor != parentColor ||
