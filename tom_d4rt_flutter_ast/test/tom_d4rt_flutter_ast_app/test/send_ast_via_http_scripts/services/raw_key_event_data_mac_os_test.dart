...
Widget _buildMacOsKeyboardArchitecture() {
  ...
  return _rmSection('macOS Keyboard Architecture', [
    ...
    _buildArchLayer(
      1,
      'Hardware (IOKit HID)',
      'Physical key press generates USB HID scancode',
      Icons.keyboard,
      _rmDarkSlate,
    ),
    _buildArchArrow(),
    _buildArchLayer(
      2,
      'IOHIDSystem',
      'macOS kernel converts HID to IOKit event',
      Icons.memory,
      _rmCharcoal,
    ),
    _buildArchArrow(),
    _buildArchLayer(
      3,
      'WindowServer',
      'Routes raw event to the frontmost application',
      Icons.window,
      _rmMidGray,
    ),
    _buildArchArrow(),
    _buildArchLayer(
      4,
      'NSEvent',
      'Cocoa framework wraps event with keyCode, characters, modifiers',
      Icons.event,
      _rmAccentBlue,
    ),
    _buildArchArrow(),
    _buildArchLayer(
      ...
      'Flutter engine\'s macOS embedder receives NSEvent',
      Icons.extension,
      _rmAccentGreen,
    ),
    _buildArchArrow(),
    _buildArchLayer(
      6,
      'Platform Channel',
      'Event data serialized and sent to Dart side',
    ...
    _buildArchLayer(
      7,
      'RawKeyEventDataMacOs',
      'Dart object with all macOS-specific fields',
      Icons.data_object,
      _rmCharcoal,
    ),
    _rmDivider(),
...
Widget _buildEventDataFields() {
  ...
  return _rmSection('Event Data Fields', [
    ...
    _rmInfoRow(
      'logicalKey',
      'LogicalKeyboardKey — layout-dependent key identity',
    ),
    _rmInfoRow(
      'physicalKey',
      'PhysicalKeyboardKey — layout-independent key identity',
    ),
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(desc, style: const TextStyle(color: _rmMidGray, fontSize: 11.5)),
      ],
    ),
  );
}
...
Widget _buildFunctionKeyHandling() {
  ...
  return _rmSection('Function Key Handling', [
    ...
    _rmInfoRow(
      'fn flag in modifiers',
      'NSEventModifierFlagFunction bit is set',
    ),
    _rmInfoRow(
      'Special keys',
      'Some keys always need fn (Home, End, PageUp/Down)',
    ),
  ]);
}
...
Widget _buildDeadKeysAndIME() {
  ...
  return _rmSection('Dead Keys & Input Methods', [
    ...
    _buildIMEStage(
      'Candidates',
      'System shows possible characters',
      _rmAccentPurple,
    ),
...
Widget _buildPlatformChannelIntegration() {
  ...
  return _rmSection('Platform Channel Integration', [
    ...
    _rmInfoRow(
      '2. Serialize',
      'Converts NSEvent fields to Map<String, dynamic>',
    ),
...
Widget _buildKeyEventMigration() {
  ...
  return _rmSection('Migration: RawKeyEvent → KeyEvent', [
    ...
    _rmInfoRow(
      'Platform data',
      'Old: platform-specific subclasses\nNew: unified API',
    ),
    _rmInfoRow(
      'Repeat events',
      'Old: RawKeyDownEvent with isRepeat\nNew: dedicated KeyRepeatEvent',
    ),
    _rmInfoRow(
      'Synchronization',
      'Old: can desync on focus loss\nNew: auto-syncs modifier state',
    ),
    _rmInfoRow(
      'IME handling',
      'Old: receives raw composition events\nNew: cleaner IME integration',
    ),
    _rmDivider(),
    ...
    Container(
      ...
      child: Column(
        ...
        children: [
          _buildMigrationStep(
            1,
            'Replace RawKeyboard.instance.addListener with '
            'HardwareKeyboard.instance.addHandler',
            _rmCharcoal,
          ),
          _buildMigrationStep(
            2,
            'Replace RawKeyboardListener widget with '
            'KeyboardListener widget',
            _rmCharcoal,
          ),
          _buildMigrationStep(
            3,
            'Replace platform-specific data access '
            '(event.data as RawKeyEventDataMacOs) with '
            'event.logicalKey / event.physicalKey',
            _rmCharcoal,
          ),
          _buildMigrationStep(
            4,
            'Update modifier checking from '
            'isModifierPressed to HardwareKeyboard.instance.isLogicalKeyPressed',
            _rmCharcoal,
          ),
          _buildMigrationStep(
            5,
            'Test on all platforms to verify behavior parity',
            _rmCharcoal,
          ),
        ],
      ),
    ...
    Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _rmBody(
            '✦ Fields: keyCode, characters, charactersIgnoringModifiers, modifiers',
          ),
          _rmBody('✦ keyCode is layout-independent (physical position)'),
          _rmBody(
            '✦ characters respects modifiers; ...IgnoringModifiers does not',
          ),
          _rmBody(
            '✦ Modifier bitmask from NSEventModifierFlags (⌘ ⌥ ⇧ ⌃ fn CapsLock)',
          ),
