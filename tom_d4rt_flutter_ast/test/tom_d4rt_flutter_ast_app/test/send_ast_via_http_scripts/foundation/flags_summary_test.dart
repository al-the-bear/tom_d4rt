...
dynamic build(BuildContext context) {
    ...
    child: Column(
      children: [
        ...
        Container(
          ...
          child: Text(
            'extends DiagnosticsProperty<Map<String, T?>>',
            style: TextStyle(
              fontSize: 11.0,
              fontWeight: FontWeight.bold,
              color: Colors.white,
  ...
  final fsBoolFlags = FlagsSummary<bool>('widgetState', <String, bool?>{
    'enabled': true,
    'visible': true,
    'selected': false,
    'focused': true,
  ...
  final fsNonNull = fsBoolFlags.value.entries
      .where((e) => e.value != null)
      .toList();
  final fsNullEntries = fsBoolFlags.value.entries
      .where((e) => e.value == null)
      .toList();
  ...
  Widget fsFilterSection = Container(
    ...
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 4.0),
        Text(
          'FlagsSummary only displays entries where value != null. Null entries are silently excluded.',
  ...
  print(
    'Empty: level=${fsEmpty.level}, isHidden=${fsEmpty.level == DiagnosticLevel.hidden}',
  );
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              SizedBox(height: 4.0),
              Text(
                'Automatically filtered from toString output',
                style: TextStyle(
                  fontSize: 10.5,
  ...
  final fsAllNull = FlagsSummary<int>('nullFlags', <String, int?>{
    'a': null,
    'b': null,
    'c': null,
  });
  ...
  final fsWithIfEmpty = FlagsSummary<int>('nullFlags', <String, int?>{
    'a': null,
    'b': null,
  ...
  Widget fsIfEmptySection = Container(
    ...
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 4.0),
        Text(
          'Customizes what appears when no flags have non-null values:',
          style: TextStyle(fontSize: 11.5, color: Colors.grey.shade700),
        ),
        SizedBox(height: 10.0),
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8.0),
              Text(
                'With ifEmpty: "no flags set":',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11.0),
              ),
              Text(
                'valueToString → "${fsWithIfEmpty.valueToString()}"',
                style: TextStyle(
                  fontFamily: 'monospace',
                  fontSize: 11.0,
                  color: Colors.indigo,
  ...
  final fsIntFlags = FlagsSummary<int>('metrics', <String, int?>{
    'width': 100,
    'height': 200,
    'depth': null,
    'layers': 3,
  ...
  final fsStringFlags = FlagsSummary<String>('labels', <String, String?>{
    'title': 'Hello',
    'subtitle': null,
    'footer': 'World',
  });
  ...
  final fsDebugFlags = FlagsSummary<bool>('debugFlags', {
    'verbose': true,
  }, level: DiagnosticLevel.debug);
  final fsWarningFlags = FlagsSummary<bool>('warnFlags', {
    'overflow': true,
  ...
  final fsShowName = FlagsSummary<bool>(
    'flags',
    {'a': true},
    showName: true,
    showSeparator: true,
  ...
  Widget fsJsonSection = Container(
    ...
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 4.0),
        Text(
          'Serializes flag data for DevTools and debugging tools:',
          style: TextStyle(fontSize: 11.5, color: Colors.grey.shade700),
        ),
        SizedBox(height: 10.0),
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              ...
              fontSize: 12.0,
              color: current ? color : Colors.grey.shade700,
            ),
          ),
          SizedBox(width: 6.0),
          Text(
        ...
        child: Column(
          ...
          children: [
            ...
            fsAnatomyRow(
              'FlagsSummary()',
              'name, Map<String, T?>, ...',
              'Constructor taking a name and map of flags',
              Color(0xFFD81B60),
            ),
            fsAnatomyRow(
              '.value',
              'Map<String, T?>',
              'The raw flag map (includes null entries)',
              Color(0xFFD81B60),
            ),
            fsAnatomyRow(
              '.valueToString()',
              'String',
              'Comma-separated non-null flag names',
              Color(0xFFD81B60),
            ),
            fsAnatomyRow(
              '.level',
              'DiagnosticLevel',
              'Hidden if empty/all-null, else user-specified',
              Color(0xFFD81B60),
            ),
            fsAnatomyRow(
              '.ifEmpty',
              'String?',
              'Custom text when no flags are non-null',
              Color(0xFFD81B60),
            ),
            fsAnatomyRow(
              '.toJsonMap()',
              'Map<String, Object?>',
              'Serialization for DevTools',
              Color(0xFFD81B60),
            ),
            ...
            SizedBox(height: 4.0),
            Text(
              'Widget state flags — the most common use case:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            Wrap(
              children: fsBoolFlags.value.entries
                  .map((e) => fsFlagChip(e.key, e.value, Color(0xFFD81B60)))
                  .toList(),
            ),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Map: {a: null, b: null, c: null}',
                    style: TextStyle(fontFamily: 'monospace', fontSize: 11.0),
                  ),
                  SizedBox(height: 4.0),
                  Text(
                    'Level: ${fsAllNull.level.name}',
                    style: TextStyle(
                      fontFamily: 'monospace',
                      fontSize: 11.0,
                      color: Colors.grey.shade600,
                  ...
                  Text(
                    'Treated same as empty — all filtered away',
                    style: TextStyle(
                      fontSize: 10.5,
                      fontStyle: FontStyle.italic,
                      color: Colors.grey.shade600,
            ...
            SizedBox(height: 8.0),
            Wrap(
              children: fsIntFlags.value.entries
                  .map((e) => fsFlagChip(e.key, e.value, Color(0xFF1565C0)))
                  .toList(),
            ),
            SizedBox(height: 6.0),
            Text(
              'Output: ${fsIntFlags.valueToString()}',
              style: TextStyle(
                fontFamily: 'monospace',
                fontSize: 11.0,
                color: Colors.grey.shade700,
            ...
            SizedBox(height: 8.0),
            Wrap(
              children: fsStringFlags.value.entries
                  .map((e) => fsFlagChip(e.key, e.value, Color(0xFF2E7D32)))
                  .toList(),
            ),
            SizedBox(height: 6.0),
            Text(
              'Output: ${fsStringFlags.valueToString()}',
              style: TextStyle(
                fontFamily: 'monospace',
                fontSize: 11.0,
                color: Colors.grey.shade700,
            ...
            SizedBox(height: 4.0),
            Text(
              'Override the diagnostic level for different severity contexts:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'showName: true, showSeparator: true',
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      fontSize: 11.0,
                    ),
            ...
            SizedBox(height: 4.0),
            Text(
              'Where FlagsSummary appears in the framework:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            fsUseCaseRow(
              'RenderBox',
              'needsLayout, needsPaint, needsCompositing',
              Icons.crop_square,
              Color(0xFFD81B60),
            ),
            fsUseCaseRow(
              'RenderObject',
              'sizedByParent, debugNeedsLayout, debugDoingThisResize',
              Icons.account_tree,
              Color(0xFFD81B60),
            ),
            fsUseCaseRow(
              'GestureRecognizer',
              'drag flags: down, start, update, end',
              Icons.touch_app,
              Color(0xFFD81B60),
            ),
            fsUseCaseRow(
              'ScrollPosition',
              'atEdge, outOfRange, keepScrollOffset',
              Icons.swap_vert,
              Color(0xFFD81B60),
            ),
            fsUseCaseRow(
              'FocusNode',
              'hasFocus, hasPrimaryFocus, canRequestFocus',
            ...
            Wrap(
              children: [
                ...
                fsSummaryTile(
                  'Bool Flags',
                  '${fsBoolFlags.value.length}',
                  Color(0xFFE3F2FD),
                  Colors.blue.shade700,
                ),
                fsSummaryTile(
                  'Int Flags',
                  '${fsIntFlags.value.length}',
                  Color(0xFFF3E5F5),
