...
dynamic build(BuildContext context) {
  ...
  final dlLevels = <Map<String, dynamic>>[
    {
      ...
      'purpose': 'Never shown in output',
      'usage': 'Filtered away at all levels',
    },
    {
      'level': DiagnosticLevel.fine,
      'color': Color(0xFF78909C),
      'icon': Icons.tune,
      'purpose': 'Low-value that matches defaults',
      'usage': 'Properties that equal their default value',
    },
    {
      'level': DiagnosticLevel.debug,
      'color': Color(0xFF42A5F5),
      'icon': Icons.bug_report,
      'purpose': 'Fine-grained debug info',
      'usage': 'Verbose details for investigating issues',
    },
    {
      'level': DiagnosticLevel.info,
      'color': Color(0xFF66BB6A),
      'icon': Icons.info,
      'purpose': 'Important diagnostic to show',
      'usage': 'Main informational properties of a widget',
    },
    {
      'level': DiagnosticLevel.warning,
      'color': Color(0xFFFFA726),
      'icon': Icons.warning,
      'purpose': 'Potentially problematic values',
      'usage': 'Properties with unexpected or risky values',
    },
    {
      'level': DiagnosticLevel.hint,
      'color': Color(0xFF26C6DA),
      'icon': Icons.lightbulb,
      'purpose': 'Best-practice suggestion',
      'usage': 'Suggestions to improve widget configuration',
    },
    {
      'level': DiagnosticLevel.summary,
      'color': Color(0xFF7E57C2),
      'icon': Icons.summarize,
      'purpose': 'Summarizes other diagnostics',
      'usage': 'Compact child that summarises sub-tree details',
    },
    {
      'level': DiagnosticLevel.error,
      'color': Color(0xFFEF5350),
      'icon': Icons.error,
      'purpose': 'Error or unexpected condition',
      'usage': 'Critical issues like constraint violations',
    },
    {
      'level': DiagnosticLevel.off,
      'color': Color(0xFF424242),
      'icon': Icons.block,
      'purpose': 'Sentinel to show nothing',
      'usage': 'Used as filter level to suppress all diagnostics',
    },
  ];
    ...
    child: Column(
      children: [
        ...
        Container(
          ...
          child: Text(
            '9 severity levels for diagnostic filtering',
            style: TextStyle(
              fontSize: 12.0,
              fontWeight: FontWeight.bold,
              color: Colors.white,
  ...
  Widget dlSpectrumBar = Container(
    ...
    child: Column(
      ...
      children: [
        ...
        SizedBox(height: 4.0),
        Text(
          'Levels are ordered by severity index (0=lowest, 8=highest):',
  ...
  Widget dlFilterDemo(DiagnosticLevel minLevel) {
    final shown = DiagnosticLevel.values
        .where((l) => l.index >= minLevel.index && l != DiagnosticLevel.off)
        .toList();
    final hidden = DiagnosticLevel.values
        .where((l) => l.index < minLevel.index || l == DiagnosticLevel.off)
        .toList();
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Container(
                padding: EdgeInsets.symmetric(horizontal: 6.0, vertical: 2.0),
                decoration: BoxDecoration(
                  color: Color(0xFFF57F17).withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(4.0),
                ),
                child: Text(
                  minLevel.name,
          ...
          SizedBox(height: 4.0),
          Text(
            'Showing ${shown.length} / ${DiagnosticLevel.values.length} levels',
  ...
  final dlInfoProp = StringProperty(
    'name',
    'DiagnosticLevel Demo',
    level: DiagnosticLevel.info,
  );
  final dlDebugProp = IntProperty('count', 42, level: DiagnosticLevel.debug);
  final dlWarningProp = StringProperty(
    'status',
    'deprecated widget used',
    level: DiagnosticLevel.warning,
  );
  final dlHintProp = MessageProperty(
    'tip',
    'Consider using Container instead',
    level: DiagnosticLevel.hint,
  );
  final dlErrorProp = StringProperty(
    'error',
    'Constraints violated',
    level: DiagnosticLevel.error,
  );
  final dlHiddenProp = StringProperty(
    'internal',
    'not for display',
    level: DiagnosticLevel.hidden,
  );
  final dlFineProp = StringProperty(
    'alignment',
    'Alignment.center',
    level: DiagnosticLevel.fine,
  );
  ...
  for (final prop in dlDiagProps) {
    print(
      '  ${prop.name}: level=${prop.level.name}, value=${prop.toStringDeep().trim()}',
    );
  }
  ...
  print(
    'error > warning: ${DiagnosticLevel.error.index > DiagnosticLevel.warning.index}',
  );
  print(
    'info > debug: ${DiagnosticLevel.info.index > DiagnosticLevel.debug.index}',
  );

  Widget dlComparisonRow(
    DiagnosticLevel left,
    DiagnosticLevel right,
    String op,
  ) {
    final result = op == '>'
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 4.0),
            Text(
              'Each level serves a distinct role in the diagnostics pipeline:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            dlLevelCard(
            ...
            SizedBox(height: 4.0),
            Text(
              'When toStringDeep(minLevel: X) is called, only levels with index >= X are included:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            ...
            SizedBox(height: 20.0),
            Text(
              '14. Live DiagnosticsNode Properties',
              style: TextStyle(
                fontSize: 18.0,
                fontWeight: FontWeight.bold,
                color: Color(0xFFF57F17),
            ...
            SizedBox(height: 4.0),
            Text(
              'Real DiagnosticsProperty instances with different levels:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            ...
            SizedBox(height: 4.0),
            Text(
              'Severity is determined by comparing index values:',
              style: TextStyle(fontSize: 12.0, color: Colors.grey.shade700),
            ),
            SizedBox(height: 8.0),
            dlComparisonRow(
            ...
            Wrap(
              children: [
                dlSummaryTile(
                  'Values',
                  '${DiagnosticLevel.values.length}',
                  Color(0xFFFFF8E1),
                  Color(0xFFF57F17),
                ),
                dlSummaryTile(
                  'First',
                  DiagnosticLevel.values.first.name,
                  Color(0xFFECEFF1),
                  Color(0xFF616161),
                ),
                dlSummaryTile(
                  'Last',
                  DiagnosticLevel.values.last.name,
                  Color(0xFFFBE9E7),
                  Color(0xFFBF360C),
                ),
                dlSummaryTile(
