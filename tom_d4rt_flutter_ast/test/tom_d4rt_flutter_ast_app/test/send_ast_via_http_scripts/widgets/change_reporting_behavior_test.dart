...
Widget _crBody(String text) {
  return Padding(
    ...
    child: Text(
      text,
      style: const TextStyle(color: _crDarkText, fontSize: 12.5, height: 1.5),
    ),
  );
}
        ...
        child: Column(
          children: [
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _crChip(
                        'Notify',
                        _crWhite.withValues(alpha: 0.25),
                        _crWhite,
                      ),
                      _crChip(
                        'Silent',
            ...
            _crSection('3 · Notification Flow Diagram', [
              _crBody('How a mutation flows through the reporting pipeline:'),
            ...
            _crSection('12 · Summary', [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: const LinearGradient(colors: [_crSage, _crMedSage]),
                ...
                child: Column(
                  children: [
                    _crSummaryRow(
                      ...
                      'Always mode: immediate rebuild on every change',
                    ),
                    _crSummaryRow(
                      Icons.notifications_off,
                      'Silent mode: mutate without triggering rebuilds',
                    ),
                    _crSummaryRow(
                      Icons.filter_alt,
                      'Conditional mode: only notify when value differs',
                    ),
                    _crSummaryRow(
                      Icons.batch_prediction,
                      'Batch updates: silent → mutate → notify once',
                    ),
                    _crSummaryRow(
                      Icons.speed,
                      'Performance: reduce unnecessary widget rebuilds',
                    ),
                    _crSummaryRow(
                      Icons.architecture,
                      'Architecture: separate mutation from notification',
                    ),
                  ],
                ),
              ),
  ...
  final modes = <Map<String, dynamic>>[
    {
      'mode': 'Always',
      'desc':
          'Every mutation fires notifyListeners(). Simplest, '
    ...
    },
    {
      'mode': 'Silent',
      'desc':
          'Mutations update the value but skip notifications. '
    ...
    },
    {
      'mode': 'Conditional',
      'desc':
          'Only notifies when the new value differs from the '
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(m['icon'] as IconData, color: _crWhite, size: 18),
              ),
...
Widget _buildRebuildComparison() {
  return Container(
    ...
    child: Row(
      children: [
        Expanded(
          child: _crComparisonCard('Always Mode', '3 mutations → 3 rebuilds', [
            'set name → rebuild',
            'set age → rebuild',
            'set email → rebuild',
          ], _crAccentOrange),
  ...
  final reasons = <Map<String, dynamic>>[
    {
      ...
      'desc': 'Set 5 fields, notify once',
      'icon': Icons.batch_prediction,
      'color': _crAccentBlue,
    },
    {
      'reason': 'Initialization',
      'desc': 'Set defaults without rebuilds',
      'icon': Icons.play_arrow,
      'color': _crSage,
    },
    {
      'reason': 'Undo / Redo',
      'desc': 'Restore state silently first',
      'icon': Icons.undo,
      'color': _crAccentPurple,
    },
    {
      'reason': 'Server sync',
      'desc': 'Apply API response atomically',
      'icon': Icons.cloud_download,
      'color': _crAccentTeal,
    },
    {
      'reason': 'Animation frames',
      'desc': 'Avoid mid-tween rebuilds',
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(r['icon'] as IconData, color: _crWhite, size: 14),
            ),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
...
Widget _buildBatchUpdateFlow() {
  final steps = <Map<String, dynamic>>[
    {
      'step': 'Begin',
      'desc': 'Save current behavior, set to silent',
      'icon': Icons.lock,
      'color': _crAccentBlue,
    },
    {
      'step': 'Mutate 1',
      'desc': 'controller.name = "Alice"',
      'icon': Icons.edit,
      'color': _crSage,
    },
    {
      'step': 'Mutate 2',
      'desc': 'controller.age = 30',
      'icon': Icons.edit,
      'color': _crSage,
    },
    {
      'step': 'Mutate 3',
      'desc': 'controller.bio = "Engineer"',
      'icon': Icons.edit,
      'color': _crSage,
    },
    {
      'step': 'End',
      'desc': 'Restore behavior, notifyListeners()',
  ...
  final patterns = <Map<String, dynamic>>[
    {
      'name': 'ChangeNotifier',
      'desc':
          'Flutter core. Always reporting by default. '
    ...
    },
    {
      ...
      'desc':
          'Conditional by default — only notifies when '
    ...
    },
    {
      ...
      'desc':
          'Always mode — every add() fires. Use '
    ...
    },
    {
      'name': 'BLoC / Cubit',
      'desc':
          'Conditional via Equatable. State emission '
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(p['icon'] as IconData, color: _crWhite, size: 18),
          ),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
  ...
  final fields = <Map<String, dynamic>>[
    {
      ...
      'value': 'Alice Chen',
      'behavior': 'Always',
      'rebuilds': 'Header, sidebar',
      'color': _crAccentOrange,
    },
    {
      'field': 'Bio',
      'value': 'Flutter developer & designer',
      'behavior': 'Conditional',
      'rebuilds': 'Profile card only',
      'color': _crAccentPurple,
    },
    {
      'field': 'Theme Pref',
    ...
    {
      'field': 'Last Login',
      'value': '2026-04-08 09:30',
      'behavior': 'Silent',
      'rebuilds': 'None (metadata)',
      'color': _crAccentTeal,
    },
  ];
      ...
      children: [
        ...
        ...fields.map((f) {
          return Container(
            ...
            decoration: BoxDecoration(
              border: Border(
                bottom: BorderSide(color: _crLightSage.withValues(alpha: 0.3)),
              ),
            ),
            child: Row(
              children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(ap['icon'] as IconData, size: 16, color: _crAccentRed),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  ap['bad'] as String,
          ...
          Row(
            children: [
              const Icon(Icons.check_circle, size: 14, color: _crAccentTeal),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  ap['fix'] as String,
                  style: const TextStyle(color: _crAccentTeal, fontSize: 10.5),
                ),
              ),
            ],
          ),
