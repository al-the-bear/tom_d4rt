    ...
    return ListView(
      ...
      children: <Widget>[
        ...
        _SectionCard(
          ...
          title: 'What it is',
          body:
              'DraggableScrollableActuator is a tree-level marker holding '
        ...
        _SectionCard(
          ...
          body:
              'Place one above any sheet whose position you want to '
        ...
        _SectionCard(
          ...
          body:
              '• User taps background — collapse sheet.\n'
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      'Programmatic reset for descendant draggable sheets',
                      style: TextStyle(
          ...
          child: Row(
            children: <Widget>[
              Container(
                ...
                decoration: BoxDecoration(
                  color: s.primary.withValues(alpha: 0.14),
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      '${dist.toStringAsFixed(1)} km · open now',
...
class _ExtentRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        Text(
          '$label · ${extent.toStringAsFixed(2)}',
                  ...
                  ValueListenableBuilder<double>(
                    ...
                    builder: (BuildContext ctx, double e, Widget? _) => Wrap(
                      ...
                      children: <Widget>[
                        ...
                        _Tag(
                          label: 'init ${_kInitialExtent.toStringAsFixed(2)}',
                        ),
                        _Tag(label: 'max ${_kMaxExtent.toStringAsFixed(2)}'),
                        _Tag(
                          label: 'now ${e.toStringAsFixed(2)}',
                          highlight: true,
                        ),
                      ],
                    ),
...
class _GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double frac = ((extent - minExtent) / (maxExtent - minExtent)).clamp(
      0.0,
      1.0,
    );
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ColorScheme s = Theme.of(context).colorScheme;
    final Color bg = highlight ? s.primary : s.primary.withValues(alpha: 0.10);
          ...
          children: <Widget>[
            ...
            Expanded(
              child: Text(
                ...
                style: TextStyle(fontSize: 12, color: s.onSecondaryContainer),
              ),
            ),
    ...
    return ListView(
      ...
      children: <Widget>[
        _SectionCard(
          ...
          body:
              'Reset dispatches a _ResetNotification that each sheet\'s '
...
class _ResetAnimationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _event(
      canvas,
      Offset(left + (right - left) * 0.35, yMax),
      'reset pressed',
      scheme,
    );
    _event(
      canvas,
      Offset(left + (right - left) * 0.75, yInit),
      'initialChildSize reached',
      scheme,
    );
  ...
  void _drawGuide(
    ...
    final Paint p = Paint()
      ..color = s.outlineVariant.withValues(alpha: 0.7)
      ..strokeWidth = 0.8;
    const double dash = 6;
    double x = left;
    while (x < right) {
    ...
    return ListView(
      ...
      children: <Widget>[
        _SectionCard(
          ...
          body:
              'The actuator exposes a ResetNotifier (ChangeNotifier) '
...
class _FlowDiagramPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final List<_FlowNode> nodes = <_FlowNode>[
      _FlowNode(
        label: 'User event',
        subtitle: 'FilledButton tap / gesture',
        accent: scheme.primary,
      ),
      _FlowNode(
        label: 'Actuator.reset(ctx)',
        subtitle: 'static method',
        accent: scheme.secondary,
      ),
      _FlowNode(
        label: '_ResetNotifier',
        subtitle: 'ChangeNotifier',
        accent: scheme.tertiary,
      ),
      _FlowNode(
        label: '_SheetState.addListener',
        subtitle: 'listens in subtree',
        accent: scheme.primary,
      ),
      _FlowNode(
        label: 'AnimationController',
        subtitle: 'animates extent',
        accent: scheme.secondary,
      ),
      _FlowNode(
        label: 'initialChildSize',
    ...
    for (int i = 0; i < nodes.length; i++) {
      ...
      final Rect r = Rect.fromLTWH(marginX, 10 + i * rowH, usable, rowH - 18);
      final RRect rr = RRect.fromRectAndRadius(r, const Radius.circular(14));
      canvas.drawRRect(rr, Paint()..color = n.accent.withValues(alpha: 0.12));
      canvas.drawRRect(
        rr,
        Paint()
          ..color = n.accent
  ...
  @override
  bool shouldRepaint(covariant _FlowDiagramPainter old) => old.scheme != scheme;
}
  ...
  Widget build(BuildContext context) {
    ...
    final List<_UseCase> cases = <_UseCase>[
      _UseCase(
        ...
        scenario:
            'User browses a place card up close, then backs out. The '
      ...
      _UseCase(
        ...
        title: 'Filter drawer',
        scenario:
            'Filter sheet expanded for multi-select; applying filters '
      ...
      _UseCase(
        ...
        title: 'Ride-hailing sheet',
        scenario:
            'Driver selected from the list; sheet glides to a compact '
            '"driver arriving" state.',
        trigger: 'Driver selected',
        accent: s.tertiary,
      ),
      _UseCase(
        ...
        title: 'Cart summary',
        scenario:
            'User expanded the cart to read policies. After checkout, '
            'collapse it.',
        trigger: 'Checkout tapped',
        accent: s.primary,
      ),
      _UseCase(
        ...
        scenario:
            'Expanded queue obscures the now-playing art; tap cover to '
            'reset the sheet.',
        trigger: 'Cover tap',
        accent: s.secondary,
      ),
      _UseCase(
        ...
        scenario:
            'After picking a result, the results sheet collapses so '
            'the selected page shows.',
        trigger: 'Result selected',
        accent: s.tertiary,
      ),
    ];
          ...
          children: <Widget>[
            Row(
              children: <Widget>[
                ...
                const SizedBox(width: 10),
                const Text(
                  'Snap + reset interaction',
                  style: TextStyle(fontWeight: FontWeight.w700),
                ),
                const Spacer(),
                Builder(
                  builder: (BuildContext inner) => FilledButton.icon(
                    onPressed: () => DraggableScrollableActuator.reset(inner),
                    icon: const Icon(Icons.restart_alt),
                    label: const Text('Reset'),
                  ),
                ),
...
class _SnapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final double s in snaps) {
      ...
      canvas.drawCircle(Offset(x, y), 6, Paint()..color = scheme.secondary);
      final TextPainter tp = TextPainter(
        text: TextSpan(
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: <Widget>[
        _PitfallTile(
          number: 1,
          title: 'No actuator ancestor',
          description:
              'Calling DraggableScrollableActuator.reset(context) '
        ...
        _PitfallTile(
          number: 2,
          title: 'Nested actuators',
          description:
              'When two DraggableScrollableActuators are nested, '
        ...
        _PitfallTile(
          number: 3,
          title: 'Reset during drag',
          description:
              'If the user is actively dragging a sheet when you '
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            Row(
              children: <Widget>[
                Icon(Icons.menu_book_outlined, color: s.primary, size: 18),
