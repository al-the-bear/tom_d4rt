...
final ValueNotifier<Duration> _selectedDuration = ValueNotifier<Duration>(
  const Duration(milliseconds: 300),
);
final ValueNotifier<Curve> _selectedCurve = ValueNotifier<Curve>(
  Curves.easeInOut,
);
final List<ExpansibleController> _groupControllers =
    List<ExpansibleController>.generate(4, (_) => ExpansibleController());

void _toggle(ExpansibleController c) =>
...
class _HeroTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _Section(
      ...
      subtitle:
          'The building-block primitive that powers ExpansionTile, ExpansionPanel, and any custom collapsible surface you design.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Container(
                ...
                Row(
                  children: <Widget>[
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: scheme.onPrimary.withAlpha(32),
                        borderRadius: BorderRadius.circular(12),
                      ),
                      child: Icon(
                        Icons.unfold_more,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          const SizedBox(height: 4),
                          Text(
                            'Stateful primitive. Stateless by your design.',
            ...
            children: const <Widget>[
              Expanded(
                child: _HeroBullet(
                  ...
                  title: 'Total control',
                  body:
                      'You build header and body. Expansible only owns the clip-and-size animation.',
                ),
              ),
              SizedBox(width: 12),
              Expanded(
                child: _HeroBullet(
                  ...
                  body:
                      'ExpansibleController.expand / collapse toggles state externally.',
                ),
              ),
              SizedBox(width: 12),
              Expanded(
                child: _HeroBullet(
                  ...
                  title: 'Composable',
                  body:
                      'expansibleBuilder lets you arrange header and body freely.',
                ),
              ),
            ],
          ),
          ...
          const _Explainer(
            ...
            title: 'Mental model',
            body:
                'Expansible is to ExpansionTile what RawGestureDetector is to GestureDetector. It is the unstyled core. Everything visible on screen is your callback. The only thing Expansible owns is the height-factor animation driven by an AnimationController bound to the controller you pass in.',
          ),
        ],
      ),
    );
...
class _BasicTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _Section(
      title: '2. Basic usage',
      subtitle:
          'A single Expansible with one ExpansibleController. External buttons drive controller.expand(), controller.collapse(), and a toggle. The body is a colorful gradient card.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          AnimatedBuilder(
            animation: _basicController,
            builder: (BuildContext ctx, Widget? _) => Row(
              children: <Widget>[
                FilledButton.icon(
                  onPressed: _basicController.isExpanded
                      ? null
                      : _basicController.expand,
          ...
          Material(
            ...
            child: Expansible(
              ...
              headerBuilder: (BuildContext ctx, Animation<double> a) =>
                  _TapHeader(
                    controller: _basicController,
                    animation: a,
              ...
              bodyBuilder: (BuildContext ctx, Animation<double> a) =>
                  _GradientBody(animation: a),
            ),
          ),
          const SizedBox(height: 20),
          const _Explainer(
            ...
            body:
                'Expansible owns an AnimationController. When you call controller.expand() / collapse() the internal animation runs forward or reverse. The animation is passed to headerBuilder and bodyBuilder so any caller widget can listen. The body is wrapped in ClipRect + Align(heightFactor: t).',
          ),
        ],
      ),
    );
...
class _StatePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ColorScheme scheme = Theme.of(context).colorScheme;
    final Color bg = isExpanded
        ? scheme.primaryContainer
        : scheme.tertiaryContainer;
    final Color fg = isExpanded
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          Icon(
            isExpanded ? Icons.check_circle : Icons.radio_button_unchecked,
            size: 16,
            color: fg,
          ),
...
class _HeadersTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _Section(
      ...
      subtitle:
          'Three Expansibles share the same body but demonstrate dramatically different headers. Everything above the fold is headerBuilder output.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          _HeaderVariant(
            label: 'Variant A - plain text header',
            note:
                'Bare Text wrapped in a tap target. Closest thing to ExpansionTile default.',
            controller: _headerPlainController,
            headerBuilder: (BuildContext ctx, Animation<double> a) =>
                _PlainHeader(controller: _headerPlainController, animation: a),
          ),
          const SizedBox(height: 18),
          _HeaderVariant(
            label: 'Variant B - gradient pill',
            note:
                'Full-bleed gradient with a chevron rotating on animation.value.',
          ...
          const SizedBox(height: 18),
          _HeaderVariant(
            label: 'Variant C - icon + badge',
            note:
                'Decorated leading circle and a badge that scales with the animation.',
          ...
          const _Explainer(
            ...
            title: 'Rule of thumb',
            body:
                'headerBuilder receives the animation. Anything that should visually react to expansion (chevron rotation, color tween, badge scale) reads animation.value via RotationTransition, ScaleTransition, or AnimatedBuilder.',
          ),
        ],
      ),
    );
      ...
      child: Container(
        ...
        child: Row(
          children: <Widget>[
            ...
            RotationTransition(
              turns: Tween<double>(begin: 0.0, end: 0.5).animate(animation),
              child: Icon(Icons.expand_more, color: scheme.onPrimary),
            ),
          ],
        ),
      ),
            ...
            Expanded(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  const SizedBox(height: 2),
                  Text(
                    'ScaleTransition lives inside the header.',
                    style: TextStyle(
                      fontSize: 12,
                      color: scheme.onSurfaceVariant,
                    ),
...
class _BodiesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _Section(
      ...
      subtitle:
          'bodyBuilder is just a Widget factory. The body is clipped and Aligned by Expansible, but its content is entirely yours.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          _BodyVariant(
            label: 'Variant A - list of items',
            controller: _bodyListController,
            titleText: 'Recent activity',
            bodyBuilder: (BuildContext ctx, Animation<double> a) =>
                _ListBody(animation: a),
          ),
          const SizedBox(height: 18),
          _BodyVariant(
            label: 'Variant B - grid of chips',
            controller: _bodyGridController,
            titleText: 'Apply filters',
            bodyBuilder: (BuildContext ctx, Animation<double> a) =>
                _ChipGridBody(animation: a),
          ),
          const SizedBox(height: 18),
          _BodyVariant(
            label: 'Variant C - rich text block',
            controller: _bodyRichController,
            titleText: 'Release notes',
            bodyBuilder: (BuildContext ctx, Animation<double> a) =>
                _RichTextBody(animation: a),
          ),
          const SizedBox(height: 20),
          const _Explainer(
            ...
            title: 'Body anatomy',
            body:
                'Expansible wraps bodyBuilder output in Offstage + TickerMode + ClipRect + Align(heightFactor: t). Your body can be any widget tree - Wrap, Grid, bounded ListView, CustomPaint - and Expansible handles the vertical reveal.',
          ),
        ],
      ),
    );
...
class _LayoutTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _Section(
      ...
      subtitle:
          'Override expansibleBuilder to place header and body however you like. The default stacks them in a Column; here are two alternatives.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
            'Variant 1 - stacked with an animated separator bar',
            style: Theme.of(
              context,
            ).textTheme.titleSmall?.copyWith(color: scheme.primary),
          ),
            ...
            child: Expansible(
              ...
              headerBuilder: (BuildContext ctx, Animation<double> a) =>
                  _TapHeader(
                    ...
                    subtitle: 'Header | accent bar | body.',
                    leading: Icons.tune,
                  ),
              bodyBuilder: (BuildContext ctx, Animation<double> a) =>
                  FadeTransition(
          ...
          Text(
            'Variant 2 - side-by-side header and body',
            style: Theme.of(
              context,
            ).textTheme.titleSmall?.copyWith(color: scheme.primary),
          ),
                  ...
                  FadeTransition(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: <Widget>[
                          for (final (String head, String body)
                              in const <(String, String)>[
                                ('Scene 1', 'Header is a fixed-width sidebar.'),
                                (
                                  'Scene 2',
                                  'Body expands horizontally within the Row slot.',
                                ),
                                (
                                  'Scene 3',
                                  'Height is still animated as children grow.',
                                ),
                              ]) ...<Widget>[
                            Text(
                              head,
          ...
          const _Explainer(
            ...
            title: 'expansibleBuilder callback',
            body:
                'Signature: Widget Function(BuildContext, Widget header, Widget body, Animation<double>). The default returns Column([header, body]). Return any widget that includes both - Stack, Row, LayoutBuilder, RepaintBoundary.',
          ),
        ],
      ),
    );
...
class _TimingTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _Section(
      ...
      subtitle:
          'Pick a duration and a curve. Tap Expand below - Expansible rebuilds with the chosen animationStyle. Picking values while collapsed makes the difference most visible.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Text(
          ...
          ValueListenableBuilder<Duration>(
            ...
            builder: (BuildContext ctx, Duration value, Widget? _) => Wrap(
              spacing: 8,
              runSpacing: 8,
              children: <Widget>[
                for (final (String label, Duration d) in durations)
                  _Pill(
                    label: label,
                    selected: value == d,
                    onTap: () => _selectedDuration.value = d,
                  ),
              ],
            ),
          ),
          ...
          ValueListenableBuilder<Curve>(
            ...
            builder: (BuildContext ctx, Curve value, Widget? _) => Wrap(
              spacing: 8,
              runSpacing: 8,
              children: <Widget>[
          ...
          AnimatedBuilder(
            ...
            builder: (BuildContext ctx, Widget? _) => Row(
              children: <Widget>[
                FilledButton.icon(
                  onPressed: _durationController.isExpanded
                      ? null
                      : _durationController.expand,
          ...
          ValueListenableBuilder<Duration>(
            valueListenable: _selectedDuration,
            builder: (BuildContext ctx, Duration duration, Widget? _) =>
                ValueListenableBuilder<Curve>(
                  valueListenable: _selectedCurve,
                  builder: (BuildContext ctx, Curve curve, Widget? _) => Container(
                    ...
                    child: Expansible(
                      key: ValueKey<String>(
                        'timing-${duration.inMilliseconds}-${curve.hashCode}',
                      ),
                      ...
                      headerBuilder: (BuildContext ctx, Animation<double> a) =>
                          _TapHeader(
                            ...
                            subtitle:
                                'duration=${duration.inMilliseconds}ms, curve=${curve.runtimeType}',
          ...
          const _Explainer(
            icon: Icons.speed,
            title: 'animationStyle vs duration/curve',
            body:
                'The deprecated duration, curve, and reverseCurve parameters still work, but animationStyle is current. AnimationStyle.noAnimation short-circuits the animation - handy for snapshot tests.',
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    const List<(IconData, String, String)> items = <(IconData, String, String)>[
      (
        Icons.code,
        'Code review',
        'Every controller is independent. Expanding one does not touch any other.',
      ),
      (
        Icons.bug_report_outlined,
        'Issues',
        'Expand-all and collapse-all iterate the List<ExpansibleController>.',
      ),
      (
        Icons.rocket_launch_outlined,
        'Deployment',
        'Each Expansible has its own controller, so you can persist, lift, or stream state.',
      ),
      (
        Icons.settings_outlined,
        'Preferences',
        'This is essentially how ExpansionPanelList works: a list of Expansibles + List<bool>.',
      ),
    ];
    return _Section(
      title: '7. Multiple controllers',
      subtitle:
          'Four Expansibles each with their own ExpansibleController. Group operations iterate the list and call expand or collapse on every controller.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          ...
          const _Explainer(
            ...
            body:
                'Since ExpansibleController is a ChangeNotifier, you can build an aggregator that listens to all controllers and derives a group-level isAllExpanded bool. Or, like this demo, iterate and call expand() on each.',
          ),
        ],
      ),
    );
...
class _GroupBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (BuildContext ctx, Widget? _) {
        final int expanded = controllers
            .where((ExpansibleController c) => c.isExpanded)
            .length;
        ...
        return Row(
          children: <Widget>[
            FilledButton.icon(
              onPressed: allExpanded
                  ? null
                  : () {
                      for (final ExpansibleController c in controllers) {
                        c.expand();
                      }
                    },
              icon: const Icon(Icons.unfold_more),
      ...
      children: <Widget>[
        for (int i = 0; i < controllers.length; i++) ...<Widget>[
          Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              color: controllers[i].isExpanded
                  ? scheme.primary
...
class _MechanismTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _Section(
      ...
      subtitle:
          'A tap on the header calls controller.expand or collapse. That toggles an internal AnimationController. When the animation ticks, headerBuilder and bodyBuilder are re-invoked with the live Animation<double>.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Container(
            ...
            children: const <Widget>[
              Expanded(
                child: _Step(
                  ...
                  title: 'User input',
                  body:
                      'Tap on headerBuilder output invokes controller.expand() or collapse().',
                ),
              ),
              SizedBox(width: 10),
              Expanded(
                child: _Step(
                  index: '2',
                  title: 'Controller notifies',
                  body:
                      'ExpansibleController flips _isExpanded and notifyListeners() fires.',
                ),
              ),
            ],
          ),
            ...
            children: const <Widget>[
              Expanded(
                child: _Step(
                  ...
                  body:
                      '_ExpansibleState calls forward() or reverse() on the AnimationController.',
                ),
              ),
              SizedBox(width: 10),
              Expanded(
                child: _Step(
                  ...
                  title: 'Rebuilds',
                  body:
                      'AnimatedBuilder wraps everything; each tick re-invokes headerBuilder and bodyBuilder.',
                ),
              ),
            ],
          ),
...
class _MechPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _arrow(
      ...
      Offset(ctrl.left, ctrl.center.dy),
      'tap',
    );
    _arrow(
      canvas,
      arrow,
      Offset(ctrl.center.dx, ctrl.bottom),
      Offset(anim.center.dx, anim.top),
      'listener',
    );
    _arrow(
      canvas,
      arrow,
      Offset(anim.left, anim.center.dy),
    ...
    _text(
      canvas,
      'Expansible internal data flow',
      Offset(w / 2 - 110, h * 0.38),
      color: onSurface,
      size: 15,
      weight: FontWeight.w700,
    );
    ...
    _text(
      canvas,
      '  _ExpansibleState  ',
      Offset(boundary.left + 12, boundary.top - 8),
      color: primary,
      size: 11,
      weight: FontWeight.w700,
      bg: surface,
  ...
  void _arrow(
    ...
    _text(
      canvas,
      label,
      Offset((start.dx + end.dx) / 2, (start.dy + end.dy) / 2 - 14),
      color: paint.color,
      size: 11,
...
class _CompareApiTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _Section(
      ...
      subtitle:
          'How Expansible relates to higher-level widgets, and every parameter you can pass to it.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: const <Widget>[_Compare(), SizedBox(height: 22), _ApiSheet()],
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    const List<List<String>> rows = <List<String>>[
      ...
      <String>[
        ...
        'Total (headerBuilder)',
        'Partial (title/leading/trailing)',
        'Partial (headerBuilder)',
        'N/A',
      ],
      <String>[
        'Layout customization',
        'Total (expansibleBuilder)',
        'Fixed Column',
        'Fixed panel',
        'layoutBuilder',
      ],
      <String>[
        'Animation type',
        'Height Align + your fade',
        'Same (uses Expansible)',
        'Height + divider',
        'Cross-fade of two widgets',
      ],
      <String>[
        'State shape',
        'bool on controller',
      ...
      <String>[
        'Best when',
        'Custom collapsible surface',
        'Material list item',
        'Accordion list',
        'Swap two alternatives',
      ],
    ];
    return Container(
      decoration: BoxDecoration(
          ...
          Container(
            ...
            child: Row(
              children: <Widget>[
                ...
                const SizedBox(width: 8),
                Text(
                  'Expansible vs ExpansionTile vs ExpansionPanelList vs AnimatedCrossFade',
                  style: TextStyle(
                    color: scheme.onPrimaryContainer,
                    fontWeight: FontWeight.w700,
                  ),
...
class _CompareRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? cell = Theme.of(
      context,
    ).textTheme.bodySmall?.copyWith(color: scheme.onSurface, height: 1.35);
    final TextStyle? expansible = cell?.copyWith(
      color: scheme.primary,
      fontWeight: FontWeight.w600,
    );
    return Container(
...
class _ApiSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ColorScheme scheme = Theme.of(context).colorScheme;
    const List<(String, String, String, bool)>
    ctor = <(String, String, String, bool)>[
      (
        'controller',
        'ExpansibleController',
        'Required. External source of truth. Drives expand/collapse and notifies listeners.',
        false,
      ),
      (
        'headerBuilder',
        'Widget Function(BuildContext, Animation<double>)',
        'Required. Builds the always-visible part. Usually a tap target calling controller.expand/collapse.',
        false,
      ),
      (
        'bodyBuilder',
        'Widget Function(BuildContext, Animation<double>)',
        'Required. Builds the collapsible part. Wrapped in ClipRect + Align(heightFactor: t).',
        false,
      ),
      (
        'expansibleBuilder',
        'Widget Function(BuildContext, Widget header, Widget body, Animation<double>)?',
        'Optional. Default stacks header and body in a Column. Override for custom layouts.',
        false,
      ),
      (
        'animationStyle',
        'AnimationStyle?',
        'Preferred way to override duration, curve, and reverseCurve in one shot.',
        false,
      ),
      (
        'duration',
        'Duration',
        'Default 200 ms. DEPRECATED - prefer animationStyle.duration.',
        true,
      ),
      (
        'curve',
        'Curve',
        'Default Curves.ease. DEPRECATED - prefer animationStyle.curve.',
        true,
      ),
      (
        'reverseCurve',
        'Curve?',
        'Defaults to curve. DEPRECATED - prefer animationStyle.reverseCurve.',
        true,
      ),
      (
        'maintainState',
        'bool',
        'Default true. If false the body is removed from the tree when collapsed.',
        false,
      ),
    ];
    const List<(String, String, String, bool)>
    ctrl = <(String, String, String, bool)>[
      (
        'isExpanded',
        'bool (getter)',
        'True between expand() and collapse(). Does not wait for the animation to finish.',
        false,
      ),
      (
        'expand()',
        'void',
        'Sets state to expanded and fires listeners. Not safe to call during build.',
        false,
      ),
      (
        'collapse()',
        'void',
        'Sets state to collapsed and fires listeners. Not safe to call during build.',
        false,
      ),
      (
        'addListener / removeListener',
        'inherited from ChangeNotifier',
        'React to expand/collapse without rebuilding. Pair with AnimatedBuilder.',
        false,
      ),
      (
        'ExpansibleController.of(context)',
        'ExpansibleController',
        'Walks up to the enclosing Expansible. Throws if none found.',
        false,
      ),
      (
        'ExpansibleController.maybeOf(context)',
        'ExpansibleController?',
