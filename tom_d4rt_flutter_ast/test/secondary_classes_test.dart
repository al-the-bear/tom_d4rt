...
void main() {
  ...
  group('rendering/ individual', () {
    ...
    test('render_custom_single_child_layout_box_test.dart', () async {
      // 1944 TODO C.21 (2026-05-31): historical 20260524-2003
      // §6/E14+T15 cold-start wrapper REMOVED. Script runs in ~2.0 s
      // under normal load (httpMs=1975) on a 1.1 MB bundle.
      final result = await SendTestRunner.send(
    ...
    test('render_ignore_baseline_test.dart', () async {
      // 1944 TODO C.22 (2026-05-31): historical 20260524-2003 §6/E15
      // cold-start wrapper REMOVED. Script runs in ~1.5 s under
      // normal load (httpMs=1462).
      final result = await SendTestRunner.send(
    ...
    test('render_proxy_box_with_hit_test_behavior_test.dart', () async {
      // 1944 TODO C.23 (2026-05-31): historical 20260524-2003 §6/T16
      // cold-start wrapper REMOVED. Script runs in ~2.0 s under
      // normal load (httpMs=1955) on a 1.0 MB bundle.
      final result = await SendTestRunner.send(
  ...
  group('services/ individual', () {
    ...
    test(
      'hybrid_android_view_controller_test.dart',
      () async {
        final result = await SendTestRunner.send(
          'services/hybrid_android_view_controller_test.dart',
          // 20260523-1056 baseline §1.3/E2: under parallel-driver
          // contention (ast+test apps booting concurrently) the /build
          // for this 1399-line, 52 KB script can exceed the default
          // 25 s HTTP cap on the first request after the test app
          // cold-start (measured 25001 ms cold vs 1661 ms warm — a 15×
          // gap). Serial isolated re-runs complete in ~1.7 s. 50 s
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E4: TimeoutException after 30s
          // — cold-start contention pushed the dart-test wrapper past
          // its 30s default. Isolated serial re-run completes in
          // ~1.5 s (httpMs=1316). Same family as E1/E2: 50 s leaves
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E6: TimeoutException after 30s
          // — cold-start contention. This 2285-line / 84 KB script
          // (1 MB AST bundle) takes ~2.8 s on a serial cold run and
          // ~2.5 s warm. Same family as E1/E2/E4: 50 s leaves 10 s of
          // headroom under the 60 s dart-test wrapper for contention.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
      },
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E8: TimeoutException after 30s
          // — cold-start contention. This 1456-line / 54 KB script
          // (607 KB bundle) takes ~1.3–1.6 s in both variants. Same
          // family as E1/E2/E4/E6/E7: 50 s leaves 10 s of headroom
          // under the 60 s dart-test wrapper.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
      },
    ...
    test(
      ...
      () async {
        final result = await SendTestRunner.send(
          ...
          // 20260523-1056 baseline §1.3/E9: TimeoutException after 30s
          // — cold-start contention. This 1096-line / 40 KB script
          // (429 KB bundle) takes ~1.7 s in both variants. Same
          // family as E1/E2/E4/E6/E7/E8: 50 s leaves 10 s of headroom
          // under the 60 s dart-test wrapper.
          httpBuildTimeout: const Duration(seconds: 50),
        );
        expect(result.success, isTrue, reason: result.error);
