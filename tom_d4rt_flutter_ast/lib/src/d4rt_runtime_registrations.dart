...
void _registerInterfaceProxies() {
  ...
  // `D4.unwrapAs<Listenable>` falls through and rejects the
  // InterpretedInstance.
  //
  ...
  // recurses. The `_forwarding*` flags short-circuit re-entry to the
  // native default impl on the mixin.
...
void registerD4rtInterfaceProxyOverrides() {
  ...
  //
  // Mirror of the flutter_test registration. Scripts that subclass
  // `TextSelectionControls` (e.g. the `widgets/text_selection_controls_test.dart`
  // "_TscFoundryTextSelectionControls" teaching demo) need a real native
  // `TextSelectionControls` instance so they can be passed to
  // `TextField(selectionControls: …)` and the bridge's
  // `extractBridgedArg<TextSelectionControls?>` succeeds.
  //
  // The flutter_ast 20260526-1401 sweep happened to pass this script
  // because of a fallback path in tom_d4rt_ast's `extractBridgedArg`,
  // but registering an explicit proxy makes the contract identical
  // across both runners and shields against fallback regressions.
...
class _InterpretedStatefulWidget extends StatefulWidget
  ...
  static bool _usesAutomaticKeepAliveClientMixin(InterpretedClass klass) {
    ...
    bool walk(InterpretedClass? c) {
      if (c == null) return false;
      if (!visited.add(c)) return false;
      if (c.bridgedMixins.any(
        (m) => m.name == 'AutomaticKeepAliveClientMixin',
      )) {
        return true;
      }
      if (walk(c.superclass)) return true;
...
class _InterpretedParentDataWidget extends ParentDataWidget<ParentData>
    implements D4InterpretedProxy {
  const _InterpretedParentDataWidget(
    this._visitor,
    this._instance, {
    required super.child,
    super.key,
