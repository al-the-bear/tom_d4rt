...
- `widgets/inherited_theme_test.dart` — exact-type lookup machinery resolved; residual null-context boundary later fixed (Plan E2, `920032c7` / `80c5d1d4`)
...
**Plan E2 (resolved):** The two residual gii failures
(`Cannot invoke method 'dependOnInheritedWidgetOfExactType' on
null` — a null receiver `BuildContext` reaching the call from a
closure that lost the captured context) were fixed in the
interpreter via the `nativeStateProxy` getter fallback
(`920032c7`, C14) + `80c5d1d4`; regression tests
