...
- [ ] **3.** `transport_clear_wedge` cluster (31 scripts, 8 ast + 23 test) — root cause is the still-open U28 wedge accumulator (per the architectural finding documented in commit `42588be2`). Instrument: add per-`/clear` counter dumps for `D4._nativeToInterpreted` Expando length + D4 static cache sizes in each test_app's `/clear` handler; rerun the affected subsuites; observe which counter grows monotonically. Once the real accumulator is identified, add a targeted reset to `FlutterD4rt.resetScript()` / `SourceFlutterD4rt.resetScript()`. This single fix should clear all 31 entries.

- [ ] **4.** `test_30s_timeout` cluster (48 scripts, 12 ast + 36 test) — Flutter test wrapper's 30 s budget fires before test completes. Two sub-classes:
    - **Cold-start performance (§U25)** — affects the TEST source-direct path more heavily (36 vs 12). Triage candidates: `dropdown_menu_close_behavior_test.dart`, `axis_direction_test.dart`, the retest cluster (10+ items). Either bump per-test `timeout:` parameter on slow scripts OR optimise the source interpreter cold-start parse pipeline.
    - **Genuinely-blocked scripts** — `widgets/raw_keyboard_listener_test.dart`, `widgets/route_aware_test.dart`, etc. (the `hardly_relevant_5` group) hit timeout on BOTH projects, suggesting intrinsic script slowness or a bridge-side hang. Inspect each.

- [ ] **5.** `build_30s_timeout` — 1 AST failure (`timeout_tests/retest: widgets/app_kit_view_test.dart`, "Expected: true Actual: false, Build timed out after 30 seconds"). Either raise the test-app build budget for this script or identify the hang. The script involves AppKit native view embedding which is desktop-platform-sensitive (similar to §U17 / android_view skip pattern).
