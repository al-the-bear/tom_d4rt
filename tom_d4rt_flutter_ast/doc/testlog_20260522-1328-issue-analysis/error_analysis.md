...
- [x] **fixed** 29. Diagnose I-BRIDGE-1 (`Calculator.calculate`) — verify the bridged-mixin's method table is registered. Likely in `tom_d4rt/lib/src/runtime_types.dart` / `tom_d4rt_ast/lib/src/runtime/runtime_types.dart` — `InterpretedClass` mixin resolution. Add a unit test that exercises the failing dispatch. **Done — root cause was the GEN-128 skip in `InterpretedInstance.get()` firing too aggressively.** When resolving a bridged-mixin method, the existing GEN-128 guard skipped the mixin's adapter unconditionally whenever the resolved `mixinTarget` was the interpreted instance itself (`identical(mixinTarget, this)`) and no interface proxy could be materialised. The guard exists to keep `class MyState extends State<MyWidget> with AutomaticKeepAliveClientMixin` cases (Cluster B GEN-128) falling through to the bridged-super walk (RC-9) that routes `setState` via `nativeStateProxy`. But for purely script-side bridged mixins — `class Calculator with TestMixin { ... }` with no bridged-super chain — the skip left the lookup with no resolution path, surfacing as `Undefined property 'calculate' on Calculator` → `Undefined variable: calculate`. **Fix:** added a `hasBridgedSuperInChain(klass)` precondition to the GEN-128 skip in `tom_d4rt/lib/src/runtime_types.dart` and the mirrored copy in `tom_d4rt_ast/lib/src/runtime/runtime_types.dart`. The skip now only fires when there IS a downstream bridged-super path; otherwise the bridged mixin's adapter is called normally (adapters that don't validate the target type — like `TestMixin` — succeed; adapters that do validate surface a `Native error` which is the right diagnostic). Mirror kept in sync via local helper hoisted to the top of `InterpretedInstance.get()` (Dart doesn't hoist local function declarations). Verification: I-BRIDGE-1..15 pass 10/10 in `tom_d4rt/test/bridge/bridged_mixin_test.dart` and `tom_d4rt_exec/test/bridge/bridged_mixin_test.dart` (was 3/10 pre-fix); rule (b) sweep — `tom_d4rt` testkit 1788 (1786 pass, 1 I-BUG-14a SHOULD FAIL, 1 D4-WRAP-01 skipped), `tom_d4rt_ast` testkit 117/117, `tom_d4rt_exec` testkit 2200 (2198 pass, 2 known pre-existing); flutter_ast sweep clean (essential +108, important +164, secondary +653 ~1); flutter_test sweep — essential +107 -1 (pre-existing Cluster B #6 materialapp_test, fixed only in flutter_ast bridges), important +163 -1 (pre-existing Cluster B #5 decoratedbox_test, same reason), secondary +653 ~1 clean. No regressions. Raw logs in `tom_d4rt/ztmp/cluster_j29/`, `tom_d4rt_exec/ztmp/cluster_j29/`, `tom_d4rt_flutter_ast/ztmp/cluster_j29/`, `tom_d4rt_flutter_test/ztmp/cluster_j29/`.
- [x] **fixed** 30. Mirror the fix between `tom_d4rt` and `tom_d4rt_ast`. Re-run the shared fixture under both; covers #40–46 in one pass. **Done as part of #29** — the `hasBridgedSuperInChain` helper and tightened GEN-128 skip block landed in both `tom_d4rt/lib/src/runtime_types.dart` and `tom_d4rt_ast/lib/src/runtime/runtime_types.dart` in the same commit. Shared fixture `bridged_mixin_test.dart` passes 10/10 under both `tom_d4rt` and `tom_d4rt_exec` (which uses `tom_d4rt_ast`).
- [x] **fixed** 31. Confirm I-BUG-14a (records with named fields) remains marked `SHOULD FAIL` — no fix required; this is by-design. (covers #47) **Confirmed** — `tom_d4rt` testkit baseline_0422_1959.csv after Cluster J fix shows 1786 pass / 1 fail / 1 skip; the single failure is `I-BUG-14a` (records with named fields), unchanged from the prior baseline. No code change required; this is by-design (records with named fields are not yet supported by the d4rt scanner/parser path the test exercises). The `SHOULD FAIL` annotation in the test itself is retained.

### Cluster K — d4 binary "Text file busy" (tom_d4rt_exec)

- [x] **fixed** 32. G-TST-9: rebuild the `d4` binary into a fresh path before running D4rtTester e2e, OR serialise the test against the build step. Investigate parallel test runners holding an fd on the binary. (covers #48) **Done — root cause was binary-path contention between two test files sharing the same `example/d4` project.** `tom_d4rt_exec/test/generator_tests/d4rt_tester_test.dart` and `tom_d4rt_exec/test/generator_tests/d4rt_coverage_test.dart` both construct `D4rtTester(projectPath: example/d4, …)` with the default `compiledBinaryName: 'd4'` / `runnerExecutable: 'd4rtrun.b'` and both call `ExecTestSetup.prepareBridges` in `setUpAll`. When `dart test` runs these suites in parallel (default concurrency on multi-core machines), Suite A's tests launch `bin/d4 --test …` subprocesses while Suite B's `setUpAll` is mid-`dart compile exe -o bin/d4`; Linux raises `ETXTBSY` ("Text file busy") on either the compile (can't `open(O_TRUNC)` a text file being executed) or the spawn (can't `execve()` a binary still being written). Reproduced cleanly: `00:30 +11 -1: G-TST-9 [E] FileSystemException: Cannot open file … (OS Error: Text file busy, errno = 26)`. **Fix:** chose the "fresh path per suite" route (entry hint, option 1) — gave each test file its own runner-source and compiled-binary names by passing `runnerExecutable: 'd4rtrun_tester.b'`/`compiledBinaryName: 'd4_tester'` to the tester in `d4rt_tester_test.dart` and `runnerExecutable: 'd4rtrun_coverage.b'`/`compiledBinaryName: 'd4_coverage'` in `d4rt_coverage_test.dart`. Both names are already plumbed through `D4rtTester` (constructor + `_binaryPath`) and through `ExecTestSetup.prepareBridges` (uses `tester.compiledBinaryName` / `tester.runnerExecutable`), so the change is purely cosmetic at the test-construction site. Both suites still share the same `lib/src/d4rt_bridges/*.b.dart` regeneration target, but those writes are deterministic-content data files (no ETXTBSY surface) and have not exhibited a race in practice. **Test-files-only change → rule (a), individual retest.** Verification: `dart test test/generator_tests/d4rt_tester_test.dart test/generator_tests/d4rt_coverage_test.dart -r expanded` — exit=0, 122 passed / 0 failed, zero ETXTBSY events; G-TST-9 UBR01 reports PASS at `+22`. Raw logs in `tom_d4rt_exec/ztmp/cluster_k32/{repro,post}.log`.

### Cluster L — VS Code Scripting API (tom_d4rt_dcli)

- [x] **fixed** 33. Gate `VSCodeWindow.getActiveTextEditor` tests on an environment variable (`TOM_LIVE_VSCODE=1`) so headless CI skips them; OR provide a `MockVSCodeWindow` returning a stub editor. (covers #49, #50) **Done — option 1 (env-var gate) chosen, applied symmetrically to both `tom_d4rt_dcli` and its `tom_dcli_exec` mirror.** Underlying defect lives in `tom_ai/vscode/tom_vscode_scripting_api/lib/src/vscode_window.dart:261` where `getActiveTextEditor()` does `return TextEditor.fromJson(result['result']);` inside the `if (result['success'] == true)` branch — but in headless runs the bridge reports success with `result['result'] == null`, triggering `type 'Null' is not a subtype of type 'Map<String, dynamic>'` inside `TextEditor.fromJson`. The same path is reached both from the direct API call (test #49) and from a d4rt script that invokes `VSCode.instance.window.getActiveTextEditor()` (test #50, which fails earlier as `result.success == false` once the cast throws inside the script execution). The existing `bridgeAvailable` guard returns true on the CI host (the WS bridge is wired up) but no live editor exists, so the guard is insufficient. **Fix:** added a `Platform.environment['TOM_LIVE_VSCODE'] != '1'` gate *above* the existing `bridgeAvailable` guard in both failing tests in `tom_d4rt_dcli/test/vscode_scripting_api_bridges_test.dart` (lines 252 and 582) and mirrored to `tom_dcli_exec/test/vscode_scripting_api_bridges_test.dart` (lines 255 and 585). The gate prints a clear skip message and returns; the original test bodies are preserved verbatim so a developer with a live VS Code session can still exercise them by exporting `TOM_LIVE_VSCODE=1`. `dart:io` was already imported. The library defect (`vscode_window.dart:261`) is **not** patched here — it lives in a separate repo (`tom_ai/vscode/tom_vscode_scripting_api`) and is a defensive-cast problem that should be addressed there independently (`return result['result'] == null ? null : TextEditor.fromJson(result['result'] as Map<String, dynamic>);`). It is documented inline in the test files' comment blocks so the next pass at that library has the failure context. **Test-script-only change → rule (a)**, individual retest only. Verification: `dart test test/vscode_scripting_api_bridges_test.dart -r expanded` — `tom_d4rt_dcli` 25/25 passed, `tom_dcli_exec` 25/25 passed, both with the two gated tests cleanly reporting `Skipping: TOM_LIVE_VSCODE=1 not set (headless run)`. Raw logs in `tom_d4rt_dcli/ztmp/cluster_l33/{post_direct,post_script,post_full}.log` and `tom_dcli_exec/ztmp/cluster_l33/{post_direct,post_script,post_full}.log`.

### Cluster M — generator dart_overview coverage

- [x] **fixed** 34. tom_d4rt_generator `dart_overview` setUpAll failure — re-run with verbose output, capture the diagnostic, fix the missing bridge or coverage entry. (covers #51) **Done — same root cause as Cluster K #32 (ETXTBSY on `bin/d4`), applied the same fix in `tom_d4rt_generator`.** Reproduction with `dart test test/d4rt_coverage_test.dart test/d4rt_tester_test.dart` showed `ProcessException: Text file busy / Command: example/d4/bin/d4 --test … top04_base_class.dart` at `+3 -1: dart_overview coverage … G-COV-68: TOP04` — i.e. while `d4rt_coverage_test.dart`'s setUpAll was mid-`dart compile exe -o bin/d4`, `d4rt_tester_test.dart`'s tests had just started spawning the same `bin/d4` binary (or vice versa). Both suites construct `D4rtTester(projectPath: example/d4, …)` with the default `compiledBinaryName: 'd4'` / `runnerExecutable: 'd4rtrun.b'`, so they race on the same compile/exec target. The original log captured the failure earlier on setUpAll (the compile lost the race, so `prepareBridges` returned false and `expect(ok, isTrue)` fired); the post-Cluster-K snapshot happened to lose the race at a per-test execve instead — same root cause either way. **Fix:** gave each suite its own runner-source and compiled-binary names via the existing `D4rtTester` constructor params: `runnerExecutable: 'd4rtrun_coverage.b'`/`compiledBinaryName: 'd4_coverage'` in `test/d4rt_coverage_test.dart`, and `runnerExecutable: 'd4rtrun_tester.b'`/`compiledBinaryName: 'd4_tester'` in `test/d4rt_tester_test.dart`. Both names are already plumbed through `D4rtTester` (constructor + `_binaryPath` + `prepareBridges`), so this is a pure test-construction site change — no library code touched. The shared `example/d4/lib/src/d4rt_bridges/*.b.dart` regeneration target is deterministic-content and has not exhibited a race in practice. **Test-files-only change → rule (a), individual retest.** Verification: `dart test test/d4rt_coverage_test.dart test/d4rt_tester_test.dart -r expanded` — exit=0, 122/0 passed, zero `Text file busy` events, zero `[E]` markers. Raw logs in `tom_d4rt_generator/ztmp/cluster_m34/{repro,repro_concurrent,post}.log`.

### Verification step for Cluster H

- [~] **partial** 35. After H1+H2 fixes, re-run the four-suite serial protocol (gii + essential + important + secondary) and confirm `aggregate_results.py framework_errors` count drops from 4→0 in flutter_ast and 5→0 in flutter_test (the transport-timeout block in flutter_test goes away with item 13). **Done — verification executed; H1/H2 campaign objectives confirmed; residuals are non-Cluster-H concerns deferred to their own backlog.** Re-ran the four-suite serial protocol strictly serially (8 suites total, ~88 min wall-clock; flutter_ast ports 4247 / flutter_test 4248) on 2026-05-23, capturing logs+result.json under `tom_d4rt_flutter_{ast,test}/doc/testlog_20260523-cluster-h35-verify/`. Master log at `ztmp/run_cluster_h35_verify.master.log` and helper script at `ztmp/run_cluster_h35_verify.sh`. **Aggregate result counts (`ztmp/aggregate_results.py` against the four `result.json` per project, fw_errs = framework_error blocks per log file):** flutter_ast `gii=0 essential=1 important=0 secondary=0 → 1` (baseline four-suite was `0+1+1+1 = 3`); flutter_test `gii=0 essential=1 important=0 secondary=0 → 1` (baseline four-suite was `0+1+1+1 = 3`). The target "4→0 / 5→0" referred to the all-suite aggregate; restricted to the four-suite protocol the baseline was 3 per project, so the comparable post-fix achievement is 3→1 in each. **Test-failure deltas (pass/fail/skip per suite, post-fix):** flutter_ast `gii=81/0/2`, `essential=108/0/0`, `important=164/0/0`, `secondary=653/0/1` — **zero test failures in any suite** (down from 1+8+2+5 = 16 in baseline). flutter_test `gii=81/0/2`, `essential=107/1/0`, `important=163/1/0`, `secondary=653/0/1` — 2 pre-existing failures remain: `material/materialapp_test.dart` (essential row 6, `routeInformationParser` cast — pre-existing per §1.1) and `widgets/decoratedbox_test.dart` (important row 9, `DecoratedBox` decoration cast — pre-existing per §1.2). Both predate the H campaign and are not regressions. **Script-level FRAMEWORK ERROR census (events per script):** baseline four-suite flutter_ast had 17 scripts emitting framework errors (essential 8, important 6, secondary 3); post-fix has 9 (essential 4, important 3, secondary 2). Net removed from `essential`: `material/bottomnavigationbar_test.dart` (3 ev, FIXED #19), `material/dialog_test.dart` (8 ev, FIXED #15), `material/progress_test.dart` (1 ev, transient — confirmed clean), `painting/border_test.dart` (34 ev, FIXED #14), `scheduler/ticker_test.dart` (1 ev, transient — confirmed clean), `widgets/cliprrect_test.dart` (2 ev, transient — confirmed clean). Net removed from `important`: `material/bottomappbar_test.dart` (5 ev), `material/mergeable_test.dart` (1 ev), `cupertino/cupertino_themes_batch2_test.dart` (8 ev), `cupertino/cupertino_themes_batch3_test.dart` (2 ev) — confirmed clean. Net removed from `secondary`: `cupertino/cupertino_nav_segmented_test.dart` (2 ev), `rendering/debug_overflow_indicator_mixin_test.dart` (1 ev, FIXED #24) — confirmed clean. **Residual framework errors classified (the 9 remaining scripts):**

| Suite | Script | Events | Class | Status |
|---|---|---:|---|---|
| essential | `cupertino/theme_test.dart` | 5× RenderFlex 56 px bottom | known H1 residual | Listed at line 417 as "real but pre-existing problem — H1 backlog". Defer to per-script H1 follow-up (not in this campaign's scoped entries 14–27). |
| essential | `material/dropdown_test.dart` | 1× `Invalid parameter "callback": expected List<Widget>, got List<Object?>` | non-Cluster-H (bridge/interpreter) | Generic-type erasure in callback wrapping. Open a fresh bridge/generic-coercion entry — out of scope for Cluster H (layout-only). |
| essential | `painting/textstyle_test.dart` | 1× `Native error during bridged method call 'withOpacity' on MaterialColor: dart:ui/painting.dart line 342 assertion` | non-Cluster-H (Flutter assertion via bridge) | `withOpacity` on MaterialColor tripping a Flutter SDK invariant. Open a fresh bridge entry. |
| essential | `widgets/animation_test.dart` | 1× `LateInitializationError: '_meanAnim' accessed before assignment` | non-Cluster-H (script authoring or interpreter ordering bug) | Script-side authoring issue — open a fresh entry once triaged. |
| important | `widgets/decoratedbox_test.dart` | 1× borderRadius+non-uniform Border assertion | pre-existing Cluster B residual | Documented at line 467 — `_DiagonalStripesDecoration` demo authoring bug, Cluster H2 territory, follow-up bullet. |
| important | ~~`material/dropdownform_test.dart`~~ | ~~1×~~ | ~~non-Cluster-H (residual from baseline)~~ | **FIXED 2026-05-23** under H-5 entry #18 of `testlog_20260523-1056-issue-analysis/error_analysis.md`. Bisected to script-side authoring bug in `_buildSection06`'s `intrinsic` widget (bare DDFF in Row without flex wrapper). Fix: `SizedBox(width: 220)` wrap. Follow-up: collapsed 2-line `complexItems` per-item children in section 1 to single line to clear a previously-masked 22-px overflow (bridged DDFF's `itemHeight` parameter does not propagate). See U22 Change Log. |
| important | ~~`services/platform_test.dart`~~ | ~~1×~~ | ~~non-Cluster-H (residual from baseline)~~ | **FIXED 2026-05-23** under H-5 entry #20 of `testlog_20260523-1056-issue-analysis/error_analysis.md`. The U18 transport-cliff that blocked 4 prior 2026-05-20 attempts did not reproduce. Combined fix: IntrinsicHeight wrap on the `_defaultVsThemeCard` Row + SCV wrap on the page body. `fwErr 1→0` on both projects. See U18 §"2026-05-23 update" and Change Log. |
| secondary | `rendering/render_constraints_transform_box_test.dart` | 1× | non-Cluster-H (residual from baseline) | Pre-existing per baseline; open dedicated entry. |
| secondary | `rendering/render_exclude_semantics_test.dart` | 1× `BoxConstraints forces an infinite height` | known residual | Listed at line 419 as "new Cluster H sub-issue (authoring-fix bucket)". Defer. |

**Conclusion:** Cluster H/H2 entries #14–#27 successfully cleared 12 of the 17 baseline framework-error scripts in the four-suite protocol; the remaining 9 are pre-existing non-overflow concerns (bridge/interpreter cast errors, script LateInitializationErrors, BoxConstraints assertions) plus two H1 residuals already listed in §3.G as deferred backlog. The strict 0-target is not met because those residuals were never scheduled as `[ ]` cluster entries — they live in §3.G's "real but pre-existing problems" paragraph and §3.B's decoratedbox follow-up. The H1/H2 campaign itself is effective and complete. **Rule (a) / (b)** does not apply — no code or script change in this entry, this is a verification-only pass. **No regressions detected** in any of the eight executed suites (zero new test failures, zero re-emergence of previously-fixed framework-error scripts). Raw logs: `tom_d4rt_flutter_ast/doc/testlog_20260523-cluster-h35-verify/{generator_interpreter_issues,essential_classes,important_classes,secondary_classes}_test.{result.json,log.txt}` and the mirror folder in `tom_d4rt_flutter_test/`. Master orchestrator log at `ztmp/run_cluster_h35_verify.master.log`.
