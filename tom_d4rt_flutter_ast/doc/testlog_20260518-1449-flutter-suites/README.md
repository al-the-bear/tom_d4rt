# Flutter suites baseline — `20260518-1449-flutter-suites` — tom_d4rt_flutter_ast
...
| Date | 2026-05-18 (Mon) |
| Wall window | 14:51:25 CEST → 16:20:48 CEST (≈ 1 h 29 m) |
| Git revision | `319825dc` (`319825dce462a9ef8885f65f51002483c3991e0b`) |
| Branch | `main` |
| Bridges | reused from the 1428 regen (working tree had the freshly-regenerated `.b.dart` artefacts from `testlog_20260518-1428-…`; no generator changes have landed since, so re-running `regenerate_bridges.dart` would have produced identical output) |
...
|---|---:|---:|---:|---:|---:|---:|
| blocking_tests_test | 7 | 7 | 0 | 0 | 0 | 51s |
| crashing_tests_test | 6 | 6 | 0 | 0 | 0 | 26s |
| essential_classes_test | 110 | 110 | 0 | 0 | 0 | 280s |
| generator_interpreter_issues_test | 85 | 83 | 0 | 0 | 2 | 198s |
| generator_interpreter_retest_test | 60 | 55 | 0 | 0 | 5 | 123s |
| hardly_relevant_classes_1_test | 207 | 204 | 0 | 1 | 2 | 504s |
| hardly_relevant_classes_2_test | 205 | 205 | 0 | 0 | 0 | 418s |
| hardly_relevant_classes_3_test | 203 | 203 | 0 | 0 | 0 | 498s |
| hardly_relevant_classes_4_test | 229 | 229 | 0 | 0 | 0 | 431s |
| hardly_relevant_classes_5_test | 232 | 232 | 0 | 0 | 0 | 499s |
| important_classes_test | 166 | 166 | 0 | 0 | 0 | 397s |
| interactive_tests_test | 8 | 8 | 0 | 0 | 0 | 47s |
| secondary_classes_test | 656 | 655 | 0 | 0 | 1 | 1706s |
| timeout_tests_test | 53 | 53 | 0 | 0 | 0 | 124s |
| **TOTAL** | **2227** | **2216** | **0** | **1** | **10** | |

## Delta vs `testlog_20260517-0914-test_analysis`

The most recent comparable per-file flutter baseline. Cluster
fixes 1–11 (most recent: GEN-094) landed between the two runs.
...
| secondary_classes_test | 631/22/2 | 655/0/0 | +24 | −22 | −2 |
| timeout_tests_test | 52/1/0 | 53/0/0 | +1 | −1 | 0 |

## Verdict

**No regression — substantial improvement.** All 60 prior failures
...
These have `skip: true` in the source test list — pre-existing
markers, not run today:
...
| `_summary.md` | Totals table (generated by `../../../_summarize_flutter.dart`). |
