...
**Branch:** `main`
**Package:** `tom_d4rt_flutterm` — renamed to `tom_d4rt_flutter_ast` on
2026-04-30 (`582984c02`), which is where this document now lives
**Runner:** `flutter test` per file, JSON reporter + tee'd text log, strictly serial
**Log folder:** `doc/testlog_20260424-1838-issue-analysis/` at the time of the
run; runner output now goes to `testlog/` and this run's raw files are not
versioned

> **Preserved analysis.** This document was written inside the run folder and
> was removed with it four days later (`3dd032dba`), while ten citations in
> `interpreter_issues.md` went on pointing at the deleted path — one of them
> calling this the authoritative table for Section Q. The text below is the last
> committed revision (`11241cb59`), so it carries every cluster closure that was
> written into it after the run, not just the original triage. Only the location
> changed; nothing was edited except this note.
>
> Read the paths it quotes as historical. The suite names (`essential_classes`,
> `important_classes`, `secondary_classes`, `hardly_relevant_*`) were repacked
> into `flutter_base_01..17` + `flutter_extended_01..24`, and the pass/fail
> numbers describe an interpreter from April. The durable content is the
> *classification* — the buckets, clusters and sections that `interpreter_issues.md`
> refers to by name.
