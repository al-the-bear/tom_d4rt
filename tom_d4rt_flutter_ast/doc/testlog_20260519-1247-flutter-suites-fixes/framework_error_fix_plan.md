...
3. `cupertino/cupertino_page_test.dart` *(secondary_classes_test, 1/1, B-layout/BoxConstraints — infinite height)* — **P1**. Bound the page content with a `SizedBox(height: 600, …)`.

4. `cupertino/cupertino_scroll_behavior_test.dart` *(secondary_classes_test, 1/1, B-layout/RenderFlex — overflow 4.0 px bottom)* — **P2**. Wrap the scrolled column in a `SingleChildScrollView`.

5. `cupertino/restorable_cupertino_tab_controller_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-layout/RenderFlex — "Offset argument contained a NaN value.")* — **P1+P2**. The NaN comes from an unbounded `TabBarView`; give it a bounded extent via SizedBox and wrap the inner content in a SingleChildScrollView.

6. `cupertino/route_test.dart` *(essential_classes_test, 1/1, B-bridge — "A borderRadius can only be given on borders with uniform colors.")* — **P5(a)**. Replace the multi-color `Border` with `Border.all(...)` in the route's decoration. If the test needs the assertion, switch to P5(b) and capture the FlutterError.

7. `dart_ui/shader_mask_engine_layer_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-bridge — "Runtime Error: Native error during bridged constructor 'linear'")* — **P6**. Pass `Offset` (not `List<double>`) and `List<Color>` to `Gradient.linear`.

8. `dart_ui/uniform_float_slot_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-layout/RenderFlex — overflow 2491 px bottom)* — **P1+P2**. Wrap in a bounded scrollable.

9. `dart_ui/uniform_vec2_slot_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-layout — "Offset argument contained a NaN value.")* — **P1**. Bound the demo viewport so layout-derived NaN can't propagate.

10. `dart_ui/uniform_vec3_slot_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-layout/BoxConstraints — infinite height)* — **P1**.
...
15. `foundation/diagnosticable_tree_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-layout/BoxConstraints — infinite height)* — **P1**.
...
19. `foundation/string_property_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-layout/infinite-size — Offset NaN)* — **P1**.

20. `foundation/unicode_test.dart` *(secondary_classes_test, 1/1, B-layout/BoxConstraints — infinite height)* — **P1**.

### Cluster C — `gestures/` (items 21–34)

21. `gestures/hit_testable_test.dart` *(hardly_relevant_classes_1_test, 1/1, B-bridge — uniform-colors)* — **P5(a)**.
